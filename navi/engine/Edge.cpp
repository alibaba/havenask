...
Edge::Edge(const NaviLoggerPtr &logger, const PoolPtr &pool)
...
void Edge::postInit(const PoolPtr &pool) {
    if (_inputNode) {
    ...
    _slotReadyMap =
        ReadyBitMap::createReadyBitMap(pool.get(), _outputSlots.size());
    initSlotReadyMap();
}
                   ...
                   const Node *callNode)
{
    ...
    NAVI_LOG(SCHEDULE1,
             "getData success, data [%p], output [%s], index [%d], eof [%d]",
             data.get(), getOutputName(index).c_str(), index, eof);
    return true;
}
...
bool Edge::clearData(const Node *callNode) {
    ...
    if (!isOverride()) {
        ...
        NAVI_LOG(SCHEDULE1, "schedule call by node [%s] getData",
                 callNode->getName().c_str());
        _inputNode->incNodeSnapshot();
        _inputNode->schedule(callNode);
    } else {
        if (!_eof && !flushOverrideData()) {
            return false;
        }
    }
    return true;
