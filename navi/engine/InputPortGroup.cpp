...
InputPortGroup::InputPortGroup(InputTypeDef type)
    : _type(type)
    , _autoFillIndex(true)
    , _readyMap(nullptr)
...
bool InputPortGroup::postInit(const PartInfo &partInfo, autil::mem_pool::Pool *pool) {
    if (_readyMap) {
        return true;
    }
...
bool InputPortGroup::postInit(autil::mem_pool::Pool *pool) {
    if (_readyMap) {
        return true;
    }
    ...
    _readyMap = ReadyBitMap::createReadyBitMap(pool, _inputs.size());
    _readyMap->setReady(false);
    _readyMap->setOptional(IT_OPTIONAL == _type);
    _readyMap->setFinish(false);
