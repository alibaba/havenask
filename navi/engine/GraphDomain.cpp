                         ...
                         const GraphDomain &domain)
{
    os << "(" << &domain << ","
       ...
       << ",sub_graphs{";
    autil::StringUtil::toStream(os, domain._graphIdSet.begin(), domain._graphIdSet.end());
    return os<< "})";
}
...
void GraphDomain::addSubGraph(SubGraphDef *subGraphDef) {
    _subGraphs.push_back(subGraphDef);
    _graphIdSet.insert(subGraphDef->graph_id());
}
...
bool GraphDomain::containGraph(GraphId graphId) const {
    return _graphIdSet.end() != _graphIdSet.find(graphId);
}
...
bool GraphDomain::postInit() {
    for (auto subGraphDef : _subGraphs) {
        ...
        NAVI_LOG(SCHEDULE2, "sub graph def [%s]", subGraphDef->DebugString().c_str());
...
bool GraphDomain::initPeerInfo(SubGraphDef *subGraphDef) {
    auto borderCount = subGraphDef->borders_size();
    for (int32_t i = 0; i < borderCount; i++) {
        auto border = subGraphDef->mutable_borders(i);
