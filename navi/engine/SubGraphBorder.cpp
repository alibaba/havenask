...
Port *SubGraphBorder::doCreateLocalPort(IoType ioType, const EdgeDef &def) {
    if (IOT_INPUT == ioType) {
        return NAVI_POOL_NEW_CLASS(_pool, LocalInputPort, def, this);
    } else {
        return NAVI_POOL_NEW_CLASS(_pool, LocalOutputPort, def, this);
    }
}

Port *SubGraphBorder::doCreateRemotePort(IoType ioType, const EdgeDef &def) {
    if (IOT_INPUT == ioType) {
        return NAVI_POOL_NEW_CLASS(_pool, RemoteInputPort, def, this);
    } else {
        return NAVI_POOL_NEW_CLASS(_pool, RemoteOutputPort, def, this);
    }
}
...
void SubGraphBorder::initPort(Port *port, const BorderEdgeDef &borderEdgeDef) {
    ...
    port->setPoolResource(_param->worker->getGraphMemoryPoolResource().get());
}
...
bool SubGraphBorder::addPort(Port *port) {
    ...
    if (_portNameMap.end() == it) {
        _portNameMap[portKey].push_back(port);
    } else {
        if (IOT_INPUT == port->getIoType()) {
            auto prev = it->second[0];
...
bool SubGraphBorder::doInitBorderState(const PartInfo &partInfo) {
    return _borderState.init(_pool.get(), partInfo, _portVec.size());
}
...
bool SubGraphBorder::receive(NaviBorderData &borderData) {
    ...
    for (int32_t index = 0; index < dataCount; index++) {
        ...
        // todo report error
        if (EC_NONE != ec && EC_BUFFER_FULL != ec) {
            NAVI_LOG(ERROR, "set data failed, portId[%d], ec[%s]",
                     portId, CommonUtil::getErrorString(ec));
            return false;
        }
        if (retEof) {
