...
NaviConfigContext NaviConfigContext::enter(const std::string &key) {
    ...
    return NaviConfigContext(_configPath, jsonAttrs);
}
...
NaviConfigContext NaviConfigContext::enter(size_t index) {
    if (_jsonAttrs) {
        return NaviConfigContext(_configPath, &(*_jsonAttrs)[index]);
    }

    return NaviConfigContext(_configPath, nullptr);
}

bool NaviConfigContext::hasKey(const std::string &key) {
    if (!_jsonAttrs) {
        return false;
    }
    return _jsonAttrs->HasMember(key.c_str());
}
