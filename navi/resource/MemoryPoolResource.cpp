...
bool MemoryPoolResource::config(ResourceConfigContext &ctx) {
    assert(false);
    return false;
}

ErrorCode MemoryPoolResource::init(ResourceInitContext &ctx) {
    assert(false);
    return EC_INIT_RESOURCE;
}

void MemoryPoolResource::def(ResourceDefBuilder &builder) const {
    builder.name(MEMORY_POOL_RESOURCE_ID);
}

void MemoryPoolResource::initConfigParams() {
...
bool MemoryPoolResource::init(kmonitor::MetricsReporterPtr metricsReporter) {
    ...
    _poolCacheSizeAutoScaleThread =
        autil::LoopThread::createLoopThread(std::bind(&MemoryPoolResource::poolCacheSizeAutoScale, this),
...
autil::mem_pool::Pool *MemoryPoolResource::getPool() {
...
void MemoryPoolResource::putPool(autil::mem_pool::Pool *pool) {
...
void MemoryPoolResource::initMetricsReporter(kmonitor::MetricsReporter &baseReporter) {
...
bool MemoryPoolResource::myHasInterceptorMalloc() {
#ifndef AIOS_OPEN_SOURCE
