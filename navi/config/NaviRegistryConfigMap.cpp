...
bool NaviRegistryConfig::loadConfig(const string &name,
...
void NaviRegistryConfig::Jsonize(autil::legacy::Jsonizable::JsonWrapper &json) {
    ...
    std::set<std::string> dependResource;
    json.Jsonize("dynamic_resource", dependResource, dependResource);
    if (json.GetMode() == FROM_JSON) {
        ...
        for (const auto &resource : dependResource) {
            dependResourceMap.emplace(resource, true);
        }
    }
}
