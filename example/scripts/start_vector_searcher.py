...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print 'python start_vector_searcher.py installRoot qrsHttpPort[optional]'
    ...
    configDir = os.path.join(HERE, '../config/vector_config/online_config/')
    tableConfigDir = os.path.join(configDir, 'table')
    if not util.copyAithetaIndexerPlugin(tableConfigDir):
        sys.exit(-1)

    if not os.path.exists(workDir):
        print '%s not exists, please build index with build_vector_data.py first.'
        sys.exit(-1)
