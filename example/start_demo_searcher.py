...
if __name__ == '__main__':
    ...
    if not os.path.exists(workDir):
        ...
        sys.exit(-1)
    cmd = 'cd %s && /usr/bin/python %s/usr/local/lib/python/site-packages/ha_tools/local_search_starter.py -i ./runtimedata/ -c %s -p 30468,30480 -b %s --qrsHttpArpcBindPort %s' % (workDir, installRoot, configDir, installRoot, qrsHttpPort)

    executCmd(cmd)
    
    curl_tool = os.path.abspath(os.path.join(HERE, "common", "curl_http.py"))
    for query in testQueries:
        cmd = "/usr/bin/python %s %s \"%s\"" %(curl_tool, qrsHttpPort, query)
        executCmd(cmd)
