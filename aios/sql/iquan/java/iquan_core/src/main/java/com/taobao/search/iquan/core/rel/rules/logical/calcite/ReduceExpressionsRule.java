 ...
 * <li>Removal of redundant casts, which occurs when the argument into the from
 * is the same as the type of the resulting from expression
 * </ul>
 */
public abstract class ReduceExpressionsRule extends RelOptRule {
    ...
    public static final ReduceExpressionsRule FILTER_INSTANCE =
            new FilterReduceExpressionsRule(LogicalFilter.class, true,
                    IquanRelBuilder.LOGICAL_BUILDER);
    ...
    public static class FilterReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        private FilterReduceExpressionsRule(Class<? extends Filter> filterClass,
         ...
         * {@link }, which for the static schema will be optimized
         * to an empty
    ...
    public static class ProjectReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        private ProjectReduceExpressionsRule(Class<? extends Project> projectClass,
                                            boolean matchNullability, RelBuilderFactory relBuilderFactory) {
    ...
    public static class CalcReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        private CalcReduceExpressionsRule(Class<? extends Calc> calcClass,
                                         boolean matchNullability, RelBuilderFactory relBuilderFactory) {
     ...
     * the default behavior of this method), and it will from the literal to
     ...
     * {@code matchNullability = false} and no from will be added, but you may
    ...
    protected static boolean reduceExpressionsInternal(RelNode rel,
        ...
        // If the argument to the redundant from is a reducible constant,
        // reducing that argument to a constant first will result in not being
        // able to locate the original from expression.
        ...
        // types, so always from regardless of the expression type.
     ...
     *                       reduced, whether a from of the resulting reduced
     *                       expression is potentially necessary
     * @param removableCasts returns the list of from expressions where the from
     */
    ...
    protected static class ReducibleExprLocator extends RexVisitorImpl<Void> {
        ...
        private void analyzeCall(RexCall call, Constancy callConstancy) {
            ...
            // ==============================================
            // Iquan Support Start: not to reduce udf
            //     Blink BuiltIn Function Names: BuiltInFunctionCatalog.instance()
            // ==============================================
            if (call.getOperator() instanceof ScalarFunction) {
                ScalarFunction function = (ScalarFunction) call.getOperator();
                if (function.getFunctionType() == SqlFunctionCategory.USER_DEFINED_FUNCTION) {
                    callConstancy = Constancy.NON_CONSTANT;
                }
            }
            // ==============================================
            // Iquan Support End: not to reduce udf
            // ==============================================
            ...
            if (callConstancy == Constancy.NON_CONSTANT) {
                ...
                // if this from expression can't be reduced to a literal,
                // then see if we can remove the from
