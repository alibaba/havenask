...
Status Iquan::query(IquanDqlRequest &request,
    ...
    return _impl->query(request, response, planCacheStatus);
}

Status Iquan::listCatalogs(std::string &result) {
    return _impl->listCatalogs(result);
}
...
Status
Iquan::listTables(const std::string &catalogName, const std::string &dbName, std::string &result) {
    return _impl->listTables(catalogName, dbName, result);
}

Status Iquan::listFunctions(const std::string &catalogName,
...
Status Iquan::dumpCatalog(std::string &result) {
    return _impl->dumpCatalog(result);
}

Status Iquan::warmup(const WarmupConfig &warmupConfig) {
    return _impl->warmup(warmupConfig);
}

uint64_t Iquan::getPlanCacheKeyCount() {
    return _impl->getPlanCacheKeyCount();
}

uint64_t Iquan::getPlanMetaCacheKeyCount() {
    return _impl->getPlanMetaCacheKeyCount();
}

void Iquan::resetPlanCache() {
    return _impl->resetPlanCache();
};

void Iquan::resetPlanMetaCache() {
    return _impl->resetPlanMetaCache();
};
} // namespace iquan
