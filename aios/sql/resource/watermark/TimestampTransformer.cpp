 ...
 */
#include "sql/resource/watermark/TimestampTransformer.h"
...
std::unique_ptr<swift::client::SwiftReader> TimestampTransformer::tryCreateReader() {
    auto reader = suez::SwiftReaderCreator::create(_swiftClientCreator, _configMap);
    if (!reader) {
        SQL_LOG(ERROR,
...
bool TimestampTransformer::doInit() {
    ...
    if (!_getStatusThread) {
        SQL_LOG(ERROR,
                "[%p] create get swift status thread loop failed, desc [%s]",
                this,
                getDesc().c_str());
        return false;
    }
