 ...
 */
#include "sql/ops/tvf/builtin/RerankByQuotaTvfFunc.h"
...
bool RerankByQuotaTvfFunc::init(TvfFuncInitContext &context) {
...

    {
        ...
        if (tmps.size() > _quotaFieldValues.size()) {
            SQL_LOG(ERROR,
                    "Quota field value num [%lu] is not match [%lu]",
                    tmps.size(),
    ...
    if (!autil::StringUtil::fromString<uint32_t>(context.params[5], _perCategoryCount)) {
        SQL_LOG(ERROR, "Failed to convert string[%s] to uint32_t", context.params[5].c_str());
        return false;
    }

    if (!autil::StringUtil::fromString<uint32_t>(context.params[6], _windowSize)) {
        SQL_LOG(ERROR, "Failed to convert string[%s] to uint32_t", context.params[6].c_str());
        return false;
    }
    ...
    SQL_LOG(DEBUG, "Quota field str [%s(%lu)]", _quotaFieldStr.c_str(), _quotaFields.size());
    return true;
}
...
RerankByQuotaTvfFunc::rerankByQuota(const table::TablePtr &table,
    ...
    if (!table::TableUtil::calculateGroupKeyHashWithSpecificRow(
            table, _rerankFields, group, groupKeys)) {
        SQL_LOG(ERROR, "Failed to calculate group key [%s] hash", _rerankFieldStr.c_str());
        return result;
    }
    if (group.size() != groupKeys.size()) {
        SQL_LOG(ERROR,
                "Not match between group size [%lu], and group key size [%lu]",
                group.size(),
                groupKeys.size());
        return result;
    }
    ...
    std::priority_queue<table::Row, std::vector<table::Row>, mycomparison> pq(
        mycomparison(comparator, false));
    ...
    while (count < quotaNum) {
        ...
        for (auto &pair : um) {
            ...
            if (seq >= _perCategoryCount) {
                SQL_LOG(WARN,
                        "More than upper limit [%u] in hash key [%lu]",
                        _perCategoryCount,
                        pair.first);
                flag++;
                continue;
        ...
        if (flag == um.size()) {
            SQL_LOG(WARN, "All group visit over, visit [%d], but real need [%d]", count, quotaNum);
            break;
        }
    ...
    for (auto &pair : um) {
        ...
        pTarget->insert(pTarget->end(), um[hashId].begin(), um[hashId].begin() + hsIndex[hashId]);
        targetNum += hsIndex[hashId];
        if (hsIndex[hashId] != um[hashId].size()) {
            pExtra->insert(pExtra->end(), um[hashId].begin() + hsIndex[hashId], um[hashId].end());
...
bool RerankByQuotaTvfFunc::computeTable(const table::TablePtr &table) {
    ...
    if (comparator == nullptr) {
        SQL_LOG(ERROR, "Failed to init rerankByQuota tvf combo comparator [%s]", _sortStr.c_str());
        return false;
    }
