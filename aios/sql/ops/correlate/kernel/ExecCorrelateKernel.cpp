 ...
 */
#include "sql/ops/correlate/ExecCorrelateKernel.h"
...
#include "sql/ops/correlate/CorrelateInfo.h"
...
void ExecCorrelateKernel::def(navi::KernelDefBuilder &builder) const {
    builder.name("sql.ExecCorrelateKernel")
...
bool ExecCorrelateKernel::config(navi::KernelConfigContext &ctx) {
    ...
    NAVI_JSONIZE(ctx, "uncollect_attrs", infos, infos);
...
navi::ErrorCode ExecCorrelateKernel::init(navi::KernelInitContext &context) {
    const auto &kernelName = getKernelName();
...
TablePtr ExecCorrelateKernel::doUnnestTable(const TablePtr &inputTable) {
    ...
    TablePtr nullTable;
    TablePtr outputTable(new Table(_graphMemoryPoolR->getPool()));
    // init output table
