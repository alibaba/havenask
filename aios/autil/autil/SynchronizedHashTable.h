...
public:
    SynchronizedHashTable(uint32_t bucketNumber = BUCKET_NUM_DEF,
              HashFunc hasher = KeyHash<K>());
...
SynchronizedHashTable<K, V, HashFunc>::SynchronizedHashTable(
        uint32_t bucketNumber, HashFunc hasher)
    : _hasher(hasher)
{
...
template<class K, class V, class HashFunc>
HashEntry<K, V>* SynchronizedHashTable<K, V, HashFunc>::getHashEntry(
        const K& key, ReadWriteLock* &pLock) const
{
