...
inline uint32_t Tag(uint64_t point) { return point & 0xFFFFFFFF; }
inline uint32_t Index(uint64_t point) { return point >> 32; }
union TaggedPointer {
    uint64_t mValue;
    void *mPointer;
    struct {
...
inline uint64_t MakePointer(void *address, uint16_t tag) {
    ...
    pointer.mValue = static_cast<uint64_t>(reinterpret_cast<unsigned long>(address));
    pointer.mTag = tag;
...
inline void *GetPointer(uint64_t pointer) {
    ...
    return reinterpret_cast<void *>(static_cast<unsigned long>(tagged_pointer.mValue));
}
...
inline int arpc_eventfd(unsigned int initval, int flags) { return syscall(__NR_eventfd, initval, flags); }
