...
const uint8_t BloomFilter::bit_mask[8] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
...
bool BloomFilter::DecodeFromString(std::string &filterStr) {
    ...
    if (uncompress((Bytef *)uncompress_buffer,
                   &uncompress_buffer_size,
                   (const Bytef *)(decoded_filterStr.c_str() + 12),
                   (decoded_filterStr.size() - 12)) != Z_OK) {
