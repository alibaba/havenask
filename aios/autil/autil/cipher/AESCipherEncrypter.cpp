...
AESCipherEncrypter::AESCipherEncrypter(std::unique_ptr<AESCipherUtility> utility) : _utility(std::move(utility)) {
    assert(_utility);
}
...
bool AESCipherEncrypter::encryptMessage(const std::string &plainText, std::string &cipherText) {
    ...
    size_t len;
    if (EVP_EncryptInit_ex(&ctx, _utility->getCipher(), nullptr, _utility->getKey(len), _utility->getIv(len)) <= 0) {
        AUTIL_LOG(
            ERROR, "Failed to initialize encryption operation, error:[%s].", toErrorString(ERR_get_error()).c_str());
    ...
    if (EVP_EncryptUpdate(
            &ctx, cipherBuffer, &encryptLen, (const unsigned char *)plainText.c_str(), plainText.size()) <= 0) {
...
std::string AESCipherEncrypter::getKeyHexString() const { return _utility->getKeyHexString(); }

std::string AESCipherEncrypter::getIvHexString() const { return _utility->getIvHexString(); }

std::string AESCipherEncrypter::getSaltHexString() const { return _utility->getSaltHexString(); }
