...
bool AESCipherUtility::setKeyAndIv(const std::string &hKey, const std::string &hiv) {
    ...
    if (keyLen != EVP_CIPHER_key_length(_cipher)) {
        AUTIL_LOG(ERROR,
                  "invalid hex key str [%s], key length [%d] not match with cipher [%s]",
                  hKey.c_str(),
                  keyLen,
                  CIPHER_NAME(_cipher));
        return false;
    ...
    if (ivLen != EVP_CIPHER_iv_length(_cipher)) {
        AUTIL_LOG(ERROR,
                  "invalid hex iv str [%s], iv length [%d] not match with cipher [%s]",
                  hiv.c_str(),
                  ivLen,
                  CIPHER_NAME(_cipher));
        return false;
...
bool AESCipherUtility::calculate() {
    ...
    if (_usePbkdf2) {
        ...
        if (!PKCS5_PBKDF2_HMAC(
                _passwd.c_str(), _passwd.size(), sptr, islen, _pbkdf2Iter, _dgst, iklen + ivlen, tmpkeyiv)) {
        ...
        EVP_BytesToKey(_cipher, _dgst, sptr, (const unsigned char *)_passwd.c_str(), _passwd.size(), 1, _key, _iv);
    }
    _keyLen = iklen;
    _ivLen = ivlen;
    return true;
...
size_t AESCipherUtility::getSaltHeaderLen() const { return sizeof(magic) - 1 + sizeof(_salt); }

size_t AESCipherUtility::getCipherBlockSize() const { return _cipher ? EVP_CIPHER_block_size(_cipher) : 0; }
