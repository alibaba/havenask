...
{
    if (_blockMap.size() >= _maxBlockCount
        && replaceBlocks() == 0)
    {
...
uint32_t BlockCache::replaceBlocks()
{
    ...
    ReplacePolicy::TryReplacePredicate tryReplace(_blockMap);
     uint32_t replacedNodeCount =
         _replacePolicy->replaceBlocks(tryReplace, firstReplacedNode);
...
void BlockCache::setMaxMemory(size_t maxMemory)
{
    _maxBlockCount = (maxMemory / _blockSize);
}
