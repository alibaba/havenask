...
public:
    ...
    static void sundaySearch(const char *text, const char *key, std::vector<size_t> &posVec);
    static void
    sundaySearch(const char *text, size_t textSize, const char *key, size_t keySize, std::vector<size_t> &posVec);
    ...
    static void getKVValue(
        const std::string &text, std::string &key, std::string &value, const std::string &sep, bool isTrim = false);
...
private:
    ...
    public:
        StringStreamWrapper(const std::string &str = "") : _ss(StringUtil::getStringStream()) { _ss->str(str); }
...
inline std::string StringUtil::getTypeString(const T &t) {
    ...
    std::unique_ptr<char[], void (*)(void *)> result(abi::__cxa_demangle(id, 0, 0, &status), std::free);
#else
    std::auto_ptr<char[], void (*)(void *)> result(abi::__cxa_demangle(id, 0, 0, &status), std::free);
#endif
