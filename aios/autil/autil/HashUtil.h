...
public:
    inline bool operator()(uint128_t nKey1, uint128_t nKey2) const { return nKey1 == nKey2; }
};
...
class BinarySearch {
public:
    inline uint32_t operator()(Type *szElements, uint32_t nBegin, uint32_t nEnd, Type value) {
        return std::lower_bound(szElements + nBegin, szElements + nEnd, value, Compare()) - szElements;
    }
};
