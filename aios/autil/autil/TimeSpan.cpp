...
TimePoint TimeSpan::GetCurrentTimePoint() { return GetCycles(); }

Duration TimeSpan::GetDuration() const { return GetDuration(GetCycles()); }

Duration TimeSpan::GetDuration(TimePoint end) const { return end > start_ ? (end - start_) / kCpuMhzDivider : 0; }

Duration TimeSpan::Rotate() { return Rotate(GetCycles()); }
...
TimePoint Deadline::GetDeadline(TimePoint start, Duration duration) { return start + duration * kCpuMhz; }
...
Deadline::operator bool() const { return reached_ || (reached_ = (GetCycles() >= deadline_)); }

bool Deadline::After(TimePoint now) const { return now < deadline_; }
