...
#define MULTI_VALUE_FORMAT_SUPPORTED(T)                                                                                \
    std::is_same<T, bool>::value || std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value ||               \
        std::is_same<T, int16_t>::value || std::is_same<T, uint16_t>::value || std::is_same<T, int32_t>::value ||      \
        std::is_same<T, uint32_t>::value || std::is_same<T, long>::value || std::is_same<T, unsigned long>::value ||   \
        std::is_same<T, long long>::value || std::is_same<T, unsigned long long>::value ||                             \
        std::is_same<T, int64_t>::value || std::is_same<T, uint64_t>::value ||                                         \
        std::is_same<T, autil::uint128_t>::value || std::is_same<T, float>::value || std::is_same<T, double>::value || \
        std::is_same<T, char>::value

#define STRING_TYPE_VALUE_FORMAT_SUPPORTED(T)                                                                          \
    std::is_same<const T, const std::string>::value || std::is_same<const T, const autil::StringView>::value ||        \
...
public:
    ...
    static inline size_t getEncodedCountFromFirstByte(uint8_t firstByte) { return (firstByte >> 6) + 1; }
    ...
    calculateMultiStringBufferLen(StringType data[], size_t dataCount, size_t &offsetLen) {
        size_t latestOffset = 0;
        size_t length = 0;

        for (size_t i = 0; i < dataCount; ++i) {
            size_t encodeHeaderLen = MultiValueFormatter::getEncodedCountLength(data[i].size());
    ...
    formatMultiStringToBuffer(char *buffer, size_t bufLen, size_t offsetLen, const StringType data[], size_t tokenNum) {
        ...
        *(uint8_t *)writeBuf = offsetLen;
        ...
        for (size_t i = 0; i < tokenNum; ++i) {
            size_t encodeLen = MultiValueFormatter::encodeCount(data[i].size(), dataBuf, buffer + bufLen - dataBuf);
            dataBuf += encodeLen;
    ...
    static autil::StringView encodePrefixLength(char *buf, const autil::StringView &value) {
        size_t countLen = encodeCount(value.size(), buf, 4);
        memcpy(buf + countLen, value.data(), value.size());
        return autil::StringView(buf, countLen + value.size());
    }
};
