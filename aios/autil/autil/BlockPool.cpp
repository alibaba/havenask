...
BlockPool::~BlockPool() { 
    for (std::vector<void *>::iterator it = _bigChunk.begin();
         it != _bigChunk.end(); ++it) 
    {
...
bool BlockPool::init(uint64_t capacity, uint64_t chunkSize,
                     uint32_t blockSize, uint64_t freeThreshold) 
{
    if ( blockSize < 8
        || chunkSize > capacity
        || blockSize > chunkSize
        || freeThreshold >= capacity
        || 0 != capacity % chunkSize 
        || 0 != chunkSize % blockSize) 
    {
        return false;
    }
    if (_blockSize > 0
