...
public:
    bool init(uint64_t capacity, uint64_t chunkSize, 
              uint32_t blockSize, uint64_t freeThreshold);
...
inline void* BlockPool::getBlock() {
    ...
    if (_freeCount == 0) {
        if (_chunkSize * _bigChunk.size() >= _capacity
            || false == getOneBigChunk()) 
        {
            return NULL;
        }
    }
...
inline void BlockPool::freeBlock(const std::vector<void* > &blocks) {
    ...
    MemBlock *tail = NULL;
    uint64_t count = 0;
    for (std::vector<void* >::const_iterator it = blocks.begin();
         it != blocks.end(); ++it) {
        if(NULL != *it) {
            if (NULL == head) {
                head = (MemBlock*)(*it);
...
inline uint64_t BlockPool::getCapacity() const {
    return _capacity;
}

inline uint64_t BlockPool::getUsedSize() const {
    return _usedSize;
}

inline uint32_t BlockPool::getBlockSize() const {
    return _blockSize;
}
