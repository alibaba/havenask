...
public:
    ...
    void normalizeUTF16(const uint16_t *in, size_t len, uint16_t *out);
    bool needNormalize() {
        return !(_options.traditionalSensitive && _options.widthSensitive && _options.caseSensitive);
    }
...
private:
    ...
    bool stringConverter(const std::string &input,
                         std::string &output,
                         TO_UNICODE_FUN to_unicode,
                         FROM_UNICODE_FUN from_unicode,
                         unsigned op = 0) const;
