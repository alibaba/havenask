...
void DataBuffer::dataBufferCorrupted() const { AUTIL_LEGACY_THROW(DataBufferCorruptedException, "invalid data"); }

void DataBuffer::readVarint32Fallback(uint32_t &value) { readVarint32FallbackInline(value); }
...
void DataBuffer::readVarint64Fallback(uint64_t &value) { readVarint64FallbackInline(value); }
...
void DataBuffer::fillSectionHeader(unsigned char *pbuf, const DataBuffer &metaBuffer) {
    SectionHeader sectionHeader;
    size_t secMetaLen = metaBuffer.getDataLen();
    sectionHeader.checkSum = HashAlgorithm::hashString(metaBuffer.getData(), secMetaLen, 0);
...
void DataBuffer::fillSectionFlag(unsigned char *pbuf) { *(uint32_t *)(pbuf) = SECTION_FLAG; }
...
void DataBuffer::serializeToStringWithSection(std::string &resultStr) {
    ...
    // calc new mem size
    size_t memLen = dataLen + secDataLen + secMetaBuffer.getDataLen() + sizeof(SectionHeader) + sizeof(SECTION_FLAG);
...
bool DataBuffer::extractAndCheckSectionHeader(SectionHeader &header, const unsigned char *pbuf) {
    header = *(SectionHeader *)pbuf;
    if (header.metaLen > (pbuf - _pdata)) {
        return false;
    }

    uint32_t checkSum = HashAlgorithm::hashString((const char *)(pbuf - header.metaLen), header.metaLen, 0);
...
bool DataBuffer::extractAndCheckSectionMeta(SectionMeta &secMeta, size_t metaLen, const unsigned char *metaData) {
...
void DataBuffer::extractSectionBuffer(const SectionMeta &secMeta, size_t &sectionDataLen) {
    sectionDataLen = 0;
    const auto &secInfoMap = secMeta.getSectionInfoVec();
    for (const auto &sec : secInfoMap) {
        DataBuffer *secDataBuffer = new DataBuffer(_pdata + sec.start, sec.length, _pool);
        _sectionMap.insert(std::make_pair(sec.name, secDataBuffer));
        sectionDataLen += sec.length;
    }
}

void DataBuffer::deserializeFromStringWithSection(const std::string &str, mem_pool::Pool *pool, bool needCopy) {
    // init mem
...
void DataBuffer::buildSectionMeta(SectionMeta &secMeta, size_t &secDataLen) {
    ...
    for (const auto &section : _sectionMap) {
        auto dataLen = section.second->getDataLen();
        secMeta.addSectionInfo(section.first, start, dataLen);
        start += dataLen;
        secDataLen += dataLen;
    }
}

void DataBuffer::initByString(const std::string &str, mem_pool::Pool *pool, bool needCopy) {
    reset();
    size_t len = str.size();
    if (needCopy) {
        init(len, pool);
...
void DataBuffer::makeConstString(StringView &value, char *data, uint32_t len) { value = StringView(data, len); }
...
void SectionMeta::serialize(DataBuffer &dataBuffer) const { dataBuffer.write(_sectionInfoVec); }

void SectionMeta::deserialize(DataBuffer &dataBuffer) { dataBuffer.read(_sectionInfoVec); }
