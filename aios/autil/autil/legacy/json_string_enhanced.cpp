...
inline static void trueThenThrow(bool test, const std::string &s) {
    if (test)
        AUTIL_LEGACY_THROW(JsonParserError, s.c_str());
}
} // namespace
...
void SkipExact(const std::string &is, size_t &pos, const std::string &str) {
    ...
    } catch (std::exception &ex) { result = -2; }
    trueThenThrow(result != 0, std::string("expected ") + str);
    pos += str.size();
}

void ParseJsonBoolTrue(const std::string &is, size_t &pos) { SkipExact(is, pos, "true"); }

void ParseJsonBoolFalse(const std::string &is, size_t &pos) { SkipExact(is, pos, "false"); }

void ParseJsonNull(const std::string &is, size_t &pos) { SkipExact(is, pos, "null"); }
...
Acceptable num format:
    ...
    */
    ...
    char last = is[pos - 1];
    trueThenThrow(last == 'e' || last == 'E' || last == '+' || last == '-' ||
...
void ParseJsonMap(const std::string &is, size_t &pos, std::map<std::string, Any> &ret) {
    ...
    for (; pos < is.size(); SkipSpace(is, pos)) {
        ...
        switch (byte) {
        case '}':
            trueThenThrow(!expectComma && ret.size() > 0, "element required after ,");
            ++pos;
            return;
        case ',':
...
void ParseJsonArray(const std::string &is, size_t &pos, JsonArray &ret) {
    ...
    for (; pos < is.size(); SkipSpace(is, pos)) {
        ...
        case ',':
            ...
            break;
        case ']':
            trueThenThrow(!expectComma && ret.size() > 0, "item missing between , and ]");
            ++pos;
            return;
        default:
