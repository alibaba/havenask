 ...
 */
uint16_t Utf8ToUnicode(const std::string &s, std::string::const_iterator &it);
...
std::string TrimString(const std::string &str, const char leftTrimChar = ' ', const char rightTrimChar = ' ');
...
*/
std::string ReplaceString(const std::string &origin_string, const std::string &old_value, const std::string &new_value);
...
template <>
inline std::string StringTo(const std::string &string) {
    return string;
}
...
std::pair<K, V> StringToPair(const std::string &str, const std::string &delim = ":") {
    ...
    return std::make_pair(StringTo<K>(str.substr(0, pos)), StringTo<V>(str.substr(pos + delim.size())));
}
...
template <typename K, typename V>
std::map<K, V>
StringToMap(const std::string &string, const std::string &itemDelim = ",", const std::string &keyValueDelim = ":") {
    std::map<K, V> result;
    std::vector<std::string> part = StringToVector<std::string>(string, itemDelim);
    for (std::vector<std::string>::const_iterator it = part.begin(); it != part.end(); ++it) {
...
private:
     ...
     * @param str the input string pattern; 3rd param - width (default: width_of_(2nd param)); 4th param - fill char
     * (default: '0')
     * @return result
