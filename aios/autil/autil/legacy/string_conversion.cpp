...
void autil::legacy::QuoteString(const std::string &textToEscape, std::string &ret) {
    ...
    for (std::string::const_iterator it = textToEscape.begin(); it != textToEscape.end(); ++it) {
        /**
...
std::string autil::legacy::UnquoteString(a_Istream &is) {
    bool escaped = false;
    std::string str;
    for (char byte = static_cast<char>(is.peek()); is.good(); is.get(), byte = static_cast<char>(is.peek())) {
        if (escaped) {
            switch (byte) {
            case '"':
                byte = '"';
            ...
            case 'u': {
                ...
                for (int j = 1; j <= 4; ++j) {
                    ...
                    if (b >= '0' && b <= '9')
                        ...
                        BadQuoteString("\\u needs 4 digits");
                }
...
std::string autil::legacy::LeftTrimString(const std::string &string, const char trimChar) {
    size_t pos = 0;
    while (pos < string.size() && string[pos] == trimChar)
        ++pos;
...
std::string autil::legacy::RightTrimString(const std::string &string, const char trimChar) {
    ...
    while (pos != (size_t)-1 && string[pos] == trimChar)
        --pos;
    return string.substr(0, pos + 1);
}

std::string autil::legacy::TrimString(const std::string &string, const char leftTrimChar, const char rightTrimChar) {
...
std::string autil::legacy::ToString(double data, uint32_t precision) {
    static const int MAX_SIZE_OF_LITERAL = MAX_DOUBLE_PRECISION + 1 + // for possible decimal point
...
std::string autil::legacy::operator+(const std::string &left, int32_t data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, uint32_t data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, int64_t data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, uint64_t data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, float data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, double data) { return left + ToString(data); }

std::string autil::legacy::operator+(const std::string &left, bool data) { return left + ToString(data); }

std::string autil::legacy::EscapeForCCode(const std::string &input) {
    std::string result;
    for (decltype(input.begin()) it = input.begin(); it != input.end(); ++it) {
        switch (*it) {
        case '"':
            result += "\\\"";
            break;
...
bool autil::legacy::EndWith(const std::string &input, const std::string &pattern) {
    ...
    while (it2 != pattern.rend() && *it1 == *it2) {
        ++it1;
        ++it2;
    }
...
std::string autil::legacy::FromHexString(const std::string &str) {
    ...
    if (str.size() % 2) {
        AUTIL_LEGACY_THROW(ParameterInvalidException, "The size of Hex-String must be even.");
    }
    ...
    for (size_t i = 0; i < len; ++i) {
        for (int n = 0; n < 2; ++n) {
            ...
            if (c >= 'a' && c <= 'f') {
                ...
                AUTIL_LEGACY_THROW(ParameterInvalidException, "unexpected character in Hex String.");
            }
