...
template <typename T, bool = std::is_enum<T>::value, bool = std::is_arithmetic<T>::value>
...
template <typename T>
inline typename enable_if_exist<Any, typename dispatch_trait<T>::tag_type>::type ToJson(const T &i);

template <typename T>
inline typename enable_if_exist<void, typename dispatch_trait<T>::tag_type>::type FromJson(T &i, const Any &a);
...
inline Any ToJson(const Any &a) { return a; }

inline void FromJson(Any &t, const Any &a) { t = a; }

inline Any ToJson(const std::string &s) { return s; }

inline void FromJson(std::string &s, const Any &a) { s = AnyCast<std::string>(a); }

inline Any ToJson(const char *s) { return std::string(s); }
...
Any ToJson(const std::vector<T, A> &m) {
    ...
    for (typename std::vector<T, A>::const_iterator it = m.begin(); it != m.end(); it++) {
...
void FromJson(std::vector<T, A> &tVec, const Any &a) {
    ...
    for (typename std::vector<Any>::const_iterator it = anyVec.begin(); it != anyVec.end(); it++) {
        T t;
        FromJson(t, *it);
        tVec.push_back(t);
    }
...
Any ToJson(const std::deque<T, A> &m) {
    std::vector<Any> anyVec;
    for (typename std::deque<T, A>::const_iterator it = m.begin(); it != m.end(); it++) {
        anyVec.push_back(ToJson(*it));
    }
    return anyVec;
}
...
Any ToJson(const std::list<T, A> &m) {
    std::vector<Any> anyVec;
    for (typename std::list<T, A>::const_iterator it = m.begin(); it != m.end(); it++) {
        anyVec.push_back(ToJson(*it));
    }
    return anyVec;
}
...
Any ToJson(const std::map<T, U, C, A> &m) {
    ...
    for (typename std::vector<std::pair<T, U>>::const_iterator it = vec.begin(); it != vec.end(); it++) {
        anyVec.push_back(ToJson(*it));
    }
...
Any ToJson(const std::map<std::string, T, C, A> &m) {
    ...
    for (typename std::map<std::string, T, C, A>::const_iterator it = m.begin(); it != m.end(); it++) {
...
void FromJson(std::map<std::string, T, C, A> &tMap, const Any &a) {
    ...
    for (typename std::map<std::string, Any>::const_iterator it = anyMap.begin(); it != anyMap.end(); it++) {
...
template <typename T>
std::string ToJsonString(const T &t, bool isCompact, const std::string &prefix = "") {
