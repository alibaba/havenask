...
public:
    ...
    virtual bool eof() { return mLastGetFail; }

    virtual bool bad() { return false; }

    virtual bool fail() { return false; }

    virtual ~a_IstreamBuffer() { freeAlloc(); }
...
public:
    ...
    virtual int peek() { return mBase.peek(); }

    virtual int get() { return mBase.get(); }
    ...
    virtual bool eof() { return mBase.eof(); }

    virtual bool bad() { return mBase.bad(); }

    virtual bool fail() { return mBase.fail(); }
...
public:
    ...
    virtual a_Ostream &put(char data) { return *this; }
    ...
    virtual a_Ostream &operator<<(const char *s) { return *this << std::string(s); }
...
public:
    ...
    virtual a_OstreamNull &operator<<(const char &ch) { return *this; }
    virtual a_OstreamNull &operator<<(const std::string &in) { return *this; }
    virtual a_OstreamNull &operator<<(const char *s) { return *this; }
...
public:
    ...
    virtual void write(const char *d, uint32_t len) { mBase.write(d, len); }
...
class a_IstreamLengthed : public a_Istream {
    ...
    void FetchDataFromBuff(uint32_t bufferSize) {
        ...
        } else // fill buffer
        {
            ...
            uint32_t canRead = mTotal < LENGTHED_STREAM_BUFFER_SIZE ? mTotal : LENGTHED_STREAM_BUFFER_SIZE;
...
public:
    a_IstreamLengthed(a_Istream &is, uint32_t len)
        : mBegin(0)
        , mLocation(0)
        , mEnd(0)
        , mHoldInBuff(0)
    ...
    inline uint32_t GetRemain() { return mTotal - (mLocation - mBegin); }
    ...
    virtual bool eof() { return mLastGetEof; }
...
public:
    ...
    virtual bool fail() { return mCurrent == mSources.end() ? false : (*mCurrent)->fail(); }

    virtual bool eof() { return mCurrent == mSources.end(); }
...
public:
    ...
    a_LinePositionAwareIstream(a_Istream &base) : a_IstreamWrapper(base), mLast(0), mLine(0), mCol(0) {}
...
public:
     ...
     */
    ByteArrayOstream(uint32_t buffSize = SIZE_PER_BUFF) : mCurr(0), mEnd(0), mSizePerBuff(buffSize) {}
    ...
    inline uint32_t WriteVarint(const int32_t &data) { return WriteVarint(static_cast<uint32_t>(data)); }
    inline uint32_t WriteVarint(const int64_t &data) { return WriteVarint(static_cast<uint64_t>(data)); }
    /**
    ...
    inline uint32_t WriteFixed(const int32_t &data) { return WriteFixed(static_cast<uint32_t>(data)); }
    inline uint32_t WriteFixed(const int64_t &data) { return WriteFixed(static_cast<uint64_t>(data)); }
    inline uint32_t WriteFixed(const float &data) { return WriteFixed(reinterpret_cast<const uint32_t &>(data)); }
    inline uint32_t WriteFixed(const double &data) { return WriteFixed(reinterpret_cast<const uint64_t &>(data)); }
    /**
