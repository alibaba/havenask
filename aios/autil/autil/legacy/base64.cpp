...
void Base64DecodingEx(std::istream &is, std::ostream &os, char makeupChar, char plus, char slash) {
    ...
    while (is.peek() >= 0) {
        ...
        if (bTmp == makeupChar) {
            if (index == 0 || index == 1) {
                AUTIL_LEGACY_THROW(BadBase64Exception,
                                   "\n\nInvaild input!\nInput must be a multiple of four!!\n= must be the third or "
                                   "fourth place in the last four characters!\n");
            } else if (index == 2) {
                bTmpNext = is.get();
...
std::string Base64DecodeFast(const std::string &input) { return Base64DecodeFast(input.c_str(), input.size()); }

std::string Base64EncodeFast(const std::string &input) { return Base64EncodeFast(input.c_str(), input.size()); }
