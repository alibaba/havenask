...
int StrCaseCmpUtf8(const std::string &str1, const std::string &str2) {
    ...
    while (++currPos < smallerLen) {
        if (str1[currPos] == str2[currPos]) {
            continue;
        } else if (!(str1[currPos] & 0x80) && !(str2[currPos] & 0x80) &&
...
std::string::size_type FindNextCharUtf8(const std::string &str, std::string::size_type pos) {
    ...
    while (currPos < byteCount) {
        if (!(str[currPos] & 0x80) || (str[currPos] & 0xE0) == 0xC0 || (str[currPos] & 0xF0) == 0xE0 ||
            (str[currPos] & 0xF8) == 0xF0 || (str[currPos] & 0xFC) == 0xF8 || (str[currPos] & 0xFE) == 0xFC) {
            break;
        }
        if ((str[currPos] & 0xC0) != 0x80) {
            AUTIL_LEGACY_THROW(BadUtf8Format, "");
...
std::string UnicodeToUtf8(uint16_t unicode) {
    ...
    if (unicode <= 0x7F) {
        ...
        utf8 += static_cast<char>(((unicode & 0x7C0) >> 6) | UTF8_2BYTE_PREFIX);
        utf8 += static_cast<char>((unicode & 0x3F) | UTF8_BYTE_PREFIX);
    } else /*if(unicode <= 0xFFFF)*/
    {
        utf8 += static_cast<char>(((unicode & 0xF000) >> 12) | UTF8_3BYTE_PREFIX);
        utf8 += static_cast<char>(((unicode & 0x0FC0) >> 6) | UTF8_BYTE_PREFIX);
...
#define trueThenThrow(test, str)                                                                                       \
    if (test)                                                                                                          \
        AUTIL_LEGACY_THROW(autil::legacy::ParameterInvalidException, str);
    trueThenThrow(it == s.end(), "unexpected end in utf8");
    uint16_t unicode = 0;
    char c = *it;
    if (UTF8_3BYTE_PREFIX == (c & UTF8_3BYTE_MASK)) { // 3 byte
