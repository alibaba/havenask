...
public:
    ...
    template <typename ValueType>
    Any(const ValueType &value) : mContent(new Wrapper<ValueType>(value)) {}
    ...
    void Swap(Any &other) { std::swap(mContent, other.mContent); }
...
public:
    ...
    std::string GetMessage() const override { return "autil::legacy::AnyNumberCast: Given is not of number type."; }
};
...
const ValueType &AnyCast(const Any &operand) {
    if (operand.GetType() == typeid(ValueType)) {
        ...
        AUTIL_LEGACY_THROW(BadAnyCast,
                           std::string("autil::legacy::AnyCast: can't cast from ") + operand.GetType().name() + " to " +
                               typeid(ValueType).name() + ", content is " + operandStr);
    }
}
...
#define IF_CAST_RETURN(tp)                                                                                             \
    ...
    else IF_CAST_RETURN(uint32_t) else IF_CAST_RETURN(int64_t) else IF_CAST_RETURN(uint64_t) else IF_CAST_RETURN(double) else IF_CAST_RETURN(
        uint8_t) else IF_CAST_RETURN(int8_t) else IF_CAST_RETURN(uint16_t) else IF_CAST_RETURN(int16_t) else IF_CAST_RETURN(float) else {
