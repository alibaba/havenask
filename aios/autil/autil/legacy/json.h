...
struct JsonNumber : public std::string {
    ...
    std::string &AsString() { return dynamic_cast<std::string &>(*this); }

    const std::string &AsString() const { return dynamic_cast<const std::string &>(*this); }
...
inline bool IsJsonNull(const Any &any) { return any.GetType() == typeid(void); }

inline bool IsJsonBool(const Any &any) { return any.GetType() == typeid(bool); }

inline bool IsJsonArray(const Any &any) { return any.GetType() == typeid(JsonArray); }

inline bool IsJsonMap(const Any &any) { return any.GetType() == typeid(JsonMap); }

inline bool IsJsonString(const Any &any) { return any.GetType() == typeid(std::string); }

inline bool IsJsonNumber(const Any &any) { return any.GetType() == typeid(JsonNumber); }
...
inline std::string GetJsonTypeName(const Any &any) {
    ...
    return type == typeid(void)          ? "null"
           ...
           : type == typeid(JsonNumber) || type == typeid(uint8_t) || type == typeid(int8_t) ||
                   type == typeid(uint16_t) || type == typeid(int16_t) || type == typeid(uint32_t) ||
                   type == typeid(int32_t) || type == typeid(uint64_t) || type == typeid(int64_t) ||
