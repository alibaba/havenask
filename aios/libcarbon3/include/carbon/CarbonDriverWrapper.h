...
public:
    ...
    virtual bool init(const std::string &applicationId,
                      ...
                      bool isNewStart,
                      uint32_t amnitorPort, bool withBuffer) = 0;

    // proxySpec: read from env if missing.
    virtual bool init(const std::string &applicationId,
                      const std::string &hippoZk,
                      const std::string &carbonZk,
                      const std::string &backupRoot,
                      ...
                      const DriverOptions& opts = DriverOptions()) = 0;
    ...
    virtual bool releaseSlots(
        ...
        uint64_t leaseMs, ErrorInfo *errorInfo) const = 0;
