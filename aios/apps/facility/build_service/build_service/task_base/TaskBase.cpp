...
TaskBase::TaskBase(const std::string& epochId) : TaskBase(nullptr, epochId) {}

TaskBase::TaskBase(indexlib::util::MetricProviderPtr metricProvider, const std::string& epochId)
...
bool TaskBase::init(const std::string& jobParam)
{
    ...
    ConfigDownloader::DownloadErrorCode errorCode = ConfigDownloader::downloadConfig(configPath, localConfPath);
...
bool TaskBase::initKeyValueMap(const string& jobParam, KeyValueMap& kvMap)
{
    try {
        FromJsonString(kvMap, jobParam);
    } catch (const ExceptionBase& e) {
        string errorMsg = "Invalid job param[" + jobParam + "], from json failed[" + string(e.what()) + "]";
        BS_LOG(ERROR, "%s", errorMsg.c_str());
        return false;
    }
    return true;
...
proto::PartitionId TaskBase::createPartitionId(uint32_t instanceId, Mode mode, const string& role)
{
...
bool TaskBase::startMonitor(const Range& range, Mode mode)
{
    ...
    KmonitorUtil::getTags(range, _jobConfig.getClusterName(), _dataTable, _jobConfig.getGenerationId(), tags);
...
bool TaskBase::getIndexPartitionOptions(indexlib::config::IndexPartitionOptions& options)
{
    string clusterName = _jobConfig.getClusterName();
    string mergeConfigName = getValueFromKeyValueMap(_kvMap, MERGE_CONFIG_NAME);
    bool ret =
        IndexPartitionOptionsWrapper::getIndexPartitionOptions(*_resourceReader, clusterName, mergeConfigName, options);
    if (!ret) {
        return false;
    }
    ...
    IndexPartitionOptionsWrapper::initBatchMask(_kvMap, options);
    IndexPartitionOptionsWrapper::initGenerationId(_kvMap, options);
    IndexPartitionOptionsWrapper::initSchemaUpdateStandard(_kvMap, options);

    return initDocReclaimConfig(options, mergeConfigName, clusterName);
}

bool TaskBase::initDocReclaimConfig(indexlib::config::IndexPartitionOptions& options, const string& mergeConfigName,
                                    const string& clusterName)
{
    ...
    if (!resourceReader->getClusterConfigWithJsonPath(clusterName, jsonPath, reclaimSource, isExist) || !isExist) {
    ...
    string docReclaimConfigPath = fslib::util::FileUtil::joinFilePath(
        IndexPathConstructor::constructGenerationPath(_kvMap[INDEX_ROOT_PATH], _kvMap[CLUSTER_NAMES], generationId),
        IndexPathConstructor::DOC_RECLAIM_DATA_DIR);
    string docReclaimFile = fslib::util::FileUtil::joinFilePath(docReclaimConfigPath, BS_DOC_RECLAIM_CONF_FILE);
    bool isFileExist = false;
    if (!fslib::util::FileUtil::isExist(docReclaimFile, isFileExist)) {
...
bool TaskBase::addDataDescription(const string& dataTable, KeyValueMap& kvMap)
{
    ...
    if (!_resourceReader->getDataTableConfigWithJsonPath(dataTable, "data_descriptions", dataDescriptions.toVector())) {
    ...
    for (size_t i = 0; i < dataDescriptions.size(); i++) {
        ...
        if (getValueFromKeyValueMap(dataDescription.kvMap, READ_SRC) == src) {
            BS_LOG(INFO, "use data description[%s]", autil::legacy::ToJsonString(dataDescription).c_str());
            for (auto it = dataDescription.begin(); it != dataDescription.end(); ++it) {
                if (kvMap.find(it->first) == kvMap.end()) {
                    kvMap[it->first] = it->second;
                }
            }
            BS_LOG(INFO, "kv map after merge[%s]", autil::legacy::ToJsonString(kvMap).c_str());
            return true;
        }
    }
...
bool TaskBase::getIncBuildParallelNum(uint32_t& parallelNum) const
{
    ...
    if (!_resourceReader->getClusterConfigWithJsonPath(clusterName, "cluster_config.builder_rule_config",
                                                       buildRuleConfig)) {
        string errorMsg = "parse cluster_config.builder_rule_config for [" + clusterName + "] failed";
        BS_LOG(ERROR, "%s", errorMsg.c_str());
        return false;
    }
