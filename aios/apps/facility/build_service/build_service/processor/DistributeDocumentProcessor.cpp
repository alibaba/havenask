...
bool DistributeDocumentProcessor::init(const DocProcessorInitParam& param)
{
    ...
    vector<string> fieldValueRules = StringTokenizer::tokenize(StringView(ruleStr), FIELD_NAME_SEP);
    for (size_t i = 0; i < fieldValueRules.size(); i++) {
        vector<string> keyValue = StringTokenizer::tokenize(StringView(fieldValueRules[i]), KEY_VALUE_SEP);
        if (keyValue.size() != 2) {
            string errorMsg = "distribute_rule[" + ruleStr + "] is invalid";
            BS_LOG(ERROR, "%s", errorMsg.c_str());
            return false;
        }
        vector<string> ruleClusters = StringTokenizer::tokenize(StringView(keyValue[1]), CLUSTERS_SEP);
        for (size_t clusterId = 0; clusterId < ruleClusters.size(); ++clusterId) {
            if (find(param.clusterNames.begin(), param.clusterNames.end(), ruleClusters[clusterId]) ==
                param.clusterNames.end()) {
                string errorMsg =
                    "invalid cluster[" + ruleClusters[clusterId] + "] in distribute_rule[" + ruleStr + "]";
                BS_LOG(ERROR, "%s", errorMsg.c_str());
                return false;
            }
        }
    ...
    vector<string> defaultClusters = StringTokenizer::tokenize(StringView(defaultStr), CLUSTERS_SEP);
    ...
    _otherClusters = StringTokenizer::tokenize(StringView(otherStr), CLUSTERS_SEP);
    ...
    if (emptyStr.empty()) {
        ...
        _emptyClusters = StringTokenizer::tokenize(StringView(emptyStr), CLUSTERS_SEP);
    }
...
bool DistributeDocumentProcessor::process(const ExtendDocumentPtr& document)
{
    ...
    if (fittedClusters->size() > 0) {
        ...
        for (size_t i = 0; i < metas.size(); i++) {
            if (find(fittedClusters->begin(), fittedClusters->end(), metas[i].clusterName) != fittedClusters->end()) {
...
void DistributeDocumentProcessor::destroy() { delete this; }
