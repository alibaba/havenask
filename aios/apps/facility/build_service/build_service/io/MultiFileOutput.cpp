...
bool MultiFileOutput::init(const KeyValueMap& initParams)
{
    ...
    for (const auto& fileName : fileList) {
        if (StringUtil::endsWith(fileName, TEMP_MULTI_FILE_SUFFIX)) {
            FslibWrapper::DeleteFileE(FslibWrapper::JoinPath(_destDirectory, fileName), DeleteOption::NoFence(false));
            continue;
        }
...
indexlib::file_system::BufferedFileWriter* MultiFileOutput::createFile(const KeyValueMap& params)
{
    ...
    auto fileWriter = new indexlib::file_system::BufferedFileWriter(writerOption);
    if (auto ec = fileWriter->Open(filePath, filePath).Code(); ec != FSEC_OK) {
