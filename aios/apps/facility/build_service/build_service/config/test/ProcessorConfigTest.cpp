...
{
    string content;
    string filePath = GET_TEST_DATA_PATH() + "processor_config_test/full_processor_config.json";
    ASSERT_TRUE(fslib::util::FileUtil::readFile(filePath, content));
...
{
    string content;
    string filePath = GET_TEST_DATA_PATH() + "processor_config_test/batch_processor_config.json";
    ASSERT_TRUE(fslib::util::FileUtil::readFile(filePath, content));
    ProcessorConfig processorConfig;
    ASSERT_NO_THROW(FromJsonString(processorConfig, content));
    EXPECT_EQ(uint32_t(16), processorConfig.processorThreadNum);
