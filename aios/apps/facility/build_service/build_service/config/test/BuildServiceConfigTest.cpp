...
#define VALIDATE_EMPTY_FIELD(field)                                                                                    \
    ...
    VALIDATE_EMPTY_FIELD(_swiftRoot);
...
TEST_F(BuildServiceConfigTest, testGetCounterConfig)
{
    ...
    EXPECT_EQ(string("swift"), newConfig._swiftRoot);
    ...
    EXPECT_EQ(string("redis"), newCounterConfig.position);
    EXPECT_EQ(string("aaa.aaa.bbb.ccc"), newCounterConfig.params["hostname"]);
    EXPECT_EQ(string("99"), newCounterConfig.params["port"]);
    EXPECT_EQ(string("some-password"), newCounterConfig.params["password"]);
}
