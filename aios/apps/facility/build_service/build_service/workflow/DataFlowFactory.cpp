...
{
    BS_LOG(INFO, "create consumer, docType [%s], consumerType [%d]", docType == RAW_DOC ? "raw" : "processed",
           int(type));
    assert(factory);
    switch (docType) {
    case RAW_DOC:
...
DataFlowFactory::createFlowType(BuildFlowMode mode, const FlowFactory::RoleInitParam& param)
{
    ...
    if (getValueFromKeyValueMap(param.kvMap, INPUT_DOC_TYPE) == INPUT_DOC_RAW) {
                                                             ...
                                                             BuildFlowMode mode, FlowFactory* factory)
{
    ...
    for (size_t i = 0; i < producerType.size(); ++i) {
        ...
        if (i == 0 && producerType[i] == FlowFactory::ProducerType::TP_RAW) {
            docType = RAW_DOC;
        }
