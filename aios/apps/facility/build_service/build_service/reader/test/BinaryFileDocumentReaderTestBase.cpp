...
{
    ASSERT_TRUE(fslib::util::FileUtil::writeFile(_fileName, ""));
...
void BinaryFileDocumentReaderTestBase::testReadEmptyDoc()
{
    ...
    ASSERT_TRUE(fslib::util::FileUtil::writeFile(_fileName, emptyDocStr));
    FileDocumentReaderPtr reader = createFileDocumentReader();
    ASSERT_TRUE(reader->init(_fileName, 0));
    string docString;
...
void BinaryFileDocumentReaderTestBase::checkReadDocWithOffset(uint32_t docCount, uint32_t docSize)
{
    vector<string> docStringVect;
    vector<int64_t> offsetVect;
...
void BinaryFileDocumentReaderTestBase::internalCheckReadDoc(int64_t offset, int32_t startDocCount,
...
void BinaryFileDocumentReaderTestBase::makeVarLenBinaryDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip,
                                                                    vector<string>& docStringVect,
                                                                    vector<int64_t>& offsetVect)
{
    docStringVect.clear();
    ...
    makeOneVarLenDocumentFile(_fileName, docCount, contentLen, docStringVect, offsetVect);
    if (gzip) {
...
void BinaryFileDocumentReaderTestBase::makeFixLenBinaryDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip,
                                                                    vector<string>& docStringVect,
                                                                    vector<int64_t>& offsetVect)
{
    docStringVect.clear();
    ...
    makeOneFixLenDocumentFile(_fileName, docCount, contentLen, docStringVect, offsetVect);
    if (gzip) {
        string cmd = string("gzip -f ") + _fileName;
        (void)!(system(cmd.c_str()));
        _fileName = _fileName + ".gz";
...
void BinaryFileDocumentReaderTestBase::makeOneFixLenDocumentFile(const string& fileName, uint32_t docCount,
                                                                 uint32_t contentLen, vector<string>& docStringVect,
                                                                 vector<int64_t>& offsetVect)
{
    string docString;
    int64_t offset = 0;
    ...
    ASSERT_TRUE(fslib::util::FileUtil::writeFile(fileName, content));
}

void BinaryFileDocumentReaderTestBase::makeOneVarLenDocumentFile(const string& fileName, uint32_t docCount,
                                                                 uint32_t contentLen, vector<string>& docStringVect,
                                                                 vector<int64_t>& offsetVect)
{
    string docString;
...
void BinaryFileDocumentReaderTestBase::makeDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip,
                                                        vector<string>& docStringVect, vector<int64_t>& offsetVect)
{
    if (_isFixLen) {
        _fixLen = contentLen;
        makeFixLenBinaryDocumentFile(docCount, contentLen, gzip, docStringVect, offsetVect);
    } else {
        makeVarLenBinaryDocumentFile(docCount, contentLen, gzip, docStringVect, offsetVect);
    }
}
