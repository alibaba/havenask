...
{
    ASSERT_TRUE(fslib::util::FileUtil::writeFile(_fileName, _docPrefix + _docSuffix));
    FormatFileDocumentReader reader(_docPrefix, _docSuffix, _bufferSize);
    ASSERT_TRUE(reader.init(_fileName, 0));
    string docString;
...
{
    ASSERT_TRUE(fslib::util::FileUtil::writeFile(_fileName, _docPrefix + "abc" + _docSuffix));
    FormatFileDocumentReader reader(_docPrefix, _docSuffix, _bufferSize);
    ASSERT_TRUE(reader.init(_fileName, 0));
    string doc1, doc2;
...
void FileDocumentReaderTestBase::checkReadDocWithOffset(uint32_t docCount, uint32_t docSize)
{
    vector<string> docStringVect;
    vector<int64_t> offsetVect;
    ...
    internalCheckReadDoc(endWithSep, offsetVect[docCount - 1], docCount, docStringVect, offsetVect);
...
void FileDocumentReaderTestBase::internalCheckReadDoc(bool endWithSep, int64_t offset, int32_t startDocCount,
...
void FileDocumentReaderTestBase::makeDocumentFile(uint32_t docCount, uint32_t docContentPrefixLen, bool endWithSep,
                                                  bool gzip, vector<string>& docStringVect, vector<int64_t>& offsetVect)
{
    docStringVect.clear();
    offsetVect.clear();
    _fileName = _baseDir + "/FileDocumentReaderTestBase";
    makeOneDocumentFile(_fileName, docCount, docContentPrefixLen, endWithSep, docStringVect, offsetVect);
    if (gzip) {
...
void FileDocumentReaderTestBase::makeOneDocumentFile(const string& fileName, uint32_t docCount, uint32_t contentLen,
                                                     bool endWithSep, vector<string>& docStringVect,
                                                     vector<int64_t>& offsetVect)
{
    string docString;
