...
public:
    void makeDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip, std::vector<std::string>& docStringVect,
                          std::vector<int64_t>& offsetVect);

    void makeVarLenBinaryDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip,
                                      std::vector<std::string>& docStringVect, std::vector<int64_t>& offsetVect);

    void makeFixLenBinaryDocumentFile(uint32_t docCount, uint32_t contentLen, bool gzip,
                                      std::vector<std::string>& docStringVect, std::vector<int64_t>& offsetVect);

protected:
    void internalCheckReadDoc(int64_t offset, int32_t startDocCount, std::vector<std::string>& docStringVect,
                              std::vector<int64_t>& offsetVect);
    void checkReadDoc(uint32_t docCount, uint32_t docSize);
    void checkReadDocWithOffset(uint32_t docCount, uint32_t docSize);

    void makeOneFixLenDocumentFile(const std::string& fileName, uint32_t docCount, uint32_t contentLen,
                                   std::vector<std::string>& docStringVect, std::vector<int64_t>& offsetVect);

    void makeOneVarLenDocumentFile(const std::string& fileName, uint32_t docCount, uint32_t contentLen,
                                   std::vector<std::string>& docStringVect, std::vector<int64_t>& offsetVect);
