...
class ToolsConfig(object):
    ...
    def __getCmd(self, cmd):
        envCmd = ''
        if self.use_asan:
            asanOpt = " ASAN_OPTIONS=\"detect_odr_violation=0 detect_leaks=1 handle_segv=0 disable_coredump=0\" "
            envCmd += asanOpt
        if self.lib_path:
    ...
    def getHippoAdapter(self):
        return self.__getCmd(self.install_prefix + '/bin/bs_admin_starter')

    def getFsUtil(self):
        return self.__getCmd(self.install_prefix + '/bin/fs_util')
