...
class JsonWriter(object):
    ...
    def _appendSpace(self):
        spaceNum = self._spaceBase * self._position
    ...
    def write(self, obj, format=False, escaped_forward_slash=False):
        self._position = 0
        self._escaped_forward_slash = escaped_forward_slash
	self._format = format
        self._results = []
        self._write(obj)
	str = "".join(self._results)
	if (self._format):
	    return str.replace('\/', '/')
        else:
	    return str
...
def write(obj, format=False, escaped_forward_slash=False):
    return JsonWriter().write(obj, format, escaped_forward_slash)
