...
namespace navi {
class MemoryPoolRTest : public TESTBASE {
private:
    typedef std::shared_ptr<autil::mem_pool::Pool> PoolPtr;
public:
    MemoryPoolRTest() : _disablePoolModeAsan("naviPoolModeAsan", "0") {}
    void setUp() {}
    void tearDown(){}
private:
...
TEST_F(MemoryPoolRTest, testInitParam) {
    {
        MemoryPoolR poolRes;
...
TEST_F(MemoryPoolRTest, testTrimPool) {
...
TEST_F(MemoryPoolRTest, testLimitPool) {
    MemoryPoolR poolRes;
    poolRes._poolCacheSizeLimit = 10;
    vector<PoolPtr> poolPtrVec;
    for (int i = 0; i < 100; i++) {
        auto *pool = new autil::mem_pool::Pool(poolRes._poolChunkSize);
        pool->allocate(100);
...
TEST_F(MemoryPoolRTest, testAutoScale) {
...
TEST_F(MemoryPoolRTest, testRelease) {
    autil::EnvGuard _("naviPoolModeAsan", "0");
    auto poolRes = make_shared<MemoryPoolR>();
    {
        auto p1 = poolRes->getPoolPtr();
        auto p2 = poolRes->getPoolPtr();
        auto p3 = poolRes->getPoolPtr();
    }
}
