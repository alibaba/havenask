...
bool OutputPortGroup::postInit(const PartInfo &partInfo) {
    if (_readyMap) {
        return true;
    }
...
bool OutputPortGroup::postInit() {
    ...
    _readyMap = ReadyBitMap::createReadyBitMap(_outputs.size());
    _readyMap->setReady(true);
    _readyMap->setOptional(false);
    _readyMap->setFinish(false);
...
void OutputPortGroup::forceEof() const {
    for (const auto &pair : _outputs) {
        pair.second->forceEof();
    }
}
