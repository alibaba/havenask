...
bool Port::link(Port *downStream, const BorderConnectInfo &connectInfo) {
    ...
    if (!_dataQueues->init(_logger.logger,
                           connectInfo.toPartInfo,
                           connectInfo.eofPartInfo,
                           this,
                           downStream,
                           connectInfo.storeType,
...
ErrorCode Port::setEofAll() {
    NaviPartId dataSize = _partInfo.getUsedPartCount();
    auto fromPartId = _border->getPartId();
    DataPtr data;
    bool eof = true;
    for (NaviPartId index = 0; index < dataSize; index++) {
        ...
        if (PST_DATA == _dataQueues->getStoreType()) {
            bool retEof;
            setDataToData(partId, fromPartId, data, eof, retEof);
        } else {
            setDataToSerialize(partId, fromPartId, data, eof);
        }
    }
    return EC_NONE;
}
...
ErrorCode Port::setSerializeToData(const NaviPortData &data, bool &retEof) {
    ...
    if (!SerializeData::deserialize(_logger, data, _dataType, dataPtr)) {
                                   ...
                                   bool &eof)
{
    ...
    if (!SerializeData::deserialize(_logger, serializeData, _dataType, data)) {
        NAVI_LOG(ERROR, "deserialize failed");
        return EC_DESERIALIZE;
    }
    return EC_NONE;
