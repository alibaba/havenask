...
{
public:
    Edge(const NaviLoggerPtr &logger);
    ~Edge();
private:
    Edge(const Edge &);
...
public:
    ...
    EdgeOutputInfo getUpStreamOutputInfo() const;
    Edge *getDownStream(IndexType slotId) const;
...
public:
    ...
    Edge *getDownStream() const {
        return _edge->getDownStream(_slotId);
    }
    bool finished() const {
    ...
    void startUpStreamSchedule(const Node *callNode) const {
        _edge->startUpStreamSchedule(callNode);
    }
    Node *getInputNode() const {
        return _edge->getInputNode();
    }
private:
    Edge *_edge;
    IndexType _slotId;
