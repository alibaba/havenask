                           ...
                           const std::shared_ptr<ModuleManager> &moduleManager)
{
    ...
    if (!startNavi(gigConfigStr, moduleManager)) {
        std::cout << "start navi failed, server: " << _name << std::endl;
        return false;
    }
    return true;
}
        ...
        const std::shared_ptr<ModuleManager> &moduleManager)
{
    ...
    if (!navi->update(_loader, _configPath, loadParam, rootResourceMap, moduleManager)) {
        std::cout << "navi update failed, server: " << _name << ", navi: " << (void *)(long)navi->getInstanceId() << std::endl;
        return false;
    }
...
std::shared_ptr<ModuleManager> NaviTestServer::getModuleManager() const {
    if (_navi) {
        return _navi->getModuleManager();
    } else {
        return nullptr;
    }
}
...
bool NaviTestServer::createGigServer() {
    ...
    if (!gigServer->init(serverConfig)) {
        std::cout << "gig server init failed" << std::endl;
        return false;
    }
    {
        ...
        if (!gigServer->startArpcServer(desc)) {
            std::cout << "startArpcServer failed" << std::endl;
            return false;
        }
    ...
    }
    {
        ...
        if (!gigServer->startHttpArpcServer(desc)) {
            std::cout << "startHttpArpcServer failed" << std::endl;
            return false;
        }
