...
examples:
    ...
    def processSourceStatus(self, response):
        ...
        for generationInfo in response.generationInfos:
            dataDescriptions = json.loads(str(generationInfo.dataDescriptions))
    ...
    def printJobStatus(self, prefix, jobInfo, commandType):
        ...
        dataSourceStr = ''
        try:
            dataSourceMap = json.loads(str(jobInfo.dataDescription)) # jobInfo.dataDescription is typeof unicode
    ...
    def printProcessorStatus(self, prefix, processorInfo, commandType, processorTaskCount):
        ...
        dataSourceStr = ''
        try:
            dataSourceMap = json.loads(str(processorInfo.dataDescription)) # processorInfo.dataDescription is typeof unicode
            for k,v in dataSourceMap.items():
                dataSourceStr += '%s=%s;' % (k,v)
        except Exception, e:
            pass
