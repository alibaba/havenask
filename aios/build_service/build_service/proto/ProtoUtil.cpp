...
bool ProtoUtil::partitionIdToRoleId(const PartitionId &pid, string &roleId) {
    ...
    if (pid.role() == ROLE_BUILDER || pid.role() == ROLE_MERGER ||
        pid.role() == ROLE_JOB || pid.role() == ROLE_TASK) {
...
bool ProtoUtil::roleIdToPartitionId(const string &roleId,
                                    ...
                                    PartitionId &pid)
{
    ...
    uint16_t from, to;
    if (!StringUtil::fromString(strVec[4], from)
        || !StringUtil::fromString(strVec[5], to))
    {
        return false;
    }
    pid.mutable_range()->set_from(from);
    pid.mutable_range()->set_to(to);
    if (role == ROLE_BUILDER || role == ROLE_MERGER || role == ROLE_TASK) {
...
bool ProtoUtil::partitionIdToCounterFileName(const PartitionId &pid,
            string &counterFileName)
{
    ...
    common::ProcessorTaskIdentifier processorIdentifier;
...
string ProtoUtil::adminToRoleId(uint32_t generationId,
                                const string &datatable,
                                const string &clusterName)
{
    stringstream ss;
    ss << "admin." << datatable
       << "." << StringUtil::toString(generationId)
       << "." << clusterName;
    return ss.str();
}

string ProtoUtil::getRawDocTopicName(
        const string &prefix, const BuildId &buildId)
{
    return prefix + "_raw_" + buildId.datatable() + "_"
        + StringUtil::toString(buildId.generationid());
}

string ProtoUtil::getProcessedDocTopicName(
        const string &prefix, const BuildId &buildId,
        const string &clusterName)
{
    return prefix + "_processed_"
        + StringUtil::toString(buildId.generationid())
        + "_" + clusterName;
}
...
RoleType ProtoUtil::fromRoleString(const string &roleStr) {
    if (roleStr == "builder") {
        return ROLE_BUILDER;
    } else if (roleStr == "processor") {
        return ROLE_PROCESSOR;
    } else if (roleStr == "merger") {
        return ROLE_MERGER;
    } else if (roleStr == "job") {
        return ROLE_JOB;
    } else if (roleStr == "task") {
        return ROLE_TASK;
    } else {
        return ROLE_UNKNOWN;
    }
...
bool ProtoUtil::isEqual(const BuildId& left, const BuildId& right)
{
    ...
    if (left.has_datatable() && right.has_datatable()
        && left.datatable() != right.datatable()) {
        return false;
    }
    // check generationid
...
bool ProtoUtil::strToBuildId(const string& str, proto::BuildId& buildId)
{
    ...
    try {
        ...
        BS_LOG(ERROR, "buildId [%s] from json fail, reason [%s]",
               str.c_str(), e.what());
        return false;
    }
...
void ProtoUtil::partitionIdToBeeperTags(const proto::PartitionId& pid,
                                        beeper::EventTags &tags)
{
    ...
    tags.AddTag("range", StringUtil::toString(pid.range().from()) + "_" +
                 StringUtil::toString(pid.range().to()));
    if (pid.clusternames().size() > 0) {
        tags.AddTag("clusters", StringUtil::toString(
                        ProtoUtil::getClusterNames(pid), " "));
    }
    tags.AddTag("buildStep", ProtoUtil::toStepString(pid));
    tags.AddTag("roleType", ProtoUtil::toRoleString(pid.role()));
}

void ProtoUtil::buildIdToBeeperTags(const proto::BuildId& buildId,
                                    beeper::EventTags &tags)
{
