...
BuildId ProtoCreator::createBuildId(const string &dataTable,
...
PartitionId ProtoCreator::createPartitionId(
        RoleType role, BuildStep step,
        uint16_t from, uint16_t to,
        const string &dataTable,
        generationid_t generationId,
        const string &clusterNames,
        ...
        const std::string &appName)
{
    ...
    *partitionId.mutable_buildid() = createBuildId(dataTable, generationId, appName);
    vector<string> clusters = StringTokenizer::tokenize(ConstString(clusterNames), ",");
...
vector<PartitionId> ProtoCreator::createPartitionIds(
        uint32_t partCount, uint32_t parallelNum, 
        RoleType role, BuildStep step,
        uint16_t from, uint16_t to,
        ...
        const string &clusterNames, const string &mergeConfigName)
{
    ...
    for (size_t i = 0; i < ranges.size(); i++) {
        pids.push_back(createPartitionId(
                        role, step, ranges[i].from(), 
                        ranges[i].to(), dataTable, gid, 
                        clusterNames, mergeConfigName));
    }
    return pids;
}
