...
void ErrorCollector::addErrorInfo(
    ServiceErrorCode errorCode, const string &errorMsg, ErrorAdvice errorAdvice) const {
...
void ErrorCollector::fillErrorInfos(
        vector<ErrorInfo> &errorInfos) const
{
    autil::ScopedLock lock(_mutex);
...
void ErrorCollector::fillErrorInfos(
        deque<ErrorInfo> &errorInfos) const
{
    autil::ScopedLock lock(_mutex);
    errorInfos.insert(errorInfos.end(), _errorInfos.begin(), _errorInfos.end());
}

ErrorInfo ErrorCollector::makeErrorInfo(
        ServiceErrorCode errorCode, const string &errorMsg, ErrorAdvice errorAdvice) const
{
    ErrorInfo errorInfo;
...
void ErrorCollector::setBeeperCollector(const string& id)
{
    _beeperCollector = id;
}
