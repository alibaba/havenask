...
bool JobConfig::init(const KeyValueMap &kvMap) {
    ...
    if (zkRoot.empty()) {
        string errorMsg = "zookeeper root is empty!";
        BS_LOG(WARN, "%s", errorMsg.c_str());
    }
    initBuildRuleConfig(kvMap);

    _buildPartitionFrom = getValueWithDefault<int32_t>(kvMap,
            BUILD_PARTITION_FROM, 0);
    _buildPartitionCount = getValueWithDefault<int32_t>(kvMap,
            BUILD_PARTITION_COUNT, _config.partitionCount - _buildPartitionFrom);
    _generationId = getValueWithDefault<generationid_t>(kvMap,
            GENERATION_ID, INVALID_GENERATIONID);
    ...
    if (_config.partitionCount < _buildPartitionFrom + _buildPartitionCount) {
        ...
        ss << "Invalid buildPartitionFrom and buildPartitionCount,"
           << "buildPartitionFrom[" << _buildPartitionFrom
           << "]+buildPartitionCount[" << _buildPartitionCount
           << "] should not greater than partitionCount["
...
Range JobConfig::calculateBuildMapRange(uint32_t instanceId) const {
    ...
    vector<Range> rangeVec = RangeUtil::splitRange(
            buildParts[partId].from(), buildParts[partId].to(), _config.mapReduceRatio);
...
vector<int32_t> JobConfig::calcMapToReduceTable() const {
    return calcMapToReduceTable(_buildPartitionFrom, _buildPartitionCount,
                                _config.partitionCount, _config.buildParallelNum);
}
...
vector<Range> JobConfig::splitBuildParts() const {
    ...
    for (size_t i = 0; i < rangeVec.size(); ++i) {
        vector<Range> onePart = RangeUtil::splitRange(
                rangeVec[i].from(), rangeVec[i].to(), _config.buildParallelNum);
        parallelRangeVec.insert(parallelRangeVec.end(), onePart.begin(),
                                onePart.end());
    }
...
vector<int32_t> JobConfig::calcMapToReduceTable(
        ...
        int32_t partitionCount, int32_t buildParallelNum)
{
    ...
    vector<Range> partRangeVec = RangeUtil::splitRange(
            RANGE_MIN, RANGE_MAX, partitionCount);
    partRangeVec.assign(partRangeVec.begin() + buildPartFrom,
                        partRangeVec.begin() + buildPartFrom + buildPartCount);
    vector<Range> reducePartRangeVec;
    for (size_t i = 0; i < partRangeVec.size(); ++i) {
        vector<Range> parallelPartRanges = RangeUtil::splitRange(
                partRangeVec[i].from(), partRangeVec[i].to(), buildParallelNum);
        reducePartRangeVec.insert(reducePartRangeVec.end(),
                parallelPartRanges.begin(), parallelPartRanges.end());
    }
    vector<int32_t> table(RANGE_MAX + 1, -1);
    for (size_t reduceId = 0; reduceId < reducePartRangeVec.size(); ++reduceId) {
        for (uint32_t hashId = reducePartRangeVec[reduceId].from();
             hashId <= reducePartRangeVec[reduceId].to(); ++hashId)
        {
...
template <typename T>
T JobConfig::getValueWithDefault(const KeyValueMap &kvMap,
                                 ...
                                 const T &defaultValue)
{
    ...
    if (!StringUtil::fromString(valueStr.c_str(), value)) {
        string errorMsg = "Invalid " + keyName + "["
                          + valueStr + "], must be integer.";
        BS_LOG(WARN, "%s", errorMsg.c_str());
        return defaultValue;
    }
    return value;
