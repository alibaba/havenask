...
protected:
    ExtendDocumentPtr createExtendDocument(int64_t timestamp,
            const std::string &modifiedFields,
            const std::string &subModifiedFields = "");

    // void checkIndexDocument(const IE_NAMESPACE(document)::DocumentPtr& indexDoc,
...
ExtendDocumentPtr ExtendDocumentTest::createExtendDocument(int64_t timestamp,
        const string &modifiedFields, const string &subModifiedFields)
{
    ...
    if (!modifiedFields.empty()) {
        ...
        doc->getIndexExtendDoc()->setModifiedFieldSet(destModifiedFields);
    }
    ...
    if (!subModifiedFields.empty()) {
        ...
        set<fieldid_t> destModifiedFields(inputModifiedFields.begin(), inputModifiedFields.end());
        doc->getIndexExtendDoc()->setSubModifiedFieldSet(destModifiedFields);
    }
