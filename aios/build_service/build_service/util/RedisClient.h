#ifndef ISEARCH_BS_REDISCLIENT_H
#define ISEARCH_BS_REDISCLIENT_H
...
struct RedisInitParam {
    ...
    std::string toString() const {
        return "hostname : " + hostname + ", port:" +
            autil::StringUtil::toString(port) +
            ", pass:" + password + ".";
    }
...
public:
    ...
    int setHash(const std::string &key, const std::string &fieldName,
                const std::string &value, ErrorCode &errorCode);

    std::string getHash(const std::string &key, const std::string &fieldName,
                               ErrorCode &errorCode);
...
#endif //ISEARCH_BS_REDISCLIENT_H
