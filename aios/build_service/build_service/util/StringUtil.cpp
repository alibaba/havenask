...
int32_t StringUtil::boldLineStr(const vector<string> &segments, const char *src, int32_t src_len, char *dest,
    ...
    while (*p) {
        ...
        if (' ' != *p) {
            int i = 0;
            for (vector<string>::const_iterator it = segments.begin(); it != segments.end() && i < ISEARCH_BS_MAX_BOLD_TERMS;
                 ++it, ++i) {
        ...
        if (!match) {
            if (need_begin) {
                SAFE_COPY(dest + index, ISEARCH_BS_BOLD_BEGIN, dest_len - index);
                index += strlen(ISEARCH_BS_BOLD_BEGIN);
                need_begin = false;
                need_end = true;
            }
            dest[index++] = *p++;
            ...
            if (need_end) {
                SAFE_COPY(dest + index, ISEARCH_BS_BOLD_END, dest_len - index);
                index += strlen(ISEARCH_BS_BOLD_END);
                need_end = false;
                need_begin = true;
            }
