...
private:
    ...
    struct Slot {
        T data;
        size_t size = 0;
        Slot(T &&item, size_t sz)
            : data(std::forward<T>(item)), size(sz) {}
        Slot(T item, size_t sz)
            : data(std::forward<T>(item)), size(sz) {}        
    };
...
public:
    MemControlStreamQueue(uint32_t queueSize = DEFAULT_QUEUE_SIZE,
...
public:
    ...
    void push(C &&item, size_t itemSize) {
        ...
        while (_itemCount.load(std::memory_order_relaxed) >=
                   _capacity.load(std::memory_order_relaxed) ||
               _usedMemory.load(std::memory_order_relaxed) + itemSize >
                   _memoryLimit.load(std::memory_order_relaxed)) {
