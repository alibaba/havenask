...
bool FileDocumentReader::init(const string& fileName, int64_t offset) {
    ...
    if (fileType == FileReaderBase::GZIP_TYPE) {
        _fReader.reset(new GZipFileReader(_bufferSize));
    } else {
        _fReader.reset(new FileReader);
    }
    assert(_fReader != NULL);
    if (!_fReader->init(fileName, offset)) {
        stringstream ss;
        ss << "failed to init freader with fileName[" << fileName
           << "], offset[" << offset << "]";
...
void FileDocumentReader::readUntilPos(
        char *pos, uint32_t sepLen, string &docStr) {
...
void FileDocumentReader::readUntilPos(
        char *pos, uint32_t sepLen, char* &buffer) {
