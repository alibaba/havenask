...
public:
    ...
    void testSeek();
public:
    void makeOneDocumentFile(const std::string& fileName,
                             uint32_t docCount, uint32_t docContentPrefixLen,
                             bool endWithSep,
                             std::vector<std::string>& docStringVect,
                             std::vector<int64_t>& offsetVect);
protected:
    void makeDocumentFile(uint32_t docCount, uint32_t docContentPrefixLen,
                          bool endWithSep, bool gzip,
                          std::vector<std::string>& docStringVect,
                          std::vector<int64_t>& offsetVect);
