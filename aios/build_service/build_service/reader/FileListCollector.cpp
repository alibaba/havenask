...
bool FileListCollector::collect(const KeyValueMap &kvMap,
...
bool FileListCollector::kvMap2Description(
        ...
        CollectorDescription &description)
{
    ...
    description.filePattern =
        getValueFromKeyValueMap(kvMap, RAW_DOCUMENT_FILE_PATTERN, "");
    return true;
}

bool FileListCollector::getBSRawFileMetaDir(
    ...
    std::string& metaDir)
{
    ...
    metaDir = IndexPathConstructor::getGenerationIndexPath(
        indexRoot, clusterNames[0], generationId);
    return true;
}

bool FileListCollector::parseRawDocumentPaths(
        vector<string> &paths, const string &rawDocumentDir)
{
    vector<string> pathList = StringUtil::split(rawDocumentDir,
            RAW_DOCUMENT_PATH_SEPERATOR, true);
    for (size_t i = 0; i < pathList.size(); ++i) {
        StringUtil::trim(pathList[i]);
        bool exist = false;
        if (!FileUtil::isExist(pathList[i], exist) || !exist) {
...
bool FileListCollector::collectAllResults(
    ...
    CollectResults &result)
{
    ...
    for (size_t i = 0; i < rawDocPaths.size(); i++) {
        ...
        FilePatterns filePatterns = FilePatternParser::parse(
                desc.filePattern, rawDocPaths[i]);
...
bool FileListCollector::getCollectResultsFromFile(
    const KeyValueMap & kvMap, CollectResults &result)
{
    ...
    string metaFile = FileUtil::joinFilePath(metaDir, BS_RAW_FILES_META);
    string content;
    if (!FileUtil::readFile(metaFile, content))
    {
...
bool FileListCollector::collect(
    const KeyValueMap & kvMap,
...
void FileListCollector::calculateHashIds(
...
void FileListCollector::calculateFileIds(
        const vector<FilePatterns> &filePatternsVec,
        const vector<CollectResults> &resultsVec,
        CollectResults &allResults)
{
    FilePatterns allPatterns;
    for (size_t i = 0; i < filePatternsVec.size(); i++) {
        allPatterns.insert(allPatterns.end(),
                           filePatternsVec[i].begin(),
                           filePatternsVec[i].end());
    }
...
bool FileListCollector::getAllCandidateFiles(const string &path, fslib::FileList &result) {
    ...
    if (!FileUtil::isFile(path, fileExist)) {
    ...
    if (fileExist) {
        ...
        return FileUtil::listDirRecursiveWithAbsolutePath(path, result, true);
    }
}
