...
using IE_NAMESPACE(util)::TaskScheduler;
using IE_NAMESPACE(util)::TaskItem;
using IE_NAMESPACE(util)::TaskItemPtr;
...
bool RealtimeBuilderImplBase::prepareIntervalTask(
        ...
        MetricProviderPtr metricProvider)
{
    ...
    if (!resourceReader->getConfigWithJsonPath(relativePath,
                    "build_option_config.max_recover_time", _maxRecoverTime))
    {
        string errorMsg = "fail to read maxRecoverTime from config path "
                          + relativePath;
    ...
    if (_supportForceSeek)
    {
        if (!resourceReader->getConfigWithJsonPath(
                relativePath,
                "build_option_config.seek_to_latest_when_recover_fail",
                _seekToLatestWhenRecoverFail))
        {
            string errorMsg = "fail to read seek_to_latest_when_recover_fail from config path "
                + relativePath;
            BS_PREFIX_LOG(ERROR, "%s", errorMsg.c_str());
            return false;
        }
    ...
    if (resourceReader->getConfigWithJsonPath(relativePath,
                    "build_option_config.recover_delay_tolerance_ms", latencyInMs))
    {
        if (latencyInMs < 0)
        {
            BS_PREFIX_LOG(ERROR, "invalid recover latency [%ld], "
...
void RealtimeBuilderImplBase::skipRtBeforeTimestamp() {
    ...
    Locator locatorToJump;
    locatorToJump.setSrc(latestLocator.getSrc());
    locatorToJump.setOffset(_timestampToSkip);
...
bool RealtimeBuilderImplBase::getLatestLocator(Locator &latestLocator, bool &fromInc) {
    ...
    int64_t seekTs = OnlineJoinPolicy::GetRtSeekTimestamp(
        incVersionTime, rtLocator.getOffset(), fromInc);
    
    if (fromInc) {
        latestLocator.setSrc(rtLocator.getSrc());
        latestLocator.setOffset(seekTs);
        BS_INTERVAL_LOG2(120, INFO, "incVersionTime[%ld], rt locator[%s], inc covers rt",
                         incVersionTime, rtLocator.toDebugString().c_str());
    } else {
...
void RealtimeBuilderImplBase::checkRecoverBuild() {
    ...
    if (producerReadTimestamp + _recoverLatency >= maxTimestamp) {
        _isRecovered = true;
        BS_PREFIX_LOG(INFO, "realtime build recover succeed.locator offset in producer:%ld,"
                      "maxtimestamp in producer: %ld", producerReadTimestamp, maxTimestamp);
    }
}
...
void RealtimeBuilderImplBase::setTimestampToSkip(int64_t timestamp) {
    ...
    if(_timestampToSkip != timestamp) {
        BS_PREFIX_LOG(INFO, "set timestampToSkip for [%s] as [%ld]", _partitionId.ShortDebugString().c_str(), timestamp);
...
bool RealtimeBuilderImplBase::isRecovered() const {
    autil::ScopedLock lock(_realtimeMutex);
    return _isRecovered;
}

void RealtimeBuilderImplBase::getErrorInfo(RealtimeErrorCode &errorCode,
                                       string &errorMsg,
...
bool RealtimeBuilderImplBase::needAutoSuspend() {
    ...
    IndexPartition::MemoryStatus memStatus =
        _indexPartition->CheckMemoryStatus();
...
BuildFlow *RealtimeBuilderImplBase::createBuildFlow(
    const IE_NAMESPACE(partition)::IndexPartitionPtr &indexPartition,
    const SwiftClientCreatorPtr& swiftClientCreator) const
{
    return new BuildFlow(swiftClientCreator, indexPartition,
                         _buildFlowThreadResource);
}

void RealtimeBuilderImplBase::setErrorInfoUnsafe(
        RealtimeErrorCode errorCode, const string &errorMsg)
{
