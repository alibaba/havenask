...
bool DocumentClusterFilterProcessor::init(const DocProcessorInitParam &param)
{
    ...
    if (filterRule.empty()) {
        BS_LOG(ERROR, "[%s] is empty in DocumentClusterFilterProcessor",
               FILTER_RULE.c_str());
        return false;
    }
...
bool DocumentClusterFilterProcessor::parseRule(const string& ruleStr)
{
    ...
    StringTokenizer st(ruleStr, " ", StringTokenizer::TOKEN_IGNORE_EMPTY
...
DocumentProcessor* DocumentClusterFilterProcessor::clone()
{
    return new DocumentClusterFilterProcessor(*this);
}
