                                  ...
                                  const ProcessorChainSelectorConfigPtr &chainSelectorConfig)
{
    ...
    if (!chainSelectorConfig
        || chainSelectorConfig->selectFields.empty()
        || chainSelectorConfig->selectRules.empty())
    {
    ...
    for (size_t i = 0; i < chainConfigVec->size(); i++)
    {
        ...
        if (chainName.empty())
        {
            BS_LOG(ERROR, "init ProcessorChainSelector failed, "
                   "chainName of chain[%zu] is empty.", i);
            return false;
        }
        if (chainNameToChainIdMap.find(chainName) != chainNameToChainIdMap.end())
        { 
            BS_LOG(ERROR, "init ProcessorChainSelector failed, "
                   "chainName[%s] is duplicated.", chainName.c_str());
            return false;
        }
        chainNameToChainIdMap.insert(make_pair(chainName, (uint16_t)i));
    }
    _selectFields.insert(_selectFields.end(),
                         chainSelectorConfig->selectFields.begin(),
    ...
    for (; it != chainSelectorConfig->selectRules.end(); it++)
    {
        ...
        for (auto chainName : rule.destChains)
        {
            ...
            {
                BS_LOG(INFO, "dest chain[%s] will not take effect for SelectRule[%d].",
                       chainName.c_str(), idx);
                continue;
            }
        ...
        BS_LOG(INFO, "init chain selector[%s] to chain[%s]",
               chainKey.c_str(), chainNames.c_str());
        idx++;
    }
    return true;
}

const ProcessorChainSelector::ChainIdVector* ProcessorChainSelector::selectChain(
        const RawDocumentPtr &rawDoc) const
{
    // when user does not config ProcessorChainSelector
    if (alwaysSelectAllChains()) {
        return &_allChainIds;
    }
    ...
    if (chainIds == _chainsMap.end()) {
        ERROR_COLLECTOR_LOG(INFO, "cant not find chains for document[%s], fieldValue[%s]",
                            rawDoc->toString().c_str(), chainKey.c_str());
        return NULL;
    }
