...
SortedBuilder::SortedBuilder(const IndexBuilderPtr &indexBuilder,
...
SortedBuilder::~SortedBuilder() {
    stop();
}

bool SortedBuilder::initBuilder(const BuilderConfig &builderConfig,
...
bool SortedBuilder::init(const BuilderConfig &builderConfig,
                         IE_NAMESPACE(misc)::MetricProviderPtr metricProvider)
{
    ...
    const IndexPartitionSchemaPtr &schema =  
        _indexBuilder->GetIndexPartition()->GetSchema();
    ...
    _running = true;
    _batchBuildThreadPtr = Thread::createThread(
        std::tr1::bind(&SortedBuilder::buildThread, this), "BsBatchBuild");
...
bool SortedBuilder::reserveMemoryQuota(
...
void SortedBuilder::initConfig(const BuilderConfig &builderConfig) {
    ...
    BS_PREFIX_LOG(INFO, "sortQueueMem [%ld MB], sortQueueSize [%u]",
                  _sortQueueMem, _sortQueueSize);
}

bool SortedBuilder::initSortContainers(const BuilderConfig &builderConfig,
                                       const IndexPartitionSchemaPtr &schema)
{
...
bool SortedBuilder::initOneContainer(const BuilderConfig &builderConfig,
                                     const IndexPartitionSchemaPtr &schema,
                                     SortDocumentContainer &container)
{
    common::Locator locator = common::INVALID_LOCATOR;
...
void SortedBuilder::stop(int64_t stopTimestamp) {
    ...
    BEEPER_REPORT(INDEXLIB_BUILD_INFO_COLLECTOR_NAME,
            "SortedBuilder flush sortQueue before stop.");
    flush();
...
void SortedBuilder::flushUnsafe() {
    ...
    BEEPER_FORMAT_REPORT_WITHOUT_TAGS(INDEXLIB_BUILD_INFO_COLLECTOR_NAME,
            ...
            "docCount [%ld] memoryUse [%ld]", endSortTs - beginSortTs,
            _collectContainer.allDocCount(), _collectContainer.memUse());
...
int64_t SortedBuilder::calculateSortQueueMemory(
        const BuilderConfig &builderConfig,
        size_t buildTotalMemMB)
{
