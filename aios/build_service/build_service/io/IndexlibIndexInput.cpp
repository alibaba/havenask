...
IndexlibIndexInput::IndexlibIndexInput(
    const config::TaskInputConfig& inputConfig)
    : Input(inputConfig)
{
}
...
bool IndexlibIndexInput::getIndexPath(
    const KeyValueMap& params, std::string& indexPath) {
    ...
    indexPath = IndexPathConstructor::constructIndexPath(
        indexRoot, clusterName, generationId, rangeFrom, rangeTo);
    return true;
}
...
bool IndexlibIndexInput::init(const KeyValueMap& params) {
    ...
    if (optionsStr != EMPTY_STRING) {
        try {
            ...
            string errorMsg = "parse index_partition_options fail[" +
                optionsStr + "] failed.";
    ...
    try {
        _provider = PartitionResourceProviderFactory::GetInstance()->CreateProvider(
            indexPath, options, pluginPath);
        if (!_provider) {
