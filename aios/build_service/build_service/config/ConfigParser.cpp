...
bool ConfigParser::parseRelativePath(const string &relativePath, JsonMap &jsonMap) const {

    string jsonFile = FileUtil::joinFilePath(_basePath, relativePath);
    string content;
    auto it = _fileMap.find(relativePath);
    if (it != _fileMap.end()) {
        content = it->second;
    } else if (!FileUtil::readFile(jsonFile, content)) {
        string errorMsg = "read file " + jsonFile + " failed";
         ...
         it != jsonMap.end(); it++)
    {
        ...
        if (any.GetType() == typeid(JsonMap)) {
            ...
            if (!merge(childJsonMap, isExternal)) {
                string errorMsg = "merge child " + key + " fail";
                BS_LOG(ERROR, "%s", errorMsg.c_str());
                return false;
            }
        }
