...
TEST_F(ProcessorConfiguratorTest, testAutoConf) {
    {
        ...
        ResourceReaderPtr resourceReader(new ResourceReader(TEST_DATA_PATH"/resource_reader_test/processorConfig"));

        PartitionId pid = ProtoCreator::createPartitionId(ROLE_PROCESSOR,
                BUILD_STEP_FULL, 0, 65535, "simple", 0, "cluster0", "", "1");
        ...
        ASSERT_TRUE(configurator.getSwiftWriterMaxBufferSize(resourceReader, pid, "cluster0", ds, maxBufferSize));
        EXPECT_EQ((uint64_t)(8192ll * 1024 * 1024 *
                             SwiftTopicConfig::DEFAULT_BULDER_MEM_TO_PROCESSOR_BUFFER_FACTOR / 64),
                  maxBufferSize);

        // test buffer size calculation is ineffective when topic mode is not memory_prefer
        maxBufferSize = 0;
        ASSERT_TRUE(configurator.getSwiftWriterMaxBufferSize(resourceReader, pid, "cluster1", ds, maxBufferSize));
    ...
    }
    {
        ...
        PartitionId pid = ProtoCreator::createPartitionId(ROLE_PROCESSOR,
                BUILD_STEP_FULL, 0, 65535, "simple", 0, "cluster3", "", "1");
        ...
        uint64_t maxBufferSize = 0;
        ASSERT_TRUE(configurator.getSwiftWriterMaxBufferSize(resourceReader, pid, "cluster3", ds, maxBufferSize));
    ...
    }
    {
        ...
        PartitionId pid = ProtoCreator::createPartitionId(ROLE_PROCESSOR,
                BUILD_STEP_INC, 0, 65535, "simple", 0, "cluster3", "", "1");
        ...
        // test buffer size calculation is ineffective when topic mode is not memory_prefer
        maxBufferSize = 0;
        ASSERT_TRUE(configurator.getSwiftWriterMaxBufferSize(resourceReader, pid, "cluster4", ds, maxBufferSize));
...
{
    {
        ...
        PartitionId pid = ProtoCreator::createPartitionId(ROLE_PROCESSOR,
                BUILD_STEP_FULL, 0, 65535, "simple", 0, "cluster2", "", "1");
        ...
        uint64_t maxBufferSize = 0;
        ASSERT_TRUE(configurator.getSwiftWriterMaxBufferSize(resourceReader, pid, "cluster2", ds, maxBufferSize));
    ...
    }
    {
        ...
        PartitionId pid = ProtoCreator::createPartitionId(ROLE_PROCESSOR,
                BUILD_STEP_FULL, 0, 65535, "simple", 0, "cluster4", "", "1");
...
TEST_F(ProcessorConfiguratorTest, testGetSchemaUpdatableClusterConfigVec) {
...

    {
        ...
        bool isExist;
        ASSERT_TRUE(configurator.getSchemaUpdatableClusterConfigVec(
                        resourceReader, "simple3", configVec, isExist));
        ASSERT_TRUE(isExist);
    ...
    // test empty cluster name
    {
        SchemaUpdatableClusterConfigVec configVec;
        bool isExist;
        ASSERT_FALSE(configurator.getSchemaUpdatableClusterConfigVec(
                        resourceReader, "simple4", configVec, isExist));
        ASSERT_TRUE(isExist);
        ASSERT_EQ(1, configVec.size());
        ASSERT_EQ(3, configVec[0].partitionCount);
    ...
    // test invalid cluster name
    {
        ...
        bool isExist;
        ASSERT_FALSE(configurator.getSchemaUpdatableClusterConfigVec(
                        resourceReader, "simple5", configVec, isExist));
        ASSERT_TRUE(isExist);
        ASSERT_EQ(1, configVec.size());
        ASSERT_EQ(3, configVec[0].partitionCount);
...
TEST_F(ProcessorConfiguratorTest, testGetSchemaUpdatableClusterConfig) {
...

    {
        ...
        ASSERT_TRUE(configurator.getSchemaUpdatableClusterConfig(
                        resourceReader, "simple", "cluster0", config));
...

    {
        ...
        SchemaUpdatableClusterConfig config;
        ASSERT_TRUE(configurator.getSchemaUpdatableClusterConfig(
                        resourceReader, "multi_cluster", "cluster3", config));
        ASSERT_EQ(3, config.partitionCount);
        ASSERT_EQ("cluster3", config.clusterName);

        ASSERT_FALSE(configurator.getSchemaUpdatableClusterConfig(
                        resourceReader, "multi_cluster", "cluster1", config));
    }
}
...
TEST_F(ProcessorConfiguratorTest, testGetSchemaUpdatableClusters) {
...

    {
        ...
        ASSERT_TRUE(configurator.getSchemaUpdatableClusters(
                        resourceReader, "multi_cluster", clusterNames));
...
TEST_F(ProcessorConfiguratorTest, testGetAllClusters) {
...

    {
        //multi cluster
        vector<string> clusterNames;
        ASSERT_TRUE(configurator.getAllClusters(
                        resourceReader, "multi_cluster", clusterNames));
...
TEST_F(ProcessorConfiguratorTest, testGetSchemaNotUpdatableClusters) {
...

    {
        //multi cluster
        vector<string> clusterNames;
        ASSERT_TRUE(configurator.getSchemaNotUpdatableClusters(
                        resourceReader, "multi_cluster", clusterNames));
        ASSERT_EQ(1, clusterNames.size());
        ASSERT_EQ("cluster1", clusterNames[0]);
    }
