...
public:
    ...
    void tearDown();
protected:
    static std::string readTestFileContent(const std::string& file);
};
...
TEST_F(DocProcessorChainConfigTest, testInvalidFormat) {
    DocProcessorChainConfig config;
    EXPECT_THROW(FromJsonString(config, "invalid json string"),
...
TEST_F(DocProcessorChainConfigTest, testEmptySections) {
    {
        ...
        ASSERT_FALSE(fileContent.empty());
        DocProcessorChainConfig config;
        EXPECT_THROW(FromJsonString(config, fileContent),
...
string DocProcessorChainConfigTest::readTestFileContent(const string& file) {
    string fileContent;
    string filePath = TEST_DATA_PATH"doc_processor_config_test/" + file;
