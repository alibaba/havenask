...
public:
    ...
    virtual int32_t getFieldPosition(const std::string &indexName, const std::string &fieldname) const = 0;
    virtual std::vector<std::string> getFieldList(const std::string &indexName) const = 0;
};

typedef std::shared_ptr<IndexInfoHelper> IndexInfoHelperPtr;
