...
public:
    ...
    ~ANetRPCMessageCodec();
public:
    anet::Packet *EncodeRequest(const CodecContext *context,
                                version_t version) const;
    CodecContext *DecodeRequest(anet::Packet *packet, const RPCServer *rpcServer,
                                version_t version) const;
    anet::Packet *EncodeResponse(RPCMessage *response, Tracer *tracer,
                                 version_t version,
                                 const std::shared_ptr<google::protobuf::Arena> &arena) const;
protected:
    anet::Packet *createPacket(uint32_t pcode,
                               anet::DataBufferSerializable *serializable) const;
private:
