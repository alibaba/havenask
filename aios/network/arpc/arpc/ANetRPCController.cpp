...
void ANetRPCController::setProtoArena(const std::shared_ptr<google::protobuf::Arena> &arena) {
    _arena = arena;
}

bool ANetRPCController::Failed() const
{
    return _failed;
}

string ANetRPCController::ErrorText() const
{
    return _reason;
}
...
bool ANetRPCController::IsCanceled() const
{
    return _canceled;
}

void ANetRPCController::NotifyOnCancel(RPCClosure *callback)
{
    _cancelList.push_back (callback);
}

void ANetRPCController::SetErrorCode(ErrorCode errorCode)
{
    _errorCode = errorCode;
}

ErrorCode ANetRPCController::GetErrorCode()
{
    return _errorCode;
}

int ANetRPCController::GetExpireTime()
{
    return _expireTime;
}
 ...
 */
void ANetRPCController::SetExpireTime(int milliseconds)
{
    _expireTime = milliseconds;
}

void ANetRPCController::SetClientAddress(const string &clientAddress)
{
    _clientAddress = clientAddress;
}

std::string &ANetRPCController::GetClientAddress()
{
    return _clientAddress;
}

void ANetRPCController::SetTraceFlag(bool flag)
{
    _tracer.SetTraceFlag(flag);
}

bool ANetRPCController::GetTraceFlag() const
{
    return _tracer.GetTraceFlag();
}

Tracer &ANetRPCController::GetTracer()
{
    return _tracer;
}

const Tracer &ANetRPCController::GetTracer() const
{
    return _tracer;
}

const TraceInfo *ANetRPCController::GetTraceInfo() const
{
    return _traceInfo;
}

void ANetRPCController::SetTraceInfo(TraceInfo *traceInfo)
{
...
void ANetRPCController::PrintTraceInfo()
{
    ...
    if (cnt % 100 == 0) {
        ARPC_LOG(INFO, "serverQueueSize: %ld\nhandleRequestTime: %ld\nworkItemWaitProcessTime: %ld\nworkItemProcessTime: %ld\nrequestOnServerTime: %ld\nrtt: %ld",
                 serverQueueSize / cnt, handleRequestTime / cnt, workItemWaitProcessTime / cnt, workItemProcessTime / cnt, requestOnServerTime / cnt, rtt / cnt);
    }
}
