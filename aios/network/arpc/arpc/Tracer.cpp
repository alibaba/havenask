...
TraceInfo *Tracer::ExtractServerTraceInfo(google::protobuf::Arena *arena) const
{
    ...
    traceInfo->set_handlerequesttime(
        calcTime(_beginHandleRequest, _endHandleRequest));
    traceInfo->set_workitemwaitprocesstime(
        calcTime(_endHandleRequest, _beginWorkItemProcess));
    traceInfo->set_workitemprocesstime(
        calcTime(_beginWorkItemProcess, _beginPostResponse));
    traceInfo->set_requestonservertime(
        calcTime(_beginHandleRequest, _beginPostResponse));
