...
public:
     ...
     */
    virtual Connection *createConnection(IPacketStreamer *streamer, IServerAdapter *adapter = NULL) { return NULL; }
    ...
    inline void updateUseTime(int64_t now) { _lastUseTime = now; }
     ...
     * 是否连接状态, 包括正在连接状态
     */
    inline bool isConnectState() { return getState() < ANET_CLOSING; }

    inline bool isClosed() { return getState() >= ANET_CLOSING; }
     ...
     * 得到重连标志
     */
    inline bool isAutoReconn() { return (_autoReconn && !_isServer); }

    /***
     * Is this IOComponent referenced by read/write thread of transport
     */
    bool isReferencedByReadWriteThread() { return _refByRreadWriteThread; }

    /***
     * Set _refByRreadWriteThread flag
     */
    inline void referencedByReadWriteThread(bool flag) { _refByRreadWriteThread = flag; }

    virtual void lock();
    virtual void unlock();

    inline IOCState getState() { return _state; }
    ...
    virtual void dump(std::ostringstream &buf) {
        ...
        buf << "------IOC State------\n";
        buf << "IOC Address: " << this << "\t"
            << "Owner transport: " << _owner << "\t"
            << "Socket: " << _socket << std::endl;
        buf << "IOC State: " << _state << "\t"
            << "Ref Count: " << atomic_read(&_refcount) << "\t"
            << "Last Use: " << _lastUseTime << "-" << std::string(timestr);
        buf << "Enable Read: " << _enableRead << "\t"
            << "Enable Write: " << _enableWrite << "\t"
            << "Ref By RW Thread: " << _refByRreadWriteThread << std::endl;
        buf << "Max Idle Time: " << _maxIdleTimeInMicroseconds << "us" << std::endl;
        buf << "Prev IOC: " << _prev << "\t"
            << "Next IOC: " << _next << std::endl;
    ...
    IocType getType() const { return _type; }
