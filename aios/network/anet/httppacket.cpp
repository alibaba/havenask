...
HTTPPacket::~HTTPPacket() {
    ...
    if (_reasonPhrase)
        ::free(_reasonPhrase);
    for (HeadersType::iterator it = _headers.begin(); it != _headers.end(); it++) {
        ::free(it->first);
        ::free(it->second);
    }
}
...
HTTPPacket::HTTPPacketType HTTPPacket::getPacketType() { return _type; }
...
HTTPPacket::HTTPVersion HTTPPacket::getVersion() { return _version; }
...
HTTPPacket::HTTPMethod HTTPPacket::getMethod() { return _method; }
...
const char *HTTPPacket::getURI() { return _URI; }
...
bool HTTPPacket::removeHeader(const char *key) {
    if (NULL == key)
        return false;
    HeadersType::iterator it = _headers.find((char *)key);
    char *oldKey = NULL;
    char *oldValue = NULL;
    if (it != _headers.end()) {
        oldKey = it->first;
...
int HTTPPacket::getStatusCode() { return _statusCode; }
...
void HTTPPacket::disableContentLengthHeader() { _addContentLengthHeader = false; }
...
bool HTTPPacket::encode(DataBuffer *output) {
    return (encodeStartLine(output) && encodeHeaders(output) && encodeBody(output));
}
...
bool HTTPPacket::encodeHeaders(DataBuffer *output) {
    ...
    if (_addContentLengthHeader && (NULL == getHeader("Content-Length"))) {
...
int64_t HTTPPacket::getSpaceUsed() { return _headerLen + sizeof(HTTPPacket) + getBodyLen(); }

HTTPPacket::ConstHeaderIterator HTTPPacket::headerBegin() { return _headers.begin(); }

HTTPPacket::ConstHeaderIterator HTTPPacket::headerEnd() { return _headers.end(); }
