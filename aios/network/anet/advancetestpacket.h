...
{
public:
    AdvanceTestPacket() { randomEncodeError = randomDecodeError = false; nBits = 0;}
...
bool AdvanceTestPacket<T>::encode(DataBuffer *output) 
{
    ...
    if (rc && randomEncodeError) {
        ...
        if (dataLen > 0) offset = sizeof(AdvancePacketHeader) + getRandInt(dataLen) - 4;
        ...
        ANET_LOG(INFO, "Going to update %ld offset of packet data from %d to %d", (char *)p - output->getData(), *p32, v);
