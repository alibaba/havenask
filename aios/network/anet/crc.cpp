 ...
 */
/* Content of this file comes majorly from apsara/common/error_detection/crc32c.cpp */
...
enum {
    ANET_CRC32C_LOOKUP = 0,
    ANET_CRC32C_INTEL = 1,
    ANET_CRC32C_AARCH64 = 2
};
static const char *CrcFuncStr[] = {"CRC32C_Lookup", "CRC32C_Intel", "CRC32C_Aarch64"};
...
uint32_t DoCrc32c_Lookup(uint32_t crc, const uint8_t *data, uint64_t length) {
    while (length--) {
        crc = Crc32cTable[(*data++ ^ crc) & 0xFF] ^ (crc >> 8);
    }

    return crc;
}

static inline uint32_t DoCrc32c_Intel_Byte(uint32_t crc, const uint8_t *data, uint32_t length) {
    while (length--) {
        asm volatile("crc32b %[data], %[crc]\n\t" : [crc] "=r"(crc) : "0"(crc), [data] "r"(*data));
        data++;
    }
...
uint32_t DoCrc32c_Intel(uint32_t crc, const uint8_t *data, uint64_t length) {
    unsigned int quotient = length / 8;
    unsigned int remainder = length % 8;
    const uint64_t *p = (const uint64_t *)data;
    uint64_t crc8 = crc;

    while (quotient--) {
        asm volatile("crc32q %[data], %[crc];" : [crc] "=r"(crc8) : "0"(crc8), [data] "r"(*p));
        p++;
    }
    crc = (uint32_t)crc8;
    if (remainder) {
        crc = DoCrc32c_Intel_Byte(crc, (const uint8_t *)p, remainder);
    }
...
uint32_t DoCrc32c_Aarch64(uint32_t crc, const uint8_t *data, uint64_t len) {
...
#define CRC32CB(crc, value) __asm__("crc32cb %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(value))
    ...
    for (; length > (int64_t)(8 * sizeof(uint64_t)); length -= 8 * sizeof(uint64_t)) {
...
#if __x86_64__
    return IsSSE42Supported() ? &DoCrc32c_Intel : &DoCrc32c_Lookup;
#elif __aarch64__
    return &DoCrc32c_Aarch64;
#endif
}
...
#if __x86_64__
    if (IsSSE42Supported()) {
        return CrcFuncStr[ANET_CRC32C_INTEL];
    } else {
        return CrcFuncStr[ANET_CRC32C_LOOKUP];
    }
#elif __aarch64__
    return CrcFuncStr[ANET_CRC32C_AARCH64];
#endif
}
