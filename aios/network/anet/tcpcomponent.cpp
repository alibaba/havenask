...
RECONNErr TCPComponent::reconnect() {
    ...
    if (_autoReconn) {
        ...
        char tmp[32];
        ANET_LOG(INFO, "connection is being/already established while manual connect is invoked: "
                       "ioc state: %d, target: %s. ", getState(), _socket->getAddr(tmp,32));
    ...
    if (!_socket->connect()) {
        ...
        if (error != EINPROGRESS && error != EWOULDBLOCK) {
            ...
            ANET_LOG(ERROR, "failed to connect to %s, %s(%d)",
                     _socket->getAddr(tmp, 32), errStr, error);
            return false;
        }
    }
...
bool TCPComponent::handleWriteEvent() {
    ...
    if (getState() == ANET_CONNECTING) {
        ...
        if (now - _startConnectTime > 1000000) {
            char spec[32];
            ANET_LOG(WARN, "slow connect to %s, cost time:%d us",
                _socket->getAddr(spec, 32), (int)(now - _startConnectTime));
        }
        setState(ANET_CONNECTED);
...
bool TCPComponent::checkTimeout(int64_t now) {
    ...
    if (getState() == ANET_CONNECTING) {
        if (_startConnectTime > 0
            && _startConnectTime < (now - flags::getConnectTimeout())) {
        ...
        if (idle > _maxIdleTimeInMicroseconds) {
            _autoReconn = 0;
            closeAndSetState();
            ANET_LOG(WARN, "Closing idle connection. %s (idle time: %ld secs IOC:%p).",
...
bool TCPComponent::socketReconnect(int64_t now) {
    ...
    if (socketConnect()) {
        ...
        ANET_LOG(ERROR, "can not reconnect to target, ioc %p, _autoReconn %d!",
                        this, _autoReconn);
        rc = false;
    }
...
void TCPComponent::dump(std::ostringstream &buf) {
    ...
    buf <<  "Type: TCPComponent" << "\t" << "Start Conn: "
        << _startConnectTime << "-" << std::string(timebuf);
