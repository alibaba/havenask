...
void VIPDomainConfig::Jsonize(autil::legacy::Jsonizable::JsonWrapper &json) {
    ...
    json.Jsonize("grpc_stream_port_diff", _grpcStreamPortDiff, _grpcStreamPortDiff);
...
bool VIPDomainConfig::operator==(const VIPDomainConfig &rhs) const {
    return _vipDomain == rhs._vipDomain && _vipPortType == rhs._vipPortType &&
           _tcpPortDiff == rhs._tcpPortDiff && _arpcPortDiff == rhs._arpcPortDiff &&
           _httpPortDiff == rhs._httpPortDiff && _grpcPortDiff == rhs._grpcPortDiff &&
           _grpcStreamPortDiff == rhs._grpcStreamPortDiff &&
           _rdmaArpcPortDiff == rhs._rdmaArpcPortDiff;
}

void SubscribeClustersConfig::Jsonize(autil::legacy::Jsonizable::JsonWrapper &json) {
    json.Jsonize("cm2_clusters", cm2Clusters, cm2Clusters);
    json.Jsonize("cm2_no_topoinfo_clusters", cm2NoTopoClusters, cm2NoTopoClusters);
    json.Jsonize("vip_domains", vipDomains, vipDomains);
    json.Jsonize("istio_biz_clusters", istioBizClusters, istioBizClusters);
}

bool SubscribeClustersConfig::operator==(const SubscribeClustersConfig &rhs) const {
    return cm2Clusters == rhs.cm2Clusters && cm2NoTopoClusters == cm2NoTopoClusters &&
           vipDomains == rhs.vipDomains && istioBizClusters == rhs.istioBizClusters;
}
...
bool SubscribeClustersConfig::checkSub(bool cm2Sub, bool vipSub, bool istioSub) const {
    bool check = true;
    if (!cm2Sub && hasCm2Sub()) {
        AUTIL_LOG(ERROR, "cm2 clusters not dealed, cm2Clusters:%s, cm2NoTopoClusters:%s",
                  StringUtil::toString(cm2Clusters).c_str(),
                  StringUtil::toString(cm2NoTopoClusters).c_str());
        check = false;
    }
    if (!vipSub && hasVipSub()) {
        AUTIL_LOG(ERROR, "vip domains not dealed:%s", ToJsonString(vipDomains).c_str());
        check = false;
    }
