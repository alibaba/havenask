...
GrpcServerWorker::GrpcServerWorker(GigRpcServer &owner)
    : _owner(owner), _randomGenerator(0), _callBackThreadPool(nullptr) {}
...
int32_t GrpcServerWorker::getPort() const { return _listenPort; }
...
bool GrpcServerWorker::init(const GrpcServerDescription &desc) {
    ...
    if (!secureConfig.pemRootCerts.empty()) {
        ...
        _builder.AddListeningPort(spec, grpc::SslServerCredentials(option),
                                  &_listenPort);
    } else {
        _builder.AddListeningPort(spec, grpc::InsecureServerCredentials(),
                                  &_listenPort);
    }
    ...
    if (!_server) {
        AUTIL_LOG(ERROR, "start grpc server failed, port [%s], threadNum [%lu]",
                  port.c_str(), threadNum);
        return false;
    } else {
        for (size_t i = 0; i < _completionQueues.size(); i++) {
            _workThreads.push_back(autil::Thread::createThread(
                std::bind(&GrpcServerWorker::workLoop, this, i,
                          _completionQueues[i]),
                "grpc_server_io"));
        }
    }
    _desc = desc;
    AUTIL_LOG(INFO, "start grpc server success, port [%s], threadNum [%lu]",
              port.c_str(), threadNum);
    return true;
}
...
GrpcServerWorker::getSslOptions(const SecureConfig &secureConfig) const {
    ...
    option.client_certificate_request =
...
void GrpcServerWorker::workLoop(size_t idx,
                                ServerCompletionQueueStatusPtr cqsPtr) {
    auto &cqs = *cqsPtr;
    auto &cq = *cqs.cq;
    AUTIL_LOG(INFO, "server work loop started, worker [%p] idx [%lu] cq [%p]",
              this, idx, &cq);
    {
        GrpcServerStreamHandlerPtr handler(
            new GrpcServerStreamHandler(this, cqsPtr));
    ...
    AUTIL_LOG(INFO, "server work loop stopped, worker [%p] idx [%lu] cq [%p]",
              this, idx, &cq);
}
