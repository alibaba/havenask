...
#define DEFINE_METRIC(kMonitor, metric, metricName, metricType, metricLevel,   \
                      metricTags)                                              \
    ({                                                                         \
        metric = NULL;                                                         \
        ...
        std::call_once(once_##metric, [&]() {                                  \
            if ((kMonitor)) {                                                  \
                kMonitor->Register(gigMetricName, kmonitor::metricType,        \
        ...
        if ((kMonitor && metricTags)) {                                        \
            metric = (kMonitor)->DeclareMetric(                                \
                gigMetricName,                                                 \
                const_cast<kmonitor::MetricsTags *>(metricTags.get()));        \
            if (!metric) {                                                     \
...
#define DEFINE_MUTABLE_METRIC(kMonitor, metric, metricName, metricType,        \
                              metricLevel)                                     \
    {                                                                          \
        metric = NULL;                                                         \
        std::string gigMetricName = std::string("gig.") + metricName;          \
        if (kMonitor) {                                                        \
            metric.reset(kMonitor->RegisterMetric(                             \
                gigMetricName, kmonitor::metricType, kmonitor::metricLevel));  \
        }                                                                      \
    }
