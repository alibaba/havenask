...
CallBack::CallBack(const SearchServiceResourcePtr &resource,
                   const CallerPtr &caller, bool isRetry)
    : _resource(resource), _caller(caller), _isRetry(isRetry) {}
...
void CallBack::run(void *responseData, MultiCallErrorCode ec,
                   const string &errorString, const string &responseInfo) {
    ...
    if (_span) {
        ...
        _span->setStatus(ec <= MULTI_CALL_ERROR_DEC_WEIGHT
    ...
    if (!isFailed) {
        ...
        reply->updateDetectInfo(_resource->getBizName(),
                                _resource->retryEnabled());
    }
