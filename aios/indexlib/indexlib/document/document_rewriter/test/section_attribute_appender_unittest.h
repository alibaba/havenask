...
private:
    void InnerTestAppendSectionAttribute(const std::string& despStr,
					 const std::string& indexFilter,
					 bool isUniqCompress,
					 bool hasSectionWeight,
					 bool hasFieldId);

    void ResetSchema();
    void OverwriteSchema(const config::IndexPartitionSchemaPtr& schema,
			 const std::string& indexFilter,
			 bool isUniqCompress,
			 bool hasSectionWeight,
			 bool hasFieldId);
    ...
    void InnerCheckDocument(const document::IndexDocumentPtr& indexDocument,
			    const std::string& despStr);

    void CheckSectionAttribute(const autil::ConstString& sectionAttr,
			       const config::PackageIndexConfigPtr& packIndexConfig,
			       const std::string& despStr);

    void CheckUnpackSectionAttribute(section_len_t* lengthBuf, section_fid_t* fidBuf,
				     section_weight_t* weightBuf, uint32_t sectionCount,
				     const std::vector<int>& sectionCountsPerField,
				     const config::PackageIndexConfigPtr& packIndexConfig);
