...
void PackAttributeRewriterTest::TestRewriteCheckPackField()
{
    ...
    IndexPartitionSchemaPtr fixSchema = SchemaMaker::MakeKKVSchema(
            field, "pkey", "skey", "value");
    IndexPartitionSchemaPtr varSchema = SchemaMaker::MakeKKVSchema(
            field, "pkey", "skey", "value;pkey;");
    string docStr = "cmd=add,pkey=pkey1,skey=1,value=1,ts=101;";

    Pool pool;
    string str(6, 'a');
    StringAttributeConvertor convertor;
    ConstString newValue = convertor.Encode(ConstString(str), &pool);
    
    {
        // fix 
        NormalDocumentPtr doc = DocumentCreator::CreateDocument(fixSchema, docStr);
    ...
    {
        // var
        NormalDocumentPtr doc = DocumentCreator::CreateDocument(varSchema, docStr);
        PackAttributeRewriter rewriter;
    ...
    bool mainSchemaHasPack, bool subSchemaHasPack)
{
    ...
    else
    {
        mainSchema = SchemaLoader::LoadSchema(
            string(TEST_DATA_PATH)+"pack_attribute/",
            "main_schema_with_nopack.json");
    }
    
    IndexPartitionSchemaPtr subSchema;
    if (subSchemaHasPack)
    {
        subSchema = SchemaLoader::LoadSchema(
            string(TEST_DATA_PATH)+"pack_attribute/",
    ...
    else
    {
        subSchema = SchemaLoader::LoadSchema(
            string(TEST_DATA_PATH)+"pack_attribute/",
            "sub_schema_with_nopack.json");
    }
    mainSchema->SetSubIndexPartitionSchema(subSchema);
    return mainSchema;
}

void PackAttributeRewriterTest::InnerTestRewrite(
    bool mainSchemaHasPack, bool subSchemaHasPack)
{
    IndexPartitionSchemaPtr schema =
        CreateSchema(mainSchemaHasPack, subSchemaHasPack);
    NormalDocumentPtr doc = DocumentCreator::CreateDocument(schema, mDocStr);
    PackAttributeRewriter rewriter;
