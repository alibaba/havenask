...
void AttributeDocumentFieldExtractorTest::TestSimpleProcess()
{
    ...
    NormalDocumentPtr doc = DocumentCreator::CreateDocument(
                    schema,
                    "cmd=add,string1=hello,attr1=11,attr2=12 20,"
    ...
    ConstString fieldValue = extractor->GetField(attrDoc, 0, pool);
    EXPECT_EQ(ConstString::EMPTY_STRING, fieldValue);
...
void AttributeDocumentFieldExtractorTest::TestExceptionCase()
{
    ...
    IndexPartitionSchemaPtr schema = 
        SchemaMaker::MakeSchema(fields, index, attribute, summary);

    AttributeSchemaPtr attrSchema = schema->GetAttributeSchema();

    NormalDocumentPtr doc = DocumentCreator::CreateDocument(
                    schema, "cmd=add,string1=hello,attr1=11,attr4=14 24,attr5=15,attr6=hello,"
                    "modify_fields=attr1#attr2#attr3#attr4#attr5#attr6");
    ...
    EXPECT_EQ(ConstString::EMPTY_STRING, extractor->GetField(emptyAttrDoc, 1, &mPool));
    EXPECT_TRUE(ConstString::EMPTY_STRING != extractor->GetField(attrDoc, 1, &mPool));    
    attrDoc->ClearFields({1});
    EXPECT_EQ(ConstString::EMPTY_STRING, extractor->GetField(attrDoc, 1, &mPool));
    // get field which is no in AttributeSchema
    EXPECT_EQ(ConstString::EMPTY_STRING, extractor->GetField(attrDoc, 6, &mPool));
    // get field which is non-exist
    EXPECT_EQ(ConstString::EMPTY_STRING, extractor->GetField(attrDoc, 7, &mPool)); 
}

// expectValues.size() == 0, represents expect EMPTY_STRING
// FieldType support ft_int32 & ft_string only
void AttributeDocumentFieldExtractorTest::CheckValue(
        const AttributeDocumentFieldExtractorPtr& extractor, const AttributeSchemaPtr& attrSchema,
        const AttributeDocumentPtr& attrDoc, fieldid_t fieldId, FieldType fieldType,
        const vector<string>& expectValues)
{
    if (expectValues.size() == 0)
    {
        EXPECT_EQ(ConstString::EMPTY_STRING, extractor->GetField(attrDoc, fieldId, &mPool));
        return;
    }
    ...
    ConstString fieldValue = extractor->GetField(attrDoc, fieldId, &mPool);
