...
private:
    ...
    void MakeFieldString(const string& str, SectionVector& sectionVec, uint32_t maxSectionLen)
    {
        ...
        StringTokenizer st(str, ";", autil::StringTokenizer::TOKEN_TRIM 
                           | autil::StringTokenizer::TOKEN_IGNORE_EMPTY);
        for (size_t i = 0; i < st.getNumTokens(); ++i)
        {
            TokenVector tokenVec;
            StringTokenizer st2(st[i], ",", autil::StringTokenizer::TOKEN_TRIM 
                    | autil::StringTokenizer::TOKEN_IGNORE_EMPTY);
    ...
    {
        for (SectionVector::const_iterator it = sectionVec.begin();
             it != sectionVec.end(); ++it)
        {
            section_weight_t weight = (*it).first;
            const vector<string>& tokenVec = (*it).second;

            builder.BeginSection(weight);
            for (TokenVector::const_iterator it2 = tokenVec.begin();
                 it2 != tokenVec.end(); ++it2)
            {
                if (*it2 == "N")
                {
                ...
                else 
                {
                    DefaultHasher hasher;
                    uint64_t hashKey;
                    hasher.GetHashKey(it2->c_str(), hashKey);
                    INDEXLIB_TEST_TRUE(
                            builder.AddToken(hashKey, weight % 5));
                }
            }
            builder.EndSection();
        }
        ...
        for (size_t i = 0; i < sectionVec.size(); ++i)
        {
            ...
            for (size_t j = 0; j < tokenVec.size(); ++j)
            {
                if (tokenVec[j] != "N")
                {
                    ...
                    INDEXLIB_TEST_EQUAL((pos_t)(totalTokens + j + sectionGap) - pos, 
                            token->GetPosIncrement());
