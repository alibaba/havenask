...
#define TEST_SUMMARY_DOCUMENT_SERIALIZE_EQUAL(summary_doc1, summary_doc2) \
    do {                                                                \
        INDEXLIB_TEST_EQUAL(summary_doc1.mFields.size(),                \
                            summary_doc2.mFields.size());               \
        for(size_t fieldidx = 0; fieldidx < summary_doc1.mFields.size(); fieldidx++) \
        {                                                               \
            INDEXLIB_TEST_EQUAL(summary_doc1.mFields[fieldidx],         \
                    summary_doc2.mFields[fieldidx]);                    \
        }                                                               \
    } while(0)

#define TEST_ATTRIBUTE_DOCUMENT_SERIALIZE_EQUAL(attr_doc1, attr_doc2) \
    do {                                                                \
        INDEXLIB_TEST_EQUAL(attr_doc1.mNormalAttrDoc.mFields.size(),    \
                            attr_doc2.mNormalAttrDoc.mFields.size());   \
        for(size_t fieldidx = 0; fieldidx < attr_doc1.mNormalAttrDoc.mFields.size(); fieldidx++) \
        {                                                               \
            INDEXLIB_TEST_EQUAL(attr_doc1.mNormalAttrDoc.mFields[fieldidx], \
                    attr_doc2.mNormalAttrDoc.mFields[fieldidx]);                    \
        }                                                               \
        INDEXLIB_TEST_EQUAL(attr_doc1.mPackFields.size(), attr_doc2.mPackFields.size()); \
        for (size_t i = 0; i < attr_doc1.mPackFields.size(); ++i)\
        {                                                        \
            INDEXLIB_TEST_EQUAL(attr_doc1.mPackFields[i],        \
                                attr_doc2.mPackFields[i]);       \
        }                                                        \
    } while(0)
...
#define TEST_INDEX_DOCUMENT_SERIALIZE_EQUAL(index_doc1, index_doc2)     \
    do {                                                                \
        ...
        {                                                               \
            if (fields1[fieldidx] == nullptr && fields2[fieldidx] == nullptr) continue; \
            INDEXLIB_TEST_TRUE(*fields1[fieldidx] == *fields2[fieldidx]); \
	}								\
	INDEXLIB_TEST_EQUAL(index_doc1.GetMaxIndexIdInSectionAttribute(), index_doc2.GetMaxIndexIdInSectionAttribute()); \
...
#define TEST_DOCUMENT_SERIALIZE_EQUAL(doc1, doc2)                       \
    do {                                                                \
        TEST_INDEX_DOCUMENT_SERIALIZE_EQUAL((*doc1.GetIndexDocument()), \
                (*doc2.GetIndexDocument()));                            \
        TEST_ATTRIBUTE_DOCUMENT_SERIALIZE_EQUAL((*doc1.GetAttributeDocument()), \
                (*doc2.GetAttributeDocument()));                        \
        TEST_SUMMARY_SERIALIZED_DOCUMENT_SERIALIZE_EQUAL((*doc1.GetSummaryDocument()), \
                (*doc2.GetSummaryDocument()));                          \
