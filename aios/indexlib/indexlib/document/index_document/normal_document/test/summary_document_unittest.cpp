...
public:
    ...
    void TestCaseSerializeSummaryDocument()
    {
        ...
        for(size_t i = 0; i < summaryDocuments1.size(); i++)
        {
            TEST_SUMMARY_DOCUMENT_SERIALIZE_EQUAL((*summaryDocuments1[i]),
                    (*summaryDocuments2[i]));
        }
    }
    ...
    {
        autil::ConstString expectValue = autil::ConstString("abc", &mPool);
        ...
        fieldid_t fieldId = 0;
        const autil::ConstString& value = iter.Next(fieldId);
    ...
    void GenerateSummaryDocuments(size_t num, vector<SummaryDocumentPtr>& documents)
    {
        for(size_t i = 0; i < num; ++i)
        {
            ...
            for (size_t j = 0; j < i; ++j)
            {
                ...
                if (j % 2 == 0)
                {
                    ptr->SetField(j % 16, autil::ConstString(ss.str(), &mPool));
                }
            }
