...
bool SingleDocumentParser::Init(const IndexPartitionSchemaPtr& schema,
                                AccumulativeCounterPtr& attrConvertErrorCounter)
{
    mSchema = schema;
    ...
    {
        ERROR_COLLECTOR_LOG(ERROR, "init SingleDocumentParser fail, "
...
NormalDocumentPtr SingleDocumentParser::Parse(const IndexlibExtendDocumentPtr& document)
{
    ...
    if (rawDoc->getDocOperateType() == SKIP_DOC ||
    ...
    if (rawDoc->getDocOperateType() == DELETE_DOC ||
        rawDoc->getDocOperateType() == DELETE_SUB_DOC)
    {
        ...
        if (attrSchema && attrSchema->IsInAttribute(fieldId)) {
            if (rawDoc->getDocOperateType() == UPDATE_FIELD) {
                ...
                if (attributeConfig->IsAttributeUpdatable() &&
                    rawDoc->exist(attributeConfig->GetAttrName()) &&
...
void SingleDocumentParser::SetPrimaryKeyField(
...
{
    const IndexlibExtendDocument::FieldIdSet &modifiedFieldSet =
        document->getModifiedFieldSet();
    for (IndexlibExtendDocument::FieldIdSet::const_iterator it =
             modifiedFieldSet.begin(); it != modifiedFieldSet.end(); ++it)
    {
        indexDoc->AddModifiedField(*it);
    }

    const IndexlibExtendDocument::FieldIdSet &subModifiedFieldSet =
        document->getSubModifiedFieldSet();
    for (IndexlibExtendDocument::FieldIdSet::const_iterator it =
             subModifiedFieldSet.begin(); it != subModifiedFieldSet.end(); ++it) {
...
bool SingleDocumentParser::Validate(const IndexlibExtendDocumentPtr &document)
{
    ...
    if (mHasPrimaryKey && classifiedDocument->getPrimaryKey().empty()) {
        IE_LOG(WARN, "primary key is empty");
