#include "indexlib/common/field_format/date/test/date_term_unittest.h"
...
IE_LOG_SETUP(common, DateTermTest);
...
void DateTermTest::CaseSetUp()
{
}

void DateTermTest::CaseTearDown()
{
}

pair<uint64_t, uint64_t> DateTermTest::ParseTime(uint64_t left, uint64_t right,
                                                 DateLevelFormat format)
{
    TimestampUtil::Date leftDate = TimestampUtil::ConvertToDate(left); 
    TimestampUtil::Date rightDate = TimestampUtil::ConvertToDate(right);
...
void DateTermTest::TestSimpleProcess()
{
    ...
    {
        DateLevelFormat format;
        format.Init(3415, DateLevelFormat::MILLISECOND); //no middle level
        // from 2014-11-22 00:05:12 to 2014-11-22 02:01:52        
        tie(leftDateTerm, rightDateTerm) = ParseTime(1416614712000, 1416621712000, format);
        vector<uint64_t> terms;
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
        vector<uint64_t> expectTerms;
        ...
        sort(expectTerms.begin(), expectTerms.end());
    ...
    {
        DateLevelFormat format;
        format.Init(3415, DateLevelFormat::SECOND); //no middle level
        // from 2014-11-22 00:05:12 to 2016-07-27 12:15:11
        tie(leftDateTerm, rightDateTerm) = ParseTime(1416614712000, 1469621711000, format);
        vector<uint64_t> terms;
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
        vector<uint64_t> expectTerms;
        DateTermMaker::MakeDateTerms(44, 11, 22, 0, 5, 12, 59, expectTerms);
...
void DateTermTest::TestCalculateTerms()
{
    ...
    }
    {
        ...
        DateTermMaker::MakeDateTerms(44, 11, 22, 1, 1, expectTerms);
        sort(expectTerms.begin(), expectTerms.end());
        tie(leftDateTerm, rightDateTerm) = ParseTime(1416614760000LL, 1416621720000LL, format);
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
        sort(terms.begin(), terms.end());
        //ASSERT_EQ(expectTerms.size(), terms.size());
        ASSERT_EQ(expectTerms, terms);
    }
    {
        ...
        sort(expectTerms.begin(), expectTerms.end());
        tie(leftDateTerm, rightDateTerm) = ParseTime(1516076340000LL,1516096320000LL, format);
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
        sort(terms.begin(), terms.end());
        ASSERT_EQ(expectTerms, terms);
    }
    {
        format.Init(3415, DateLevelFormat::DAY);
        ...
        tie(leftDateTerm, rightDateTerm) = ParseTime(1504137600000LL, 1514678400000LL, format);
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
        sort(terms.begin(), terms.end());
        sort(expectTerms.begin(), expectTerms.end());
    ...
    }
    {
        ...
        vector<uint64_t> expectTerms, terms;
        tie(leftDateTerm, rightDateTerm) = ParseTime(1416948000123LL, 1516939889000LL, format);
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
    ...
    {
        vector<uint64_t> terms;
        tie(leftDateTerm, rightDateTerm) = ParseTime(1504137600000LL, 1500678400000LL, format);
        DateTerm::CalculateTerms(leftDateTerm, rightDateTerm, format, terms);
