...
IE_LOG_SETUP(common, LineIntersectOperator);

LineIntersectOperator::LineIntersectOperator()
{
}
...
LineIntersectOperator::ComputeIntersect(const Point& p1, const Point& p2,
...
LineIntersectOperator::IntersectType
LineIntersectOperator::ComputeCollinearIntersect(
        ...
        IntersectVertics& intersectVertics)
{
    ...
    if (p1q1p2 && q1p1q2) {
        return q1 == p1 ? POINT_INTERSECTION : COLLINEAR_INTERSECTION;
    }
    if (p1q1p2 && q1p2q2) {
        return q1 == p2 ? POINT_INTERSECTION : COLLINEAR_INTERSECTION;
    }
    if (p1q2p2 && q1p1q2) {
        return q2 == p1 ? POINT_INTERSECTION : COLLINEAR_INTERSECTION;
    }
    if (p1q2p2 && q1p2q2) {
        return q2 == p2 ? POINT_INTERSECTION : COLLINEAR_INTERSECTION;
    }
    return NO_INTERSECTION;
}

int LineIntersectOperator::SignOfDet2x2(double x1, double y1,
                                        double x2, double y2)
{
...
bool LineIntersectOperator::IntersectBoudingBox(
        const Point& p1, const Point& p2,
        const Point& q1, const Point& q2)
{
    double minq = min(q1.GetX(), q2.GetX());
    double maxq = max(q1.GetX(), q2.GetX());
    double minp = min(p1.GetX(), p2.GetX());
    double maxp = max(p1.GetX(), p2.GetX());
    ...
    minq = min(q1.GetY(), q2.GetY());
    maxq = max(q1.GetY(), q2.GetY());
    minp = min(p1.GetY(), p2.GetY());
    maxp = max(p1.GetY(), p2.GetY());
...
int LineIntersectOperator::OrientationIndex(const Point& p1,
        const Point& p2, const Point& q)
{
...
bool LineIntersectOperator::IntersectEnvelope(
        const Point& p1, const Point& p2, const Point& q)
{
