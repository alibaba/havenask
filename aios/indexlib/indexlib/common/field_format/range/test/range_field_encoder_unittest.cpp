#include "indexlib/common/field_format/range/test/range_field_encoder_unittest.h"
...
IE_LOG_SETUP(common, RangeFieldEncoderTest);
...
void RangeFieldEncoderTest::TestTopLevel()
{
    ...
    AssertEncode(0, left + 65536, Ranges(),
                 {MakeRange(17592186044416L, 17592186044416L, 4),
                         MakeRange(0, 0, 15)});
}
...
void RangeFieldEncoderTest::TestBigNumber()
{
    ...
    AssertEncode(leftNumber, maxNumber - 1, {Range(leftNumber, maxNumber - 1)},
                 Ranges());
    AssertEncode(leftNumber, maxNumber, Ranges(),
                 {MakeRange(maxNumber >> 4, maxNumber >> 4, 1)});
    leftNumber = maxNumber - 65536 - 255;
    AssertEncode(leftNumber, maxNumber, Ranges(),
                 {MakeRange(leftNumber >> 8, leftNumber >> 8, 2),
                         MakeRange(maxNumber >> 16, maxNumber >> 16, 4)});
    ...
    AssertEncode(leftNumber, rightNumber, Ranges(),
                 {MakeRange(leftNumber >> 4, leftNumber >> 4, 1),
                         MakeRange(rightNumber >> 12, rightNumber >> 12, 3),
                         MakeRange(maxSeg, maxSeg, 4)});
}

RangeFieldEncoderTest::Range RangeFieldEncoderTest::MakeRange(
        uint64_t leftTerm, uint64_t rightTerm, size_t level)
{
    return Range(leftTerm | level << 60, rightTerm | level << 60);
}

void RangeFieldEncoderTest::AssertEncode(
        uint64_t leftTerm, uint64_t rightTerm,
        const Ranges& bottomLevelRanges, const Ranges& higherLevelRanges)
{
    Ranges bottom;
    Ranges higher;
