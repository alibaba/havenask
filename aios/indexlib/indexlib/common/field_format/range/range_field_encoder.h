...
{
public:
    RangeFieldEncoder(const config::IndexPartitionSchemaPtr& schema);
    ~RangeFieldEncoder();
public:
    enum RangeFieldType
    {
    ...
    bool NormalizeToUInt64(
            fieldid_t fieldId, const std::string& fieldValue, uint64_t& value);
    static RangeFieldType GetRangeFieldType(FieldType fieldType);
    static void ConvertTerm(const common::Int64Term* term, RangeFieldType fieldType,
                            uint64_t& leftTerm, uint64_t& rightTerm);
    static void CalculateSearchRange(uint64_t leftTerm, uint64_t rightTerm,
            Ranges& bottomLevelRanges, Ranges& higherLevelRanges);
    static void AlignedTerm(bool alignLeft, bool alignRight,
                             uint64_t& leftTerm, uint64_t& rightTerm);
...
//////////////////////////////////////////////////////////////
inline void RangeFieldEncoder::ConvertTerm(const common::Int64Term* term, RangeFieldType fieldType,
        uint64_t& leftTerm, uint64_t& rightTerm)
{
...
inline void RangeFieldEncoder::AlignedTerm(bool alignLeft, bool alignRight,
        uint64_t& leftTerm, uint64_t& rightTerm)
{
...
inline void RangeFieldEncoder::CalculateSearchRange(uint64_t leftTerm, uint64_t rightTerm,
        Ranges& bottomLevelRanges, Ranges& higherLevelRanges)
{
    uint64_t mask = 15;
...
inline bool RangeFieldEncoder::NormalizeToUInt64(
        fieldid_t fieldId, const std::string& fieldValue, uint64_t& value)
{
                              ...
                              std::vector<dictkey_t>& dictKeys)
{
    ...
    for (size_t levelIdx = 1; levelIdx < 16; levelIdx++)
    {
        value >>= 4;
