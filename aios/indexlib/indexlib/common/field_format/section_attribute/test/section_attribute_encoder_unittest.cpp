...
public:
    ...
    void TestEncodeOneSection()
    {
        TestEncode(1);
    }

    void TestEncodeSeveralSections()
    {
        TestEncode(7);
    }
    ...
    void TestEncodeWithShortEncodeBuffer()
    {
        ...
        CreateSectionData(30, answer, lengths, fids, 
                          weights);

        const uint32_t MAX_ENCODED_BUF_LEN = 3 * sizeof(uint32_t) +
                                             sizeof(SectionMeta) * MAX_SECTION_COUNT_PER_DOC;
        uint8_t encodedBuf[MAX_ENCODED_BUF_LEN];
        ASSERT_THROW(encoder.Encode(lengths, fids, weights, 30, encodedBuf, 30),
    ...
    void TestDecodeWithAllWeightZero()
    {
        ...
        uint32_t encodedLen = encoder.Encode(lengths, fids, weights, 
                1, encodedBuf + 4087, 9);
    ...
    void TestDecodeWithShortDecodeBuffer()
    {
        ...
        uint8_t encodedBuf[MAX_ENCODED_BUF_LEN];
        uint32_t encodedLen = encoder.Encode(lengths, fids, weights, 
                sectionCount, encodedBuf, MAX_ENCODED_BUF_LEN);

        const uint32_t SIZE_PER_SECTION = sizeof(section_fid_t) + sizeof(section_len_t)
        ...
        ASSERT_THROW(encoder.Decode(encodedBuf, encodedLen, decodedBuf, 30),
                BufferOverflowException);
    }

private:
    void CreateSectionData(uint32_t sectionCount, vector<SectionMeta>& answer,
                           section_len_t* lengths, section_fid_t* fids,
                           section_weight_t* weights)
    {
    ...
    void CheckSections(const MultiSectionMeta& multiMeta, uint32_t sectionCount, 
                       vector<SectionMeta>& answer)
    {
    ...
    void TestEncode(uint32_t sectionCount)
    {
        ...
        section_fid_t fids[MAX_SECTION_COUNT_PER_DOC];
        section_weight_t weights[MAX_SECTION_COUNT_PER_DOC];
        CreateSectionData(sectionCount, answer, lengths, fids, 
                          weights);
	...
	char* base = (char*)decodedBuf;
        encoder.Decode(encodedBuf, encodedLen,
                       decodedBuf, MAX_DECODED_BUF_LEN);
