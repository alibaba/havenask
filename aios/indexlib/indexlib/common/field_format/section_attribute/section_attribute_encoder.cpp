...
const uint8_t SectionAttributeEncoder::itemNumPerUnit[16] = {28, 21, 21, 21,
                                                             14, 9, 8, 7,
                                                             6, 6, 5, 5,
                                                             4, 3, 2, 1 };

uint32_t SectionAttributeEncoder::Encode(section_len_t *lengths, section_fid_t *fids, 
...
void SectionAttributeEncoder::EncodeSectionWeights(const section_weight_t* srcWeights,
        uint32_t sectionCount, uint8_t*& cursor, uint8_t* bufEnd)
{
...
void SectionAttributeEncoder::EncodeWeightUnit(const section_weight_t* weightUnit,
...
void SectionAttributeEncoder::EncodeFieldIds(const section_fid_t* srcFids,
        uint32_t sectionCount, uint8_t*& cursor, uint8_t* bufEnd)
{
    uint32_t unitCount = S16Compressor::Encode((uint32_t*)cursor, (uint32_t*)bufEnd, 
            srcFids, sectionCount);
    cursor += sizeof(uint32_t) * unitCount;
}

void SectionAttributeEncoder::EncodeSectionLens(const section_len_t* srcLengths,
        uint32_t sectionCount, uint8_t*& cursor, uint8_t* bufEnd)
{
    ...
    uint32_t unitCount = S16Compressor::Encode((uint32_t*)cursor, (uint32_t*)bufEnd,
            bufferToEncode, sectionCount + 1);
    cursor += sizeof(uint32_t) * unitCount;
}

void SectionAttributeEncoder::Decode(const uint8_t* srcBuf, uint32_t srcBufLen, 
                    uint8_t* destBuf, uint32_t destBufLen)
{
...
void SectionAttributeEncoder::DecodeSectionLens(const uint8_t*& srcCursor, 
        uint8_t*& destCursor, uint8_t* destEnd)
{
    ...
    uint32_t itemCount = S16Compressor::DecodeOneUnit(sectionLenCursor, 
            (section_len_t*)destEnd, (const uint32_t*)srcCursor);
...
void SectionAttributeEncoder::DecodeSectionFids(const uint8_t*& srcCursor,
...
void SectionAttributeEncoder::DecodeSectionWeights(const uint8_t*& srcCursor,
        const uint8_t* srcEnd, uint32_t sectionCount, uint8_t*& destCursor, uint8_t* destEnd)
{
