#ifndef __INDEXLIB_NOSSE_NEW_PFORDELTA_COMPRESSOR_H
#define __INDEXLIB_NOSSE_NEW_PFORDELTA_COMPRESSOR_H
...
struct NosseNewPForDeltaCompressorHeader 
{
    ...
    uint32_t dataNum:7;
    inline NosseNewPForDeltaCompressorHeader(uint32_t x = 0) 
    {
...
class NosseNewPForDeltaCompressor
{
public:
...
template <typename T>
size_t NosseNewPForDeltaCompressor::Compress(uint32_t* dest, size_t destLen, const T* src, size_t srcLen)
{
...
template <typename T>
inline uint32_t NosseNewPForDeltaCompressor::SelectCompressBits(const T* block,
...
template <typename T>
size_t NosseNewPForDeltaCompressor::CompressBlock(uint32_t* dest, size_t destLen, const T* src,
...
template <typename T>
inline size_t NosseNewPForDeltaCompressor::TransformToFrame(uint32_t* dest, size_t destLen,
...
template <typename T>
inline void NosseNewPForDeltaCompressor::Pack(uint32_t* dest, const T* src, size_t srcLen, uint32_t frameBits)
{
...
template <typename T>
size_t NosseNewPForDeltaCompressor::Decompress(T* dest, size_t destLen, const uint32_t* src, size_t srcLen)
{
...
template <typename T>
size_t NosseNewPForDeltaCompressor::DecompressBlock(T* dest, size_t destLen, uint32_t*& src, size_t& srcLen, bool& isLastBlock)
{
    ...
    //
    NosseNewPForDeltaCompressorHeader header(*src);
...
inline uint32_t NosseNewPForDeltaCompressor::MakeHeader(uint32_t frameBits,
        size_t firstExceptionPos, size_t codeLen, uint32_t numExceptions,
        uint32_t exceptionIntRange, bool isLastBlock)
{
    NosseNewPForDeltaCompressorHeader header;
...
inline void NosseNewPForDeltaCompressor::PackException(
...
inline uint32_t NosseNewPForDeltaCompressor::HighBitIdx(uint32_t value)
{
    int r = 0;
...
template<typename T>
inline size_t NosseNewPForDeltaCompressor::S9Encode(uint32_t* dest,
        size_t destLen, T* src, size_t srcLen)
{
    return S9Compressor::Encode(dest, destLen, src, srcLen);
}

inline void NosseNewPForDeltaCompressor::FastMemZero(uint32_t* dest, size_t destLen)
{
...
template <typename T>
inline void NosseNewPForDeltaCompressor::S9Decode(T* dest, uint32_t* src, size_t srcLen,
                                      size_t firstExceptionPos, uint32_t frameBits)
{
...
typedef std::tr1::shared_ptr<NosseNewPForDeltaCompressor> NosseNewPForDeltaCompressorPtr;

///////////////////////////////////////////////////////////
inline size_t NosseNewPForDeltaCompressor::GetCompressedLength(uint32_t header) const
{
    // Header component is encoded as follow:
    // 
    // 7bit : dataNum - 1
    ...
    // 1bit : has next frame or not
    //
    NosseNewPForDeltaCompressorHeader compressHeader(header);
...
inline bool NosseNewPForDeltaCompressor::IsLastBlock(uint32_t header) const
{
    NosseNewPForDeltaCompressorHeader compressHeader(header);
