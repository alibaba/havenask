#ifndef __INDEXLIB_S9_COMPRESSOR_H
#define __INDEXLIB_S9_COMPRESSOR_H
...
inline size_t S9Compressor::CalculateBytes(uint32_t *bits, uint32_t len) 
{
    ...
    while (curPos < len)
    {
        for (uint32_t k = 0; k < 9; k++ )
        {
            ...
            if (len <= curPos + curSize - 1) continue;
...
template <typename T>
inline size_t S9Compressor::Encode(uint32_t* dest, size_t destLen,
                                   const T* src, size_t srcLen)
{
    ...
    while (curPos < srcLen)
    {
        for (uint32_t k = 0; k < 9; k++ )
        {
            ...
            if (k >= 8)
            {
                INDEXLIB_FATAL_ERROR(Runtime, "Encode posting-list "
                        "FAILED: [%u] exceeds 28 bit", (uint32_t)src[curPos]);
            }
        }
    }
...
#endif //__INDEXLIB_S9_COMPRESSOR_H
