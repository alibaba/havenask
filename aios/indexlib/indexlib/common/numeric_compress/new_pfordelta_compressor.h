#ifndef __INDEXLIB_NEW_PFORDELTA_COMPRESSOR_H
#define __INDEXLIB_NEW_PFORDELTA_COMPRESSOR_H
...
public:
     ...
     */
    size_t CompressInt32(uint32_t* dest, size_t destLen,
     ...
     * @return compressed length in int32 size
     */
    size_t CompressInt16(uint32_t* dest, size_t destLen,
     ...
     * @return compressed length in int32 size
     */
    size_t CompressInt8(uint32_t* dest, size_t destLen,
     ...
     */
    size_t DecompressInt32(uint32_t* dest, size_t destLen,
                           const uint32_t* src, size_t srcLen) const;
     ...
     * @return number of decompressed integers
     */
    size_t DecompressInt16(uint16_t* dest, size_t destLen,
                          const uint32_t* src, size_t srcLen) const;
     ...
     * @return number of decompressed integers
     */
    size_t DecompressInt8(uint8_t* dest, size_t destLen,
                          const uint32_t* src, size_t srcLen) const;
...
public:
    ...
    template <typename T>
    static size_t DecompressBlock(T* dest, size_t destLen, uint32_t*& src,
                                  size_t& srcLen, bool& lastFlag);
...
protected:
    template <typename T>
    static uint32_t SelectCompressBits(const T* block, size_t blkLen, uint32_t& numExceptions);
...
protected:
    static uint32_t MakeHeader(uint32_t frameBits, size_t firstExceptionPos, 
                               size_t codeLen, uint32_t numException, 
                               uint32_t exceptionIntRange, bool isLastBlock);

    static void PackException(uint32_t* dest, size_t exceptionOffsetIntSize, 
    ...
    template <typename T>
    static void S9Decode(T* dest, uint32_t* src, size_t srcLen, 
                  size_t firstExceptionPos, uint32_t frameBits);
...
size_t NewPForDeltaCompressor::Compress(uint32_t* dest, size_t destLen, const T* src, size_t srcLen)
{
    ...
    for (size_t off = 0, blk = 0; blk < totalBlk; off += BLOCK_SIZE, ++blk)
    {
        ...
        uint32_t bestFrameBits = SelectCompressBits(src + off, len, bestExceptions); 
        size_t blkCompSize = CompressBlock(destPtr, destLeftLen, src + off, len,
                bestFrameBits, bestExceptions, (blk == totalBlk - 1));
        destPtr += blkCompSize;
...
template <typename T>
inline uint32_t NewPForDeltaCompressor::SelectCompressBits(const T* block,
        size_t blkLen, uint32_t& numExceptions)
{
    ...
    for (int32_t i = FRAME_BIT_MAP[maxBits]; i >= 0; i--)
    {
        ...
        if (exceptionNum > maxExceptionNum 
            || maxBits > frameBits + 28)
        {
            break;
        }
...
size_t NewPForDeltaCompressor::CompressBlock(uint32_t* dest, size_t destLen, const T* src,
        size_t srcLen, uint32_t frameBits, uint32_t numExceptions, bool isLastBlock)
{
    uint32_t pre =  0;
    T max = (T)(1 << frameBits);
    T code[BLOCK_SIZE]{};
    ...
    if (numExceptions == 0 )
    {
        ...
        return TransformToFrame(dest, destLen, frameBits, 0,
                                src, srcLen, NULL, NULL, 0, isLastBlock);
    }
    ...
    return TransformToFrame(dest, destLen, frameBits, firstExceptionPos, code, srcLen, 
                            exception, exceptionOffset, numExceptions, isLastBlock);
}

template <typename T>
inline size_t NewPForDeltaCompressor::TransformToFrame(uint32_t* dest, size_t destLen,
        uint32_t frameBits, size_t firstExceptionPos, const T* code, size_t codeLen,
        uint32_t* exception, uint32_t* exceptionOffset, size_t numExceptions, bool isLastBlock)
{
    ...
    size_t exceptionRange = S9Encode(dest + exceptionOffsetIntSize, destLen - exceptionOffsetIntSize,
    ...
    *dest++ = MakeHeader(frameBits, firstExceptionPos, codeLen,
                         numExceptions, exceptionRange, isLastBlock);
...
template <typename T>
inline void NewPForDeltaCompressor::Pack(
        uint32_t* dest, const T* src, size_t srcLen, uint32_t frameBits)
{
    typedef void (*pack_function)(uint32_t*, const T*, uint32_t n);
    static pack_function pack_func[33] =
    {
        pack_0<T>,  pack_1<T>,  pack_2<T>,  pack_3<T>,  
        pack_4<T>,  pack_5<T>,  pack_6<T>,  pack_7<T>,  
        pack_8<T>,  pack_9<T>,  pack_10<T>, pack_11<T>,
        pack_12<T>, pack_13<T>, pack_14<T>, pack_15<T>,
        pack_16<T>, pack_17<T>, pack_18<T>, pack_19<T>,
        pack_20<T>, pack_21<T>, pack_22<T>, pack_23<T>,
        pack_24<T>, pack_25<T>, pack_26<T>, pack_27<T>,
        pack_28<T>, pack_29<T>, pack_30<T>, pack_31<T>,
        pack_32<T>
    };
...
template <typename T>
size_t NewPForDeltaCompressor::Decompress(
        T* dest, size_t destLen, const uint32_t* src, size_t srcLen)
{
    ...
    while (!isLastBlock)
    {
        totalDecodeSize += DecompressBlock(dest + totalDecodeSize, destLen - totalDecodeSize,
                srcPtr, encodeLen, isLastBlock);
    }
    return totalDecodeSize;
}

template <typename T>
inline size_t NewPForDeltaCompressor::DecompressBlock(
        T* dest, size_t destLen, uint32_t*& src, size_t& srcLen, bool& isLastBlock)
{
    ...
    {
        unpack_0<T>,  unpack_1<T>,  unpack_2<T>,  unpack_3<T>,  
        unpack_4<T>,  unpack_5<T>,  unpack_6<T>,  unpack_7<T>,  
        unpack_8<T>,  unpack_9<T>,  unpack_10<T>, unpack_11<T>,
        unpack_12<T>, unpack_13<T>, unpack_14<T>, unpack_15<T>,
        unpack_16<T>, unpack_17<T>, unpack_18<T>, unpack_19<T>,
        unpack_20<T>, unpack_21<T>, unpack_22<T>, unpack_23<T>,
        unpack_24<T>, unpack_25<T>, unpack_26<T>, unpack_27<T>,
        unpack_28<T>, unpack_29<T>, unpack_30<T>, unpack_31<T>,
        unpack_32<T>
    };
    ...
    if (firstExceptionPos != 0 )
    {
        S9Decode(dest, src + intOffsetForExceptionRange, exceptionIntRange,
                 firstExceptionPos - 1, frameBits);
    }
...
template<>
inline size_t NewPForDeltaCompressor::DecompressBlock<uint32_t>(
        uint32_t* dest, size_t destLen, uint32_t*& src, size_t& srcLen, bool& isLastBlock)
{
    // Step 1. decode header value
    // Header component is encoded as follow:
    // 
    ...
    {
        decompress_sse4_c0,  decompress_sse4_c1,  decompress_sse4_c2,  decompress_sse4_c3,  
        decompress_sse4_c4,  decompress_sse4_c5,  unpack_6<uint32_t>,  decompress_sse4_c7,  
        decompress_sse4_c8,  decompress_sse4_c9,  decompress_sse4_c10, decompress_sse4_c11,
        decompress_sse4_c12, decompress_sse4_c13, decompress_sse4_c14, unpack_15<uint32_t>,
        decompress_sse4_c16, decompress_sse4_c17, decompress_sse4_c18, decompress_sse4_c19,
        decompress_sse4_c20, decompress_sse4_c21, decompress_sse4_c22, decompress_sse4_c23,
        decompress_sse4_c24, decompress_sse4_c25, decompress_sse4_c26, decompress_sse4_c27,
        decompress_sse4_c28, decompress_sse4_c29, decompress_sse4_c30, decompress_sse4_c31,
        decompress_sse4_c32
    };
...
inline uint32_t NewPForDeltaCompressor::MakeHeader(uint32_t frameBits,
        size_t firstExceptionPos, size_t codeLen, uint32_t numExceptions,
        uint32_t exceptionIntRange, bool isLastBlock)
{
...
inline void NewPForDeltaCompressor::PackException(
        uint32_t* dest, size_t exceptionOffsetIntSize,
        uint32_t* compressedExceptionDatum, size_t len)
{
...
template<typename T>
inline size_t NewPForDeltaCompressor::S9Encode(uint32_t* dest,
        size_t destLen, T* src, size_t srcLen)
{
...
template <typename T>
inline void NewPForDeltaCompressor::S9Decode(T* dest, uint32_t* src, size_t srcLen,
                                      size_t firstExceptionPos, uint32_t frameBits)
{
...
typedef std::tr1::shared_ptr<NewPForDeltaCompressor> NewPForDeltaCompressorPtr;
...
#endif //__INDEXLIB_NEW_PFORDELTA_COMPRESSOR_H
