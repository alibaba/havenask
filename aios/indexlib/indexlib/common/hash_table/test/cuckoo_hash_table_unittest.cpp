...
void _CheckFind(const char* file, int32_t line,
                const CuckooHashTable<_KT, _VT>& hashTable,
...
template <typename _KT, typename _VT> static
void _CheckFind(const char* file, int32_t line,
                const CuckooHashTable<_KT, TimestampValue<_VT>>& hashTable,
                const _KT& key, bool expectFound,
                bool expectIsDeleted = false,
...
static
void _CheckFind(const char* file, int32_t line,
                const CuckooHashTable<_KT, SpecialValue<_VT, _EV, _DV>>& hashTable,
                const _KT& key, bool expectFound,
                bool expectIsDeleted = false,
...
static
void _CheckFind(const char* file, int32_t line,
                const CuckooHashTable<_KT, OffsetValue<_VT, _EV, _DV>>& hashTable,
                const _KT& key, bool expectFound,
                bool expectIsDeleted = false,
...
void _InsertAndCheck(const char* file, int32_t line,
                     CuckooHashTable<_KT, _VT>& hashTable,
...
void CuckooHashTableTest::TestCuckooKick()
{
    ...
    for (uint32_t num = 0; num < 100; ++num)
    {
        ...
        uint64_t blockId = HashTable::GetFirstBucketIdInBlock(hash,
                hashTable.mBlockCount) / 4;
        if (blockId == 0)
        {
            uint64_t hash1 = hashTable.CuckooHash(num, 1);
            uint64_t blockId1 = HashTable::GetFirstBucketIdInBlock(hash1,
                hashTable.mBlockCount) / 4;
            if (blockId1 == 1) 
            {
                tester = num;
    ...
    {
        ASSERT_TRUE(hashTable.Insert(zeros[i],10));
        ASSERT_TRUE(hashTable.Insert(ones[i],20));
    }

    ASSERT_EQ(8, hashTable.Size());
    const int32_t* value;
    Status status = hashTable.Find(zeros[4], value);
    ...
    ASSERT_TRUE(hashTable.Insert(tester, 100));
...
void CuckooHashTableTest::TestTableInsertMany()
{
    ...
    {
        key = key * 3 - 1;
        Status status = hashTable.Find(key, value);
        if (i % 7 <= 2)
        {
...
void CuckooHashTableTest::TestOffsetValue()
{
    ...
    hashTable.MountForWrite(buffer, sizeof(char) * buffLen);
    //TEst Insert()
    ASSERT_TRUE(hashTable.Insert(1UL, VT(10)));
    ASSERT_TRUE(hashTable.Insert(3UL, VT(30)));
    //Test Find()
    const VT* value = NULL;
    ASSERT_EQ(NOT_FOUND, hashTable.Find(2UL, value));
    ASSERT_EQ(OK, hashTable.Find(1UL, value));
...
void CuckooHashTableTest::TestInsertSpecialKey()
{
    ...
    //Test Insert()
    ASSERT_TRUE(hashTable.Insert(1UL, 10));
    ASSERT_TRUE(hashTable.Insert(Bucket::EmptyKey(), 30));
    //Test Find()
    const int32_t* value;
    ASSERT_EQ(NOT_FOUND, hashTable.Find(2UL, value));
    ASSERT_EQ(OK, hashTable.Find(1UL, value));
...
void CuckooHashTableTest::TestIterator()
{
    ...
    for (; count < TOTAL && !hashTable.IsFull(); ++count)
    {
        key = key * 11 + 17;
        keyArr[count] = key;
        ASSERT_TRUE(hashTable.Insert(key,  count));
    }
    ASSERT_EQ(count, hashTable.Size());
...
void CuckooHashTableTest::TestFileIterator()
{
    ...
    static const uint32_t TOTAL = 100000;
    size_t size = Traits::HashTable::CapacityToTableMemory(TOTAL, 100);
    unique_ptr<char[]> buffer(new char[size]);
    ...
    fileWriter->Write(buffer.get(), hashTable.MemoryUse());
    fileWriter->Close();

    file_system::FileReaderPtr fileReader = directory->CreateFileReader(filename, file_system::FSOT_IN_MEM);
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        if (key % 3 == 2)
        {
            ASSERT_EQ(DELETED, reader.Find(key, actualValue,  nullptr)) << "key:" << key;
        }
        else
        {
            ASSERT_EQ(OK, reader.Find(key, actualValue, nullptr)) << "key:" << key;
...
void CuckooHashTableTest::TestIncHashFunc()
{
    ...
    for (uint64_t i = 0; i < 1000 ; ++i)
    {
        ...
        if (HashTable::GetFirstBucketIdInBlock(hash0, 10) == 0 &&
            HashTable::GetFirstBucketIdInBlock(hash1, 10) == 16)
        {
            keyVec.push_back(i);
        }
    }
    ...
    {
        if (!hashTable.Insert(keyVec[i], VT(0, i * 3)))
        {
            ASSERT_FALSE(true);
            break;
        }
...
void CuckooHashTableTest::TestShrinkFail()
{
    ...
    size_t size = TOTAL * sizeof(HashTable::Bucket)
                  + sizeof(HashTable::HashTableHeader);
    ...
    for (uint64_t i = 0; i < 8; ++i)
    {
        ASSERT_EQ(hashTable.CuckooHash(keys[i], 0) % 2,
                  hashTable.CuckooHash(keys[i], 1) % 2);
        ASSERT_TRUE(hashTable.Insert(keys[i], VT(i, keys[i]))) << "key:" << keys[i];
    }
    ASSERT_EQ(8, hashTable.Size());
...
void CuckooHashTableTest::TestStretch()
{
    ...
    size_t stretchSize = bucketSize * HashTable::STRETCH_MEM_RATIO + 1
                         + HashTable::BLOCK_SIZE * sizeof(HashTable::Bucket);
    ASSERT_EQ(headerSize + specialBucketSize + bucketSize,
              HashTable::CapacityToTableMemory(100, 100));
    ASSERT_EQ(headerSize + specialBucketSize + bucketSize + stretchSize,
              HashTable::CapacityToTableMemory(100, options));
    size_t allocatedSize = headerSize + specialBucketSize + 
    ...
    ASSERT_EQ(stretchSize / HashTable::BLOCK_SIZE,
              hashTable.Header()->stretchSize / HashTable::BLOCK_SIZE);
    ASSERT_EQ(100, hashTable.BucketCount());

    for (size_t i = 0; i < 90; ++i)
    {
        uint64_t key = i;
        ASSERT_TRUE(hashTable.Insert(key, i));
    }
...
void CuckooHashTableTest::TestBuildMemoryToCapacity()
{
    ...
    size_t maxKeyCountThreshhold = HashTable::MAX_NUM_BFS_TREE_NODE *
                                   HashTable::BLOCK_SIZE * 100 / 100;
    size_t threshholdSize =
        HashTable::CapacityToBuildMemory(maxKeyCountThreshhold, 100);
    ASSERT_EQ(maxKeyCountThreshhold,
              HashTable::BuildMemoryToCapacity(threshholdSize, 100));

    size_t oneBlockTableSize = sizeof(HashTable::HashTableHeader) + 
                              HashTable::BLOCK_SIZE * sizeof(HashTable::Bucket) +
                              2 * sizeof(HashTable::SpecialBucket);
    size_t oneBlockBuildSize = oneBlockTableSize + 
                               sizeof(uint32_t) +
                               sizeof(HashTable::TreeNode);
