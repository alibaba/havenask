...
template<typename _KT, typename _VT,
         typename Bucket, bool HasSpecialKey>
...
public:
    ...
    void Reset() { mCursor = -1; MoveToNext(); }
    void MoveToNext()
    {
        assert(IsValid());
        ++mCursor;
    ...
    void SetValue(const _VT& value)
    {
        mBucket[mCursor].UpdateValue(value);
    }

private:
    static bool KeyCompare(const Bucket& left, Bucket& right)
...
class ClosedHashTableIterator<_KT, _VT, Bucket, true> :
    public ClosedHashTableIterator<_KT, _VT, Bucket, false>
{
private:
...
public:
    ...
    {
        return Base::IsValid() 
            || mCursor == mBucketCount
            || mCursor == mBucketCount + 1;
    }
...
private:
    ...
    const SpecialBucket* EmptyBucket() const
    { return reinterpret_cast<const SpecialBucket*> (&mBucket[mBucketCount]); }
