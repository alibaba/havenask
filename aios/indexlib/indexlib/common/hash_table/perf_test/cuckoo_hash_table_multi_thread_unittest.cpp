...
#include "indexlib/common/hash_table/perf_test/cuckoo_hash_table_multi_thread_unittest.h"
...
IE_LOG_SETUP(common, CuckooHashTableMultiThreadTest);
...
template<>
bool ValueEqual<TimestampValue<uint64_t>>
(uint32_t realValue, uint32_t realTs, TimestampValue<uint64_t>& value)
{
    bool ret = (realValue != value.Value() || realTs != value.Timestamp());
    if (ret)
    {
        cerr << "wrong value i: " << realTs
             << " key:" << (uint32_t)realValue
             << " value:" << value.Value()
...
template<>
bool ValueEqual<SpecialValue<uint32_t>>
(uint32_t realValue, uint32_t realTs, SpecialValue<uint32_t>& value)
{

    bool ret = (realValue != value.Value());
    if (ret)
    {
        cerr << " key(real value):" << (uint32_t)realValue
...
void ReadHashTable(const HashTable& hashTable, bool* isSucess,
                   bool onlyOnce = false)
{
    ...
    while (!DHTIsDone)
    {
        ...
        for (uint32_t i = 0; i < CUCKOO_COUNT; ++i)
        {
            key = key * 5 + i;
            VT value;
            Status status = hashTable.FindForReadWrite(key, value);
            if ((status = hashTable.FindForReadWrite(key, value)) != OK)
            {
...
{
    if (realValue == numeric_limits<uint32_t>::max()
        || realValue == numeric_limits<uint32_t>::max() - 1)
    {
        realValue = 0;
    }
...
void WriteHashTable(HashTable& hashTable)
{
    ...
    {
        key = key * 5 + i;
        if (!hashTable.Insert(key, CreateValue<VT>(i, (uint32_t)key)))
        {
            break;
        }       
        if (i % 100000 == 0)
...
void CuckooHashTableMultiThreadTest::TestMultiThreadWithTs()
{
    ...
    for (size_t i = 0; i < NUM_THREADS; ++i) {
        ThreadPtr thread = Thread::createThread(
                tr1::bind(&ReadHashTable<HashTable>, hashTable, &(status[i]), false));
        readThreads.push_back(thread);
    }
    ThreadPtr writeThread = Thread::createThread(tr1::bind(&WriteHashTable<HashTable>, hashTable));
    readThreads.clear();
