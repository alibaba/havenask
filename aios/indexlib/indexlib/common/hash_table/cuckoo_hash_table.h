...
private:    
    struct TreeNode
    {
        ...
        TreeNode(uint64_t bucketId, uint64_t parent, uint8_t inBlockId)
            : mBucketId(bucketId), mParent(parent), mInBlockId(inBlockId) {}
    };
...
public:
    ...
    {
        return (occupancy > 97)? 97 : occupancy;
    }
...
public:
    ...
    uint64_t Capacity() const { return BucketCountToCapacity(mBucketCount, mOccupancyPct); }
    bool IsFull() const { return Size() >= Capacity(); }
    uint64_t BucketCount() const { return mBucketCount; }
    uint64_t MemoryUse() const;
    uint64_t BuildAssistantMemoryUse() const;
    ...
    typedef ClosedHashTableIterator<
        _KT, _VT, Bucket, 
        ClosedHashTableTraits<_KT, _VT, useCompactBucket>::HasSpecialKey> Iterator;
...
///////////////////////////////////////////
IE_LOG_SETUP_TEMPLATE4_CUSTOM(typename, typename, bool, bool, util, CuckooHashTable);

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline size_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Compress(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CapacityToTableMemory(
        uint64_t maxKeyCount, const HashTableOptions& options)
{
    int32_t occupancyPct = options.occupancyPct;
    size_t bucketSize = 
        sizeof(Bucket) * CapacityToBucketCount(maxKeyCount, occupancyPct);
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CapacityToBuildMemory(
        uint64_t maxKeyCount, const HashTableOptions& options)
{
    size_t hashTableSize = CapacityToTableMemory(maxKeyCount, options);
    ...
    size_t bfsTreeSize = 
        std::min(blockCount, (size_t)MAX_NUM_BFS_TREE_NODE) * sizeof(TreeNode);
    return hashTableSize + callIdSize + bfsTreeSize;
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline size_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::TableMemroyToCapacity(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline size_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BuildMemoryToCapacity(
        size_t buildMemory, int32_t occupancyPct)
{
    ...
    size_t threshhold = CapacityToBuildMemory(maxKeyCountThreshhold, occupancyPct);
    ...
    {
        size_t blockCount = leftMemory /
                            (sizeof(uint32_t) + sizeof(TreeNode) + BLOCK_SIZE * sizeof(Bucket));
        bucketCount = blockCount * BLOCK_SIZE;
    }
    return bucketCount * occupancyPct / 100;
}
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline size_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BuildMemoryToTableMemory(
        size_t buildMemory, int32_t occupancyPct)
{
    size_t maxKeyCount = BuildMemoryToCapacity(buildMemory, occupancyPct);
    return CapacityToTableMemory(maxKeyCount, occupancyPct);
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CapacityToBucketCount(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BucketCountToCapacity(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::MountForWrite(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::MountForRead(
        const void* data, size_t size)
{
    ...
    {
        IE_LOG(INFO, "mount for read, headerSize[%lu], mBucket[%lu]",
               sizeof(HashTableHeader), (uint64_t)mBucket);
    }
    return true;
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Insert(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Delete(
        const _KT& key, const _VT& value)
{
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline misc::Status CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Find(
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline misc::Status CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::FindForReadWrite(
        const _KT& key, _VT& value) const
{
    while (true)
    {
        const Bucket* bucket = FindBucketForRead(key, Header()->numHashFunc);
        misc::Status status = misc::NOT_FOUND;
        ...
        value = bucket->Value();
        status = (bucket->IsDeleted()) ? misc::DELETED : misc::OK;
        ...
        if (likely(!bucket->IsEmpty() && bucket->IsEqual(key)
                   && bucket->Value() == value))
        {
            return status;
        }
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline void CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::PrintHashTable() const
{
    std::stringstream ss;
    for (uint32_t i = 0; i < mBlockCount; ++i)
    {
        ...
        for (uint32_t inBlockId = 0; inBlockId < BLOCK_SIZE; ++inBlockId)
        {
            ...
            {
                ss << key << "[";
                for (uint8_t hashFuncId = 0; hashFuncId < mNumHashFunc - 1;
                     ++hashFuncId)
                {
                    ss << (uint32_t)CuckooHash(key, hashFuncId)
                        % (uint32_t)mBlockCount << ",";
                }
                ss << (uint32_t)CuckooHash(key, mNumHashFunc - 1)
                    % (uint32_t)mBlockCount << "]  ";
            }
        }
        ss << std::endl;
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CheckBucket(
        const _KT& key, uint64_t targetBucketId) const
{
    ...
    for (uint32_t hashFuncId = 0; hashFuncId < mNumHashFunc; ++hashFuncId)
    {
        ...
        uint64_t blockId = GetFirstBucketIdInBlock(hash, mBlockCount) /
                           BLOCK_SIZE;
    ...
    ss << GetFirstBucketIdInBlock(hash, mBlockCount) / BLOCK_SIZE 
       << "] to wrong block [" << targetBlockId << "]";
    std::cerr << ss.str() << std::endl;
    return false;
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Shrink(int32_t occupancyPct)
{
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Stretch()
{
    uint64_t newBlockCount = Header()->stretchSize / sizeof(Bucket) / BLOCK_SIZE
                             + mBlockCount;
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::ReHash(uint64_t newBucketCount)
{
    uint64_t oldBucketCount = mBucketCount;
    ...
    for (uint64_t i = 0; i < newBucketCount; ++i)
    {
        ...
        if (unlikely(!EvictionInsert(evictedBucket, bitmap)))
        {
            ...
            mBlockCount = mBucketCount / BLOCK_SIZE;
            IE_LOG(ERROR, "ReHash failed, newBucketCount[%lu], oldBucketCount[%lu]",
                   newBucketCount, oldBucketCount);
            return false;
        }
    }
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::EvictionInsert(
        Bucket seekingBucket, std::vector<bool>& bitmap)
{
    while (true)
    {
        ...
        if (!bucket) // can not find a bucket, rehash
        {
            ...
            {
                IE_LOG(ERROR, "can not find bucket to store key[%lu] numHashFunc[%u]",
                       (uint64_t)seekingBucket.Key(), mNumHashFunc);
                return false;
            }
            ++mNumHashFunc;
            ++Header()->numHashFunc;
            IE_LOG(WARN, "Increase numHashFunc to [%u], size[%lu], bucketCount[%lu]",
                   mNumHashFunc, Size(), mBucketCount);
            continue;
        }
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::FindBucketForEviction(
        const _KT& key, std::vector<bool>& bitmap)
{
    TreeNodeVec bfsTree;
    for (uint32_t hashFuncId = 0; hashFuncId < mNumHashFunc; ++hashFuncId)
    {
        ...
        for (uint32_t curBucketId = firstBucketId; 
             curBucketId < firstBucketId + BLOCK_SIZE; ++curBucketId)
        {
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BFSFindBucketForEviction(
        TreeNodeVec& bfsTree, std::vector<bool>& bitmap)
{
    ++mCurCallId;
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::MemoryUse() const
{
    return mBucketCount * sizeof(Bucket) + sizeof(HashTableHeader);
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline uint64_t CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BuildAssistantMemoryUse() const
{
    size_t callIdSize = mCallIdVec.size() * sizeof(uint32_t);
    size_t bfsTreeSize =
        std::min(mBlockCount, (size_t)MAX_NUM_BFS_TREE_NODE) * sizeof(TreeNode);
    return callIdSize + bfsTreeSize;
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::InternalFindBucket(const _KT& key)
{
    while (true)
    {
    ...
    IE_LOG(ERROR, "can not find bucket to store key[%lu] numHashFunc[%u]",
           (uint64_t)key, mNumHashFunc);
    return NULL;
}

template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::DoInternalFindBucket(const _KT& key)
{
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline const typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::FindBucketForRead(
        const _KT& key, uint8_t numHashFunc) const
{
    ...
    for (uint32_t hashFuncId = 2; hashFuncId < numHashFunc; ++hashFuncId)
    {// line 1,2 put out of this loop because of performance
        const _KT& hash = CuckooHash(key, hashFuncId);
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
template <typename functor>
inline bool CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::InternalInsert(
        const _KT key, const _VT&value)
{
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::BFSFindBucket(TreeNodeVec& bfsTree)
{
    ++mCurCallId;
    for (uint32_t i = 0; i < bfsTree.size(); ++i)
    {
    ...
    while (idx < bfsTree.size())
    {
        ...
        for (uint32_t inBlockId = 0; inBlockId < BLOCK_SIZE; ++inBlockId)
        {
            ...
            for (uint32_t hashFuncId = 0; hashFuncId < mNumHashFunc; ++hashFuncId)
            {
                ...
                if (bfsTree.size() >= MAX_NUM_BFS_TREE_NODE)
                {
                    IE_LOG(WARN, "too many[%lu] node in bfs tree more than [%lu], skip it",
                            bfsTree.size(), MAX_NUM_BFS_TREE_NODE);
                    continue;
                }
                mCallIdVec[firstBucketId / BLOCK_SIZE] = mCurCallId;
                bfsTree.push_back(TreeNode(firstBucketId, idx, inBlockId));
                ...
                Bucket* curBucket = &(mBucket[firstBucketId]);
                for (uint32_t inCurBlockId = 0; inCurBlockId < BLOCK_SIZE;
                     ++inCurBlockId, ++curBucket)
                {
                    if (curBucket->IsEmpty())
                    {
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline typename CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::Bucket*
CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CuckooKick(
        TreeNodeVec& bfsTree, uint32_t emptyInBlockId)
{
    ...
    while (prevIdx != TreeNode::BFS_ROOT_IDX)
    {
        ...
        assert(CheckBucket(prevBucket.Key(),
                           bfsTree[emptyIdx].mBucketId + emptyInBlockId));
...
template<typename _KT, typename _VT, bool HasSpecialKey, bool useCompactBucket>
inline _KT CuckooHashTable<_KT, _VT, HasSpecialKey, useCompactBucket>::CuckooHash(const _KT& key, uint32_t hashFuncId)
{
    ...
    return autil::MurmurHash::MurmurHash64A(&key, sizeof(key),
            CUCKOO_MURMUR_SEED_MULTIPLIER * hashFuncId);
}
...
class CuckooHashTable<_KT, _VT, true, useCompactBucket> : public CuckooHashTable<_KT, _VT, false, useCompactBucket>
{
public:
    typedef typename ClosedHashTableTraits<_KT, _VT, useCompactBucket>::Bucket Bucket;
    typedef typename ClosedHashTableTraits<_KT, _VT, useCompactBucket>::SpecialBucket SpecialBucket;
private:
    typedef CuckooHashTable<_KT, _VT, false, useCompactBucket> Base;
...
public:
    ...
    {
        return Base::CapacityToBuildMemory(maxKeyCount, options) +
            sizeof(SpecialBucket) * 2;
    }
    static size_t TableMemroyToCapacity(size_t tableMemory, int32_t occupancyPct)
    {
    ...
    static size_t BuildMemoryToCapacity(size_t buildMemory, int32_t occupancyPct)
    {
        return Base::BuildMemoryToCapacity(
                buildMemory - sizeof(SpecialBucket) * 2, occupancyPct);
    }
    static size_t BuildMemoryToTableMemory(size_t buildMemory, int32_t occupancyPct)
    {
        size_t maxKeyCount = BuildMemoryToCapacity(buildMemory, occupancyPct);
