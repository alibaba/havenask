...
protected:
    ...
    { return (HashTableHeader*)((uint8_t*)mItem - sizeof(HashTableHeader)); }
...
template<typename _KT, typename _VT>
inline uint64_t ChainHashTable<_KT, _VT>::CapacityToTableMemory(
...
template<typename _KT, typename _VT>
inline size_t ChainHashTable<_KT, _VT>::TableMemroyToCapacity(
        size_t memoryUse, int32_t occupancyPct)
{
    return 0;
}
...
template<typename _KT, typename _VT>
inline uint64_t ChainHashTable<_KT, _VT>::CapacityToBucketCount(
...
template<typename _KT, typename _VT>
inline bool ChainHashTable<_KT, _VT>::MountForWrite(
        void* data, size_t size, const int32_t occupancyPct)
{
    ...
    IE_LOG(INFO, "mount for write, size[%lu], ntopRatio[%d], itemCount[%lu], bucketCount[%lu], mBucket[%lu]",
           size, mRatio, mItemCount, mBucketCount, (uint64_t)mBucket);
    return true;
}

template<typename _KT, typename _VT>
inline bool ChainHashTable<_KT, _VT>::MountForRead(
        const void* data, size_t size)
{
    ...
    size_t totSize = sizeof(HashTableHeader) + mItemCount * sizeof(Item)
                     + mBucketCount * sizeof(Bucket);
    if (size < totSize)
    {
        IE_LOG(ERROR, "too small size[%lu], header size[%lu], item size[%lu], bucket size[%lu]",
               size, sizeof(HashTableHeader), mItemCount * sizeof(Item), 
               mBucketCount * sizeof(Bucket));
        return false;
    }
...
template<typename _KT, typename _VT>
inline bool ChainHashTable<_KT, _VT>::Insert(
...
template<typename _KT, typename _VT>
inline bool ChainHashTable<_KT, _VT>::Delete(
        const _KT& key, const _VT& value)
{
    return true;
}
...
template<typename _KT, typename _VT>
inline misc::Status ChainHashTable<_KT, _VT>::Find(
...
template<typename _KT, typename _VT>
inline misc::Status ChainHashTable<_KT, _VT>::FindForReadWrite(
...
template<typename _KT, typename _VT>
inline misc::Status ChainHashTable<_KT, _VT>::InternalFind(
        const _KT& key, const _VT*& value) const
{
    uint64_t itemId = key % mItemCount;
...
inline bool ChainHashTable<_KT, _VT>::Finish()
{
    ...
    for (uint64_t i = 0; i < mItemCount; ++i)
    {
        ...
        {
            mBucket[BucketId].Set(
                    mVecTable[i][j].first, mVecTable[i][j].second);
            ++BucketId;
            ++mKeyCount;
        }
    }
