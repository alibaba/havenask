#ifndef __INDEXLIB_SEPARATE_CHAIN_HASH_TABLE_H
#define __INDEXLIB_SEPARATE_CHAIN_HASH_TABLE_H
...
public:
    ...
    struct KeyNode
    {
        KeyNode(KeyType _key,
                ValueType _value,
                uint32_t _next = INVALID_KEY_OFFSET)
            : key(_key)
            , value(_value)
            , next(_next)
        {}
    ...
    {
    public:
        HashTableIterator(const KeyNodeVectorPtr& keyNodes)
            : mCursor(0)
            , mKeyNodes(keyNodes)
        { assert(keyNodes); }
    public:
        bool IsValid() const
        { return mCursor < mKeyNodes->Size(); }
        ...
        { assert(IsValid()); return &(*mKeyNodes)[mCursor]; }

        const KeyType& Key() const
        {
            return GetCurrentKeyNode()->key;
        }

        const ValueType& Value() const
        {
            return GetCurrentKeyNode()->value;
        }
        ...
        size_t Size() const
        { return mKeyNodes->Size(); }
        
    private:
        static bool Comp(const KeyNode& left, KeyNode& right)
        {
...
public:
    ...
    void Init(uint32_t bucketCount, size_t reserveSize)
    {
        ...
        mKeyNodes->Init(util::MMapAllocatorPtr(new util::MMapAllocator),
                        reserveSize / sizeof(KeyNode) + 1);
    }

    HashTableIterator* CreateKeyNodeIterator()
    {
        return new HashTableIterator(mKeyNodes);
    }

    size_t GetUsedBytes() const
    {
        return mBucketCount * sizeof(uint32_t) + mKeyNodes->GetUsedBytes();
    }
...
template<typename KeyType, typename ValueType>
inline void SeparateChainHashTable<KeyType, ValueType>::Insert(
...
#endif //__INDEXLIB_SEPARATE_CHAIN_HASH_TABLE_H
