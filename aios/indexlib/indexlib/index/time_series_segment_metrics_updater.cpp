    ...
    const config::IndexPartitionOptions& options, const util::KeyValueMap& parameters)
{
    ...
    auto minDocCountStr
        = GetValueFromKeyValueMap(parameters, MIN_DOC_COUNT_PARAM, DEFAULT_MIN_DOC_COUNT);
    auto continuousLenStr
        = GetValueFromKeyValueMap(parameters, CONTINUOUS_LEN_PARAM, DEFAULT_CONTINUOUS_LEN);
...
#define CREATE_PROCESSOR(fieldType)                                                               \
    case fieldType:                                                                               \
        mProcessor = new TimeSeriesSegmentMetricsProcessorImpl<                                   \
            FieldTypeTraits<fieldType>::AttrItemType>(                                            \
            attrName, period, minDocCount, continuousLen);                                        \
        if (!mProcessor->Init(fieldType, attributeConfig->GetFieldConfig()))                      \
        {                                                                                         \
            delete mProcessor;                                                                    \
            mProcessor = nullptr;                                                                 \
...
void TimeSeriesSegmentMetricsUpdater::Update(const document::DocumentPtr& doc)
{
    mProcessor->Process(doc);
}

void TimeSeriesSegmentMetricsUpdater::UpdateForMerge(segmentid_t oldSegId, docid_t oldLocalId)
{
...
template <typename T> T TimeSeriesSegmentMetricsProcessorImpl<T>::GetSubKey(T key)
{
    return key / mPeriod;
}

template <> float TimeSeriesSegmentMetricsProcessorImpl<float>::GetSubKey(float key)
{
    return (int64_t)key / mPeriod;
}

template <> double TimeSeriesSegmentMetricsProcessorImpl<double>::GetSubKey(double key)
{
    return (int64_t)key / mPeriod;
}
