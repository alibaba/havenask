...
std::string LifecycleSegmentMetricsUpdater::GenerateLifeCycleTag() const
{
    ...
    if (checkedSegment == 0 || mThresholdInfos.IsEmpty()
        || mMaxMinUpdater.GetCheckedDocCount() == 0)
    {
...
bool LifecycleSegmentMetricsUpdater::InitThresholdInfos()
{
    ...
    StringTokenizer st(
        mLifecycleParam, ";", StringTokenizer::TOKEN_TRIM | StringTokenizer::TOKEN_IGNORE_EMPTY);
    T lastThreshold = T();
    for (size_t i = 0; i < st.getNumTokens(); ++i)
    {
        StringTokenizer innerSt(
            st[i], ":", StringTokenizer::TOKEN_TRIM | StringTokenizer::TOKEN_IGNORE_EMPTY);
...
void LifecycleSegmentMetricsUpdater::Update(const document::DocumentPtr& doc)
{
    mMaxMinUpdater.Update(doc);
}

void LifecycleSegmentMetricsUpdater::UpdateForMerge(segmentid_t oldSegId, docid_t oldLocalId)
{
    mMaxMinUpdater.UpdateForMerge(oldSegId, oldLocalId);
}

template <typename T>
bool LifecycleSegmentMetricsUpdater::GetSegmentAttrValue(
    const SegmentMetrics& metrics, T& maxValue, T& minValue) const
{
