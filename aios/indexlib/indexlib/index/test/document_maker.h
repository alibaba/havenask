...
public:
    ...
    struct MockDoc
    {
        MockDoc()
            ...
            , fieldMap(0)
        {
            posList.reserve(1024);
        }
        
        static bool lessThan(const MockDoc& doc1, const MockDoc& doc2) 
        {
...
public:
     ...
     */
    static void MakeDocuments(
            const std::string& documentsStr,
    ...
    static void UpdateDocuments(const config::IndexPartitionSchemaPtr& schema,
                                DocumentVect& segDocVect, 
                                MockIndexPart& mockIndexPart);

    static void MakeDocumentsStr(uint32_t docCount, 
                                 const config::FieldSchemaPtr& fieldSchema,
                                 std::string& documentsStr,
                                 const std::string& pkField = "",
                                 int32_t pkStartSuffix = -1);

    static void MakeSortedDocumentsStr(uint32_t docCount, 
            const config::FieldSchemaPtr& fieldSchema,
            std::string& documentsStr,
    ...
    static void MakeMultiSortedDocumentsStr(uint32_t docCount, 
            const config::FieldSchemaPtr& fieldSchema,
            std::string& documentsStr,
    ...
    static document::NormalDocumentPtr MakeDeletionDocument(
            docid_t docId, int32_t pkSuffix);

    static document::NormalDocumentPtr MakeDeletionDocument(const std::string& pk,
            int64_t timeStamp);

    static document::NormalDocumentPtr MakeUpdateDocument(
            const config::IndexPartitionSchemaPtr& schema, 
            const std::string& pk,
            const std::vector<fieldid_t>& fieldIds,
            const std::vector<std::string>& fieldValues,
            int64_t timestamp);

    static void ReclaimDocuments(const MockIndexPart& oriIndexPart, 
                                 const ReclaimMapPtr& reclaimMap,
                                 MockIndexPart& newIndexPart);

    static void ReclaimDeletiionMap(const MockDeletionMap& oriDeletionMap, 
                                    const ReclaimMapPtr& reclaimMap,
                                    MockDeletionMap& newDeletiionMap);
    static void ReclaimPrimaryKey(const MockPrimaryKey& oriPrimaryKey, 
                                  const ReclaimMapPtr& reclaimMap,
                                  MockPrimaryKey& newPrimaryKey);
    static void ReclaimAttribute(const MockAttributes& oriAttributes, 
                                 const ReclaimMapPtr& reclaimMap,
                                 MockAttributes& newAttributes);
    static void ReclaimSummary(const MockSummary& oriSummary, 
                               const ReclaimMapPtr& reclaimMap,
                               MockSummary& newSummary);
    static void ReclaimIndexs(const MockIndexes& oriIndexes, 
                              const ReclaimMapPtr& reclaimMap,
                              MockIndexes& newIndexes);

    static document::NormalDocumentPtr MakeDocument(
            docid_t docId, const std::string& docStr,
            const config::IndexPartitionSchemaPtr& schema,
            MockIndexPart& answer);
    ...
    static document::Field* MakeField(const config::FieldSchemaPtr& fieldSchema, 
...
private:
    static void InternalMakeDocStr(uint32_t docCount, 
                                   const config::FieldSchemaPtr& fieldSchema,
                                   std::string& documentsStr,
                                   const std::string& sortFieldName,
                                   const std::string& pkField,
    ...
    static void InitAttributes(const config::IndexPartitionSchemaPtr& schema,
                               MockAttributes& attributes);

    static document::NormalDocumentPtr CreateOneDocument(
            const config::IndexPartitionSchemaPtr& schema);

    static void MakeMockIndex(const config::IndexSchemaPtr& indexSchema, 
                              ...
                              std::map<std::string ,std::set<std::string> >& firstPosSets,
                              MockIndexes& indexes);

    static void SetDocPayload(document::IndexDocumentPtr& indexDoc, 
                              docpayload_t docPayload, 
                              document::IndexTokenizeField* field);
    static void FieldToString(const document::Field* field,
                              const HashKeyToStrMap &hashKeyToStrMap,
                              std::string& str);
private:
    static void InitDefaultAttrDocValue(document::AttributeDocumentPtr& attrDoc,
            const config::AttributeSchemaPtr& attrSchema, const config::FieldSchemaPtr& fieldSchema,
