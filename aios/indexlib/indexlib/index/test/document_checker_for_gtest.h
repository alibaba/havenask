...
public:
    ...
    ~DocumentCheckerForGtest();
public:
    void CheckData(const partition::IndexPartitionReaderPtr& reader,
                   const config::IndexPartitionSchemaPtr& schema,
                   const MockIndexPart& mockIndexPart);
    
    void CheckIndexesData(const IndexReaderPtr& indexReader,
                          const config::IndexSchemaPtr& indexSchema,
                          const MockIndexPart& mockIndexPart);

    void CheckIndexData(const IndexReaderPtr& indexReader,
                        const config::IndexSchemaPtr& indexSchema,
                        const std::string& indexName,
                        const MockIndexPart& mockIndexPart,
                        const std::vector<std::string>& toCheckTokens);
    ...
    void CheckBitmapData(const IndexReaderPtr& indexReader,
                         const config::IndexConfigPtr& indexConfig,
                         const MockIndex& mockIndex,
                         const MockDeletionMap& deletionMap);

    void CheckPrimaryKeyIndexData(const IndexReaderPtr& indexReader,
                                  const config::IndexConfigPtr& indexConfig, 
                                  const MockPrimaryKey& primaryKey,
                                  const MockDeletionMap& deletionMap);
    
    void CheckSummaryData(const SummaryReaderPtr& summaryReader,
    ...
    void CheckDeletionMap(const DeletionMapReaderPtr& actualDeletionMap, 
                          const MockDeletionMap& deletionMap);
private:
    IE_LOG_DECLARE();
};
