...
void DocumentMaker::MakeDocuments(const string& documentsStr,
                                  ...
                                  MockIndexPart& answer)
{
    ...
    {
        NormalDocumentPtr doc = MakeDocument((docid_t)(i + baseDocId),
                st[i], schema, answer);
                                  ...
                                  const string& docIdStr)
{
    ...
    for (size_t i = 0; i < st.getNumTokens(); ++i)
    {
        NormalDocumentPtr doc = MakeDocument(docIds[i],
                st[i], schema, answer);
        docVect.push_back(doc);
    }
    answer.docCount += docVect.size();
...
void DocumentMaker::UpdateDocuments(const IndexPartitionSchemaPtr& schema,
                                    DocumentVect& segDocVect, 
                                    MockIndexPart& mockIndexPart)
{
    ...
    for (size_t i = 0; i < segDocVect.size(); ++i)
    {
        ...
        // get attribute doc
        AttributeDocumentPtr attributeDocument = 
            segDocVect[i]->GetAttributeDocument();
        ...
        {
            fieldid_t fieldId = -1;
            const autil::ConstString fieldVal = iter.Next(fieldId);
            ...
            autil::StringUtil::strToUInt32(fieldVal.c_str(), newFieldVal);
            newFieldVal++;
            string newFieldValStr = autil::StringUtil::toString(newFieldVal);

            ConstString encodeStr = convertor->Encode(
                    ConstString(newFieldValStr), segDocVect[i]->GetPool());
...
void DocumentMaker::MakeDocumentsStr(uint32_t docCount,
                                     const FieldSchemaPtr& fieldSchema,
                                     string& documentsStr, 
                                     const string& pkField,
                                     int32_t pkStartSuffix)
{
    InternalMakeDocStr(docCount, fieldSchema, documentsStr, "",
                       pkField, pkStartSuffix);
}

void DocumentMaker::MakeSortedDocumentsStr(uint32_t docCount,
        const FieldSchemaPtr& fieldSchema,
        string& documentsStr, 
        const string& sortFieldName,
        const string& pkField,
        int32_t pkStartSuffix)
{
    InternalMakeDocStr(docCount, fieldSchema, documentsStr, sortFieldName,
                       pkField, pkStartSuffix);
}

void DocumentMaker::MakeMultiSortedDocumentsStr(uint32_t docCount, 
        const config::FieldSchemaPtr& fieldSchema,
        std::string& documentsStr,
        const StringVec& sortFieldNameVec,
        const StringVec& sortPatterns,
        const std::string& pkField,
        int32_t pkStartSuffix)
{
    InternalMakeDocStr(docCount, fieldSchema, documentsStr, sortFieldNameVec,
                       sortPatterns, pkField, pkStartSuffix);
}
...
NormalDocumentPtr DocumentMaker::MakeDeletionDocument(
        docid_t docId, int32_t pkSuffix)
{
...
NormalDocumentPtr DocumentMaker::MakeUpdateDocument(
        const IndexPartitionSchemaPtr& schema, 
        const string& pk,
        const vector<fieldid_t>& fieldIds,
        const vector<string>& fieldValues,
        int64_t timestamp)
{
    NormalDocumentPtr doc(new NormalDocument());
    doc->SetTimestamp(timestamp);
    ...
    for (size_t i = 0; i < fieldIds.size(); i++)
    {
        ...
        AttributeConvertorPtr attrConvertor(
                AttributeConvertorFactory::GetInstance()
                ->CreateAttrConvertor(fieldConfig));
        ConstString encodeStr = attrConvertor->Encode(
                ConstString(fieldValues[i]), doc->GetPool());
...
void DocumentMaker::InternalMakeDocStr(uint32_t docCount, 
                                       const FieldSchemaPtr& fieldSchema,
                                       string& documentsStr,
                                       const string& sortFieldName,
                                       const string& pkField,
...
NormalDocumentPtr DocumentMaker::MakeDocument(docid_t docId,
                                              const string& docStr,
                                              const IndexPartitionSchemaPtr& schema,
                                              MockIndexPart& answer)
{
    ...
    autil::StringTokenizer st(str, ",", autil::StringTokenizer::TOKEN_IGNORE_EMPTY
                       | autil::StringTokenizer::TOKEN_TRIM);
    ...
    if (cursor < st.getNumTokens() 
        && st[cursor].find('[') == string::npos 
        && st[cursor][0] == '#')
    {
        // parse docVersion 
    ...
    for (; cursor < st.getNumTokens(); ++cursor)
    {
        ...
        {
            summaryDoc->SetField(field->GetFieldId(), ConstString(strField, pool));
        ...
        if (attributeSchema && attributeSchema->IsInAttribute(field->GetFieldId()))
        {
            ...
            assert(convertor);
            ConstString encodeStr = convertor->Encode(ConstString(strField), pool);
        ...
        if (indexSchema && indexSchema->IsInIndex(field->GetFieldId()))
        {
            ...
            if (indexSchema->HasPrimaryKeyIndex() && 
                indexSchema->GetPrimaryKeyIndexFieldId() == field->GetFieldId())
            {
            ...
            {
                MakeMockIndex(indexSchema, docId, docPayload, field, hashKeyToStrMap,
                        vecBasePos, firstPosSets, indexes);
                if (docPayload)
                {
                    SetDocPayload(indexDoc, docPayload, dynamic_cast<IndexTokenizeField*>(field));
                }
            }
        }
        else
...
void DocumentMaker::InitAttributes(const IndexPartitionSchemaPtr& schema,
...
Field* DocumentMaker::MakeField(const FieldSchemaPtr& fieldSchema,
                                const string& str,
                                ...
                                autil::mem_pool::Pool* pool)
{
    ...
    autil::StringTokenizer st1(fieldStr, ":", autil::StringTokenizer::TOKEN_IGNORE_EMPTY
                        | autil::StringTokenizer::TOKEN_TRIM);
    ...
    if (fieldConfig->GetFieldType() == ft_raw)
    {
        ...
        field->SetData(ConstString(st1[1], pool));
        return field;
    }

    KeyHasherPtr hasher(KeyHasherFactory::CreateByFieldType(fieldConfig->GetFieldType()));
    ...
    while (true)
    {
        ...
        string oneSection = sectionStr.substr(sectionStart + 1, 
                sectionEnd - sectionStart - 1);
        autil::StringTokenizer st2(oneSection, " ", autil::StringTokenizer::TOKEN_IGNORE_EMPTY
                            | autil::StringTokenizer::TOKEN_TRIM);
        ...
        {
            autil::StringTokenizer st3(st2[i], "^", autil::StringTokenizer::TOKEN_IGNORE_EMPTY
                    | autil::StringTokenizer::TOKEN_TRIM);
            ...
            dictkey_t key;
            hasher->GetHashKey(st3[0].c_str(), key);
...
void DocumentMaker::InitDefaultAttrDocValue(
...
{
    for (fieldid_t fid = 0;
         fid < (fieldid_t)fieldSchema->GetFieldCount(); ++fid)
    {
        if (attrSchema && attrSchema->IsInAttribute(fid))
        {
            ...
            AttributeConvertorPtr convertor(
                    AttributeConvertorFactory::GetInstance()->CreateAttrConvertor(fieldConfig));
            ConstString encodeStr = convertor->Encode(ConstString::EMPTY_STRING, pool);
            attrDoc->SetField(fid, encodeStr);
        }
    }
}

void DocumentMaker::MakeMockIndex(const IndexSchemaPtr& indexSchema, 
                                  docid_t docId, docpayload_t docPayload,
                                  const Field* field, 
    ...
    for (uint32_t i = 0; i < indexSchema->GetIndexCount(); ++i)
    {
        ...
        for (auto fieldIter = tokenizeField->Begin(); fieldIter != tokenizeField->End(); ++fieldIter)
        {
            ...
            for (size_t j = 0; j < section->GetTokenCount(); ++j)
            {
                ...
                if (mockPosting.size() == 0 || 
                    mockPosting[mockPosting.size()-1].docId != docId)
                {
                    MockDoc newMockDoc;
                ...
                if (indexType == it_expack)
                {
                    ...
                    int32_t fieldIdx = 
                        packageIndexConfig->GetFieldIdxInPack(field->GetFieldId());
                    mockDoc.fieldMap |= (fieldmap_t)(1 << fieldIdx);
                }
                mockDoc.posList.push_back(make_pair(vecBasePos[i]++, 
                                token->GetPosPayload()));
            }
            vecBasePos[i]++;
        }
...
void DocumentMaker::SetDocPayload(IndexDocumentPtr& indexDoc, 
                                  docpayload_t docPayload, IndexTokenizeField* field)
{
...
void DocumentMaker::FieldToString(
        const Field* field, const HashKeyToStrMap &hashKeyToStrMap, string& str)
{
    if (field->GetFieldTag() == Field::FieldTag::RAW_FIELD)
    {
        auto rawField = dynamic_cast<const IndexRawField*>(field);
        assert(rawField);
        str = rawField->GetData().toString();
        return;
    }
...
void DocumentMaker::ReclaimDocuments(const MockIndexPart& oriIndexPart, 
                                     ...
                                     MockIndexPart& newIndexPart)
{
    ...
    ReclaimAttribute(oriIndexPart.attributes, reclaimMap, 
                     newIndexPart.attributes);
    ReclaimPrimaryKey(oriIndexPart.primaryKey, reclaimMap, 
                      newIndexPart.primaryKey);
    ReclaimDeletiionMap(oriIndexPart.deletionMap, reclaimMap, 
                        newIndexPart.deletionMap);
    newIndexPart.docCount = oriIndexPart.docCount -
                            reclaimMap->GetDeletedDocCount();
}

void DocumentMaker::ReclaimIndexs(const MockIndexes& oriIndexes, 
                                  const ReclaimMapPtr& reclaimMap,
                                  MockIndexes& newIndexes)
{
    ...
    for (; it != oriIndexes.end(); ++it) 
    {
        ...
        for ( ; indexIt != mockIndex.end(); ++indexIt)
        {
            ...
            {
                sort(newMockPosting.begin(), newMockPosting.end(), 
                     MockDoc::lessThan);
            }
        }
    }
...
void DocumentMaker::ReclaimSummary(const MockSummary& oriSummary, 
                                   const ReclaimMapPtr& reclaimMap,
...
void DocumentMaker::ReclaimAttribute(const MockAttributes& oriAttributes, 
                                     const ReclaimMapPtr& reclaimMap,
...
void DocumentMaker::ReclaimPrimaryKey(const MockPrimaryKey& oriPrimaryKey, 
                                      const ReclaimMapPtr& reclaimMap,
...
void DocumentMaker::ReclaimDeletiionMap(const MockDeletionMap& oriDeletionMap, 
                                        const ReclaimMapPtr& reclaimMap,
