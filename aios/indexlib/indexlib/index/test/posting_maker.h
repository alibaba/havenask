...
public:
    ...
    typedef std::tr1::shared_ptr<DocRecord> DocRecordPtr;
    ...
    typedef std::tr1::shared_ptr<FieldIdVec> FieldIdVecPtr;
...
public:
    ...
    static void BuildPostingData(WriterType& writer,
                                 const DocMap& docMap,
                                 docid_t baseDocId);

    static PostingList MakePostingData(uint32_t docCount,
                                       docid_t& maxDocId,
                                       tf_t& totalTf);

    static PostingList MakePostingData(
            const std::vector<uint32_t>& numDocsInRecords,
    ...
    static PostingList SortByWeightMergePostingLists(
            std::vector<PostingList>& postLists,
    ...
    static void CreateReclaimedPostingList(PostingList& destPostingList,
            const PostingList& srcPostingList, const ReclaimMap& reclaimMap);

    static PostingList MakePositionData(uint32_t docCount, docid_t& maxDocId, 
            uint32_t& totalTF);
    static PostingList MakePositionData(const std::vector<uint32_t>& numDocsInRecords, 
            docid_t& maxDocId, uint32_t& totalTF);

private:
    static void ParseKeyValue(const std::string& str, const std::string& splite,
...
typedef std::tr1::shared_ptr<PostingMaker> PostingMakerPtr;

template<typename WriterType>
inline void PostingMaker::BuildPostingData(
        WriterType& writer, const DocMap& docMap, docid_t baseDocId)
{
    for (DocMap::const_iterator it = docMap.begin();
         it != docMap.end(); ++it)
    {
