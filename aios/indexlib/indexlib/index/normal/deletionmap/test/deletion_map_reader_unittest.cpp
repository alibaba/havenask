...
void DeletionMapReaderTest::CaseSetUp()
{
...
void DeletionMapReaderTest::TestCaseForGetDeletedDocCount()
{
    PartitionDataMaker::MakePartitionDataFiles(
            0, 0, GET_PARTITION_DIRECTORY(), "0,0,0;1,10,10;2,10,20");
    PartitionDataPtr partitionData =
        PartitionDataMaker::CreatePartitionData(GET_FILE_SYSTEM());
...
void DeletionMapReaderTest::InnerTestDeletionmapReadAndWrite(
        uint32_t segmentDocCount, const string& toDeleteDocs,
        const string& deletedDocs, const PartitionDataPtr& partitionData)
{
    InMemorySegmentPtr inMemSegment = partitionData->CreateNewSegment();
...
void DeletionMapReaderTest::CheckDeletedDocs(
        const string& deletedDocs, const PartitionDataPtr& partitionData)
{
...
void DeletionMapReaderTest::CheckResult(
        const PartitionDataPtr& partitionData, const string& resultStr)
{
    DeletionMapReader reader;
