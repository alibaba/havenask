...
protected:
    config::IndexConfigPtr CreateIndexConfig(uint64_t key,
            const std::string& indexName, bool hasPKAttribute = true);
    config::IndexConfigPtr CreateIndexConfig(autil::uint128_t key,
            const std::string& indexName, bool hasPKAttribute = true);
    virtual PrimaryKeyIndexType GetPrimaryKeyIndexType() const
    { return pk_sort_array; }
...
private:
    ...
    template <typename Key>
    merger::SegmentDirectoryPtr CreateData2Merge(const string& pksStr,
                                                index_base::SegmentInfos& segInfos);

    template <typename Key>
    index_base::SegmentInfo MakeOneSegment(
            segmentid_t segmentId, const std::string& pksStr);

    template <typename Key>
    void MakeSegmentMergeInfos(
        const index_base::SegmentInfos& segInfos, 
        const DeletionMapReaderPtr& delReader, 
        index_base::SegmentMergeInfos& segMergeInfos);

    template <typename Key>
    void CheckMergeData(const PrimaryKeyIndexReaderTyped<Key>& reader, 
...
template <typename Key>
void PrimaryKeyIndexMergerTypedTest::TestMerge(
        IndexTestUtil::ToDelete toDel, bool hasPKAttr)
{
    ...
    stringstream ss;
    ss << mRootDir << SEGMENT_FILE_NAME_PREFIX << '_' << segmentId << "_level_0/";
    string segPath = ss.str();
    FileSystemWrapper::MkDir(ss.str(), true);
    string indexPath = ss.str() + INDEX_DIR_NAME + '/';
    ...
    outputInfo.path = indexPath;
    outputInfo.directory = file_system::DirectoryCreator::Create(indexPath);
    ...
    segInfo.Store(segPath + SEGMENT_INFO_FILE_NAME);
...
template <typename Key>
merger::SegmentDirectoryPtr PrimaryKeyIndexMergerTypedTest::CreateData2Merge(
        const string& pksStr, index_base::SegmentInfos& segInfos)
{
    StringTokenizer st(pksStr, ";", StringTokenizer::TOKEN_IGNORE_EMPTY
...
template <typename Key>
index_base::SegmentInfo PrimaryKeyIndexMergerTypedTest::MakeOneSegment(
        segmentid_t segmentId, const std::string& pksStr)
{
    ...
    PrimaryKeyIndexWriterTyped<Key> pkWriter;
...
template <typename Key>
void PrimaryKeyIndexMergerTypedTest::MakeSegmentMergeInfos(
        const index_base::SegmentInfos& segInfos, 
        const DeletionMapReaderPtr& delReader, 
        index_base::SegmentMergeInfos& segMergeInfos)
{
    ...
    for (size_t i = 0; i < segInfos.size(); ++i)
    {
        index_base::SegmentMergeInfo segMergeInfo((segmentid_t)i, segInfos[i], 
                delReader->GetDeletedDocCount((segmentid_t)i),
                baseDocId);
...
template <typename Key>
void PrimaryKeyIndexMergerTypedTest::CheckMergeData(
        const PrimaryKeyIndexReaderTyped<Key>& reader, 
        docid_t answer[], uint32_t answerSize)
{
    ...
    for (uint32_t i = 0; i < answerSize; ++i)
    {
        stringstream ss;
        ss << "pkstr" << i;
        INDEXLIB_TEST_EQUAL(answer[i], reader.Lookup(ss.str()));
