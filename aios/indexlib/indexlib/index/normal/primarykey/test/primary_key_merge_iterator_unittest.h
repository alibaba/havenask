...
private:
    void InitReclaimMap(const ReclaimMapPtr& reclaimMap,
                        uint32_t docCount,
                        const std::string& mapStr = "");

    void InnerTestIterator(
        const std::string& docStr, const std::string& reclaimMapStr,
        const std::string& expectResults);
    
    void InnerTestIteratorOnPkIndexType(
        PrimaryKeyIndexType pkIndexType,
        const std::string& docStr, const std::string& reclaimMapStr,
        const std::string& expectResults);
    
    template <typename Key>
    void CheckIterator(PrimaryKeyMergeIterator<Key>& iter,
                       bool sortKey,
...
template<typename Key>
void PrimaryKeyMergeIteratorTest::CheckIterator(
    PrimaryKeyMergeIterator<Key>& iter, bool sortKey, const std::string& resultStr)
{
