...
public:
    ...
    void TestIterate();
private:
    void InnerTestIterator(
        PrimaryKeyIndexType pkIndexType, const std::string& docStr,
        const std::string& expectResults);

    template <typename Key>
    void CheckIterator(SequentialPrimaryKeyIterator<Key>& iter, bool sortKey,
...
INDEXLIB_UNIT_TEST_CASE(SequentialPrimaryKeyIteratorTest, TestIterate);

/////////////////////////////////////////////////////////////////////
template<typename Key>
bool CompPkPair(const std::pair<Key, docid_t>& left,
...
template<typename Key>
void SequentialPrimaryKeyIteratorTest::CheckIterator(
    SequentialPrimaryKeyIterator<Key>& iter, bool sortKey,
    const std::vector<size_t>& pkCountVec,
    const std::string& resultStr)
{
    ...
    if (sortKey)
    {
        ...
        {
            sort(expectResults.begin() + cursor,
                 expectResults.begin() + cursor + pkCountVec[i],
                 CompPkPair<Key>);
            cursor += pkCountVec[i];
        }
    }
