...
private:
    static void ReserveSliceFile(const file_system::SliceFilePtr& sliceFile,
                                 size_t sliceFileLen);
...
/////////////////////////////////////////////////////
template <typename Key>
void HashPrimaryKeyFormatter<Key>::Format(
    ...
    const file_system::FileWriterPtr& fileWriter) const
{
    ...
    // TODO: use pool
    size_t bufLen = PrimaryKeyHashTable<Key>::CalculateMemorySize(
        pkIter->GetPkCount(), pkIter->GetDocCount());
    char* buffer = new char[bufLen];
    DeserializeToBuffer(pkIter, buffer);
    fileWriter->Write(buffer, bufLen);
    delete []buffer;
}
    ...
    SegmentOutputMapper<PKMergeOutputData>& outputMapper) const
{
    ...
    {
        const auto& output = outputs[i];
        output.fileWriter->Write(buffers[i].data(), buffers[i].size());
    }
}

template <typename Key>
inline void HashPrimaryKeyFormatter<Key>::SerializeToBuffer(
    char* buffer, size_t docCount, const HashMapTypePtr& hashMap) const
{
    assert(hashMap);
    assert(buffer);
...
template <typename Key>
void HashPrimaryKeyFormatter<Key>::DeserializeToBuffer(
    const PrimaryKeyIteratorPtr& pkIterator, char* buffer) const
{
...
template <typename Key>
void HashPrimaryKeyFormatter<Key>::SerializeToFile(
...
template <typename Key>
inline void HashPrimaryKeyFormatter<Key>::ReserveSliceFile(
        const file_system::SliceFilePtr& sliceFile, size_t sliceFileLen)
{
    //1m : 900ms
    //4k : 600ms ~ 700ms
...
template <typename Key>
void HashPrimaryKeyFormatter<Key>::DeserializeToSliceFile(
    const PrimaryKeyIteratorPtr& pkIterator,
    const file_system::SliceFilePtr& sliceFile,
    size_t sliceFileLen)
{
    file_system::SliceFileReaderPtr sliceFileReader = 
        sliceFile->CreateSliceFileReader();
    assert(sliceFileReader);
    IE_LOG(DEBUG, "begin load pk [%s] to hash...",
           sliceFileReader->GetPath().c_str());
    ReserveSliceFile(sliceFile, sliceFileLen);
    char* baseAddress = (char*) sliceFileReader->GetBaseAddress();
    DeserializeToBuffer(pkIterator, baseAddress);
    IE_LOG(DEBUG, "end load pk [%s] to hash...",
           sliceFileReader->GetPath().c_str());
}

template <typename Key>
size_t HashPrimaryKeyFormatter<Key>::EstimatePkCount(
