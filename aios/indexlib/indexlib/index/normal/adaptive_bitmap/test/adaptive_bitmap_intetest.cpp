...
class AdaptiveBitmapInteTest : public INDEXLIB_TESTBASE
{
public:
...
public:
    ...
    void TestCaseForAdaptiveBitmapWithoutHFDict()
    {
        InnerTestAdaptiveBitmap("title", "DOC_FREQUENCY#3",
                                "token1#0,1,2,3,4,5;token2#1,2,3,4,6;token3#2,3,4,5;token4#3,4,6", 
                                false, hp_bitmap);

        InnerTestAdaptiveBitmap("title", "DOC_FREQUENCY#3",
                                "token1#0,1,2,4,5,6;token2#1,2,3,4,5;token3#1,2,5,6;token4#2,3,5", 
                                false, hp_bitmap, false, true);
        ...
        InnerTestAdaptiveBitmap("title", "INDEX_SIZE#-1",
                                "token1#0,1,2,3,4,5;token2#1,2,3,4,6;token3#2,3,4,5;token4#3,4,6;token5#4,5", 
                                false, hp_bitmap);

        InnerTestAdaptiveBitmap("title", "INDEX_SIZE#-1",
                                "token1#0,1,2,4,5,6;token2#1,2,3,4,5;token3#1,2,5,6;token4#2,3,5;token5#2,6", 
                                false, hp_bitmap, false, true);
    ...
    void TestCaseForAdaptiveBitmapWithHFDict()
    {
        InnerTestAdaptiveBitmap("title", "DOC_FREQUENCY#3",
                                "token1#0,1,2,3,4,5;token2#1,2,3,4,6;token3#2,3,4,5;token4#3,4,6", 
                                true, hp_bitmap);
    ...
    void TestCaseForNotAdaptiveBitmapWithRealTime()
    {
        InnerTestAdaptiveBitmap("title", "DOC_FREQUENCY#3",
                                "token1#0,1,2,3,4,5", 
                                true, hp_both, true);
    ...
    void InnerTestAdaptiveBitmap(
            string indexName, string adaptiveRuleStr, string expectedBtimapStr, bool hasHfDict,
    ...
    void TestCaseForMergeMultiSegment()
    {
        ...
        string dictName = SetHighFrequenceDictionary(indexName, hasHfDict, type, mSchema);
        string subDictName = SetHighFrequenceDictionary(
                subIndexName, hasHfDict, type, subSchema);
        ...
        INDEXLIB_TEST_TRUE(psm.Init(mSchema, mOptions, GET_TEST_DATA_PATH()));
        ...
        INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_FULL, docString, "title:t1", 
                        "docid=0;docid=1;docid=2;docid=3;"));
        INDEXLIB_TEST_TRUE(psm.Transfer(QUERY, "", "sub_title:s1", 
        ...
        INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_INC_NO_MERGE, docString2, "title:t1", 
                        "docid=0;docid=1;docid=2;docid=3;docid=4;"));
        ...
        DeployIndexChecker::CheckDeployIndexMeta(GET_TEST_DATA_PATH(), 2, INVALID_VERSION);
        DeployIndexChecker::CheckDeployIndexMeta(GET_TEST_DATA_PATH(), 2, 1);
    }        
    ...
    void TestCaseForMultiSegmentSort() 
    {
        ...
        partitionMeta.AddSortDescription("userid", sp_asc);
        partitionMeta.Store(GET_PARTITION_DIRECTORY());

        autil::legacy::FromJsonString(mOptions.GetMergeConfig().GetSplitSegmentConfig(), mergeConfigStr);
        PartitionStateMachine psm;
        INDEXLIB_TEST_TRUE(psm.Init(schema, mOptions, GET_TEST_DATA_PATH()));
...
private:
    ...
    string SetHighFrequenceDictionary(const string& indexName, bool hasHighFreq,
    ...
    // adaptiveRuleStr:
    // DOC_FREQUENCY#10, PERCENT#20, INDEX_SIZE#-1
    void SetAdaptiveHighFrequenceDictionary(const string& indexName,
    ...
    void BuildIndex(const IndexPartitionSchemaPtr& schema, 
                    const string& rawDocStr, const partition::IndexPartitionPtr& partition)
    {
        ...
        if (partition)
        {
            builder.reset(new partition::IndexBuilder(partition, quotaControl));
        }
        else
        {
            builder.reset(new partition::IndexBuilder(mRootDir, mOptions, schema, quotaControl));
        }
    ...
    void CheckIndex(const string& indexName, const string& expectedBitmapStr, 
                    HighFrequencyTermPostingType type, 
                    ...
                    string dictName)
    {
        ...
        CheckIndex(indexName, expectedBitmapStr, type, indexPartReader, 
                   partition != NULL, dictName, mSchema);
    }

    void CheckIndex(const string& indexName, const string& expectedBitmapStr, 
                    HighFrequencyTermPostingType type, 
                    const partition::IndexPartitionReaderPtr& indexPartReader,
                    bool realtime,
                    string dictName,
                    const IndexPartitionSchemaPtr& schema)
    {
        IndexReaderPtr indexReader = indexPartReader->GetIndexReader(indexName);
    ...
    void CheckTerm(const string& indexName,
                   ...
                   HighFrequencyTermPostingType type)
    {
        ...
        PostingIterator *iter = indexReader->Lookup(term, 1000, pt_bitmap, NULL);
        ...
        if (type == hp_bitmap)
        {
            PostingIterator *iter = indexReader->Lookup(term, 1000, pt_normal, NULL);
            INDEXLIB_TEST_TRUE(iter == NULL);
        }
        else // hp_both
        {
    ...
    void CheckDictionary(const string& indexName, 
                         size_t bitmapTermCount,
                         bool realtime,
                         string dictName,
                         const IndexPartitionSchemaPtr& schema)
    {
        ...
        HighFrequencyVocabularyPtr vol = indexConf->GetHighFreqVocabulary();
        if (!realtime)
        {
        ...
        if (realtime)
        {
            rootPath = FileSystemWrapper::JoinPath(rootPath, "rt_index_partition");
            string adaptivePath = FileSystemWrapper::JoinPath(rootPath, ADAPTIVE_DICT_DIR_NAME);
            INDEXLIB_TEST_TRUE(!FileSystemWrapper::IsExist(adaptivePath));
        }
        else 
        {
            string adaptivePath = FileSystemWrapper::JoinPath(rootPath, ADAPTIVE_DICT_DIR_NAME);
            ArchiveFolderPtr archiveFolder(new ArchiveFolder(false));
            archiveFolder->Open(adaptivePath);
            FileWrapperPtr dictFile = archiveFolder->GetInnerFile(dictName, fslib::READ);
            string content;
            FileSystemWrapper::AtomicLoad(dictFile.get(), content);
            StringTokenizer tokens(content, "\n", 
...
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForAdaptiveBitmapWithoutHFDict);
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForAdaptiveBitmapWithHFDict);
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForNotAdaptiveBitmapWithRealTime);
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForSubDoc);
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForMergeMultiSegment);
INDEXLIB_UNIT_TEST_CASE(AdaptiveBitmapInteTest, TestCaseForMultiSegmentSort);
