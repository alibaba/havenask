...
#include "indexlib/index/normal/reclaim_map/test/reclaim_map_unittest.h"
...
void ReclaimMapTest::CaseSetUp()
{
    ...
    attrConfig.reset(new config::AttributeConfig);
    attrConfig->SetAttrId(0);
    config::FieldConfigPtr fieldConfig(
            new config::FieldConfig("attr_name", ft_integer, false));
    attrConfig->Init(fieldConfig);

    AttributeConfigPtr attrConfig1;
    attrConfig1.reset(new config::AttributeConfig);
    attrConfig1->SetAttrId(1);
    config::FieldConfigPtr fieldConfig1(
            new config::FieldConfig("attr_name1", ft_integer, false));
    attrConfig1->Init(fieldConfig1);
    ...
    config::FieldConfigPtr mainJoinFieldConfig(
            new config::FieldConfig(MAIN_DOCID_TO_SUB_DOCID_ATTR_NAME, 
                    ft_int32, false));
    ...
    mAttrConfigs.push_back(attrConfig);
    mAttrConfigs.push_back(attrConfig1);
    mPatterns.push_back(sp_desc);
    mPatterns.push_back(sp_asc);
...
{
    {
        ...
        reclaimMap.Init(segMergeInfos, mDelMapReader, readerContainer, true);
...
void ReclaimMapTest::TestMergePartWithoutDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergePart(IndexTestUtil::NoDelete, false, 
                  expectedDocIdStr, needReverseMapping,
...
void ReclaimMapTest::TestMergePartWithDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergePart(IndexTestUtil::DeleteEven, false, 
                  expectedDocIdStr, needReverseMapping,
                  expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}
...
void ReclaimMapTest::TestSortMergePartWithoutDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergePart(IndexTestUtil::NoDelete, true, 
                  expectedDocIdStr, needReverseMapping,
                  expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}
...
void ReclaimMapTest::TestSortMergePartWithDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergePart(IndexTestUtil::DeleteEven, true, 
                  expectedDocIdStr, needReverseMapping,
                  expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}
...
{
    {
        ...
        InnerSubReclaimMapInit(IndexTestUtil::NoDelete, IndexTestUtil::NoDelete, false, true,
                               expectedDocIdStr, expectedReverseDocIdStr, expectedSegIdStr,
                               expectedMainJoinStr, expectedSubJoinStr);
    }
    {
        ...
        InnerSubReclaimMapInit(IndexTestUtil::NoDelete, IndexTestUtil::DeleteEven, false, true,
                               expectedDocIdStr, expectedReverseDocIdStr, expectedSegIdStr,
                               expectedMainJoinStr, expectedSubJoinStr);
    }
    {
        ...
        InnerSubReclaimMapInit(IndexTestUtil::DeleteEven, IndexTestUtil::NoDelete, false, true,
                               expectedDocIdStr, expectedReverseDocIdStr, expectedSegIdStr,
                               expectedMainJoinStr, expectedSubJoinStr);
    }
    {
        ...
        InnerSubReclaimMapInit(IndexTestUtil::DeleteEven, IndexTestUtil::DeleteEven, false, true,
                               expectedDocIdStr, expectedReverseDocIdStr, expectedSegIdStr,
                               expectedMainJoinStr, expectedSubJoinStr);
    }
    {
        ...
        InnerSubReclaimMapInit(IndexTestUtil::DeleteEven, IndexTestUtil::DeleteEven, true, true,
                               expectedDocIdStr, expectedReverseDocIdStr, expectedSegIdStr,
                               expectedMainJoinStr, expectedSubJoinStr);
    }
}

void ReclaimMapTest::InnerSubReclaimMapInit(
        ...
        const string& expectedMainJoinValueStr, const string& expectedSubJoinValueStr)
{
    ...
    if (sortByWeight)
    {
        CreateSegmentMergeInfosForSort(mDocCounts, mSegIds, 
                mDelMapReader, mMergePlan, segMergeInfos, 
                mHighAttrValue, mLowAttrValue);
        mainReclaimMap.Init(segMergeInfos, mDelMapReader, readerContainer, mAttrConfigs, mPatterns,
            mSegDir, needReverseMapping);
    }
    else
    {
        CreateSegmentMergeInfos(mDocCounts, mSegIds, mDelMapReader, mMergePlan, segMergeInfos);
        mainReclaimMap.Init(segMergeInfos, mDelMapReader, readerContainer, needReverseMapping);
    }

    SegmentMergeInfos subSegMergeInfos;    
    CreateSegmentMergeInfos(mSubDocCounts, mSegIds, mSubDelMapReader,
                            mMergePlan, subSegMergeInfos, true);

    ReclaimMap subReclaimMap;
    subReclaimMap.Init(&mainReclaimMap, mSegDir, mMainJoinAttrConfig,
                       segMergeInfos, subSegMergeInfos,
                       mSubDelMapReader, needReverseMapping);
    
    CheckAnswer(subReclaimMap, expectedDocIdStr, needReverseMapping, 
                expectedOldDocIdStr, expectedSegIdStr);

    CheckJoinValue(mainReclaimMap, expectedMainJoinValueStr);
    CheckJoinValue(subReclaimMap, expectedSubJoinValueStr);
}

void ReclaimMapTest::TestMergePart(IndexTestUtil::ToDelete toDel, 
                                   bool sortByWeight, 
                                   const string& expectedDocIdStr,
                                   bool needReverseMapping,
                                   const string& expectedOldDocIdStr, 
                                   const string& expectedSegIdStr,
                                   bool splitSegment)
{
    string docCountsStr = mDocCountStr;
    string segIdStr = "0,1,2,3";
    ...
    InnerTestMerge(docCountsStr, segIdStr, seg2MergeStr, sortByWeight,
                   needReverseMapping, toDel, expectedDocIdStr, 
                   expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}
...
void ReclaimMapTest::TestMergeAllWithoutDelete(bool needReverseMapping, bool multiTargetSegment)
{
    ...
    TestMergeAll(IndexTestUtil::NoDelete, false, 
                 expectedDocIdStr, needReverseMapping,
...
void ReclaimMapTest::TestMergeAllWithDelete(bool needReverseMapping, bool multiTargetSegment)
{
    ...
    TestMergeAll(IndexTestUtil::DeleteEven, false, 
                 expectedDocIdStr, needReverseMapping,
                 expectedOldDocIdStr, expectedSegIdStr, multiTargetSegment);
}
...
void ReclaimMapTest::TestSortMergeAllWithoutDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergeAll(IndexTestUtil::NoDelete, true, expectedDocIdStr, needReverseMapping, expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}
...
void ReclaimMapTest::TestSortMergeAllWithDelete(bool needReverseMapping, bool splitSegment)
{
    ...
    TestMergeAll(IndexTestUtil::DeleteEven, true, 
                 expectedDocIdStr, needReverseMapping,
                 expectedOldDocIdStr, expectedSegIdStr, splitSegment);
}

void ReclaimMapTest::TestMergeAll(IndexTestUtil::ToDelete toDel,
                                  bool sortByWeight,
                                  const string& expectedDocIdStr,
                                  bool needReverseMapping, 
                                  const string& expectedOldDocIdStr, 
...
void ReclaimMapTest::TestMergeSegmentApart(IndexTestUtil::ToDelete toDel,
    const string& expectedDocIdStr, bool needReverseMapping, bool splitSegment)
{
    string docCountsStr = mDocCountStr;
    string segIdStr = "1,3,4,6";
    string seg2MergeStr = "3,6";
    InnerTestMerge(docCountsStr, segIdStr, seg2MergeStr, false,
                   needReverseMapping, toDel, expectedDocIdStr, "", "", splitSegment);
}

void ReclaimMapTest::Prepare(const string& docCountsStr,
                             const string& segIdStr,
                             const string& seg2MergeStr,
                             IndexTestUtil::ToDelete toDel,
                             IndexTestUtil::ToDelete subDocToDel,
                             bool multiTargetSegment)
{
    mDocCounts.clear();
    mSegIds.clear();
    ...
    mSegDir.reset(new MockSegmentDirectory(mRootDir, v));
    mSegDir->Init(true);
    mSubSegDir = mSegDir->GetSubSegmentDirectory();
    mDelMapReader.reset();
    mDelMapReader = IndexTestUtil::CreateDeletionMap(
            mSegDir->GetVersion(), mDocCounts, toDel);
    mSubDelMapReader.reset();
    mSubDelMapReader = IndexTestUtil::CreateDeletionMap(
            mSubSegDir->GetVersion(), mSubDocCounts, subDocToDel);
                        ...
                        
{
    ...
    {
        size_t cnt = 0;
        reclaimMap.SetSegmentSplitHandler([cnt](segmentid_t, docid_t) mutable -> segmentindex_t {
            auto ret = cnt % 2;
            ++cnt;
            return ret;
        });
    ...
    if (sortByWeight)
    {
        CreateSegmentMergeInfosForSort(mDocCounts, mSegIds, 
                mDelMapReader, mMergePlan, segMergeInfos, 
                mHighAttrValue, mLowAttrValue);
        reclaimMap.Init(segMergeInfos, mDelMapReader,readerContainer, mAttrConfigs,
                        mPatterns, mSegDir, needReverseMapping);
    }
    else
    {
        CreateSegmentMergeInfos(mDocCounts, mSegIds, mDelMapReader, mMergePlan, segMergeInfos);
        reclaimMap.Init(segMergeInfos, mDelMapReader, readerContainer, needReverseMapping);
    }
    CheckAnswer(reclaimMap, expectedDocIdStr, 
                needReverseMapping, expectedReverseDocIdStr,
                expectedSegIdStr);
}

void ReclaimMapTest::CheckJoinValue(const ReclaimMap& reclaimMap, 
                                    const string& expectedJoinValueStr)
{
...
void ReclaimMapTest::CheckAnswer(const ReclaimMap& reclaimMap, 
                                 const string& expectedDocIdStr,
                                 bool needReverseMapping,
                                 const string& expectedOldDocIdStr,
                                 const string& expectedSegIdStr)
{
    ...
    if (needReverseMapping)
    {
        ...
        StringUtil::fromString(expectedOldDocIdStr, 
                               expectedOldDocIds, ",");
        StringUtil::fromString(expectedSegIdStr, expectedSegIds, ",");
        INDEXLIB_TEST_EQUAL(expectedSegIds.size(), 
                            expectedOldDocIds.size());
        INDEXLIB_TEST_EQUAL(expectedSegIds.size(), 
                            reclaimMap.GetNewDocCount());
        for (uint32_t i = 0; i < reclaimMap.GetNewDocCount(); i++)
        {
            segmentid_t segId = INVALID_SEGMENTID;
            docid_t docId = 
                reclaimMap.GetOldDocIdAndSegId((docid_t)i, segId);
...
void ReclaimMapTest::CreateSegmentMergeInfos(
        ...
        bool isSubDoc)
{
    ...
    for (size_t i = 0; i < docCounts.size(); ++i)
    {
        ...
        uint32_t deletedDocCount = 
            deletionMapReader->GetDeletedDocCount(segmentIds[i]);
        SegmentMergeInfo segMergeInfo(segmentIds[i], segInfo, 
                deletedDocCount, newBaseDocId);

        if (!isSubDoc)
        {
            vector<docid_t> joinAttrs(
                    mMainJoinAttrValue.begin() + oldBaseDocId,
                    mMainJoinAttrValue.begin() + oldBaseDocId + docCounts[i]);

            AttributeSegmentReaderPtr joinAttrSegReader(
                    new FakeAttributeSegmentReaderRM(joinAttrs));

            std::vector<AttributeSegmentReaderPtr> attrSegReaders;
            attrSegReaders.push_back(joinAttrSegReader);

            InMemorySegmentReaderPtr inMemSegReader(
                    new FakeInMemorySegmentReader(attrSegReaders));

            //TODO: use in memory segment reader
            segMergeInfo.inMemorySegmentReader = inMemSegReader;
        }
...
void ReclaimMapTest::CreateSegmentMergeInfosForSort(
        const vector<uint32_t>& docCounts, 
        const vector<segmentid_t>& segmentIds,
        const DeletionMapReaderPtr& deletionMapReader, 
        const MergePlan& plan,
        ...
        const vector<int32_t>& lowAttrValues)
{
    ...
    for (size_t i = 0; i < docCounts.size(); ++i)
    {
        ...
        vector<int32_t> localHighAttrs(
                highAttrValues.begin() + oldBaseDocId,
                highAttrValues.begin() + oldBaseDocId + docCounts[i]);

        AttributeSegmentReaderPtr highAttrSegReader(
                new FakeAttributeSegmentReaderRM(localHighAttrs));

        vector<int32_t> localLowAttrs(
                lowAttrValues.begin() + oldBaseDocId,
                lowAttrValues.begin() + oldBaseDocId + docCounts[i]);

        AttributeSegmentReaderPtr lowAttrSegReader(
                new FakeAttributeSegmentReaderRM(localLowAttrs));
...
void ReclaimMapTest::TestLoadDocIdArrayWithOldVersion()
{
    ...
    bufferedFileWriter.Write(&docCount, sizeof(docCount));
    bufferedFileWriter.Write(oldDocFormat.data(), sizeof(oldDocFormat[0]) * docCount);
