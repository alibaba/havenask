...
void NormalInDocPositionIteratorTest::CaseSetUp()
{
	...
	"test_pack", MAX_FIELD_COUNT);
    
    mIndexConfig = DYNAMIC_POINTER_CAST(
	PackageIndexConfig,
	mSchema->GetIndexSchema()->GetIndexConfig("test_pack"));
}
...
void NormalInDocPositionIteratorTest::TestCaseForSectionFeature()
{
    TestForSectionFeature(OPTION_FLAG_ALL);
}

void NormalInDocPositionIteratorTest::TestCaseForNoPayloadSectionFeature()
{
    TestForSectionFeature(NO_PAYLOAD);
}
...
void NormalInDocPositionIteratorTest::TestCaseForTfBitmap()
{
    TestForSectionFeature(OPTION_FLAG_ALL | of_tf_bitmap);
}
...
void NormalInDocPositionIteratorTest::TestForSectionFeature(optionflag_t optionFlag)
{
    ...
    IndexTestUtil::BuildOneSegmentFromDataString(str, filePath, 
            0, answerMap, indexFormatOption);
    ...
    SectionAttributeReaderImplPtr sectionReaderImpl;
    ...
    BufferedPostingIterator iter(postingFormatOption, NULL);

    uint8_t compressMode = ShortListOptimizeUtil::GetCompressMode(
            (df_t)2, (ttf_t)7, PFOR_DELTA_COMPRESS_MODE);
    SegmentPosting segPosting(postingFormatOption);
    segPosting.Init(compressMode, sliceListPtr, 0, SegmentInfo());
    ...
    InDocPositionIteratorPtr posIter = termMathData.GetInDocPositionIterator();
    ...
    if (optionFlag & of_position_list)
    {
        ...
        //test the second cloned iter
        {
            ...
            InDocPositionIteratorPtr posIter2
                = termMathData2.GetInDocPositionIterator();
...
SectionAttributeReaderImplPtr NormalInDocPositionIteratorTest::PrepareSectionReader()
{
