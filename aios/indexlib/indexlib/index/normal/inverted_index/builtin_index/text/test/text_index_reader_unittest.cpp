...
public:
    ...
    void TestDumpEmptySegment(optionflag_t optionFlag, bool hasHighFreq = false)
    {
        ...
        {
            NormalIndexWriter writer(HASHMAP_INIT_SIZE, IndexPartitionOptions());
...
private:
    ...
    void CheckIndexReaderLookup(const vector<uint32_t>& docNums,
                                TextIndexReaderPtr& indexReader,
                                const IndexConfigPtr& indexConfig,
                                Answer& answer,
                                bool usePool = true)
    {
        ...
        for (; keyIter != postingAnswerMap.end(); ++keyIter)
        {
            ...
            {
                PostingIterator *bitmapIt = 
                    indexReader->Lookup(term, 1000, pt_bitmap, pPool);
    ...
    TextIndexReaderPtr CreateIndexReader(const vector<uint32_t>& docNums, 
            const IndexConfigPtr& indexConfig)
    {
