...
public:
    ...
    public:
        ...
        typedef std::tr1::shared_ptr<IndexReaderType> IndexReaderPtrType;
        typedef IndexWriterType_ IndexWriterType;
        typedef std::tr1::shared_ptr<IndexWriterType> IndexWriterPtrType;
...
private:
    ...
    void TestLookUpWithManyDoc(optionflag_t optionFlag, bool hasHighFreq = false)
    {
        ...
        typename TestTraitsType::IndexReaderPtrType indexReader = CreateIndexReader<TestTraitsType>(docNums, indexConfig);
    ...
    void TestDumpEmptySegment(optionflag_t optionFlag, bool hasHighFreq = false)
    {
        ...
        {
            typename TestTraitsType::IndexWriterType writer(
                    HASHMAP_INIT_SIZE, IndexPartitionOptions());
                                ...
                                Answer& answer)
    {
        ...
        for (; keyIter != postingAnswerMap.end(); ++keyIter)
        {
            ...
            PostingIteratorPtr iter(indexReader->Lookup(term));
            INDEXLIB_TEST_TRUE(iter.get() != NULL);

            bool hasPos = iter->HasPosition();
            if (hasHighFreq && mVol->Lookup(term.GetWord()))
            {
                PostingIteratorPtr bitmapIt(
                        indexReader->Lookup(term, 1000, pt_bitmap));

                INDEXLIB_TEST_EQUAL(bitmapIt->GetType(), pi_bitmap);
                BitmapPostingIteratorPtr bmIt =
                    dynamic_pointer_cast<BitmapPostingIterator>(bitmapIt);
    ...
    template<typename TestTraitsType>
    SingleFieldIndexConfigPtr CreateIndexConfig(
            optionflag_t optionFlag,
            bool hasHighFreq = false)
    {
        SingleFieldIndexConfigPtr indexConfig;
        IndexDocumentMaker::CreateSingleFieldIndexConfig(indexConfig, "NumberIndex", 
                it_number, TestTraitsType::fieldType, optionFlag);
        if (hasHighFreq)
        {
...
#define REGISTER_TEST_CASE_INT_TYPE(type)                               \
    typedef NumberIndexReaderTest::TestTraits<NumberInt##type##IndexReader, NormalIndexWriter, ft_int##type> TestTraitsInt##type; \
    REGISTER_TEST_CASE(TestTraitsInt##type)

#define REGISTER_TEST_CASE_UINT_TYPE(type)                              \
    typedef NumberIndexReaderTest::TestTraits<NumberUInt##type##IndexReader, NormalIndexWriter, ft_uint##type> TestTraitsUInt##type; \
