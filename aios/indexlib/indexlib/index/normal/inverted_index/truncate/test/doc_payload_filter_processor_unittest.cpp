...
public:
    index::TermMeta *GetTermMeta() const override {return NULL ;};
    ...
    void SetDocPayload(std::string payloads)
    {
        autil::StringUtil::fromString(payloads, mDocPayloads, ";");    
    }
...
public:
    ...
    void check_data(std::string docpayloads, std::string boolvalues, 
                    DocPayloadFilterProcessor& docPayloadFilter)
    {
        FakeTruncatePostingIterator* fakeIt = new FakeTruncatePostingIterator();
        fakeIt->SetDocPayload(docpayloads);
        PostingIteratorPtr pIt(fakeIt);
        INDEXLIB_TEST_TRUE(docPayloadFilter.BeginFilter(0, pIt));
