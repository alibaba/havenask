...
private:
    ...
    void CheckWriter(df_t dfThreshold, bool expectNeedTruncate, 
                     const std::string& dfForIndexWriter, 
                     const std::string& expectAddPostingCount)
    {
        ...
        INDEXLIB_TEST_TRUE(multiWriter.NeedTruncate(
                        TruncateTriggerInfo(0, dfThreshold)) == expectNeedTruncate);
        ...
        PostingIteratorPtr postingIter(new FakePostingIterator(docIdVect));
        multiWriter.AddPosting(0, postingIter, postingIter->GetTermMeta()->GetDocFreq());

        for (size_t i = 0; i < expectAddPostingCountVec.size(); ++i)
        {
            FakeTruncateIndexWriterPtr fakeTruncateIndexWriter = 
                dynamic_pointer_cast<FakeTruncateIndexWriter>(indexWriterVec[i]);
            INDEXLIB_TEST_EQUAL((uint32_t)expectAddPostingCountVec[i], 
                    fakeTruncateIndexWriter->GetAddPostingCounter());
        }
    }
