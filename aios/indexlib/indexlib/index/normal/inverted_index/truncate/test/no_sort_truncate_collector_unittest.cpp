...
public:
    ...
    void TestCaseForNoDistinct()
    {
        ...
        DocCollectorPtr docCollector(
                new NoSortTruncateCollector(EXPECTED_DOC_COUNT,
                        EXPECTED_DOC_COUNT, DocFilterProcessorPtr(),
                        DocDistinctorPtr(), mBucketVecAllocator));
        ...
        PostingIteratorPtr postingIt(new FakePostingIterator(addDocIds));
...
private:
    void InnerTestForDistinct(size_t docCountToAdd, size_t expectedDocCount,
    ...
    DocDistinctorPtr CreateDocDistinctor(string attrValue, 
            uint64_t minDistinctValueCountToReserve)
    {
        FakeTruncateAttributeReader<int32_t>* pAttrReader = 
            new FakeTruncateAttributeReader<int32_t>();
        pAttrReader->SetAttrValue(attrValue);
        TruncateAttributeReaderPtr attrReader(pAttrReader);
        return DocDistinctorPtr (new DocDistinctorTyped<int32_t>(
