...
public:
    MatchInfoTuple(docid_t baseDocId, const std::vector<docid_t>& docids,
...
private:
    void InnerTestSeekDoc(const index::PostingIteratorPtr& iter,
                          docid_t startDocId,
                          const std::vector<docid_t>& expectDocIds,
                          const std::vector<int32_t>& expectValues); 
private:
    std::vector<SegmentMatchInfo> MakeSegMatchInfos(
        const std::vector<MatchInfoTuple> &matchInfos,
