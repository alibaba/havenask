...
public:
    void Init(const index_base::PartitionDataPtr& partitionData,
              segmentid_t segmentId) override;
    
    void AddPatchFile(const file_system::DirectoryPtr& directory,
    ...
    size_t Next(docid_t& docId, uint8_t* buffer, size_t bufferLen) override;
    ...
    size_t GetPatchFileLength() const override { assert(false); return 0; };
...
private:
    AttributePatchReader* CreateSubAttributePatchReader(
...
private:
    typedef std::priority_queue<PatchReaderItem, std::vector<PatchReaderItem>,
        PatchReaderItemComparator> PatchReaderItemHeap;
