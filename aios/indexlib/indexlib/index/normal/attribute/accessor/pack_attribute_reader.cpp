    ...
    const PartitionDataPtr& partitionData)
{
    ...
    if (!mDataConvertor)
    {
        IE_LOG(ERROR, "fail to create AttributeConvertor"
    ...
    InitBuffer();
    return mDataReader->Open(attrConfig, partitionData);
}

void PackAttributeReader::InitBuildResourceMetricsNode(
        BuildResourceMetrics* buildResourceMetrics)
{
    if (buildResourceMetrics)
    {
        ...
        mBuildResourceMetricsNode->Update(BMT_CURRENT_MEMORY_USE,
                mSimplePool.getUsedBytes());
    }
}

AttributeReference* PackAttributeReader::GetSubAttributeReference(
    const string& subAttrName) const
{
    ...
    AttributeReference* attrRef =
        mPackAttrFormatter->GetAttributeReference(subAttrName);
    ...
    attrid_t subAttrId) const
{
    ...
    AttributeReference* attrRef =
        mPackAttrFormatter->GetAttributeReference(subAttrId);
    IncreaseAccessCounter(attrRef);
    return attrRef;
}
...
PackAttributeIterator* PackAttributeReader::CreateIterator(Pool *pool)
{
    ...
    AttributeIteratorTyped<MultiChar>* iterTyped =
        dynamic_cast<AttributeIteratorTyped<MultiChar>*>(iter);
    assert(iterTyped);
    return IE_POOL_COMPATIBLE_NEW_CLASS(pool, PackAttributeIterator, iterTyped, pool);
}

AttributeIteratorBase* PackAttributeReader::CreateIterator(Pool *pool,
        const string& subAttrName) const
{
...
template<typename T>
AttributeIteratorBase* PackAttributeReader::CreateIteratorTyped(Pool *pool,
        const string& subAttrName, bool isMulti) const
{
    ...
    if (isMulti)
    {
        ...
        return IE_POOL_COMPATIBLE_NEW_CLASS(pool, PackAttributeIteratorTyped<MultiValueType<T> >,
                refTyped, iterTyped, pool);
    }
    else
    {
        AttributeReferenceTyped<T>* refTyped = 
            dynamic_cast<AttributeReferenceTyped<T>* >(ref);
        if (refTyped == NULL)
        {
            POOL_DELETE_CLASS(iter);
            return NULL;
        }
        return IE_POOL_COMPATIBLE_NEW_CLASS(pool, PackAttributeIteratorTyped<T>,
                refTyped, iterTyped, pool);
    }
}

bool PackAttributeReader::Read(docid_t docId, const string& attrName,
                               string& value, Pool* pool) const
{
    const char* baseAddr = GetBaseAddress(docId);
    if (!baseAddr)
    {
        return false;
    }
...
bool PackAttributeReader::UpdatePackFields(
    docid_t docId, const PackAttributeFields& packFields,
    bool hasHashKeyInFields)
{
    const char* data = GetBaseAddress(docId);
    if (!data)
    {
        IE_LOG(ERROR, "update pack attribute fields"
    ...
    ConstString mergeValue = 
        mPackAttrFormatter->MergeAndFormatUpdateFields(
                data, packFields, hasHashKeyInFields, *mBuffer);
    ...
    // remove hashKey
    common::AttrValueMeta decodeValue =
        mDataConvertor->Decode(mergeValue);
    
    return mDataReader->UpdateField(
        docId, (uint8_t*)decodeValue.data.data(), decodeValue.data.size());    
}
...
void PackAttributeReader::InitBuffer()
{
    ...
    if (mAttrMetrics)
    {
        mAttrMetrics->IncreasePackAttributeReaderBufferSizeValue(
