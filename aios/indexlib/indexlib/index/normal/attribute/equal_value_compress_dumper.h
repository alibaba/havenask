#ifndef __INDEXLIB_EQUAL_VALUE_COMPRESS_DUMPER_H
#define __INDEXLIB_EQUAL_VALUE_COMPRESS_DUMPER_H
...
{
public:
    EqualValueCompressDumper(bool needMagicTail, 
...
public:
    ...
    { mCompressWriter.CompressData(data, count); }

    void CompressData(const std::vector<T>& data)
    { CompressData((T*)data.data(), data.size());}

    void CompressData(const common::TypedSliceList<T>* data);
...
private:
    ...
    common::EquivalentCompressWriter<T> mCompressWriter;
    bool mNeedMagicTail;
private:
...
template <typename T>
EqualValueCompressDumper<T>::EqualValueCompressDumper(
        bool needMagicTail, autil::mem_pool::PoolBase* pool)
    : mPool(pool)
...
template <typename T>
inline void EqualValueCompressDumper<T>::CompressData(
...
template <typename T>
inline void EqualValueCompressDumper<T>::Dump(
        const file_system::FileWriterPtr& file)
{
    ...
    common::EquivalentCompressReader<T> reader(buffer);

    if (reader.Size() > 0)
    {
        uint32_t optSlotItemCount = EqualValueCompressAdvisor<T>::EstimateOptimizeSlotItemCount(
	    reader, EQUAL_COMPRESS_SAMPLE_RATIO);

        mCompressWriter.Init(optSlotItemCount);
        mCompressWriter.CompressData(reader);
    }
    ...
    file->ReserveFileNode(reserveSize);
    
    mCompressWriter.Dump(file);
    IE_POOL_COMPATIBLE_DELETE_VECTOR(mPool, buffer, compressLength);
    if (mNeedMagicTail)
    {
        DumpMagicTail(file);
    }
    assert(file->GetLength() == reserveSize);
}

template <>
inline void EqualValueCompressDumper<uint32_t>::DumpMagicTail(
        const file_system::FileWriterPtr& file)
{
    uint32_t magicTail = UINT32_OFFSET_TAIL_MAGIC;
    file->Write(&magicTail, sizeof(uint32_t));
}

template <>
inline void EqualValueCompressDumper<uint64_t>::DumpMagicTail(
        const file_system::FileWriterPtr& file)
{
...
template <typename T>
inline void EqualValueCompressDumper<T>::DumpMagicTail(
        const file_system::FileWriterPtr &file)
{
    // do nothing
    assert(false);
...
#endif //__INDEXLIB_EQUAL_VALUE_COMPRESS_DUMPER_H
