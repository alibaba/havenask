#ifndef __INDEXLIB_ATTRIBUTE_UPDATE_INFO_H
#define __INDEXLIB_ATTRIBUTE_UPDATE_INFO_H
...
{
public:
    SegmentUpdateInfo(segmentid_t segId = INVALID_SEGMENTID,
                      uint32_t docCount = 0)
        : updateSegId(segId)
    ...
    {
        return updateSegId == other.updateSegId
        && updateDocCount == other.updateDocCount;
    }
...
public:
    ...
    {
    public:
        Iterator(const SegmentUpdateInfoMap& map)
            : mCursor(map.begin())
            , mEndCur(map.end())
        {}
...
public:
    ...
    bool Add(const SegmentUpdateInfo& segUpdateInfo)
    {
        ...
        ret = mUpdateMap.insert(
            std::make_pair(segUpdateInfo.updateSegId, segUpdateInfo));
        return ret.second;
    }

    Iterator CreateIterator() const
    { return Iterator(mUpdateMap); }

    size_t Size() const
    {
        return mUpdateMap.size();
    }
...
#endif //__INDEXLIB_ATTRIBUTE_UPDATE_INFO_H
