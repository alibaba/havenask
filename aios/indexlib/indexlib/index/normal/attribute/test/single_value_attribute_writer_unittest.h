...
private:
    template<typename T>
    void AddData(SingleValueAttributeWriter<T>& writer,
                 T expectedData[], uint32_t docNum,
                 const common::AttributeConvertorPtr& convertor,
                 util::BuildResourceMetrics& buildMetrics);

    template<typename T>
    void UpdateData(SingleValueAttributeWriter<T>& writer, 
                    T expectedData[], uint32_t docNum,
                    const common::AttributeConvertorPtr& convertor,
                    util::BuildResourceMetrics& buildMetrics);
    ...
    template<typename T>
    void CheckWriterData(const SingleValueAttributeWriter<T>& writer, 
                         T expectedData[], uint32_t docNum);
...
void SingleValueAttributeWriterTest::TestAddField()
{
    ...
    // check data file
    std::string dataFilePath = attrConfig->GetAttrName() + "/"
...
template<typename T>
void SingleValueAttributeWriterTest::CheckWriterData(const SingleValueAttributeWriter<T>& writer, 
        T expectedData[], uint32_t docNum)
{
    common::TypedSliceList<T>* data = writer.GetAttributeData();
