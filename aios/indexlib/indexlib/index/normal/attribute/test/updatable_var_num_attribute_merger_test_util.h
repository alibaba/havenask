...
{
public:
    MOCK_METHOD2(Open, void(const std::string& dir, 
                            const index_base::SegmentInfo& segInfo));
    MOCK_METHOD3(Read, bool(docid_t docId, uint8_t* buf, 
                            uint32_t bufLen));
    MOCK_METHOD4(ReadDataAndLen, bool(docid_t docId, uint8_t* &buf, 
                    uint32_t bufLen, uint32_t dataLen));
    MOCK_METHOD1(SetBufferSize, void(uint32_t bufSize));
    MOCK_CONST_METHOD0(IsInMemory, bool());
    MOCK_CONST_METHOD1(GetDataLength, uint32_t(docid_t));
    MOCK_CONST_METHOD1(GetOffset, uint64_t(docid_t));
...
{
public:
    MOCK_METHOD2_T(AddPatchFile, 
                   void(const std::string &fileName, segmentid_t segmentId));
    MOCK_METHOD3_T(Seek, 
                   size_t(docid_t docId, uint8_t *value, size_t maxLen));
...
public:
    MOCK_METHOD2_T(Merge, void(const PatchFileList& patchFileList,
                               const std::string& destPatchFile));
};
...
public:
    ...
    static uint32_t CreateAttributeReadersAndPatchReaders(
            ...
            std::vector<AttributeSegmentReaderPtr> &attributeReaderVec,
     ...
     */
    static void CreateExpectMergedDataAndOffset(
            const std::string &expectDataAndOffsetString,
            std::vector<std::string> &dataVec,
            std::vector<std::string> &offsetVec);

private:
    static void InstallOneDocInAttributeReader(
            docid_t docId, const std::string &offsetString,
            const std::string &dataString, 
            MockUpdatableVarNumAttributeSegmentReader *attributeReader);
    static void InstallOneDocInPatchReader(
            docid_t docId, const std::string &patchString, 
        ...
        std::vector<typename VarNumAttributeTestUtil<T>::PatchReaderPtr> &patchReaderVec)
{
    ...
    for (size_t segId = 0; segId < stSegs.getNumTokens(); ++segId) 
    {
        ...
        MockVarNumAttributePatchReader<T> *mockPatchReader =
            new NiceMock<MockVarNumAttributePatchReader<T> >;
        typename VarNumAttributeTestUtil<T>::PatchReaderPtr patchReader(mockPatchReader);

        autil::StringTokenizer stInOneSeg;
        stInOneSeg.tokenize(stSegs[segId], "|", autil::StringTokenizer::TOKEN_IGNORE_EMPTY | autil::StringTokenizer::TOKEN_TRIM);
        for (size_t docId = 0; docId < stInOneSeg.getNumTokens(); ++docId)
        {
            ...
            InstallOneDocInAttributeReader(docId, offsetString, dataString,
                    mockAttributeReader);
        ...
        MockUpdatableVarNumAttributeSegmentReader *attributeReader)
{
    ...
    *(uint16_t *)buffer = num;
    ...
    if (!offsetString.empty())
    {
        ...
        ON_CALL(*attributeReader, GetOffset(docId))
                .WillByDefault(Return(offset));
    }
    ON_CALL(*attributeReader, ReadDataAndLen(docId, NotNull(), _, _))
        .WillByDefault(DoAll(SetArrayArgument<1>(buffer, buffer + dataLen),
                             SetArgReferee<3>(dataLen),
                             Return(true)));
 
}

template<typename T>
void VarNumAttributeTestUtil<T>::InstallOneDocInPatchReader(
        docid_t docId, const std::string &patchString, 
        MockVarNumAttributePatchReader<T> *patchReader)
{
    autil::StringTokenizer stPatch;
    stPatch.tokenize(patchString, ",",
                     autil::StringTokenizer::TOKEN_IGNORE_EMPTY
                     | autil::StringTokenizer::TOKEN_TRIM);
    uint16_t num = (uint16_t)stPatch.getNumTokens();
    if (0 == num)
    {
        ON_CALL(*patchReader, Seek(docId, _, _))
    ...
    *(uint16_t *)buffer = num;
    ...
    ON_CALL(*patchReader, Seek(docId, NotNull(), _))
        .WillByDefault(DoAll(SetArrayArgument<1>(buffer, buffer + bufferSize),
                             Return(bufferSize)));
}

template<typename T>
void VarNumAttributeTestUtil<T>::CreateExpectMergedDataAndOffset(
        const std::string &expectDataAndOffsetString,
        std::vector<std::string> &dataVec,
        std::vector<std::string> &offsetVec)
{
    ...
    for (size_t docId = 0; docId < stDataAndOffset.getNumTokens(); ++docId)
    {
        ...
        stDataAndOffsetInOneDoc.tokenize(stDataAndOffset[docId], ":",
                autil::StringTokenizer::TOKEN_TRIM);
