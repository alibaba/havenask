...
private:
    void InnerTestAttributeMerge(uint32_t docCount, uint32_t docCountPerSegment, 
                                 uint32_t updateRound, bool multiValue);

    void CheckResult(test::SingleFieldPartitionDataProvider& provider,
                     const std::vector<std::string>& expectValues,
                     const std::string& mergeFootPrint);

    std::string PrepareFullDocs(uint32_t docCount, uint32_t docCountPerSegment, 
                                std::vector<std::string>& expectValues);

    std::string PrepareUpdateDocs(
            std::vector<std::string>& expectValues, 
            uint32_t docCountPerSegment, uint32_t updateRoundIdx);

    std::string MakeRandomMergeStrategy(size_t updateRound, 
