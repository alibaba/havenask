...
void AttributePatchMergeTest::TestSingleValueAttrPatchMerge()
{
    ...
    partitionDir->GetDirectory("segment_6_level_0", true)->MountPackageFile(PACKAGE_FILE_PREFIX);
...
void AttributePatchMergeTest::CheckMergeResult(
        SingleFieldPartitionDataProvider& provider, int segNum)
{
    ...
    UInt32AttributeReaderPtr attrReader = 
        DYNAMIC_POINTER_CAST(UInt32AttributeReader, attrReaderBase);
    uint32_t value;
    for (int docId = 0; docId < segNum; docId++) 
    {
        ASSERT_TRUE(attrReader->Read(docId, value));
...
string AttributePatchMergeTest::GenerateDocString(int segNum) 
{
    ...
    for (int segId = 1; segId < segNum; segId++) 
    {
        ...
        {
            segDocStr += ",update_field:" + 
                         StringUtil::toString(docId) + 
                         ":" + 
                         StringUtil::toString(segId);
        }
