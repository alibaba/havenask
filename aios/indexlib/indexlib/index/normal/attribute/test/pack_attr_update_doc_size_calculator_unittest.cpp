...
void PackAttrUpdateDocSizeCalculatorTest::TestEstimateUpdateDocSize()
{
    ...
    ASSERT_EQ(expectUpdateDocSize,
              calculator.EstimateUpdateDocSize(INVALID_VERSION));
        
    // inc2 : update pk3, pk2/spk22
    string inc2Doc =
        "cmd=update_field,pk=pk2,id=22,sub_pk=spk22,sub_price=32,ts=3;"
    ...
    partData = PartitionDataCreator::CreateOnDiskPartitionData(
        GET_FILE_SYSTEM(), mSchema);
    PackAttrUpdateDocSizeCalculator inc2Calculator(partData, mSchema);

    expectUpdateDocSize = 9 * 2 + 13;
    ASSERT_EQ(expectUpdateDocSize,
              inc2Calculator.EstimateUpdateDocSize(lastLoadVersion));

    expectUpdateDocSize = 9 * 3 + 13 * 2;
    ASSERT_EQ(expectUpdateDocSize,
              inc2Calculator.EstimateUpdateDocSize(INVALID_VERSION));
}
...
void PackAttrUpdateDocSizeCalculatorTest::TestEstimateUpdateDocSizeInUnobseleteRtSeg()
{
    ...
    string rtDoc1 =
        "cmd=add,pk=pk3,id=3,price=3,sub_pk=spk31,sub_id=3,sub_price=31,ts=2;";
    string rtDoc2 =
        "cmd=update_field,pk=pk1,id=11,sub_pk=spk11,sub_id=3,ts=3;";
    string rtDoc3 =
        "cmd=update_field,pk=pk2,id=22,sub_pk=spk22,sub_price=32,ts=4;";
    string rtDoc4 =
        "cmd=update_field,pk=pk3,id=31,ts=5;";
    ...
    OnlinePartitionPtr onlinePartition = DYNAMIC_POINTER_CAST(
        OnlinePartition, psm.GetIndexPartition());
    assert(onlinePartition);
    PackAttrUpdateDocSizeCalculator calculator(
        onlinePartition->GetPartitionData(), mSchema);
    
    size_t expectUpdateDocSize = 9 * 3 + 13 * 2;
    ASSERT_EQ(expectUpdateDocSize,
              calculator.EstimateUpdateDocSizeInUnobseleteRtSeg(
                  INVALID_TIMESTAMP));
    ASSERT_EQ(expectUpdateDocSize,
              calculator.EstimateUpdateDocSizeInUnobseleteRtSeg(
                  3));
...
void PackAttrUpdateDocSizeCalculatorTest::TestCheckSchemaHasUpdatablePackAttribute()
{
    ...
    PackAttrUpdateDocSizeCalculator packAttrUpdateDocSizeCalc(
        partitionData, indexPartitionSchema);
...
IndexPartitionSchemaPtr PackAttrUpdateDocSizeCalculatorTest::MakeSchemaWithPackAttribute(
    bool hasPackAttr, bool isPackUpdatable, bool isSubSchema)
{
    ...
    if (hasPackAttr)
    {
        ...
        AttributeConfigPtr attr1 = AttributeTestUtil::CreateAttrConfig(
            ft_string, true, packField1, 3, false);
        AttributeConfigPtr attr2 = AttributeTestUtil::CreateAttrConfig(
            ft_string, true, packField2, 3, false);

        attr1->SetUpdatableMultiValue(isPackUpdatable);
        attr2->SetUpdatableMultiValue(isPackUpdatable);
        
        PackAttributeConfigPtr packAttr(
            new PackAttributeConfig(packAttrName, CompressTypeOption(), 50));

        packAttr->AddAttributeConfig(attr1);
        packAttr->AddAttributeConfig(attr2);
