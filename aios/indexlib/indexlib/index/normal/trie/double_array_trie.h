...
{
public:
    typedef std::pair<autil::ConstString, int32_t> KVPair;
...
public:
    ...
    autil::ConstString Data() const;
    float CompressionRatio() const;
    static size_t EstimateMemoryUse(size_t keyCount);
    static size_t EstimateMemoryUseFactor();

    // for search
    static int32_t Match(const void* data, const autil::ConstString& key);
    static size_t PrefixSearch(const void* data, const autil::ConstString& key,
                               KVPairVector& results, int32_t maxMatches = -1);
    static size_t GetFirstReserveSize() { return INIT_SIZE * sizeof(State); }
private:
    void FindChildNodes(const KVPairVector& sortedKVPairVector,
                        const Node& parentNode, NodeVector& childNodes);
    size_t FindBegin(const KVPairVector& sortedKVPairVector,
...
/////////////////////////////////////////////////////
inline int32_t DoubleArrayTrie::Match(
        const void* data, const autil::ConstString& key)
{
    assert(data);
...
inline size_t DoubleArrayTrie::PrefixSearch(const void* data, 
        const autil::ConstString& key, KVPairVector& results, int32_t maxMatches)
{
    assert(data);
    const State* stateArray = (const State*)data;
    int32_t base = stateArray[0].base;
    ...
    for (size_t i = 0; i < keyLength; ++i)
    {
        ...
        if ((uint32_t)base == stateArray[pos].check && value < 0)
        {
            autil::ConstString retKey((const char*)keyData, i);
    ...
    if ((uint32_t)base == stateArray[pos].check && value < 0)
    {
        autil::ConstString retKey((const char*)keyData, keyLength);
        int32_t retValue = -(value + 1);
        results.push_back(std::make_pair(retKey, retValue));        
        ++matchCount;
    }
