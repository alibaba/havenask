...
void PartitionInfo::Init(const Version& version, 
                         const PartitionMetaPtr& partitionMeta,
...
globalid_t PartitionInfo::GetGlobalId(docid_t docId) const 
{
    ...
    if (mBaseDocIds.size() == 0 || 
        docId >= (docid_t)mPartitionMetrics.totalDocCount)
    {
        return INVALID_GLOBALID;
    }
...
segmentid_t PartitionInfo::GetSegmentId(docid_t docId) const
{
    ...
    size_t pos = upper_bound(mBaseDocIds.begin(), mBaseDocIds.end(), docId)
                 - mBaseDocIds.begin();
    if (pos == 0)
    {
        return INVALID_SEGMENTID;
...
bool PartitionInfo::GetDiffDocIdRanges(const PartitionInfoHint& infoHint,
                                       DocIdRangeVector &docIdRanges) const
{
    ...
    if (infoHint.lastIncSegmentId == INVALID_SEGMENTID &&
                             ...
                             const std::vector<InMemorySegmentPtr>& dumpingSegments)
{
    ...
    for (size_t i = 0; i < segmentDatas.size(); i++)
    {
        ...
        mSegIdToBaseDocId.insert(
            make_pair(segData.GetSegmentId(), segData.GetBaseDocId()));
    ...
    for (const InMemorySegmentPtr& inMemSegment: dumpingSegments)
    {
        ...
        mSegIdToBaseDocId.insert(
            make_pair(inMemSegment->GetSegmentId(), totalDocCount));        
                                         ...
                                         const std::vector<InMemorySegmentPtr>& dumpingSegments)
{
    ...
    mPartitionMetrics.delDocCount
        = mDeletionMapReader ? mDeletionMapReader->GetDeletedDocCount() : 0;
    ...
    if (dumpingSegments.empty())
    {
        for (int32_t i =  segmentDatas.size() - 1; i >=0; i--)
        {
            ...
            if (RealtimeSegmentDirectory::IsRtSegmentId(segId))
            {
                ...
                mPartInfoHint.lastRtSegmentDocCount = 
                    segmentDatas[i].GetSegmentInfo().docCount;
                break;
            }
        }        
    }
...
void PartitionInfo::AddInMemorySegment(const InMemorySegmentPtr& inMemSegment)
{
    ...
    mPartInfoHint.lastRtSegmentId = inMemSegment->GetSegmentId();
    mPartInfoHint.lastRtSegmentDocCount = inMemSegment->GetSegmentInfo()->docCount;
    mVersion.AddSegment(inMemSegment->GetSegmentId());

    if (mSubPartitionInfo)
    {
        const InMemorySegmentPtr& subInMemSegment = 
            inMemSegment->GetSubInMemorySegment();
...
PartitionInfo* PartitionInfo::Clone()
{
    return new PartitionInfo(*this);
}

bool PartitionInfo::GetIncDiffDocIdRange(segmentid_t lastIncSegId,
        DocIdRange& range) const
{
    ...
    {
        if (!OnlineSegmentDirectory::IsIncSegmentId(mVersion[i])
            || mVersion[i] <= lastIncSegId)
        {
            continue;
        }
...
bool PartitionInfo::GetRtDiffDocIdRange(segmentid_t lastRtSegmentId,
        size_t lastRtSegmentDocCount, DocIdRange& range) const
{
    if (IsKeyValueTable())
    {
        return false;
    ...
    {
        if (mVersion[i] > lastRtSegmentId
            && RealtimeSegmentDirectory::IsRtSegmentId(mVersion[i]))
        {
