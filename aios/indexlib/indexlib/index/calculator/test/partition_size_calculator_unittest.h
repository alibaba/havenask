...
private:
    ...
    void PrepareSegmentData(segmentid_t segId, file_system::DirectoryPtr segDir, 
                            const std::string& attrName, bool isSub);
    void PreparePkSegment(segmentid_t segId, 
                          const config::IndexPartitionSchemaPtr& schema,
                          size_t docCount = 1, size_t subDocCount = 1);
    void PreparePkSegmentData(
            const file_system::DirectoryPtr& segDir, 
            const config::IndexPartitionSchemaPtr& schema,
            size_t docCount);
