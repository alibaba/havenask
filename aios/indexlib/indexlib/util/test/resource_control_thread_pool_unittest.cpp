...
{
public:
    MOCK_CONST_METHOD0(GetRequiredResource, int64_t());
...
void ResourceControlThreadPoolTest::CaseSetUp()
{
    MockResourceControlWorkItem::mIdentiVec.clear();
}
...
void ResourceControlThreadPoolTest::TestSimpleProcess()
{
    ...
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(1, 70)));
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(2, 80)));
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(3, 30)));
    pool.Init(workItems);
    pool.Run("");
    ASSERT_EQ(string("1 3 2"), StringUtil::toString(
                    MockResourceControlWorkItem::mIdentiVec));
}
...
void ResourceControlThreadPoolTest::TestResourceNotEnough()
{
    ...
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(1, 70)));
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(2, 60)));
    workItems.push_back(ResourceControlWorkItemPtr(
                    new MockResourceControlWorkItem(3, 50)));

    pool.Init(workItems);
    pool.Run("");
    ASSERT_EQ(string("1 2 3"), StringUtil::toString(
                    MockResourceControlWorkItem::mIdentiVec));
}
...
{
    {
        ...
        EXPECT_CALL(*mockWorkItem, GetRequiredResource())
...

    {
        ...
        EXPECT_CALL(*mockWorkItem, GetRequiredResource())
            .WillOnce(Return(50))
            .WillRepeatedly(Throw(FileIOException()));
        EXPECT_CALL(*mockWorkItem, Destroy())
            .WillOnce(Return());
...

    {
        ...
        EXPECT_CALL(*mockWorkItem, Process())
            .WillRepeatedly(Throw(FileIOException()));
        EXPECT_CALL(*mockWorkItem, Destroy())
            .WillOnce(Return());
        ASSERT_NO_THROW(pool.Init(workItems));
