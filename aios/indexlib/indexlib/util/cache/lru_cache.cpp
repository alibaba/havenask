...
LRUHandle* LRUHandleTable::Lookup(const ConstString& key, uint32_t hash) {
...
LRUHandle* LRUHandleTable::Remove(const ConstString& key, uint32_t hash) {
...
LRUHandle** LRUHandleTable::FindPointer(const ConstString& key, uint32_t hash) {
...
Cache::Handle* LRUCacheShard::Lookup(const ConstString& key, uint32_t hash) {
  ScopedLock l(mutex_);
...
Status LRUCacheShard::Insert(const ConstString& key, uint32_t hash, void* value,
                             size_t charge,
                             void (*deleter)(const ConstString& key, void* value,
...

  {
    ...
    if (usage_ - lru_usage_ + charge > capacity_ &&
      ...
      if (handle == nullptr) {
        ...
        *handle = nullptr;
        s = Status::Incomplete(ConstString("Insert failed due to LRU cache being full."));
      }
    } else {
...
void LRUCacheShard::Erase(const ConstString& key, uint32_t hash) {
  LRUHandle* e;
  bool last_reference = false;
  {
    ScopedLock l(mutex_);
