...
Status ShardedCache::Insert(const ConstString& key, void* value, size_t charge,
...
Cache::Handle* ShardedCache::Lookup(const ConstString& key) {
  uint32_t hash = HashSlice(key);
...
void ShardedCache::Erase(const ConstString& key) {
  uint32_t hash = HashSlice(key);
