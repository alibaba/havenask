...
#ifndef __INDEXLIB_AVL_H
#define __INDEXLIB_AVL_H
...
IE_LOG_SETUP_TEMPLATE(util, AvlNode);
...
AvlNode<KeyType>::Check() const {
    ...
    if (LEFT_IMBALANCE(diffHeight) || RIGHT_IMBALANCE(diffHeight)) {
        valid = 0;
        IE_LOG(ERROR, "Height difference is %lu at node %lu", diffHeight, Key());
    }

    // Verify that balance-factor is correct
    if (diffHeight != myBal) {
        valid = 0;
        IE_LOG(ERROR, "Height difference %lu doesn't match "
               "balance-factor of %lu at node %lu", diffHeight, myBal, Key());
    }
    ...
    if ((mySubtree[LEFT])
        ...
        valid = 0;
        IE_LOG(ERROR, "Node: %lu is smaller than left subtree %lu",
               Key(), mySubtree[LEFT]->Key());
    }
    if ((mySubtree[RIGHT])
        &&
        (mySubtree[RIGHT]->Compare(Key()) == MAX_CMP)) {
        valid = 0;
        IE_LOG(ERROR, "Node: %lu is greater than right subtree %lu",
               Key(), mySubtree[RIGHT]->Key());
    }
