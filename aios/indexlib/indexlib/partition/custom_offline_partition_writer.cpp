...
string CustomOfflinePartitionWriter::GetLastLocator() const
{
    ...
    if (mBuildingSegMeta)
    {
        return mBuildingSegMeta->segmentInfo.locator.ToString();
    }
    return string("");
}
                            ...
                            const PartitionModifierPtr& modifier)
{
    ...
    if (!mPartitionData)
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             "cast CustomPartitionData failed for shema[%s]",
    ...
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             "Get DumpSegmentQueue from partitionData failed for shema[%s]",
                             mSchema->GetSchemaName().c_str()); 
    }
...
DirectoryPtr CustomOfflinePartitionWriter::PrepareNewSegmentDirectory(
    ...
    const BuildConfig& buildConfig)
{
    ...
    try
    {
        segDir = rootDir->MakeInMemDirectory(segDirName);
    }
    ...
    if (buildConfig.enablePackageFile)
    {
        mBuildingSegPackDir = mBuildingSegDir->CreatePackDirectory(PACKAGE_FILE_PREFIX);
        return mBuildingSegPackDir;
    }
    return mBuildingSegDir;;
}

DirectoryPtr CustomOfflinePartitionWriter::PrepareBuildingDirForTableWriter(
    const DirectoryPtr& segmentDir)
{
    DirectoryPtr buildingDir;
    try
    {
        buildingDir =  segmentDir->MakeInMemDirectory(CUSTOM_DATA_DIR_NAME);
    }
    catch(const misc::ExceptionBase& e)
    {
        IE_LOG(WARN, "Prepare custom data dir for segment[%s] fail : reason [%s]",
               segmentDir->GetPath().c_str(), e.what());
        return DirectoryPtr(); 
    }
    return buildingDir;
}
...
{
    ScopedLock lock1(mWriterLock);
    ScopedLock lock2(mPartDataLock);
    assert(!modifier);
    if (!PrepareTableResource(mPartitionData, mTableResource))
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             "create TableResource failed for shema[%s]",
                             mSchema->GetSchemaName().c_str());             
    }
    ...
    mTableWriter = InitTableWriter(newSegData, customDataDir, mTableResource, 
                                   mPartitionData->GetPluginManager());
    if (!mTableWriter)
    {
...
void CustomOfflinePartitionWriter::ReportPartitionMetrics(
    const PartitionDataPtr& partitionData,
...
docid_t CustomOfflinePartitionWriter::UpdateBuildingSegmentMeta(
...
bool CustomOfflinePartitionWriter::BuildDocument(const document::DocumentPtr& doc)
{
    ...
    mBuildingSegMeta->segmentInfo.docCount += mLastConsumedMessageCount;
    switch(result)
    {
    ...
    case BuildResult::BR_NO_SPACE:
        mStatus = misc::NO_SPACE;
        return false;
    case BuildResult::BR_FAIL:
        mStatus = misc::INVALID_ARGUMENT;
        return false;
    case BuildResult::BR_FATAL:
        mStatus = misc::FAIL;
        INDEXLIB_FATAL_ERROR(Runtime, "build document [%ld] failed",
               mBuildingSegMeta->segmentInfo.docCount);
    default:
...
bool CustomOfflinePartitionWriter::NeedDump(size_t memoryQuota) const
{
    ScopedLock lock(mWriterLock);
    if (mBuildingSegMeta->segmentInfo.docCount >= mOptions.GetBuildConfig().maxDocCount)
    {
        IE_LOG(INFO, "segment docCount reach max-doc-count(%d), "
               "will trigger dump", mOptions.GetBuildConfig().maxDocCount);
        return true;
    }
    ...
    {
        IE_LOG(INFO, "TableWriter [%s] is full, will trigger dump",
               mSchema->GetSchemaName().c_str());
        return true;
    }
    ...
    if (mOptions.IsOnline())
    {
        ...
        {
            IE_LOG(INFO, "buildMemUse[%lu] reach memoryQuota, will trigger dump",
                   buildMemUse);
            return true;
        }
        return false;
    ...
    if (buildMemUse >= memoryQuota * MEMORY_USE_RATIO)
    {
        IE_LOG(INFO, "buildMemUse[%zu] reach memoryQuota, will trigger dump",
               buildMemUse);
        return true;
    }
    ...
    if (fsMemUse >= memoryQuota)
    {
        INDEXLIB_FATAL_ERROR(OutOfMemory,
                             ...
                             "[FileSystemMemory %lu MB]", IE_LOG_PREFIX,
...
void CustomOfflinePartitionWriter::StoreSegmentMetas(
...
void CustomOfflinePartitionWriter::CommitVersion(segmentid_t segmentId,
...
void CustomOfflinePartitionWriter::DumpSegment()
{
    ...
    CleanFileSystemCache();
    CustomSegmentDumpItemPtr segmentDumpItem(
        new CustomSegmentDumpItem(mOptions, mSchema, mPartitionName));
    ...
    else
    {
        ...
        CommitVersion(segmentDumpItem->GetSegmentId(),
                      *(segmentDumpItem->GetSegmentInfo()),
...
bool CustomOfflinePartitionWriter::PrepareTableResource(
    ...
    TableResourcePtr& tableResource)
{
    ...
    if (!tableResource)
    {
        ...
        {
            IE_LOG(ERROR, "init TableResource[%s] failed ",
                   mSchema->GetSchemaName().c_str());
            return false;
        }
        ...
        IE_LOG(INFO, "init TableResource[%s] done, metricProvider[%p]",
               mSchema->GetSchemaName().c_str(), mMetricProvider.get());
        return true;
    }

    if (!CheckMemoryLimit(
            tableResource->EstimateInitMemoryUse(segMetas), 0))
    {
        return false;            
    }
    ...
    {
        IE_LOG(ERROR, "re-init TableResource[%s] failed ",
               mSchema->GetSchemaName().c_str());
        return false; 
    }
    if (!CheckMemoryLimit(
            tableResource->GetCurrentMemoryUse(), 0))
    {
        return false;            
    }
...
TableWriterPtr CustomOfflinePartitionWriter::InitTableWriter(
    const SegmentDataBasePtr& newSegmentData,
    ...
    const PluginManagerPtr& pluginManager)
{
    ...
    if (mOptions.IsOffline())
    {
        ...
        {
            IE_LOG(ERROR, "reseve writer init mem [%ld] failed, create TableWriter failed",
                initMemUse);
            return TableWriterPtr();
        }
    }
    ...
    if (!CheckMemoryLimit(tableResource->GetCurrentMemoryUse(),
                          tableWriter->GetCurrentMemoryUse()))
    {
        return TableWriterPtr();
    }
    return tableWriter;
...
bool CustomOfflinePartitionWriter::CheckMemoryLimit(
    size_t resourceMemUse, size_t writerMemUse) const
{
    size_t memUseToCheck = writerMemUse;
    if (HasResourceMemoryLimit())
    {
        size_t resourceMemLimit =
            mOptions.GetBuildConfig().buildResourceMemoryLimit * 1024 * 1024;
        if (resourceMemUse > resourceMemLimit)
        {
            IE_LOG(ERROR, "current resource memory use [%ld] "
    ...
    size_t buildTotalMemLimit =
        mOptions.GetBuildConfig().buildTotalMemory * 1024 * 1024;    
    if (memUseToCheck > buildTotalMemLimit)
    {
        IE_LOG(ERROR, "MemoryLimit exceeded! resourceMemUse[%zu], "
...
uint64_t CustomOfflinePartitionWriter::GetFileSystemMemoryUse() const
{
    return mFileSystem->GetFileSystemMemoryUse();
}
...
void CustomOfflinePartitionWriter::RegisterMetrics(
...
void CustomOfflinePartitionWriter::ResetPartitionData(
    const PartitionDataPtr& partitionData,
    const TableResourcePtr& tableResource)
{
    ScopedLock lock(mPartDataLock);
