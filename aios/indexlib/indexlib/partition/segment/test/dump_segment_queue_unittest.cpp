...
public:
    ...
    MOCK_METHOD0(DumpWithMemLimit, bool());
    MOCK_CONST_METHOD0(IsDumped, bool()); 
    MOCK_CONST_METHOD0(GetEstimateDumpSize, uint64_t());
    MOCK_CONST_METHOD0(GetInMemoryMemUse, size_t());
    MOCK_CONST_METHOD0(GetSegmentId, segmentid_t());
    MOCK_CONST_METHOD0(GetTableWriter, table::TableWriterPtr());
    MOCK_CONST_METHOD0(GetSegmentInfo, index_base::SegmentInfoPtr());            
};
...
void DumpSegmentQueueTest::TestBase()
{
    ...
    EXPECT_CALL(*mockItem1, IsDumped())
        .Times(1)
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*mockItem2, IsDumped())
        .Times(1)
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*mockItem3, IsDumped())
        .Times(1)
        .WillRepeatedly(Return(true));
...
void DumpSegmentQueueTest::TestGetEstimateDumpSize()
{
    ...
    size_t itemSize = 100;
    EXPECT_CALL(*mockItem1, GetEstimateDumpSize())
        .WillRepeatedly(Return(itemSize));
    EXPECT_CALL(*mockItem2, GetEstimateDumpSize())
        .WillRepeatedly(Return(itemSize));
...
void DumpSegmentQueueTest::TestGetDumpingSegmentsSize()
{
    ...
    EXPECT_CALL(*mockItem1, GetEstimateDumpSize())
        .Times(1)
        .WillOnce(Return(perItemDumpSize));

    EXPECT_CALL(*mockItem1, GetInMemoryMemUse())
        .Times(1)
        .WillOnce(Return(perItemMemUse));
    EXPECT_CALL(*mockItem2, GetEstimateDumpSize())
        .Times(1)
        .WillOnce(Return(perItemDumpSize));

    EXPECT_CALL(*mockItem2, GetInMemoryMemUse())
        .Times(1)
        .WillOnce(Return(perItemMemUse));
...
void DumpSegmentQueueTest::TestReclaimSegments()
{
    ...
    EXPECT_CALL(*mockItem1, GetSegmentInfo())
        .Times(1)
        .WillRepeatedly(Return(segInfo1));
    EXPECT_CALL(*mockItem2, GetSegmentInfo())
        .Times(2)
        .WillRepeatedly(Return(segInfo2));

    EXPECT_CALL(*mockItem1, GetSegmentId())
        .Times(1)
        .WillRepeatedly(Return(1));
    EXPECT_CALL(*mockItem2, GetSegmentId())
        .Times(1)
        .WillRepeatedly(Return(1));
...
namespace {
    class UTTableWriter : public TableWriter
    {
        ...
        size_t EstimateInitMemoryUse(
            const TableWriterInitParamPtr& initParam) const override
            {
                return 0;
            }
...
void DumpSegmentQueueTest::TestGetBuildingSegmentReaders()
{
    ...
    std::string testPartName1("test1");
    std::string testPartName2("test2");
    MockCustomSegmentDumpItem* mockItem1 = 
        new MockCustomSegmentDumpItem(mOptions, mSchema, testPartName1);
    MockCustomSegmentDumpItem* mockItem2 = 
        new MockCustomSegmentDumpItem(mOptions, mSchema, testPartName2);
