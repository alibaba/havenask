...
void InMemorySegmentCreatorTest::TestCreateSubWriterOOM()
{
    ...
    InMemorySegmentPtr inMemSegment = 
        creator.Create(PartitionSegmentIteratorPtr(),
                       segData, GET_SEGMENT_DIRECTORY(),
                       util::MemoryQuotaControllerCreator::CreatePartitionMemoryController(
                               autil::mem_pool::Pool::DEFAULT_CHUNK_SIZE),
...
void InMemorySegmentCreatorTest::TestCreateSegmentWriterOutOfMem()
{
    ...
    InMemorySegmentPtr inMemSegment = 
        creator.Create(PartitionSegmentIteratorPtr(),
                       segData, GET_SEGMENT_DIRECTORY(),
                       util::MemoryQuotaControllerCreator::CreatePartitionMemoryController(
                               autil::mem_pool::Pool::DEFAULT_CHUNK_SIZE), counterMap, nullPluginManager);
    ASSERT_TRUE(inMemSegment);
...
void InMemorySegmentCreatorTest::TestCreateWithSubDoc()
{
    ...
    InMemorySegmentPtr inMemSegment = 
        creator.Create(PartitionSegmentIteratorPtr(),
                       ...
                       mCounterMap, nullPluginManager);
    ASSERT_TRUE(inMemSegment);
    InMemorySegmentPtr subInMemSegment =
...
void InMemorySegmentCreatorTest::CheckSegmentMetrics(
        const SegmentMetricsPtr& segmentMetrics, 
        const vector<string>& indexNames, size_t expectTermCount)
{
    ASSERT_TRUE(segmentMetrics);
    for (size_t i = 0; i < indexNames.size(); i++)
    {
        ASSERT_EQ(expectTermCount, 
                  segmentMetrics->GetDistinctTermCount(indexNames[i]));
    }
}

void InMemorySegmentCreatorTest::CheckSegmentWriter(
        SegmentWriterPtr segWriter, bool isSubDocWriter)
{
    if (isSubDocWriter)
    {
        SubDocSegmentWriterPtr subDocSegWriter = DYNAMIC_POINTER_CAST(
                SubDocSegmentWriter, segWriter);
        ASSERT_TRUE(subDocSegWriter);        
        return;
    }

    SingleSegmentWriterPtr singleSegWriter = DYNAMIC_POINTER_CAST(
            SingleSegmentWriter, segWriter);
