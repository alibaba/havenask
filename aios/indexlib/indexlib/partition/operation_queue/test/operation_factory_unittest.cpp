...
void OperationFactoryTest::TestSimpleProcess()
{
    ...
    OperationBase* operation = 
        factory.CreateOperation(docs[0], docs[0]->GetPool());
    ASSERT_EQ(DELETE_DOC, operation->GetDocOperateType());

    operation = factory.CreateOperation(docs[1], docs[1]->GetPool());
    ASSERT_EQ(DELETE_DOC, operation->GetDocOperateType());

    operation = factory.CreateOperation(docs[2], docs[2]->GetPool());
    ASSERT_EQ(UPDATE_FIELD, operation->GetDocOperateType());
}
...
void OperationFactoryTest::TestInit()
{
...

    {
        ...
        CheckOperationCreator(schema, factory.mRemoveOperationCreator,
                              "remove");
        CheckOperationCreator(schema, factory.mRemoveSubOperationCreator, 
                              "subDoc:null:remove");
        CheckOperationCreator(schema, factory.mUpdateOperationCreator,
                              "subDoc:update:update");
    }
}
...
{
    {
        ...
        vector<NormalDocumentPtr> docs = DocumentCreator::CreateDocuments(schema, docString);

        OperationBase* oper = factory.CreateOperation(docs[0], &mPool);
...

    {
        ...
        OperationBase* oper1 = factory.CreateOperation(docs[0], &mPool);
        OperationBase* oper2 = factory.CreateOperation(docs[1], &mPool);
...
{
    {
        ...
        vector<NormalDocumentPtr> docs = DocumentCreator::CreateDocuments(
                schema, docString);

        OperationBase* oper = factory.CreateOperation(docs[0], &mPool);
        CheckOperationType(oper, "remove");
        oper = factory.CreateOperation(docs[1], &mPool);
...

    {
        ...
        OperationBase* oper0 = factory.CreateOperation(docs[0], &mPool);
        OperationBase* oper1 = factory.CreateOperation(docs[1], &mPool);
        OperationBase* oper2 = factory.CreateOperation(docs[2], &mPool);
        OperationBase* oper3 = factory.CreateOperation(docs[3], &mPool);
...
void OperationFactoryTest::TestCreateOperationForUpdateDocument()
{
...

    {
        ...
        OperationBase* oper2 = factory.CreateOperation(docs[2], &mPool);
        OperationBase* oper3 = factory.CreateOperation(docs[3], &mPool);
        OperationBase* oper4 = factory.CreateOperation(docs[4], &mPool);
...
void OperationFactoryTest::CheckOperationCreator(
        const IndexPartitionSchemaPtr& schema, 
        OperationCreatorPtr operationCreator,
        const string& typeStr)
{
    ...
    {
        CheckSingleOperationCreator(schema, operationCreator, 
                creatorTypeInfo[0]);
        return;
    }
    ...
    if (creatorTypeInfo.size() == 3)
    {
        ...
        CheckSubDocOperationCreator(schema, operationCreator, 
                creatorTypeInfo[1], creatorTypeInfo[2]);
        return;
    }
...
void OperationFactoryTest::CheckSingleOperationCreator(
        const IndexPartitionSchemaPtr& schema, 
        OperationCreatorPtr operationCreator,
        const string& typeStr)
{
    ...
    {
        RemoveOperationCreatorPtr removeOperCreator =
            DYNAMIC_POINTER_CAST(RemoveOperationCreator, operationCreator);
    ...
    ASSERT_EQ(schema->GetIndexSchema()->GetPrimaryKeyIndexType(), 
              operationCreator->GetPkIndexType());
}

void OperationFactoryTest::CheckSubDocOperationCreator(
        const IndexPartitionSchemaPtr& schema, 
        OperationCreatorPtr operationCreator,
        const string& mainTypeStr,
        const string& subTypeStr)
{
    ASSERT_TRUE(operationCreator);
    SubDocOperationCreatorPtr subDocOperCreator = 
        DYNAMIC_POINTER_CAST(SubDocOperationCreator, operationCreator);
    ASSERT_TRUE(subDocOperCreator);    

    CheckSingleOperationCreator(schema, subDocOperCreator->mMainCreator, mainTypeStr);
    CheckSingleOperationCreator(schema->GetSubIndexPartitionSchema(), 
                                subDocOperCreator->mSubCreator, subTypeStr);
}

void OperationFactoryTest::CheckOperationType(
        OperationBase* oper, const string& typeStr)
{
...
void OperationFactoryTest::CheckSingleOperationType(
        OperationBase* oper, const string& typeStr)
{
    if (typeStr == "null")
    {
    ...
    if (typeStr == "update")
    {
        UpdateFieldOperation<uint64_t>* updateOper = 
            dynamic_cast<UpdateFieldOperation<uint64_t>*>(oper);
    ...
    if (typeStr == "remove")
    {
        RemoveOperation<uint64_t>* removeOper = 
            dynamic_cast<RemoveOperation<uint64_t>*>(oper);
...
void OperationFactoryTest::CheckSubDocOperationType(
        OperationBase* oper, const string& mainTypeStr, 
        const string& subTypeStr)
{
