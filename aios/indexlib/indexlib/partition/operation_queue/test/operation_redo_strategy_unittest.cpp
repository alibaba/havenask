...
void OperationRedoStrategyTest::InnerTestNeedRedo(bool isIncConsistentWithRt)
{
    ...
    OperationRedoStrategy redoStrategy;
    ...
    MockOperation* mockOperation = 
        MockOperationMaker::MakeOperation(101, &pool);

    EXPECT_CALL(*mockOperation, GetDocOperateType())
        .WillRepeatedly(Return(UPDATE_FIELD));
    ...
    if (isIncConsistentWithRt)
    {
        ...
        EXPECT_FALSE(redoStrategy.NeedRedo(mockOperation, hint)); 
        EXPECT_FALSE(redoStrategy.NeedRedo(mockOperation, hint));
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint)); 
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint)); 
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint)); 
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint));
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint)); 
...
void OperationRedoStrategyTest::TestNeedRedoSpecialCondition()
{
    ...
    OperationRedoHint hint;
    {
        MockOperation* mockOperation = 
            MockOperationMaker::MakeOperation(101, &pool);
        EXPECT_CALL(*mockOperation, GetDocOperateType())
            .WillRepeatedly(Return(DELETE_DOC));
    ...
    }
    {
        MockOperation* mockOperation = 
            MockOperationMaker::MakeOperation(101, &pool);
        EXPECT_CALL(*mockOperation, GetDocOperateType())
            .WillRepeatedly(Return(DELETE_SUB_DOC));
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint));
        mockOperation->~OperationBase();        
    }
    {
        OperationRedoStrategy redoStrategy1;
        ...
        EXPECT_TRUE(redoStrategy1.NeedRedo(mockOperation, hint));
        mockOperation->~OperationBase();        
    }            
    {
        MockOperation* mockOperation = 
            MockOperationMaker::MakeOperation(100, &pool);
        EXPECT_CALL(*mockOperation, GetDocOperateType())
            .WillRepeatedly(Return(DELETE_DOC));
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint));
        mockOperation->~OperationBase();                
    ...
    {
        MockOperation* mockOperation = 
            MockOperationMaker::MakeOperation(110, &pool);
        EXPECT_CALL(*mockOperation, GetDocOperateType())
            .WillRepeatedly(Return(UPDATE_FIELD));
        EXPECT_CALL(*mockOperation, GetSegmentId())
            .WillOnce(Return(1));
    ...
    {
        MockOperation* mockOperation = 
            MockOperationMaker::MakeOperation(99, &pool);
        EXPECT_CALL(*mockOperation, GetDocOperateType())
            .WillRepeatedly(Return(UPDATE_FIELD));
        EXPECT_TRUE(redoStrategy.NeedRedo(mockOperation, hint));
        mockOperation->~OperationBase(); 
