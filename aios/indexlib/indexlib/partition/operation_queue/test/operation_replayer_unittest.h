...
private:
    ...
    void InnerTestRedoFromCursor(const std::string& normalOpStr,
                                 int inMemOpCount,
                                 int64_t onDiskVersionTimestamp);

    index_base::PartitionDataPtr MakeData(test::PartitionStateMachine& psm,
                                         const std::string& normalOpStr,
                                         int inMemOpCount,
                                         OperationTestMetas& opTestMeta);
    
    void CheckRedo(int64_t onDiskVersionTimestamp,
                   const index_base::PartitionDataPtr& partData,
                   const OperationCursor& opCursor,
                   const OperationCursor& expectLastCursor,
                   size_t expectRedoCount);
    
    size_t GetRedoOpCount(const OperationTestMetas& opTestMetas,
                          size_t beginPos, int64_t onDiskVersionTimestamp);
