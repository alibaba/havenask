...
template <typename T>
void CheckAttributeReferenceForSingleValue(
    ...
    const string& expectValueStr)
{
    ...
    AttributeReferenceTyped<T>* attrRef =
        packAttrReader->GetSubAttributeReferenceTyped<T>(attrName);
    ASSERT_TRUE(attrRef);
    const char* baseAddr = packAttrReader->GetBaseAddress(docId);
...
template <typename T>
void CheckAttributeReferenceForMultiValue(
    const PackAttributeReaderPtr& packAttrReader,
    const string& attrName,
    docid_t docId,
    const string& expectValueStr)
...
void OnlinePartitionReaderInteTest::TestOpenForIndex()
{
    ...
    const IndexPartitionReader::AccessCounterMap& counterMap = 
        indexPart->GetReader()->GetIndexAccessCounters();
...
void OnlinePartitionReaderInteTest::TestOpenForAttribute()
{
    ...
    const IndexPartitionPtr& indexPart = psm.GetIndexPartition();
    const IndexPartitionReader::AccessCounterMap& counterMap = 
        indexPart->GetReader()->GetAttributeAccessCounters();
...
void OnlinePartitionReaderInteTest::TestOpenForPackAttribute()
{
    ...
    ASSERT_TRUE(psm.Transfer(BUILD_FULL_NO_MERGE, fullDoc,
                             "pk:pk", "int8_single=1,int8_multi=1 2"));
    ...
    const IndexPartitionReader::AccessCounterMap& counterMap = 
        partReader->GetAttributeAccessCounters();
    IndexPartitionReader::AccessCounterMap::const_iterator iter1 =
        counterMap.find("int8_single");
    ASSERT_EQ((int)2, iter1->second->Get());
    IndexPartitionReader::AccessCounterMap::const_iterator iter2 =
        counterMap.find("int8_multi");
    ASSERT_EQ((int)2, iter2->second->Get());
    

    const PackAttributeReaderPtr& packAttrReader =
        partReader->GetPackAttributeReader("pack_attr");

    const PackAttributeReaderPtr& packAttrReaderCmp =
        partReader->GetPackAttributeReader(packattrid_t(0));

    ASSERT_TRUE(packAttrReader);
    ASSERT_TRUE(packAttrReaderCmp);
    ASSERT_EQ(packAttrReader.get(), packAttrReaderCmp.get());

    CheckAttributeReferenceForSingleValue<int8_t>(
        packAttrReader, "int8_single", docid_t(0), "0");
    CheckAttributeReferenceForSingleValue<int8_t>(
        packAttrReader, "int8_single", docid_t(1), "1");
    CheckAttributeReferenceForSingleValue<int8_t>(
        packAttrReader, "int8_single", docid_t(2), "2");

    // check access counter
    ASSERT_EQ((int)5, iter1->second->Get());
    ASSERT_EQ((int)2, iter2->second->Get());

    CheckAttributeReferenceForMultiValue<int8_t>(
        packAttrReader, "int8_multi", docid_t(0), "0 1");
    CheckAttributeReferenceForMultiValue<int8_t>(
        packAttrReader, "int8_multi", docid_t(1), "1 2");
    CheckAttributeReferenceForMultiValue<int8_t>(
        packAttrReader, "int8_multi", docid_t(2), "2 3");
...
void OnlinePartitionReaderInteTest::CheckReadSubAttribute(
    const PackAttributeReaderPtr& packAttrReader,
    const string& attrName,
    docid_t docId,
...
void OnlinePartitionReaderInteTest::TestOpenForSummaryWithPack()
{
    IndexPartitionSchemaPtr schema = SchemaAdapter::LoadSchema(
                ...
                "schema_for_online_partition_reader_test.json");
    ...
    const ConstString* field = summaryDoc.GetFieldValue(0);
    ASSERT_EQ(string("1"), field->toString());
    
    ASSERT_TRUE(summaryReader->GetDocument(docid_t(1), &summaryDoc));
    field = summaryDoc.GetFieldValue(0);
    ASSERT_EQ(string("2"), field->toString());
}
...
void OnlinePartitionReaderInteTest::TestSubDocOpenForIndex()
{
    IndexPartitionSchemaPtr subSchema = SchemaMaker::MakeSchema(
            "sub_string:string;sub_pk:string",
            "sub_index:string:sub_string;sub_pk:primarykey64:sub_pk",
            "", "");
...
{
    IndexPartitionSchemaPtr subSchema = SchemaMaker::MakeSchema(
            "sub_text:text;sub_pk:string",
            "sub_pack:pack:sub_text;sub_pk:primarykey64:sub_pk",
            "", "");
    mSchema->SetSubIndexPartitionSchema(subSchema);
    ...
    const IndexPartitionPtr& indexPart = psm.GetIndexPartition();
    AssertSectionLen(indexPart->GetReader()->GetSubPartitionReader(),
                     "sub_pack", 0, 3);
...
void OnlinePartitionReaderInteTest::AssertSectionLen(
        const IndexPartitionReaderPtr& reader, const string& indexName, 
        docid_t docid, section_len_t expectLen)
{
    const SectionAttributeReader* sectionReader = 
        reader->GetIndexReader(indexName)->GetSectionReader(indexName);
...
{
    IndexPartitionSchemaPtr schema = SchemaMaker::MakeSchema(
            "string1:string;long1:uint32;long2:uint32",
            "index1:string:string1;index2:number:long1",
            "long2",
            "");
    PartitionStateMachine psm;
    INDEXLIB_TEST_TRUE(psm.Init(schema, mOptions, mRootDir));
...
void OnlinePartitionReaderInteTest::TestDisableSubIndex()
{
    ...
    IndexPartitionSchemaPtr subSchema = SchemaMaker::MakeSchema(
            ...
            "sub_pk:primarykey64:sub_pk;sub_int:number:sub_int",
...
void OnlinePartitionReaderInteTest::TestDisableSpatialIndexMode2()
{
    ...
    IndexPartitionSchemaPtr schema = SchemaMaker::MakeSchema(
            ...
            "index1:string:string1;index2:number:long1;spatial_index:spatial:coordinate",
            "long2",
            "");
    PartitionStateMachine psm;
    INDEXLIB_TEST_TRUE(psm.Init(schema, mOptions, mRootDir));
    ...
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, fullDoc, "index1:hello", "docid=0"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(45 0, 90 40)", 
                             "docid=0;docid=1"));
