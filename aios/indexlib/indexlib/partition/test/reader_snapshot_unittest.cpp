...
public:
    ...
    {
        mAttrReader.reset(new index::SingleValueAttributeReader<uint32_t>(NULL));
    }
...
void ReaderSnapshotTest::TestSimpleProcess()
{
    ...
    ASSERT_TRUE(snapshot->GetAttributeReaderInfo("a1", "", info));
    ASSERT_EQ(string("tableA"), info.indexPartReader->GetSchema()->GetSchemaName());
    ASSERT_FALSE(snapshot->GetAttributeReaderInfo("b1", "", info));
    ASSERT_TRUE(snapshot->GetAttributeReaderInfo("b1", "tableA", info));
    ASSERT_EQ(string("tableA"), info.indexPartReader->GetSchema()->GetSchemaName());
    ASSERT_TRUE(snapshot->GetAttributeReaderInfo("sub_a2", "", info));
...
void ReaderSnapshotTest::TestLeadingTable()
{
    ...
    //test get attribute reader
    AttributeReaderInfo info;
    PartitionReaderSnapshotPtr leadingSnapshot(
            snapshot->CreateLeadingSnapshotReader("tableA"));
    ASSERT_TRUE(leadingSnapshot);
    ASSERT_TRUE(leadingSnapshot->GetAttributeReaderInfo("a1", "", info));
    ASSERT_EQ(string("tableA"), info.indexPartReader->GetSchema()->GetSchemaName());
    ASSERT_TRUE(leadingSnapshot->GetAttributeReaderInfo("b1", "", info));
    ASSERT_EQ(string("tableA"), info.indexPartReader->GetSchema()->GetSchemaName());
}
...
void ReaderSnapshotTest::TestLeadingTableWithJoinTable()
{
    ...
    ASSERT_TRUE(leadingSnapshot->GetAttributeReaderInfo("a1", "", info));
    ASSERT_EQ(string("tableA"), info.indexPartReader->GetSchema()->GetSchemaName());
    ASSERT_TRUE(leadingSnapshot->GetAttributeReaderInfo("b2", "", info));
