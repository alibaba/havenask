...
void IndexPartitionMaker::MakeMultiSegmentData(
        ...
        bool isUpdate)
{
    ...
    for (uint32_t i = 0; i < docCounts.size(); i++)
    {
        DocumentMaker::DocumentVect docVect = MakeOneSegmentDataWithoutDump(
                docCounts[i], indexPart4Write, mockIndexPart,
                pkStartSuffix, delMode, oldDocVect);
        writer->DumpSegment();
...
void IndexPartitionMaker::MakeOneSegmentData(
        uint32_t docCount,
        IndexPartition& indexPart4Write,
        MockIndexPart& mockIndexPart,
        int32_t pkStartSuffix,
...
{
    MakeOneSegmentDataWithoutDump(docCount, indexPart4Write, mockIndexPart, 
                                  pkStartSuffix, delMode, docsToUpdate);
    PartitionWriterPtr writer = indexPart4Write.GetWriter();
    writer->Close();
}

void IndexPartitionMaker::MakeDocuments(
        uint32_t docCount, const IndexPartitionSchemaPtr& schema,
        int32_t pkStartSuffix, MockIndexPart &mockIndexPart,
        DocumentMaker::DocumentVect& segDocVect)
{
    ...
    DocumentMaker::MakeDocumentsStr(docCount, schema->GetFieldSchema(),
                                    docStr, pkFieldName, pkStartSuffix);
    ...
    if (subSchema)
    {
        ...
        MakeDocuments(docCount, subSchema, pkStartSuffix, 
                      *mockIndexPart.subIndexPart, subSegDocVect);
...
DocumentMaker::DocumentVect IndexPartitionMaker::MakeOneSegmentDataWithoutDump(
        uint32_t docCount,
        IndexPartition& indexPart4Write,
        MockIndexPart &mockIndexPart,
        int32_t pkStartSuffix,
        ...
        DocumentVect docsToUpdate)
{
    ...
    if (docsToUpdate.size() > 0)
    {
        ...
        {
            writer->UpdateDocument(docsToUpdate[i]);
        }

        DocumentMaker::UpdateDocuments(schema, segDocVect, mockIndexPart);
        for (size_t i = 0; i < segDocVect.size(); ++i)
        {
            writer->UpdateDocument(segDocVect[i]);
        }
    }
...
void IndexPartitionMaker::InnerMakeMultiSegmentsSortedData(
        vector<uint32_t>& docCounts, 
        ...
        bool isUpdate)
{
    ...
    AttributeConfigPtr sortAttributeConfig = attributeSchema->GetAttributeConfig(
            sortFieldName);
    ...
    {
        string docStr;
        DocumentMaker::MakeSortedDocumentsStr(docCounts[i], schema->GetFieldSchema(),
                docStr, sortFieldName, "string1", pkSuffix);
    ...
    for (size_t i = 0; i < docCounts.size(); ++i)
    {
        ...
        if (isUpdate)
        {
            ...
            for (size_t j = 0; j < docCounts[i]; ++j)
            {
                bool ret = writer->UpdateDocument(docVect[curDocId - docCounts[i] + j]);
...
void IndexPartitionMaker::MakeOneDelSegmentData(uint32_t docCount,
        IndexPartition& indexPart4Write, MockIndexPart &mockIndexPart,
...
void IndexPartitionMaker::MakeMultiSegmentsSortedData(
        vector<uint32_t>& docCounts,
        IndexPartitionPtr& indexPart4Write,
        MockIndexPart &mockIndexPart,
        int32_t pkStartSuffix, 
        ...
        bool isUpdate)
{
    ...
    InnerMakeMultiSegmentsSortedData(docCounts, indexPart4Write, 
            tmpIndexPart, docVect, pkStartSuffix, sortFieldName, delMode, isUpdate);
        ...
        const DocumentVect& sortedDocVec)
{
    ...
    for (size_t i  = 0; i < sortedDocVec.size(); ++i)
    {
        ...
        if (oldIndexPart.deletionMap.find(oldDocId) == 
            oldIndexPart.deletionMap.end())
        {
...
int DescCompare(const DocumentPtr& left, const DocumentPtr& right, fieldid_t fid)
{
    ...
    const ConstString& leftField = 
        leftDoc->GetAttributeDocument()->GetField(fid);
    T leftValue = *(T*)leftField.c_str();
    const ConstString& rightField = 
        rightDoc->GetAttributeDocument()->GetField(fid);
    T rightValue = *(T*)rightField.c_str();
...
public:
    DocumentMultiComp(DocumentVect& docVect, std::vector<fieldid_t> fieldIds,
                      CompareFunVec compareFunVec, vector<uint32_t>& docCounts)
        : mFieldIds(fieldIds)
...
void IndexPartitionMaker::SortDocuments(vector<uint32_t>& docCounts,
...
void IndexPartitionMaker::DoMultiSortDocuments(std::vector<uint32_t> &docCounts,
        std::vector<FieldType> &typeVec,
        std::vector<fieldid_t> &idVec,
        std::vector<std::string> &sortPatterns,
        DocumentVect& docVect)
{
    ...
    DocumentMultiComp documentMultiComp(docVect, idVec, 
            compareFuncs, docCounts);  
...
class DocumentComp
{
    typedef DocumentMaker::DocumentVect DocumentVect;
public:
    DocumentComp(DocumentVect& docVect, fieldid_t fieldId, 
                 vector<uint32_t>& docCounts)
        : mFieldId(fieldId)
...
public:
    bool operator ()(const DocumentPtr& left, const DocumentPtr& right)
    {
        ...
        const ConstString& leftField = 
            leftDoc->GetAttributeDocument()->GetField(mFieldId);
        T leftValue = *(T*)leftField.c_str();

        const ConstString& rightField = 
            rightDoc->GetAttributeDocument()->GetField(mFieldId);
        T rightValue = *(T*)rightField.c_str();
        ...
        {
            if (leftDocId >= baseDocId + (docid_t)mDocCounts[i]
                && rightDocId >= baseDocId + (docid_t)mDocCounts[i])
            {
...
template<typename T>
void IndexPartitionMaker::DoSortDocuments(
        vector<uint32_t>& srcDocCounts,
        fieldid_t fieldId, 
        DocumentVect& docVect)
{
...
template<typename T>
void IndexPartitionMaker::DoMergeSortDocuments(
        vector<uint32_t>& srcDocCounts,
        fieldid_t fieldId, 
        DocumentVect& docVect)
{
    ...
    for (size_t i = 1; i < srcDocCounts.size(); ++i)
    {
        ...
        merge(docVect.begin(), docVect.begin() + mergedDocCount, 
              docVect.begin() + mergedDocCount, docVect.begin() + mergedDocCount + srcDocCounts[i],
              outputVect.begin(), comp);
...
void IndexPartitionMaker::MergeSortDocuments(vector<uint32_t>& srcDocCounts,
        const AttributeConfigPtr& attrConfig, DocumentVect& docVect)
{
    FieldType fieldType = attrConfig->GetFieldType();
    fieldid_t fieldId = attrConfig->GetFieldConfig()->GetFieldId();
    switch (fieldType)
...
bool IndexPartitionMaker::IsVersionFileExsit(
        const std::string& rootDir, versionid_t versionId)
{
    stringstream ss;
