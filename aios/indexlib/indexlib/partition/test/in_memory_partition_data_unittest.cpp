...
void InMemoryPartitionDataTest::TestClone()
{
    ...
    InMemoryPartitionData* subData =
        clonePartitionData->GetSubInMemoryPartitionData();
    ASSERT_TRUE(clonePartitionData->GetSubPartitionData() != 
                partitionData->GetSubPartitionData());
    ASSERT_EQ(clonePartitionData->mDumpSegmentContainer,
                partitionData->mDumpSegmentContainer);
    ASSERT_EQ(clonePartitionData->mDumpSegmentContainer,
              subData->mDumpSegmentContainer);
}
...
void InMemoryPartitionDataTest::TestCreateNewSegmentData()
{
    ...
    InMemorySegmentPtr inMemSeg = creator.Create(PartitionSegmentIteratorPtr(),
            segData, GET_SEGMENT_DIRECTORY(),
            util::MemoryQuotaControllerCreator::CreatePartitionMemoryController(
                    autil::mem_pool::Pool::DEFAULT_CHUNK_SIZE), CounterMapPtr(),
