#ifndef __INDEXLIB_INDEX_PARTITION_MAKER_H
#define __INDEXLIB_INDEX_PARTITION_MAKER_H
...
public:
    static void MakeOneSegmentData(
            uint32_t docCount, IndexPartition& indexPart4Write,
    ...
    static index::DocumentMaker::DocumentVect MakeOneSegmentDataWithoutDump(
            uint32_t docCount,
    ...
    static void MakeMultiSegmentData(
    ...
    static void MakeOneDelSegmentData(uint32_t docCount,
            IndexPartition& indexPart4Write,
            MockIndexPart &mockIndexPart,
            int32_t pkStartSuffix,
            DeletionMode delMode);
    
    static void MakeMultiSegmentsSortedData(
            std::vector<uint32_t>& docCounts,
    ...
    static void SortDocuments(std::vector<uint32_t>& docCounts,
...
private:
    static void InnerMakeMultiSegmentsSortedData(
            std::vector<uint32_t>& docCounts,
            IndexPartitionPtr& indexPart4Write, 
            MockIndexPart &mockIndexPart,
            DocumentVect& documentVect, 
            int32_t pkStartSuffix, 
            const std::string& sortFieldName,
            DeletionMode delMode,
            bool isUpdate = false);
    ...
    template<typename T>
    static void DoSortDocuments(std::vector<uint32_t>& srcDocCounts,
                                fieldid_t fieldId, 
                                DocumentVect& docVect);

    static void MergeSortDocuments(std::vector<uint32_t>& srcDocCounts,
                                   const config::AttributeConfigPtr& attrConfig,
                                   DocumentVect& docVect);
    static void DoMultiSortDocuments(std::vector<uint32_t> &docCounts,
    ...
    template<typename T>
    static void DoMergeSortDocuments(
            std::vector<uint32_t>& srcDocCounts,
            fieldid_t fieldId, 
            DocumentVect& docVect);

    static void MakeDocuments(
            uint32_t docCount, const config::IndexPartitionSchemaPtr& schema,
            int32_t pkStartSuffix, MockIndexPart &mockIndexPart,
            index::DocumentMaker::DocumentVect& segDocVect);
...
#endif //__INDEXLIB_INDEX_PARTITION_MAKER_H
