...
void InMemVirtualAttributeCleanerTest::MakeAttributeDir(
        const DirectoryPtr& directory, const string& segName,
        const string& attrName, bool isVirtual)
{
    DirectoryPtr attrDirectory = directory->GetDirectory(
            segName, true)->MakeDirectory("attribute");
    if (isVirtual)
    {
        attrDirectory->MakeInMemDirectory(attrName);
    }
    else
    {
