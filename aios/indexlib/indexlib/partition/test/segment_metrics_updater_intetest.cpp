...
index_base::SegmentMetrics SegmentMetricsUpdaterTest::LoadSegmentMetrics(segmentid_t segId)
{
...
index_base::SegmentMetrics SegmentMetricsUpdaterTest::LoadSegmentMetrics(
...
index_base::SegmentInfo SegmentMetricsUpdaterTest::LoadSegmentInfo(segmentid_t segId, bool isSub) {
    auto dir = GET_PARTITION_DIRECTORY();
    string segmentName = string("segment_") + std::to_string(segId) + "_level_0";
    SegmentInfo segInfo;
    auto segDir = dir->GetDirectory(segmentName, true);
...
template <typename T>
void SegmentMetricsUpdaterTest::assertAttribute(const SegmentMetrics& metrics,
...
void SegmentMetricsUpdaterTest::assertLifecycle(const SegmentMetrics& metrics, const string& expected)
{
    auto groupMetrics = metrics.GetSegmentCustomizeGroupMetrics();
    ASSERT_TRUE(groupMetrics.get());
...
void SegmentMetricsUpdaterTest::TestSortBuild()
{
    ...
    partMeta.AddSortDescription("long1", sp_asc);
...
void SegmentMetricsUpdaterTest::TestSubDoc()
{
    ...
    if (mMultiTargetSegment)
    {
        ...
        auto subSegInfo = LoadSegmentInfo(2, true);
        ASSERT_EQ(4, subSegInfo.docCount);
        subSegInfo = LoadSegmentInfo(3, true);
