...
private:
    ...
    void InnerTestInitInMemorySegmentCreator(bool hasSub, bool isOnline,
            bool expectUseRtOption);
    ...
    void Dump(BuildingPartitionDataPtr partitionData,
              timestamp_t ts = INVALID_TIMESTAMP);
    index_base::Version GetNewVersion(std::string dirName = RT_INDEX_PARTITION_DIR_NAME);
    index_base::Version MakeVersion(versionid_t vid, std::string segmentIds,
                               timestamp_t ts);

    void CheckPartitionInfo(const index_base::PartitionDataPtr& partData,
                            docid_t docCount, docid_t subDocCount);

    void CheckDeletionMapReader(const index::DeletionMapReaderPtr& deletionMapReader,
                                const std::string& docIds, bool isDeleted);
...
INDEXLIB_INSTANTIATE_TEST_CASE_ONE_P(BuildingPartitionDataTestMode, BuildingPartitionDataTest, testing::Values(false, true));
