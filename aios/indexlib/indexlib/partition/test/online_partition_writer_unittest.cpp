...
void OnlinePartitionWriterTest::TestSimpleProcess()
{
    ...
    OperationWriterPtr opWriter = DYNAMIC_POINTER_CAST(OperationWriter,
...
public:
    ...
    MOCK_CONST_METHOD1(CheckTimestamp, bool(const document::DocumentPtr& doc));
};
...
public:
    MockOfflinePartitionWriter()
        : OfflinePartitionWriter(IndexPartitionOptions(),
                                 DumpSegmentContainerPtr(new DumpSegmentContainer))
    {
        mTTLDecoder.reset(new TTLDecoder(mSchema));
    }
    MOCK_METHOD1(AddDocument, bool(const document::DocumentPtr& doc));
    MOCK_METHOD1(UpdateDocument, bool(const document::DocumentPtr& doc));
    MOCK_METHOD1(RemoveDocument, bool(const document::DocumentPtr& doc));    
...
public:
    MockPartitionData()
        : OnDiskPartitionData(plugin::PluginManagerPtr())
    {}
public:
    MOCK_METHOD0(UpdatePartitionInfo, void());
    MOCK_METHOD0(CommitVersion, void());
};
...
void OnlinePartitionWriterTest::TestAddDocument()
{
    ...
    doc->SetDocOperateType(ADD_DOC);
    {
        ...
        EXPECT_CALL(*onlineWriter, CheckTimestamp(_))
    ...
    }
    {
        ...
        EXPECT_CALL(*offlineWriter, AddDocument(_))
            .WillOnce(Return(false));
        EXPECT_CALL(*partitionData, UpdatePartitionInfo()).Times(0);
...
void OnlinePartitionWriterTest::TestUpdateDocument()
{
    ...
    }
    {
        ...
        EXPECT_CALL(*onlineWriter, CheckTimestamp(_))
            .WillOnce(Return(true));
        EXPECT_CALL(*offlineWriter, UpdateDocument(_))
            .WillOnce(Return(true));
        EXPECT_CALL(*partitionData, UpdatePartitionInfo()).Times(1);
        ASSERT_TRUE(onlineWriter->BuildDocument(doc));
    }
...
void OnlinePartitionWriterTest::TestRemoveDocument()
{
    ...
    }
    {
        ...
        EXPECT_CALL(*onlineWriter, CheckTimestamp(_))
            .WillOnce(Return(true));
        EXPECT_CALL(*offlineWriter, RemoveDocument(_))
            .WillOnce(Return(false));
        EXPECT_CALL(*partitionData, UpdatePartitionInfo()).Times(0);
        ASSERT_FALSE(onlineWriter->BuildDocument(doc));
    }
...
void OnlinePartitionWriterTest::TestDumpSegment()
{
    ...
    EXPECT_CALL(*offlineWriter, DumpSegment())
        .WillOnce(Return());
...
void OnlinePartitionWriterTest::TestNeedDump()
{
    ...
    EXPECT_CALL(*offlineWriter, NeedDump(1024))
        .WillOnce(Return(true));
    ASSERT_TRUE(onlineWriter->NeedDump(1024));
}

void OnlinePartitionWriterTest::OpenWriter(
