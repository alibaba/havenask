...
public:
    bool Init(const config::IndexPartitionSchemaPtr& schema,
              ...
              std::string root,
              std::string partitionName = "psm",
              std::string secondRoot = "");

    void SetTermPayloadMap(
            const std::unordered_map<std::string, termpayload_t>& termMap)
    {
        mTermPayloadMap = termMap;
    }
    ...
    config::IndexPartitionSchemaPtr GetSchema()
    {
        return mSchema;
    }
    // current PsmOptions
    ...
    void SetCurrentTimestamp(int64_t currentTs)
    { mCurrentTimestamp = currentTs; }

    bool Transfer(PsmEvent event, const std::string& data,
                  const std::string& queryString,
                  const std::string& resultString);

    const partition::IndexPartitionPtr& GetIndexPartition() const
    { return mIndexPartition; }

    bool ExecuteEvent(PsmEvent event, const std::string& data);
    ResultPtr Search(const std::string& queryString,
                     TableSearchCacheType searchCache);

    PartitionMetricsPtr GetMetrics();
    misc::MetricProviderPtr GetMetricsProvider() const
    ...
    void SetMergeTTL(int64_t ttl)
    {
        mOptions.GetBuildConfig(false).ttl = ttl;
    }

    void DisableTestQuickExit()
    {
        mOptions.TEST_mQuickExit = false;
    }
    ...
    bool CreateIndexPartition(versionid_t targetVersionId);
    partition::IndexPartitionResource& GetIndexPartitionResource()
    { return mPartitionResource; }

    static std::string SerializeDocuments(
        const std::vector<document::DocumentPtr>& inDocs);
    static std::string SerializeDocuments(
        const std::vector<document::NormalDocumentPtr>& inDocs);
...
private:
    ...
    int64_t BatchBuild(const partition::IndexBuilderPtr& builder,
        const std::string& data, bool needDump = true,
    ...
    void DoBuildEvent(PsmEvent event, const std::string& data);
    bool NeedReOpen(PsmEvent event) { return event & PE_REOPEN_MASK; }
    bool RefreshIndexPartition(PsmEvent event, const std::string& data);
    ...
    std::vector<document::DocumentPtr> DeserializeDocuments(
        const std::string& docBinaryStr);
...
private:
    static document::IndexlibExtendDocumentPtr CreateExtendDocFromRawDoc(
        const config::IndexPartitionSchemaPtr& schema,
        const testlib::RawDocumentPtr& rawDoc);

    static void ConvertModifyFields(
        const config::IndexPartitionSchemaPtr& schema,
        const document::IndexlibExtendDocumentPtr &extDoc,
        const testlib::RawDocumentPtr& rawDoc);

    static void PrepareTokenizedDoc(
        const config::IndexPartitionSchemaPtr& schema,
        const document::IndexlibExtendDocumentPtr &extDoc,
        const testlib::RawDocumentPtr& rawDoc);

    static bool TokenizeValue(
        const document::TokenizeFieldPtr &field,
        const std::string &fieldValue,
        const std::string& sep = testlib::DocumentParser::DP_TOKEN_SEPARATOR);

    static bool TokenizeSingleValueField(
        const document::TokenizeFieldPtr &field,
        const std::string &fieldValue);
...
private:
    ...
    partition::IndexPartitionPtr mIndexPartition;
    int64_t mMemUseLimit;
