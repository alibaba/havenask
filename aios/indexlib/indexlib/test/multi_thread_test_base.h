...
protected:
    ...
    void Write() { while (!mIsRun) { usleep(0); } DoWrite(); }
    void Read(int* status) { while (!mIsRun) { usleep(0); } DoRead(status); }
...
////////////////////////////////////////////////////////////
inline void MultiThreadTestBase::DoMultiThreadTest(
        size_t readThreadNum, int64_t duration)
{
    ...
    for (size_t i = 0; i < readThreadNum; ++i)
    {
        autil::ThreadPtr thread = autil::Thread::createThread(
                std::tr1::bind(&MultiThreadTestBase::Read, this, &status[i]));
        readThreads.push_back(thread);
    }
    autil::ThreadPtr writeThread = autil::Thread::createThread(
            std::tr1::bind(&MultiThreadTestBase::Write, this));
