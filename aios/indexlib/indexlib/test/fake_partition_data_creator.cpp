...
FakePartitionDataCreator::FakePartitionDataCreator()
    ...
    , mPsm(DEFAULT_MEMORY_USE_LIMIT, false, IndexPartitionResource(), mSlowDumpContainer)
{
}
...
void FakePartitionDataCreator::AddBuildingData(const std::string& rtDocs)
{
    ...
    OnlinePartitionPtr onlinePartition = 
        DYNAMIC_POINTER_CAST(OnlinePartition, mPsm.GetIndexPartition());
...
partition::IndexPartitionPtr FakePartitionDataCreator::GetIndexPartition()
{
    return mPsm.GetIndexPartition();
}
