...
{
    PartitionInfoPtr partitionInfo = 
        mOnlinePartition->GetReader()->GetPartitionInfo();
...
{
    OnlinePartitionMetrics metrics = mOnlinePartition->GetPartitionMetrics();
    ss << RECLAIMED_SLICE_COUNT << ":"
       << metrics.GetAttributeMetrics()->GetReclaimedSliceCountValue() << ","
       << CURRENT_READER_RECLAIMED_BYTES << ":"
       << metrics.GetAttributeMetrics()->GetCurReaderReclaimableBytesValue() << ","
       << VAR_ATTRIBUTE_WASTED_BYTES << ":"
       << metrics.GetAttributeMetrics()->GetVarAttributeWastedBytesValue();
}
...
bool PartitionMetrics::CheckMetrics(const std::string& expectMetricsStr)
{
    ...
    for (StringMap::iterator it = expectMetrics.begin();
         it != expectMetrics.end(); it++)
    {
        ...
        {
            IE_LOG(ERROR, "metrics [%s] value not equal, expect [%s], actual [%s]",
                   it->first.c_str(), it->second.c_str(), curIt->second.c_str());
            return false;
        }
    }
    return true;
