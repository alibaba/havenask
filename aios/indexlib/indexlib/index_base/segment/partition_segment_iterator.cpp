...
void PartitionSegmentIterator::Init(
        const vector<SegmentData>& onDiskSegments,
        const vector<InMemorySegmentPtr>& dumpingInMemSegments,
        const InMemorySegmentPtr& currentBuildingSegment,
        segmentid_t nextBuildingSegmentId)
{
...
{
    return mOnDiskSegments.empty() ?
        INVALID_SEGMENTID : mOnDiskSegments.rbegin()->GetSegmentId();
}

void PartitionSegmentIterator::InitNextSegmentId(
        const vector<SegmentData>& onDiskSegments,
        const vector<InMemorySegmentPtr>& dumpingInMemSegments,
        const InMemorySegmentPtr& currentBuildingSegment)
{
...
SegmentData PartitionSegmentIterator::GetSegmentData() const 
{
    return mCurrentIter->GetSegmentData();
}

size_t PartitionSegmentIterator::GetSegmentCount() const
{
    return mOnDiskSegments.size() + mBuildingSegments.size();
}
