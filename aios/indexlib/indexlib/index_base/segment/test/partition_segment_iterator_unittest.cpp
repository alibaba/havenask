...
void PartitionSegmentIteratorTest::CheckIterator(
        PartitionSegmentIterator& iter, const string& segInfoStr)
{
...
InMemorySegmentPtr PartitionSegmentIteratorTest::CreateBuildingSegment(
        segmentid_t segId, int32_t docCount)
{
    ...
    InMemorySegmentPtr inMemSegment(new InMemorySegment(
                    buildConfig, memController, util::CounterMapPtr()));
