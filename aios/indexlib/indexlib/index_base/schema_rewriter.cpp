...
void SchemaRewriter::Rewrite(const IndexPartitionSchemaPtr& schema,
                             ...
                             const string& root)
{
    ...
    RewriteForTruncateIndexConfigs(schema, options, root);
    RewriteForSubTable(schema);
    RewriteForPrimaryKey(schema, options);
    RewriteFieldType(schema, options);
    
    string indexFormatVersionPath = storage::FileSystemWrapper::JoinPath(
            root, INDEX_FORMAT_VERSION_FILE_NAME);

    IndexFormatVersion indexFormatVersion(INDEX_FORMAT_VERSION);
    indexFormatVersion.Load(indexFormatVersionPath, true);
                             ...
                             const DirectoryPtr& rootDir)
{
    ...
    IndexFormatVersion indexFormatVersion(INDEX_FORMAT_VERSION);
    if (!indexFormatVersion.Load(rootDir, true) && options.IsOnline())
    {
        string indexFormatVersionPath = storage::FileSystemWrapper::JoinPath(
                rootDir->GetPath(), INDEX_FORMAT_VERSION_FILE_NAME);
        INDEXLIB_FATAL_ERROR(IndexCollapsed, "[%s] not exist", indexFormatVersionPath.c_str());
    }
    RewriteForKV(schema, options, indexFormatVersion);
    schema->Check();
}

void SchemaRewriter::SetPrimaryKeyLoadParam(
...
void SchemaRewriter::RewriteForPrimaryKey(const IndexPartitionSchemaPtr& schema,
        const IndexPartitionOptions& options)
{
    ...
    if (!indexConfig || indexConfig->GetIndexType() == it_trie)
    {
        return;
    }
    ...
    if (buildConfig.speedUpPrimaryKeyReader
        || indexConfig->GetPrimaryKeyIndexType() == pk_hash_table)
    {
    ...
    bool needLookupReverse = common::DocumentDeduperHelper::DelayDedupDocument(
            options, indexConfig);
...
void SchemaRewriter::RewriteForKV(const IndexPartitionSchemaPtr& schema,
                                  const IndexPartitionOptions& options,
...
void SchemaRewriter::RewriteOneRegionForKV(
        ...
        const IndexFormatVersion& indexFormatVersion, regionid_t id)
{
    ...
    if (schema->GetTableType() == tt_kkv)
    {
        ...
        if (options.IsOnline())
        {
            ...
            {
                IE_LOG(INFO, "rewrite SuffixKeyTruncateLimits from %u to %u",
                       kkvConfig->GetSuffixKeyTruncateLimits(),
...
void SchemaRewriter::RewriteForSubTable(
...
void SchemaRewriter::AddOfflineJoinVirtualAttribute(
...
void SchemaRewriter::RewriteForDisableFields(
    const IndexPartitionSchemaPtr& schema, const IndexPartitionOptions& options)
{
    ...
    for (const string& attribute : options.GetOnlineConfig().GetDisableFieldsConfig().attributes)
    {
        ...
        if (!attrSchema->DisableAttribute(attribute) &&
            subAttrSchema && !subAttrSchema->DisableAttribute(attribute))
        {
    ...
    for (const string& packAttribute : options.GetOnlineConfig().GetDisableFieldsConfig().packAttributes)
    {
        ...
        if (packAttrConfig)
        {
            for (const auto& attrConfig : packAttrConfig->GetAttributeConfigVec())
            {
                ...
                if ((summarySchema && summarySchema->GetSummaryConfig(attribute)) ||
                    (subSummarySchema && subSummarySchema->GetSummaryConfig(attribute)))
                {
                    IE_LOG(WARN, "disable pack attribute [%s] failed for inner attribute [%s] is in summary schema", packAttribute.c_str(), attribute.c_str());
        ...
        if (!attrSchema->DisablePackAttribute(packAttribute) &&
            subAttrSchema && !subAttrSchema->DisablePackAttribute(packAttribute))
        {
...
void SchemaRewriter::RewriteForDisableIndexs(
    const IndexPartitionSchemaPtr& schema, const IndexPartitionOptions& options)
{
    if (schema->GetTableType() != tt_index)
    {
    ...
    for (const string& index : options.GetOnlineConfig().GetDisableFieldsConfig().indexes)
    {
        ...
        {
            IE_LOG(ERROR, "rewrite index[%s] exception:[%s].", 
                   index.c_str(), e.what());
        }
    }
}

void SchemaRewriter::RewriteForTruncateIndexConfigs(
        const IndexPartitionSchemaPtr& schema, 
        const IndexPartitionOptions& options,
        const string& root)
{
    ...
    {
        string truncateMetaPath = storage::FileSystemWrapper::JoinPath(
            root, TRUNCATE_META_DIR_NAME);
        const IndexSchemaPtr& indexSchema = schema->GetIndexSchema();
        indexSchema->LoadTruncateTermInfo(truncateMetaPath);
    }
                                        ...
                                        const IndexFormatVersion& indexFormatVersion)
{
    ...
    if (!globalJsonMap.empty())
    {
        ...
        {
            const SingleFieldIndexConfigPtr& pkConfig =
                schema->GetIndexSchema(id)->GetPrimaryKeyIndexConfig();
            KVIndexConfigPtr kvConfig = DYNAMIC_POINTER_CAST(KVIndexConfig, pkConfig);
            assert(kvConfig);
                                        ...
                                        const IndexFormatVersion& indexFormatVersion)
{
    ...
    for (regionid_t id = 0; id < (regionid_t)schema->GetRegionCount(); id++)
    {
        ...
        KKVIndexConfigPtr kkvConfig = DYNAMIC_POINTER_CAST(KKVIndexConfig, pkConfig);
        assert(kkvConfig);
        buildProtectionThreshold = min(buildProtectionThreshold,
                kkvConfig->GetSuffixKeyProtectionThreshold());
        skipListThreshold = min(skipListThreshold,
                kkvConfig->GetSuffixKeySkipListThreshold());
    }
    ...
    IE_LOG(INFO, "kkv table use unified build_protection_threshold [%u]",
           buildProtectionThreshold);
    IE_LOG(INFO, "kkv table use unified skip_list_threshold [%u]",
           skipListThreshold);
}
    
bool SchemaRewriter::GetRewriteBuildProtectionThreshold(
        const IndexPartitionSchemaPtr& schema,
        const IndexPartitionOptions& options,
...
void SchemaRewriter::RewriteFieldType(const IndexPartitionSchemaPtr& schema,
                                      const IndexPartitionOptions& options)
{
