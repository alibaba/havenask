...
void ProgressSynchronizerTest::MakeVersions(
        const string& str, vector<Version>& versions)
{
    ...
    for (auto versionInfo : tmp)
    {
        ...
        IndexLocator indexLocator(versionInfo[1], versionInfo[2]);
        Locator locator;
...
// str: ts,src,offset;ts,src,offset
void ProgressSynchronizerTest::MakeSegmentInfos(
        const string& str, vector<SegmentInfo>& segInfos)
{
    vector<vector<int64_t>> tmp;
    StringUtil::fromString(str, tmp, ",", ";");
    for (auto segInfo : tmp)
    {
        ...
        IndexLocator indexLocator(segInfo[1], segInfo[2]);
        segmentInfo.locator.SetLocator(indexLocator.toString());
        segInfos.push_back(segmentInfo);
    }
}

void ProgressSynchronizerTest::CheckLocator(
        const ProgressSynchronizer& ps, int64_t src, int64_t offset)
{
