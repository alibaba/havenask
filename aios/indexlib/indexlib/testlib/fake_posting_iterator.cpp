...
FakePostingIterator::FakePostingIterator(const RichPostings &p,
    ...
    , _sessionPool(sessionPool)
{
    ...
    _truncateTermMeta = POOL_COMPATIBLE_NEW_CLASS(_sessionPool, index::TermMeta,
            _richPostings.second.size(), totalTermFreq, _richPostings.first);
    _termMeta = POOL_COMPATIBLE_NEW_CLASS(_sessionPool, index::TermMeta,
            _richPostings.second.size(), totalTermFreq, _richPostings.first);
}
...
common::ErrorCode FakePostingIterator::SeekPositionWithErrorCode(pos_t occ, pos_t &nextPos) {
