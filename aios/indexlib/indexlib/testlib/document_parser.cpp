...
IE_LOG_SETUP(testlib, DocumentParser);
...
RawDocumentPtr DocumentParser::Parse(const string& docStr)
{
    ...
    if (keyValues.size() <= 1)
    {
        keyValues = StringUtil::split(docStr, DP_KEY_VALUE_SEPARATOR);
    }

    RawDocumentPtr rawDoc;
    for (size_t i = 0; i < keyValues.size(); ++i)
    {
        vector<string> keyValue = StringUtil::split(keyValues[i], 
...
TokenizeSectionPtr DocumentParser::ParseSection(
        const string& sectionStr, const std::string& sep)
{
    return partition::IndexFieldConvertor::ParseSection(sectionStr, sep);
}
