...
{
    bool operator () (const pair<MergeTaskItems, double> &lst, 
               const pair<MergeTaskItems, double> &rst)
    {
...
vector<MergeTaskItems> MergeTaskItemDispatcher::DispatchMergeTaskItem(
    const IndexMergeMeta& meta, MergeTaskItems& allItems, uint32_t instanceCount)
{
    ...
    uint32_t mainPartDocCount = 
        mergePlans[segIdx.first].GetSegmentMergeInfos()[segIdx.second].segmentInfo.docCount;
    uint32_t subPartDocCount = 0;
    if (mSubSegDir)
    {
        subPartDocCount = 
            mergePlans[segIdx.first].GetSubSegmentMergeInfos()[segIdx.second].segmentInfo.docCount;
    }
    segmentid_t segmentId = 
        mergePlans[segIdx.first].GetSegmentMergeInfos()[segIdx.second].segmentId;
    InitMergeTaskItemCost(segmentId, mainPartDocCount,
                          subPartDocCount, meta, allItems);
...
pair<int32_t, int32_t> MergeTaskItemDispatcher::GetSampleSegmentIdx(
...
void MergeTaskItemDispatcher::InitMergeTaskItemCost(
        segmentid_t sampleSegId, uint32_t mainPartDocCount, uint32_t subPartDocCount,
        const IndexMergeMeta& meta, MergeTaskItems &allItems)
{
    const auto& mergePlans = meta.GetMergePlans();
    ...
    for (size_t i = 0; i < allItems.size(); ++i)
    {
        ...
        const auto &reclaimMap =
            taskItem.mIsSubItem ? meta.GetSubReclaimMap(taskItem.mMergePlanIdx)
                                : meta.GetReclaimMap(taskItem.mMergePlanIdx);

        uint32_t targetDocCount = taskItem.mIsSubItem ? plan.GetSubTargetSegmentInfo(0).docCount
                                                      : plan.GetTargetSegmentInfo(0).docCount;
        ...
        {
            outputSegmentRatio = reclaimMap->GetTargetSegmentDocCount(taskItem.mTargetSegmentIdx)
                * 1.0 / reclaimMap->GetNewDocCount();
        }
        ...
        if (mergeType == ATTRIBUTE_TASK_NAME)
        {
            ...
            else
            {
                cost = GetDirSize(segData, taskItem.mMergeType,
                        taskItem.mName) * 1.0 / sampleDocCount;
        ...
        if (mergeType == PACK_ATTRIBUTE_TASK_NAME)
        {
            ...
            double cost = GetDirSize(segData, ATTRIBUTE_DIR_NAME,
                    packAttrName) * 1.0 / sampleDocCount;
            
            const AttributeSchemaPtr &attrSchema = schema->GetAttributeSchema();
            const PackAttributeConfigPtr& packConfig =
                attrSchema->GetPackAttributeConfig(packAttrName);
            assert(packConfig);
        ...
        if (mergeType == INDEX_TASK_NAME)
        {
            ...
            double cost = GetDirSize(segData, taskItem.mMergeType,
                    taskItem.mName) * 1.0 / sampleDocCount;
            if (indexConfig->GetIndexType() == it_primarykey64
                || indexConfig->GetIndexType() == it_primarykey128)
            {
                taskItem.mCost = cost * targetDocCount * dataRatio;
                continue;
            }
            ...
            {
                const TruncateIndexConfig &truncateIndexConfig = 
                    mTruncateOptionConfig->GetTruncateIndexConfig(name);
                ...
                || indexConfig->GetIndexType() == it_expack)
            {
                PackageIndexConfigPtr packageIndexConfig = 
                    tr1::dynamic_pointer_cast<PackageIndexConfig>(indexConfig);
                assert(packageIndexConfig);
                if (packageIndexConfig->HasSectionAttribute()) 
                {
                    cost += GetDirSize(segData, taskItem.mMergeType,
                            SectionAttributeConfig::IndexNameToSectionAttributeName(
                                    taskItem.mName))* 1.0 / sampleDocCount ;
                }
            }
        ...
        {
            string name = (taskItem.mName == DEFAULT_SUMMARYGROUPNAME) ?
                          "" : taskItem.mName;
            double cost = GetDirSize(
                segData, taskItem.mMergeType,
                name) * 1.0 / sampleDocCount;
            taskItem.mCost = cost * targetDocCount * dataRatio;
            continue;
        }
...
uint64_t MergeTaskItemDispatcher::GetDirSize(
        const DirectoryPtr& segmentDirectory,
        const string &dirName, const string &name)
{
    assert(segmentDirectory);
    string subDirName = dirName + "/" + name;
    DirectoryPtr targetDirectory = 
        segmentDirectory->GetDirectory(subDirName, false);
...
uint64_t MergeTaskItemDispatcher::GetDirSize(const DirectoryPtr& directory)
{
    ...
    directory->ListFile("", fileList, false, false);
...
vector<MergeTaskItems> MergeTaskItemDispatcher::DispatchWorkItems(
        ...
        uint32_t instanceCount)
{
    ...
    IE_LOG(INFO, "merge instanceCount [%u], idle instanceCount [%u]",
           instanceCount, idleInstanceCount);
    if (idleInstanceCount > 0)
    {
        IE_LOG(WARN, "Idle merge instance exist! idle count in total instances [%u / %u]",
               idleInstanceCount, instanceCount);
    }
    return vec;
}
