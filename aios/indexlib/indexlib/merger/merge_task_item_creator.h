...
public:
    using GenerateParallelMergeItem = std::function<std::vector<index_base::ParallelMergeItem>(
...
public:
    std::vector<MergeTaskItem> CreateMergeTaskItems(
        uint32_t instanceCount = 1, bool isOptimize = false);

private:
    void InitMergeTaskIdentify(const config::IndexPartitionSchemaPtr& schema,
                               bool isSubSchema,
                               uint32_t instanceCount,
                               bool isOptimize);
    
    void InitSummaryMergeTaskIdentify(const config::IndexPartitionSchemaPtr& schema,
                                      bool isSubSchema,
                                      uint32_t instanceCount,
                                      std::vector<MergeItemIdentify>& mergeItemIdentifys);

    void InitIndexMergeTaskIdentify(const config::IndexPartitionSchemaPtr& schema,
                                    bool isSubSchema,
                                    uint32_t instanceCount,
                                    std::vector<MergeItemIdentify>& mergeItemIdentifys);

    void InitAttributeMergeTaskIdentify(
        const config::IndexPartitionSchemaPtr& schema, bool isSubSchema,
 uint32_t instanceCount,
        bool isOptimize, std::vector<MergeItemIdentify>& mergeItemIdentifys);

    void CreateMergeTaskItemsByMergeItemIdentifiers(
        const std::vector<MergeItemIdentify>& mergeItemIdentifys, size_t instanceCount,
...
private:
    ...
    void CreateParallelMergeTaskItems(const MergeItemIdentify& identify, size_t planIdx,
        bool isEntireDataSet, size_t instanceCount, size_t& taskGroupId,
        std::vector<MergeTaskItem>& mergeTaskItems) const;

    template <typename MergerType>
    std::vector<index_base::ParallelMergeItem> DoCreateParallelMergeItemByMerger(
