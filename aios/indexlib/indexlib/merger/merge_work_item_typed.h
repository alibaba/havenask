...
{
public:
    MergeWorkItemTyped(MergerPtr& merger,
...
MergeWorkItemTyped<MergerPtr, MergerResource>::MergeWorkItemTyped(
    MergerPtr& merger,
    const SegmentDirectoryPtr& segDir, const MergerResource& resource,
    const index_base::SegmentMergeInfos& segMergeInfos,
    const index_base::OutputSegmentMergeInfos& outputSegMergeInfos,
...
void MergeWorkItemTyped<MergerPtr, MergerResource>::DoProcess()
{
    ...
    catch (misc::ExceptionBase& e)
    {
        ...
        BEEPER_FORMAT_REPORT("bs_worker_error", tags,
                             "Exception throwed when merging %s : %s", 
                             GetIdentifier().c_str(), e.ToString().c_str());
        throw;
    }
}
...
void MergeWorkItemTyped<MergerPtr, MergerResource>::processForNormal()
{
    ...
    if (useTime > (float)900)
    {
        beeper::EventTags tags;
        BEEPER_FORMAT_REPORT("bs_worker_error", tags,
                             "merge [%s] [cost: %.2f] use [%.3f] seconds, over 15 min.",
...
void MergeWorkItemTyped<MergerPtr, MergerResource>::processForSorted()
{
    ...
    int64_t timeAfterMerge = autil::TimeUtility::currentTimeInMicroSeconds();
    float useTime = (timeAfterMerge - timeBeforeMerge) / 1000000.0;
    IE_LOG(INFO, "Finish sort-by-weight merging %s, use [%.3f] seconds, cost [%.2f]", 
           GetIdentifier().c_str(), useTime, GetCost());
    if (useTime > (float)900)
    {
        beeper::EventTags tags;
...
int64_t MergeWorkItemTyped<MergerPtr, MergerResource>::DoEstimateMemoryUse() const
{
    if (mMerger)
    {
        return mMerger->EstimateMemoryUse(mSegmentDirectory, mResource, mSegmentMergeInfos,
            mOutputSegmentMergeInfos, mIsSortedMerge);
    }
    return 0;
}

template<class MergerPtr, class MergerResource>
void MergeWorkItemTyped<MergerPtr, MergerResource>::SetMergeItemMetrics(
        const util::ProgressMetricsPtr& itemMetrics)
{
    // TODO: support 
