...
{
    bool operator () (const vector<double> &lst, 
               const vector<double> &rst)
    {
        double lstSum = 0.0;
        double rstSum = 0.0;
...
void MergeTaskItemDispatcherTest::TestSimpleProcess()
{
    ...
    Version version = SegmentDirectoryCreator::MakeVersion("1:-1:2,3,7,9,10", mRootDir);
    ...
    MergeTaskItemCreator taskItemCreator(&meta, segDir, SegmentDirectoryPtr(), schema,
        PluginManagerPtr(), mergeConfig, mResMgr);    

    MergeTaskItems allItems = taskItemCreator.CreateMergeTaskItems();
    vector<MergeItemIdentify> mergeItemIdentifys = 
        taskItemCreator.mMergeItemIdentifys;
    ...
    string dirName = storage::FileSystemWrapper::JoinPath(mRootDir, "segment_" +
            autil::StringUtil::toString(maxDocCountInSegmentId) + "_level_0/" + "index/pack1");
    ...
    mergeTaskItemDispatcher.InitMergeTaskItemCost(
        maxDocCountInSegmentId, maxDocCount, 0, meta, allItems);
    ...
    for (size_t i = 0; i < allItems.size(); ++i)
    {
        ...
        else if (allItems[i].mMergeType == ATTRIBUTE_TASK_NAME && 
                 allItems[i].mName == "long1")
        {
            INDEXLIB_TEST_EQUAL(targetDocCount * 4 , allItems[i].mCost);        
        }
        else if (allItems[i].mMergeType == DELETION_MAP_TASK_NAME)
        {
            ASSERT_DOUBLE_EQ(targetDocCount * 
                    MergeTaskItemDispatcher::DELETION_MAP_COST, allItems[i].mCost);
        }
        else 
        {
...
void MergeTaskItemDispatcherTest::TestSimpleProcessWithTruncate()
{
    ...
    merger::SegmentDirectoryPtr segDir(new merger::SegmentDirectory(mRootDir, version));
    segDir->Init(false, false);
    TruncateOptionConfigPtr truncateConfig = 
        TruncateConfigMaker::MakeConfig("", "", "pack1=long2:long2#DESC:long2::");
...
void MergeTaskItemDispatcherTest::TestSimpleProcessWithSubDoc()
{
    ...
    Version version = SegmentDirectoryCreator::MakeVersion("1:-1:2,3,7,9,10", mRootDir, true);
    merger::SegmentDirectoryPtr segDir(new merger::SegmentDirectory(mRootDir, version));
    ...
    meta.mMergePlans = mergePlans;
    meta.mMergePlanResources.resize(mergePlans.size());
    MergeTaskItemCreator taskItemCreator(
        &meta, segDir, subSegDir, schema, PluginManagerPtr(), mergeConfig, mResMgr);
    MergeTaskItems allItems = taskItemCreator.CreateMergeTaskItems();
    vector<MergeItemIdentify> mergeItemIdentifys = taskItemCreator.mMergeItemIdentifys;
    ...
    string dirName = storage::FileSystemWrapper::JoinPath(mRootDir, "segment_" +
            autil::StringUtil::toString(maxDocCountInSegmentId) + "_level_0/index/pack1");
    string subDirName = storage::FileSystemWrapper::JoinPath(mRootDir, "segment_" +
            autil::StringUtil::toString(maxDocCountInSegmentId) + "_level_0/sub_segment/index/sub_pack1");
    vector<string> fileNames;
    vector<uint32_t> fileListSize;
    ...
    mergeTaskItemDispatcher.InitMergeTaskItemCost(
        maxDocCountInSegmentId, mainDocCount, subDocCount, meta, allItems);
    INDEXLIB_TEST_EQUAL(mergeItemIdentifys.size() * planCount, allItems.size());
    ...
    for (size_t i = 0; i < allItems.size(); ++i)
    {
        ...
        if (allItems[i].mMergeType == INDEX_TASK_NAME && allItems[i].mName == "pack1")
        {
            INDEXLIB_TEST_EQUAL(targetDocCount * double(700) / 700 * 20 * dataRatio,
                                allItems[i].mCost);
        }
        else if (allItems[i].mMergeType == INDEX_TASK_NAME && allItems[i].mName == "sub_pack1")
        {
            INDEXLIB_TEST_EQUAL(subTargetDocCount * double(700) / 500 * 20 * dataRatio,
                                allItems[i].mCost);
        }
        else if (allItems[i].mMergeType == ATTRIBUTE_TASK_NAME && 
                 allItems[i].mName == "long1")
        ...
        {
            uint32_t docCount = 
                allItems[i].mIsSubItem ? subTargetDocCount : targetDocCount;

            ASSERT_DOUBLE_EQ(docCount * MergeTaskItemDispatcher::DELETION_MAP_COST * dataRatio,
                    allItems[i].mCost);
        }
        else 
        {
...
void MergeTaskItemDispatcherTest::TestDispatchMergeTaskItem()
{
    ...
    string segmentRoot = storage::FileSystemWrapper::JoinPath(mRootDir, "segment_10_level_0");
    ...
    meta.mMergePlans = mergePlans;
    meta.mMergePlanResources.resize(mergePlans.size());
    MergeTaskItemCreator taskItemCreator(&meta, segDir, SegmentDirectoryPtr(),
            schema, PluginManagerPtr(), mergeConfig, mResMgr);    
    MergeTaskItems allItems = taskItemCreator.CreateMergeTaskItems();

    std::vector<MergeTaskItems> mergeTaskItemsVec =
        dispatcher.DispatchMergeTaskItem(meta, allItems, instanceCount);
    ...
    EXPECT_EQ("MergePlan[1] summary [__DEFAULT_SUMMARYGROUPNAME__] TargetSegmentIdx[0]", mergeTaskItemsVec[0][2].ToString());
    EXPECT_EQ("MergePlan[0] summary [__DEFAULT_SUMMARYGROUPNAME__] TargetSegmentIdx[0]", mergeTaskItemsVec[0][3].ToString());
...
{
    vector<string> files = StringTokenizer::tokenize(ConstString(fileStr), ';',
            StringTokenizer::TOKEN_IGNORE_EMPTY | StringTokenizer::TOKEN_TRIM);
    for (size_t i = 0; i < files.size(); ++i)
    {
        vector<string> filePatten = StringTokenizer::tokenize(ConstString(files[i]), ' ',
                StringTokenizer::TOKEN_IGNORE_EMPTY | StringTokenizer::TOKEN_TRIM);
        assert(filePatten.size() == 2);
        string dirPath = storage::FileSystemWrapper::JoinPath(segRoot, filePatten[0]);
        storage::FileSystemWrapper::MkDir(dirPath, true);

        string filePath = storage::FileSystemWrapper::JoinPath(dirPath, "test_data_file");
...
void MergeTaskItemDispatcherTest::TestDispatchMergeTaskItemWithSubDoc()
{
    ...
    string segmentRoot = storage::FileSystemWrapper::JoinPath(mRootDir, "segment_10_level_0");
    string subSegmentRoot = storage::FileSystemWrapper::JoinPath(segmentRoot, 
...
void MergeTaskItemDispatcherTest::TestGetDirSize()
{
    ...
    Version version = SegmentDirectoryCreator::MakeVersion("1:-1:2,3,10", mRootDir);

    segmentid_t segId = 2;
    string segmentName = "segment_" + autil::StringUtil::toString(segId) + "_level_0";
    string segmentPath = storage::FileSystemWrapper::JoinPath(mRootDir, segmentName);
    string phraseIndexPath = storage::FileSystemWrapper::JoinPath(segmentPath, phrasePath);
    ...
    INDEXLIB_TEST_EQUAL(allSize, mergeTaskItemDispatcher.GetDirSize(
                    segDirectory, "index", "phrase"));
}
...
void MergeTaskItemDispatcherTest::TestGetDirSizeInPackageFile()
{
    ...
    MergeTaskItemDispatcher mergeTaskItemDispatcher(
            segDir, schema, TruncateOptionConfigPtr());
...
void MergeTaskItemDispatcherTest::TestGetDirSizeWithSubDir()
{
    ...
    string pkIndexPath = storage::FileSystemWrapper::JoinPath(segmentPath, pkPath);
    CreateDirAndFile(pkIndexPath, pkPathFileNames, pkFileListSize);
    string pkAttrIndexPath = storage::FileSystemWrapper::JoinPath(segmentPath, pkAttrPath);
    ...
    file_system::DirectoryPtr segDirectory = 
        segDir->GetPartitionData()->GetRootDirectory()->GetDirectory(segmentName, true);
    INDEXLIB_TEST_EQUAL((size_t)(3072), mergeTaskItemDispatcher.GetDirSize(
                    segDirectory, "index", "pk"));
}
...
void MergeTaskItemDispatcherTest::TestGetMaxDocCount()
{
    ...
    for (size_t i = 0; i < segStrVec.size(); ++i)
    {
        ...
        MergeHelper::MakeSegmentMergeInfosWithSegId(
            segStrVec[i], segMergeInfos);
...
void MergeTaskItemDispatcherTest::TestInitMergeTaskIdentifySubSchema()
{
    ...
    schema->SetSubIndexPartitionSchema(subSchema);
    merger::SegmentDirectoryPtr segDir;
    merger::SegmentDirectoryPtr subSegDir(new merger::SegmentDirectory());
    MergeConfig mergeConfig;
    IndexMergeMeta mergeMeta;
    MergeTaskItemCreator taskItemCreator(&mergeMeta, segDir, subSegDir, schema,
                                         PluginManagerPtr(), mergeConfig,
                                         mResMgr);
    vector<MergePlan> mergePlans;
    MergePlan mergePlan;
    mergePlan.GetTargetSegmentInfo(0).docCount = 2;
...
void MergeTaskItemDispatcherTest::TestInitMergeTaskIdentify()
{
    ...
    MergeConfig mergeConfig;
    IndexMergeMeta mergeMeta;
    MergeTaskItemCreator taskItemCreator(&mergeMeta, segDir, SegmentDirectoryPtr(), schema,
                                         PluginManagerPtr(), mergeConfig,
                                         mResMgr);
    vector<MergePlan> mergePlans;
    MergePlan mergePlan;
...
{
    config::IndexPartitionSchemaPtr schema =
        test::SchemaMaker::MakeSchema(
                string("text1:text;string1:string;"), 
                string("pack1:pack:text1:false:2;strindex:string:string1:false:2"),
                "","");
    Version version = SegmentDirectoryCreator::MakeVersion("1:-1:0,1,2,3", mRootDir);
    merger::SegmentDirectoryPtr segDir(new merger::SegmentDirectory(mRootDir, version));
    segDir->Init(false, false);
    MergeTaskItemDispatcher mergeTaskItemDispatcher(
            segDir, schema, TruncateOptionConfigPtr());
    ...
    MakeDataForDir(storage::FileSystemWrapper::JoinPath(mRootDir, "segment_1_level_0/index/pack1_@_0"), 
                     "dictionary:20,posting:10");

    MakeDataForDir(storage::FileSystemWrapper::JoinPath(mRootDir, "segment_1_level_0/index/pack1_@_1"), 
                     "dictionary:30,posting:20");

    MakeDataForDir(storage::FileSystemWrapper::JoinPath(mRootDir, "segment_1_level_0/index/strindex_@_0"), 
                     "dictionary:10,posting:40");

    MakeDataForDir(storage::FileSystemWrapper::JoinPath(mRootDir, "segment_1_level_0/index/strindex_@_1"), 
...
void MergeTaskItemDispatcherTest::MakeDataForDir(
        const string& dirPath, const string& fileDespStr)
{
...
void MergeTaskItemDispatcherTest::CheckMergeTaskItemCost(
    MergeTaskItems& allItems, const string& itemName, double expectCost)
{
...
void MergeTaskItemDispatcherTest::CreateDirAndFile(const string &dirPath, 
        const vector<string> &fileNames, vector<uint32_t> fileListSize)
{
    storage::FileSystemWrapper::MkDir(dirPath, true);
    for (size_t i = 0; i < fileNames.size(); ++i) 
    {
        string filePath = storage::FileSystemWrapper::JoinPath(dirPath, fileNames[i]);
        BufferedFileWriter bufferedFileWriter;
        bufferedFileWriter.Open(filePath);
        uint8_t *buf = new uint8_t[fileListSize[i]];
        memset(buf, 0, fileListSize[i]);
        bufferedFileWriter.Write(buf, fileListSize[i]);   
        bufferedFileWriter.Close();
        delete []buf;
    }
}

void MergeTaskItemDispatcherTest::CreateDirAndFileInPackageFile(
        ...
        std::vector<uint32_t> fileListSize)
{
    ...
    string tmpSegDirPath = segmentPath + "_tmp";

    file_system::DirectoryPtr tmpSegDirectory = 
        rootDirectory->MakeInMemDirectory(tmpSegDirPath);

    file_system::PackageFileWriterPtr packFileWriter = 
        tmpSegDirectory->CreatePackageFileWriter("package_file");

    assert(innerFilePath.size() == fileListSize.size());
    for (size_t i = 0; i < innerFilePath.size(); i++)
    {
        file_system::FileWriterPtr fileWriter = 
            packFileWriter->CreateInnerFileWriter(innerFilePath[i]);
    ...
    tmpSegDirectory->Sync(true);

    string absSegPath = storage::FileSystemWrapper::JoinPath(mRootDir, segmentPath);
    string absTmpSegPath = storage::FileSystemWrapper::JoinPath(mRootDir, tmpSegDirPath);

    string packageFileMetaPathSrc = 
        storage::FileSystemWrapper::JoinPath(absTmpSegPath, "package_file.__meta__");
    string packageFileDataPathSrc = 
        storage::FileSystemWrapper::JoinPath(absTmpSegPath, "package_file.__data__0");

    string packageFileMetaPathDst = 
        storage::FileSystemWrapper::JoinPath(absSegPath, "package_file.__meta__");
    string packageFileDataPathDst = 
        storage::FileSystemWrapper::JoinPath(absSegPath, "package_file.__data__0");

    storage::FileSystemWrapper::Rename(packageFileMetaPathSrc, packageFileMetaPathDst);
    storage::FileSystemWrapper::Rename(packageFileDataPathSrc, packageFileDataPathDst);
    storage::FileSystemWrapper::DeleteDir(absTmpSegPath);
}
