...
bool DemoIndexSegmentRetriever::Open(const DirectoryPtr& indexDir)
{
    ...
    // load index
    if (!parallelMeta.Load(indexDir->GetPath()))
    {
    ...
    for (uint32_t i = 0; i < parallelMeta.parallelCount; i++)
    {
        ...
        string subDirName = ParallelMergeItem::GetParallelInstanceDirName(
            parallelMeta.parallelCount, i);
