...
bool MergeHelper::CheckMergePlan(
        const MergePlan& plan,
        const vector<segmentid_t>& inPlanIds,
        const vector<size_t>& targetPosition)
{
#define ASSERT_EQUAL(left, right)                                       \
    if ((left) != (right))                                              \
    {                                                                   \
        IE_LOG(ERROR, "check failed : left[%d] and right[%d] not match", \
               int(left), int(right));                                  \
        return false;                                                   \
    }
...
pair<SegmentMergeInfos, LevelInfo>
MergeHelper::PrepareMergeInfo(const string& levelStr)
{
    ...
    LevelTopology topo = LevelMeta::StrToTopology(metas[0]);
    ...
    for (int level = segInfos.size() - 1; level >= 0; --level)
    {
        ...
        for (size_t column = 0; column < segs.size(); ++column)
        {
            ...
            segTopo.levelTopology = (level == 0 ? topo_sequence : topo);
            ...
            segMergeInfo.segmentInfo.shardingColumnCount = columnCount;
            segMergeInfos.push_back(segMergeInfo);
        }
    }
