...
void MergeTaskItemCreatorTest::CaseSetUp()
{
...
void MergeTaskItemCreatorTest::TestSimpleProcess()
{
    ...
    for (size_t i = 0; i < identifyCount; i++)
    {
        ...
        if (i % 2 == 0)
        {
            ...
            identify.generateParallelMergeItem
                = [items = std::move(parallelMergeItems)](size_t, bool) { return items; };
    ...
    MergeTaskItemCreator mergeTaskItemCreator(&meta, SegmentDirectoryPtr(),
            SegmentDirectoryPtr(), IndexPartitionSchemaPtr(), PluginManagerPtr(),
            mergeConfig, resMgr);

    mergeTaskItemCreator.CreateMergeTaskItemsByMergeItemIdentifiers(
        mergeItemIdentifys, 1, allItems);
...
void MergeTaskItemCreatorTest::TestParallelMerge()
{
    ...
    IndexPartitionSchemaPtr schema =
        SchemaAdapter::LoadSchema(TEST_DATA_PATH, "parallel_merge/customized_index_schema.json");
    ...
    PluginResourcePtr resource(new IndexPluginResource(schema, options,
                                                       counterMap, PartitionMeta(),
                                                       pluginRoot));
    ...
    MergeTaskItemCreator mergeTaskItemCreator(
            &meta, SegmentDirectoryPtr(), SegmentDirectoryPtr(),
            schema, pluginManager, mergeConfig, resMgr);    
    vector<MergeTaskItem> allItems
        = mergeTaskItemCreator.CreateMergeTaskItems(3, true);
...
{
    return lhs.mMergePlanIdx == rhs.mMergePlanIdx && lhs.mMergeType == rhs.mMergeType
        && lhs.mName == rhs.mName && lhs.mIsSubItem == rhs.mIsSubItem && lhs.mCost == rhs.mCost
        && lhs.mTargetSegmentIdx == rhs.mTargetSegmentIdx
        && lhs.mParallelMergeItem == rhs.mParallelMergeItem;
}
...
void MergeTaskItemCreatorTest::TestInitIndexMergeIdentify()
{
    ...
    meta.mMergePlans = mergePlans;
    meta.mMergePlanResources.resize(mergePlans.size());
    MockMergeTaskItemCreator creator(&meta, SegmentDirectoryPtr(), SegmentDirectoryPtr(),
    ...
    vector<MergeTaskItem> expectedItems = {
        makeTaskItem(0, INDEX_TASK_NAME, "pk", 1, 2, 0.1, 0),
...
void MergeTaskItemCreatorTest::TestInitAttributeMergeIdentify()
{
    ...
    auto makeTaskItem = [&item1, &item2](size_t planIdx, const string& mergeType,
                            const string& name, size_t groupId) {
    ...
    vector<MergeTaskItem> expectedItems = {
        makeTaskItem(0, ATTRIBUTE_TASK_NAME, "field1", 0),
        makeTaskItem(0, ATTRIBUTE_TASK_NAME, "field2", 1),
        makeTaskItem(0, PACK_ATTRIBUTE_TASK_NAME, "pack1", 2),
        makeTaskItem(1, ATTRIBUTE_TASK_NAME, "field1", 3),
        makeTaskItem(1, ATTRIBUTE_TASK_NAME, "field2", 4),
        makeTaskItem(1, PACK_ATTRIBUTE_TASK_NAME, "pack1", 5)
    };
...
void MergeTaskItemCreatorTest::TestInitSummaryMergeIdentify()
{
    ...
    vector<MergeTaskItem> expectedItems = {
        makeTaskItem(0, SUMMARY_TASK_NAME, DEFAULT_SUMMARYGROUPNAME, 0),
