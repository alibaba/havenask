...
void IndexMergeMetaTest::CaseSetUp()
{
...
void IndexMergeMetaTest::TestStoreAndLoad()
{
    ...
    for (size_t i = 0; i < task.GetPlanCount(); ++i)
    {
        ...
        ReclaimMapPtr subReclaimMap
            = IndexTestUtil::CreateReclaimMap(subPartDocCounts, delDocIdSets);
        BucketMaps bucketMaps;
        BucketMapPtr bucketMap1 = TruncateTestHelper::CreateBucketMap((i + 1) * 10);
        bucketMaps["1"] = bucketMap1;
        BucketMapPtr bucketMap2 = TruncateTestHelper::CreateBucketMap((i + 1) * 50);
        bucketMaps["2"] = bucketMap2;
        BucketMapPtr bucketMap3 = TruncateTestHelper::CreateBucketMap((i + 1) * 37);
        ...
        meta.TEST_AddMergePlanMeta(task[i], segmentid_t(i + 10),
                              segmentInfo, subSegmentInfo,
                              segmentMergeInfos, subSegmentMergeInfos);
    ...
    for (size_t i = 0; i < meta.GetMergePlanCount(); ++i)
    {
        ...
        const BucketMaps &bucketMaps2 = newMeta.GetBucketMaps(i);
        INDEXLIB_TEST_EQUAL(bucketMaps1.size(), bucketMaps2.size());
        for (BucketMaps::const_iterator it = bucketMaps1.begin();
             it != bucketMaps1.end(); ++it)
        {
            const string &name = it->first;
            const BucketMapPtr &bucketMap1 = it->second;
            BucketMaps::const_iterator fIt = bucketMaps2.find(name);
            INDEXLIB_TEST_TRUE(fIt != bucketMaps2.end());
            const BucketMapPtr &bucketMap2 = fIt->second;
...
void IndexMergeMetaTest::TestLoadLegacyMeta()
{
    ...
    for (size_t i = 0; i < task.GetPlanCount(); ++i)
    {
        ...
        ReclaimMapPtr reclaimMap = IndexTestUtil::CreateReclaimMap(
                docCounts, delDocIdSets);
        ReclaimMapPtr subReclaimMap = IndexTestUtil::CreateReclaimMap(
                subPartDocCounts, delDocIdSets);
        BucketMaps bucketMaps;
        BucketMapPtr bucketMap1 = TruncateTestHelper::CreateBucketMap((i + 1) * 10);
        bucketMaps["1"] = bucketMap1;
        BucketMapPtr bucketMap2 = TruncateTestHelper::CreateBucketMap((i + 1) * 50);
    ...
    string legacyRoot = TEST_DATA_PATH "/legacy_merge_meta";
    IndexMergeMeta legacyMeta;
    legacyMeta.Load(legacyRoot);
...
vector<MergeTaskItems> IndexMergeMetaTest::CreateMergeTaskItems(
