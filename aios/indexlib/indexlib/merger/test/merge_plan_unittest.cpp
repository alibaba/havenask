...
void MergePlanTest::AddOneSegment(MergePlan& mergePlan, segmentid_t segId,
                                  int64_t ts, uint32_t maxTTL,
...
void MergePlanTest::TestStoreAndLoad()
{
    ...
    {
        MergePlan mergePlan;
        SegmentMergeInfo segMergeInfo(1, SegmentInfo(), 1, 1);
        mergePlan.AddSegment(segMergeInfo);
        ...
        mergePlan.SetSubSegmentMergeInfos({ SegmentMergeInfo(2, SegmentInfo(), 2, 2) });
        // mergePlan.subInPlanSegMergeInfos.push_back(SegmentMergeInfo(2, SegmentInfo(), 2, 2));
        mergePlan.Store(rootPath);
    }
    {
        MergePlan mergePlan;
        mergePlan.Load(rootPath);
