...
MergeTaskItemCreator::MergeTaskItemCreator(
        ...
        const SegmentDirectoryPtr& subSegDir,
...
vector<ParallelMergeItem> MergeTaskItemCreator::DoCreateParallelMergeItemByMerger(
    const MergerType& mergerPtr, size_t planIdx, bool isSub, bool isEntireDataSet,
    size_t instanceCount) const
{
    const auto& plan = mMergeMeta->GetMergePlan(planIdx);
    const auto& inPlanSegMergeInfos
        = isSub ? plan.GetSubSegmentMergeInfos() : plan.GetSegmentMergeInfos();
    return mergerPtr->CreateParallelMergeItems(isSub ? mSubSegDir : mSegDir, inPlanSegMergeInfos,
        instanceCount, isEntireDataSet, mResMgr);
}

void MergeTaskItemCreator::InitSummaryMergeTaskIdentify(const IndexPartitionSchemaPtr& schema,
    bool isSubSchema, uint32_t instanceCount, vector<MergeItemIdentify>& mergeItemIdentifys)
{
    ...
    for (summarygroupid_t id = 0; id < summarySchema->GetSummaryGroupConfigCount(); ++id)
    {
        const SummaryGroupConfigPtr& summaryGroupConfig =
            summarySchema->GetSummaryGroupConfig(id);
        if (!summaryGroupConfig->NeedStoreSummary())
        {
            continue;
        }

        SummaryMergerPtr summaryMerger
            = CreateSummaryMerger(summarySchema, summaryGroupConfig->GetGroupName());
        ...
        auto generateParallelItem
            = [this, isSubSchema, summaryMerger, instanceCount](
                  size_t planIdx, bool isEntireDataSet) {
                  return DoCreateParallelMergeItemByMerger(
                      summaryMerger, planIdx, isSubSchema, isEntireDataSet, instanceCount);
              };
        MergeItemIdentify identify(SUMMARY_TASK_NAME,
                                   summaryGroupConfig->GetGroupName(),
                                   isSubSchema, generateParallelItem, enableInPlanMultiSegmentParallel);
        mergeItemIdentifys.push_back(identify);
    }
}

SummaryMergerPtr MergeTaskItemCreator::CreateSummaryMerger(
...
void MergeTaskItemCreator::InitAttributeMergeTaskIdentify(const IndexPartitionSchemaPtr& schema,
    bool isSubSchema, uint32_t instanceCount, bool isOptimize,
    vector<MergeItemIdentify>& mergeItemIdentifys)
{
    ...
    for (; iter != attrConfigs->End(); iter++)
    {
        ...
        AttributeMergerPtr attrMerger =
            CreateAttributeMerger(attrSchema, attrName, false, isOptimize);
        ...
        auto generateParallelItem
            = [this, isSubSchema, attrMerger, instanceCount](
                  size_t planIdx, bool isEntireDataSet) {
                  return DoCreateParallelMergeItemByMerger(
                      attrMerger, planIdx, isSubSchema, instanceCount, isEntireDataSet);
              };
    ...
    for (; packIter != packAttrConfigs->End(); packIter++)
    {
        ...
        AttributeMergerPtr packAttrMerger =
            CreateAttributeMerger(attrSchema, attrName, true, isOptimize);
        bool enableInPlanMultiSegmentParallel = packAttrMerger->EnableMultiOutputSegmentParallel();
        if (!mMergeConfig.EnableInPlanMultiSegmentParallel())
        {
            enableInPlanMultiSegmentParallel = false;
        }
        auto generateParallelItem = [this, isSubSchema, packAttrMerger, instanceCount](
                                        size_t planIdx, bool isEntireDataSet) {
            return DoCreateParallelMergeItemByMerger(
                packAttrMerger, planIdx, isSubSchema, isEntireDataSet, instanceCount);
        };
        MergeItemIdentify identify(PACK_ATTRIBUTE_TASK_NAME, attrName, isSubSchema,
            generateParallelItem, enableInPlanMultiSegmentParallel);
        mergeItemIdentifys.push_back(identify);
    }
}

AttributeMergerPtr MergeTaskItemCreator::CreateAttributeMerger(
    const AttributeSchemaPtr &attrSchema,
    const string& attrName, bool isPackAttribute, bool isOptimize) const
{
...
void MergeTaskItemCreator::InitIndexMergeTaskIdentify(const IndexPartitionSchemaPtr& schema,
    bool isSubSchema, uint32_t instanceCount, vector<MergeItemIdentify>& mergeItemIdentifys)
{
    ...
    for (; iter != indexConfigs->End(); iter++)
    {
        ...
        auto generateParallelItem
            = [this, isSubSchema, indexMerger, instanceCount](
                  size_t planIdx, bool isEntireDataSet) {
                  return DoCreateParallelMergeItemByMerger(
                      indexMerger, planIdx, isSubSchema, isEntireDataSet, instanceCount);
              };

        MergeItemIdentify identify(INDEX_TASK_NAME, indexName, isSubSchema,
            std::move(generateParallelItem), enableInPlanMultiSegmentParallel);
        mergeItemIdentifys.push_back(identify);
    }
}

IndexMergerPtr MergeTaskItemCreator::CreateIndexMerger(
    const IndexConfigPtr& indexConfig) const
{
    IndexType indexType = indexConfig->GetIndexType();
    IndexMergerPtr indexMerger(
        IndexMergerFactory::GetInstance()->CreateIndexMerger(indexType, mPluginManager));
...
void MergeTaskItemCreator::InitMergeTaskIdentify(
    const IndexPartitionSchemaPtr &schema, bool isSubSchema,
    uint32_t instanceCount, bool isOptimize)
{
    mMergeItemIdentifys.push_back(MergeItemIdentify(DELETION_MAP_TASK_NAME, "", isSubSchema));

    InitSummaryMergeTaskIdentify(schema, isSubSchema,
                                 instanceCount, mMergeItemIdentifys);

    InitIndexMergeTaskIdentify(schema, isSubSchema,
                               instanceCount, mMergeItemIdentifys);

    InitAttributeMergeTaskIdentify(
        schema, isSubSchema, instanceCount, isOptimize, mMergeItemIdentifys);
}

vector<MergeTaskItem> MergeTaskItemCreator::CreateMergeTaskItems(
    uint32_t instanceCount, bool isOptimize)
{
    ...
    vector<MergeTaskItem>& mergeTaskItems) const
{
    ...
    for (size_t planIdx = 0; planIdx < mergePlans.size(); ++planIdx)
    {
        ...
        {
            CreateParallelMergeTaskItems(
                identify, planIdx, isEntireDataSet, instanceCount, taskGroupId, mergeTaskItems);
        }
    }
}

void MergeTaskItemCreator::CreateParallelMergeTaskItems(const MergeItemIdentify& identify,
    size_t planIdx, bool isEntireDataSet, size_t instanceCount, size_t& taskGroupId,
    vector<MergeTaskItem>& mergeTaskItems) const
{
    const auto& plan = mMergeMeta->GetMergePlan(planIdx);
