...
void BalanceTreeMergeStrategy::SetParameter(
...
MergeTask BalanceTreeMergeStrategy::CreateMergeTask(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
    ...
    for (uint32_t i = 0; i < balanceTree.size(); i++)
    {
        ...
        while (true)
        {
            ...
            uint32_t segNum = std::min((size_t)mConflictSegNum, 
                    balanceTree[currentLayer].size());
            for (uint32_t j = 0; j < segNum; j++)
            {
                ...
                mergedSegInfo.segmentInfo.docCount += docCount;
    ...
    MergeTask mergeTask;
    if (mergePlan.GetSegmentCount() > 1 
        || (mergePlan.GetSegmentCount() == 1 && isConflictDelPercent))
    {
...
void BalanceTreeMergeStrategy::CreateTopLayerMergePlan(MergeTask& mergeTask, 
        const SegmentMergeInfos& segMergeInfos)
{
    for (size_t i = 0; i < segMergeInfos.size(); i++)
    {
        ...
        if (-1 == layerIdx && LargerThanDelPercent(segMergeInfos[i]))
        {
            ...
            mergeTask.AddPlan(mergePlan);
            IE_LOG(INFO, "Merge plan generated [%s] by BalanceTree", 
                   mergePlan.ToString().c_str());
            return;
        }
    }
}
...
MergeTask BalanceTreeMergeStrategy::CreateMergeTaskForOptimize(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
