...
MergeTask ShardBasedMergeStrategy::CreateMergeTask(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
    ...
    if (topology != topo_hash_mod || levelNum < 2)
    {
...
MergeTask ShardBasedMergeStrategy::CreateMergeTaskForOptimize(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
...
uint32_t ShardBasedMergeStrategy::CalculateLevelPercent(
        double spaceAmplification, uint32_t levelNum) const
{
    uint32_t begin = 0;
    uint32_t end = 100;
...
double ShardBasedMergeStrategy::CalculateSpaceAmplification(
        uint32_t percent, uint32_t levelNum) const
{
...
vector<vector<bool>> ShardBasedMergeStrategy::GenerateMergeTag(
        const LevelInfo& levelInfo,
        const map<segmentid_t, SegmentMergeInfo>& segMergeInfoMap)
{
    size_t levelNum = levelInfo.GetLevelCount();    
    ...
    size_t columnCount = levelInfo.GetColumnCount();
...
MergeTask ShardBasedMergeStrategy::GenerateMergeTask(
        const LevelInfo& levelInfo,
        const map<segmentid_t, SegmentMergeInfo>& segMergeInfoMap,
    ...
    for (size_t i = 0; i < columnCount; ++i)
    {
        ...
        for (size_t levelIdx = 1; levelIdx < levelNum; ++levelIdx)
        {
            ...
            if (!mergeTag[levelIdx][i])
            {
                ...
                targetTopology.levelTopology = topo_hash_mod;
