...
{
protected:
    bool ParseUint32Param(const std::string& kvPair, std::string& keyName,
                          uint32_t& value);
};
...
{
    MergeTriggerDescription()
        : conflictSegmentCount(std::numeric_limits<uint32_t>::max())
        , conflictDeletePercent(100)
    {}
    bool FromString(const std::string& str);
    std::string ToString() const;
public:
