...
public:
    RangeStrategyConfigValue()
        ...
        , mMin(std::numeric_limits<T>::min())
    {}
    
    RangeStrategyConfigValue(T timeMax, T timeMin)
        : mMax(timeMax)
        , mMin(timeMin)
    {}
    ...
    bool Compare(const StrategyConfigValuePtr& configValue) override
    {
        ...
        RangeStrategyConfigValue* rangeStrategyConfigValue =
            dynamic_cast<RangeStrategyConfigValue*>(configValue.get());
        if (unlikely(rangeStrategyConfigValue == NULL))
        {
            return false;
        }
        return mMin >= rangeStrategyConfigValue->mMin && 
            mMin <= rangeStrategyConfigValue->mMax;
    }
...
public:
    ...
    StringStrategyConfigValue(std::string typeValue)
        : mValue(typeValue)
    {}
...
public:
    ...
    static StrategyConfigValue* CreateConfigValue(std::string configValueStr)
    {
        ...
        if (configValueSize >=2 && configValueStr[0] == '[' && 
            configValueStr[configValueSize - 1] == ']')
        {
            // type range
            ...
            {
                IE_LOG(ERROR, "range keyvalue size not equal 2, keyvalue is [%s]",
                    configValueStr.c_str());
                return NULL;
            }
