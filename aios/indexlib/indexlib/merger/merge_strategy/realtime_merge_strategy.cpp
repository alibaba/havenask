...
void RealtimeMergeStrategy::SetParameter(
        const MergeStrategyParameter& param)
{
    ...
    for (uint32_t i = 0; i < st.getNumTokens(); i++)
    {
        ...
        if (st2[0] == MAX_SMALL_SEGMENT_COUNT) 
        {
            ...
            if (!StringUtil::strToInt32(st2[1].c_str(), maxSmallSegmentCount) ||
                maxSmallSegmentCount < 0)
            {
                INDEXLIB_THROW(misc::BadParameterException,
                        "Invalid parameter for merge strategy: [%s]",
                        st2[1].c_str());
        ...
        else if (st2[0] == DO_MERGE_SIZE_THRESHOLD)
        {
            ...
            if (!StringUtil::strToInt64(st2[1].c_str(), doMergeSizeThreshold) ||
                doMergeSizeThreshold < 0)
            {
                INDEXLIB_THROW(misc::BadParameterException,
                        "Invalid parameter for merge strategy: [%s]",
                        st2[1].c_str());
        ...
        else if (st2[0] == DONT_MERGE_SIZE_THRESHOLD)
        {
            ...
            if (!StringUtil::strToInt64(st2[1].c_str(), dontMergeSizeThreshold) ||
                dontMergeSizeThreshold <= 0)
            {
                INDEXLIB_THROW(misc::BadParameterException,
                        "Invalid parameter for merge strategy: [%s]",
                        st2[1].c_str());
    ...
    {
        INDEXLIB_THROW(misc::BadParameterException,
                       "%s[%ld] should not be less than %s[%ld]",
                       DO_MERGE_SIZE_THRESHOLD.c_str(), mDoMergeSizeThreshold, 
                       DONT_MERGE_SIZE_THRESHOLD.c_str(), mDontMergeSizeThreshold);
    }
}
...
{
    stringstream ss;
    ss << MAX_SMALL_SEGMENT_COUNT << "=" << mMaxSmallSegmentCount << ";"
       << DO_MERGE_SIZE_THRESHOLD << "=" << (mDoMergeSizeThreshold>>20) << ";"
       << DONT_MERGE_SIZE_THRESHOLD << "=" << (mDontMergeSizeThreshold>>20);
    return ss.str();
}

MergeTask RealtimeMergeStrategy::CreateMergeTask(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
    ...
    if (segMergeInfoIdx < 0 && 
        segMergeInfoCount >= mMaxSmallSegmentCount)  
    {
        needMerge = true;
        startMergeSegIdx = 0;
    }
...
MergeTask RealtimeMergeStrategy::CreateMergeTaskForOptimize(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
