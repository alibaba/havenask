...
public:
    ...
    virtual void CreateMergePlans(
        const index_base::SegmentMergeInfos& segMergeInfos, std::vector<MergePlan>& plans)
        = 0;
...
public:
    ...
    ~TimeSeriesMergeProcessorImpl() {}
public:
    void CreateMergePlans(const index_base::SegmentMergeInfos& segMergeInfos,
                          std::vector<MergePlan>& plans) override
    {
        ...
        auto timeSeriesSegmentMergeInfos =
            TimeSeriesSegmentMergeInfoCreator::CreateTimeSeriesSegmentMergeInfo<T>(segMergeInfos,
                                                                                   mTimeSeriesStrategyConditions,
                                                                                   mTimeSeriesInputLimits.sortField,
                                                                                   maxSortFieldValue,
                                                                                   maxBaseLine);
        PrepareSortValue(
            timeSeriesSegmentMergeInfos, mTimeSeriesInputLimits.sortField, maxSortFieldValue, maxBaseLine);
        std::sort(
                timeSeriesSegmentMergeInfos.begin(), timeSeriesSegmentMergeInfos.end());
        ...
        while (idx < timeSeriesSegmentMergeInfos.size())
        {
            ...
            while (idx < timeSeriesSegmentMergeInfos.size())
            {
                ...
                if (mergeInfo.mSegmentMergeInfo.segmentInfo.docCount - 
                    mergeInfo.mSegmentMergeInfo.deletedDocCount > mTimeSeriesInputLimits.maxDocCount || 
                ...
                if (targetInterval == 0 || targetInterval == currTargetInterval)
                {
                    ...
                    {
                        const TimeSeriesSegmentMergeInfo<T>& nextMergeInfo
                            = timeSeriesSegmentMergeInfos[idx + 1];
                    ...
                    if (maxMergedSegmentDocCount > 
                            mTimeSeriesOutputLimits.maxMergedSegmentDocCount || 
...
private:
                          ...
                          const std::string& sortFiled, T maxSortFieldValue, T maxBaseline)
    {
        for (auto& timeSeriesSegmentMergeInfo : timeSeriesSegmentMergeInfos)
        {
            ...
            timeSeriesSegmentMergeInfo.mSortValue
                = maxSortFieldValue - timeSeriesSegmentMergeInfo.mMaxSortFieldValue;
        }
    }
    ...
    std::vector<TimeSeriesStrategyCondition<T> > GetStrategyCondition() const 
    {
        return mTimeSeriesStrategyConditions;
    }
...
{
public:
    TimeSeriesMergeStrategy(const SegmentDirectoryPtr &segDir,
