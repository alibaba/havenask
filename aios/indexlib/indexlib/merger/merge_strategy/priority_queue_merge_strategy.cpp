...
void PriorityQueueMergeStrategy::SetParameter(
        const MergeStrategyParameter& param)
{
    if (!mInputLimits.FromString(param.inputLimitParam))
    {
        INDEXLIB_THROW(misc::BadParameterException,
                       "Invalid input limit parameter for merge strategy: [%s]",
    ...
    {
        INDEXLIB_THROW(misc::BadParameterException,
                       "Invalid output limit parameter for merge strategy: [%s]",
                       + param.outputLimitParam.c_str());
    }

    if (!DecodeStrategyConditions(param.strategyConditions,
                                  mPriorityDesc, mMergeTriggerDesc))
    {
        INDEXLIB_THROW(misc::BadParameterException,
                       "Invalid merge conditions for merge strategy: [%s]",
                       + param.strategyConditions.c_str());
    }
}

bool PriorityQueueMergeStrategy::DecodeStrategyConditions(
        const string& conditions, PriorityDescription& priorityDesc,
...
MergeTask PriorityQueueMergeStrategy::CreateMergeTask(
...
MergeTask PriorityQueueMergeStrategy::GenerateMergeTask(
        const SegmentMergeInfos& mergeInfos)
{
    ...
    while (true)
    {
        ...
        ExtractMergeSegmentsForOnePlan(
                needMergeSegments, inPlanSegments, totalMergeDocCount, totalMergedSize);
        ...
        {
            IE_LOG(INFO, "MergePlan has only one segment[%d] and "
                   "not LargerThanDelPercent!", inPlanSegments[0].segmentId);
            continue;
        }
        ...
        uint64_t curTotalMergedSize)
{
    ...
    while (!queue.empty())
    {
        ...
        if (NeedSkipCurrentSegmentByDocCount(curTotalMergeDocCount, 
                        inPlanMergeDocCount, mergeDocCount) ||
            NeedSkipCurrentSegmentBySize(curTotalMergedSize,
...
uint32_t PriorityQueueMergeStrategy::GetMergedDocCount(
        const SegmentMergeInfo& info)
{
    return info.segmentInfo.docCount - info.deletedDocCount;
}

uint64_t PriorityQueueMergeStrategy::GetValidSegmentSize(
        const SegmentMergeInfo& info)
{
    ...
    if (docCount)
    {
        return info.segmentSize *
            (static_cast<double>(GetMergedDocCount(info)) / docCount);
    }
    return 0;
}

void PriorityQueueMergeStrategy::UpdateMergedInfo(
        const SegmentMergeInfos& inPlanSegments,
        uint64_t& mergedDocCount,
        uint64_t& mergedSize)
{
...
bool PriorityQueueMergeStrategy::NeedSkipCurrentSegmentByDocCount(
        uint64_t curTotalMergeDocCount, 
        uint64_t inPlanMergeDocCount,
        uint32_t curSegmentValidDocCount)
{
    if (inPlanMergeDocCount + curSegmentValidDocCount >
        mOutputLimits.maxMergedSegmentDocCount)
    {
        return true;
    }

    if (curTotalMergeDocCount + inPlanMergeDocCount + curSegmentValidDocCount >
        mOutputLimits.maxTotalMergedDocCount)
    {
...
bool PriorityQueueMergeStrategy::NeedSkipCurrentSegmentBySize(
        uint64_t curTotalMergedSize,
        uint64_t inPlanMergedSize,
        uint64_t curSegmentValidSize)
{
    if (inPlanMergedSize + curSegmentValidSize >
        mOutputLimits.maxMergedSegmentSize)
    {
        return true;
    }

    if (curTotalMergedSize + inPlanMergedSize + curSegmentValidSize >
        mOutputLimits.maxTotalMergedSize)
    {
...
bool PriorityQueueMergeStrategy::IsPlanValid(
        const SegmentMergeInfos& planSegments)
{
...
MergePlan PriorityQueueMergeStrategy::ConstructMergePlan(
...
bool PriorityQueueMergeStrategy::LargerThanDelPercent(
        const SegmentMergeInfo &segMergeInfo)
{
    ...
    uint32_t curPercent = ((double)segMergeInfo.deletedDocCount /
                           segMergeInfo.segmentInfo.docCount) * 100;
    return curPercent > mMergeTriggerDesc.conflictDeletePercent;
}

void PriorityQueueMergeStrategy::GenerateMergeSegments(
        PriorityQueue& queue, SegmentMergeInfos& needMergeSegments)
{
    ...
    while (queue.size() >= mMergeTriggerDesc.conflictSegmentCount)
    {
        for (size_t i = 0; i < mMergeTriggerDesc.conflictSegmentCount; i++)
        {
            QueueItem item = queue.top();
            queue.pop();
            if (item.mergeInfo.segmentSize + curTotalMergeSize
                <= mInputLimits.maxTotalMergeSize)
            {
...
void PriorityQueueMergeStrategy::PutSegMergeInfosToQueue(
        ...
        PriorityQueue& priorityQueue)
{
    for (size_t i = 0; i < segMergeInfos.size(); i++)
    {
        ...
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   " for over max-doc-count[%u], docCount[%u]",
                   segMergeInfos[i].segmentId, 
                   mInputLimits.maxDocCount, docCount);
            continue;
        }
        ...
        if (segmentSize > mInputLimits.maxSegmentSize)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mInputLimits.maxSegmentSize, segmentSize);
            continue;
        }
        ...
        if (validDocCount > mInputLimits.maxValidDocCount)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mInputLimits.maxValidDocCount, validDocCount);
            continue;
        }
        ...
        if (validSegmentSize > mInputLimits.maxValidSegmentSize)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mInputLimits.maxValidSegmentSize, validSegmentSize);
            continue;
        }
        ...
        if (validDocCount > mOutputLimits.maxMergedSegmentDocCount)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mOutputLimits.maxMergedSegmentDocCount, validDocCount);
            continue;
        }
        ...
        if (validDocCount > mOutputLimits.maxTotalMergedDocCount)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mOutputLimits.maxTotalMergedDocCount, validDocCount);
            continue;
        }
        ...
        if (validSegmentSize > mOutputLimits.maxMergedSegmentSize)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mOutputLimits.maxMergedSegmentSize, validSegmentSize);
            continue;
        }
        ...
        if (validSegmentSize > mOutputLimits.maxTotalMergedSize)
        {
            IE_LOG(INFO, "merge ignore segment_[%d]"
                   ...
                   segMergeInfos[i].segmentId, 
                   mOutputLimits.maxTotalMergedSize, validSegmentSize);
            continue;
        }
...
QueueItem PriorityQueueMergeStrategy::MakeQueueItem(
        const SegmentMergeInfo& mergeInfo)
{
    QueueItem item;
...
MergeTask PriorityQueueMergeStrategy::CreateMergeTaskForOptimize(
        const SegmentMergeInfos& segMergeInfos, const LevelInfo& levelInfo)
{
