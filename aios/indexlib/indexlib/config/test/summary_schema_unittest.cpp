...
{
    mRootDir = string(TEST_DATA_PATH);
    mJsonStringHead = R"(
    {
...
{
    string jsonString;
    string confFile = string(TEST_DATA_PATH) + "summary_group_schema.json";
...
{
    string jsonString;
    FileSystemWrapper::AtomicLoad(mRootDir + "summary_group_schema.json",
                                  jsonString);
    ...
    vector<string> fields = {"nid", "title", "pid", "provcity", "category",
                             "quantity", "zk_time", "ends",
                             "user", "user_id", "nick",
                             "vip"};
    ...
    for (size_t i = 0; i < fields.size(); ++i, ++sit)
    {
        ...
        fieldid_t fieldId = schema->GetFieldSchema()->GetFieldId(fields[i]);
    ...
    SummaryGroupConfigPtr defaultGroupConfig =
        summarySchema->GetSummaryGroupConfig(DEFAULT_SUMMARYGROUPNAME);
    ASSERT_EQ(0, summarySchema->GetSummaryGroupId(DEFAULT_SUMMARYGROUPNAME));
    ASSERT_TRUE(defaultGroupConfig ==
                summarySchema->GetSummaryGroupConfig(DEFAULT_SUMMARYGROUPID));
    ...
    for (size_t i = 0; it != defaultGroupConfig->End(); ++it, ++i)
    {
        ...
        ASSERT_EQ(0, summarySchema->FieldIdToSummaryGroupId(
                        summarySchema->GetSummaryConfig(fields[i])->GetFieldId()));
    }
    ...
    {
        ASSERT_EQ(fields[i], (*it)->GetSummaryName());
        ASSERT_EQ(1, summarySchema->FieldIdToSummaryGroupId(
                        summarySchema->GetSummaryConfig(fields[i])->GetFieldId()));
    }
    ...
    {
        ASSERT_EQ(fields[i], (*it)->GetSummaryName());
        ASSERT_EQ(2, summarySchema->FieldIdToSummaryGroupId(
                        summarySchema->GetSummaryConfig(fields[i])->GetFieldId()));
    }
    ...
    {
        ASSERT_EQ(fields[i], (*it)->GetSummaryName());
        ASSERT_EQ(3, summarySchema->FieldIdToSummaryGroupId(
                        summarySchema->GetSummaryConfig(fields[i])->GetFieldId()));
    }
}
...
void SummarySchemaTest::TestJsonize()
{
    ...
    {
        ASSERT_EQ(fields[i], (*sit)->GetSummaryName());
        fieldid_t fieldId = schema2->GetFieldSchema()->GetFieldId(fields[i]);
        ASSERT_TRUE(summarySchema->IsInSummary(fieldId));
        ASSERT_EQ(i, summarySchema->GetSummaryFieldId(fieldId));
        ASSERT_EQ(fields[i], summarySchema->GetSummaryConfig(fieldId)->GetSummaryName());
    }
