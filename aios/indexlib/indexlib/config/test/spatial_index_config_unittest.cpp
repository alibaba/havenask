...
void SpatialIndexConfigTest::CaseSetUp()
{
    mSchema = SchemaLoader::LoadSchema(
            TEST_DATA_PATH, "spatial_index_schema.json");
}
...
void SpatialIndexConfigTest::TestSimpleProcess()
{
    IndexConfigPtr indexConfig = 
        mSchema->GetIndexSchema()->GetIndexConfig("spatial_index");
    ASSERT_TRUE(indexConfig);
    SpatialIndexConfigPtr spatialConfig = 
        DYNAMIC_POINTER_CAST(SpatialIndexConfig, indexConfig);
...
{
    mSchema = SchemaLoader::LoadSchema(
            TEST_DATA_PATH, "spatial_index_schema_default.json");
    IndexConfigPtr indexConfig = 
        mSchema->GetIndexSchema()->GetIndexConfig("spatial_index");
    ASSERT_TRUE(indexConfig);
    SpatialIndexConfigPtr spatialConfig = 
