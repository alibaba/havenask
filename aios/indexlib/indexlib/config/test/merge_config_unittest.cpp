...
void MergeConfigTest::TestJsonize()
{
    ...
    ASSERT_EQ(true, mergeConfig.mergeIOConfig.enableAsyncRead);
    ...
    ASSERT_EQ(true, mergeConfig.GetEnablePackageFile());
    ASSERT_EQ(100u, mergeConfig.GetCheckpointInterval());
    ASSERT_EQ(2, mergeConfig.GetPackageFileTagConfigList().configs.size());
    ASSERT_EQ("summary_data", mergeConfig.GetPackageFileTagConfigList().Match(
                    "segment_0_level_0/summary/data", "NOMATCH"));
    ASSERT_EQ("NOMATCH", mergeConfig.GetPackageFileTagConfigList().Match(
                    "BAD.segment_0_level_0/summary/data", "NOMATCH"));
    ASSERT_EQ("summary", mergeConfig.GetPackageFileTagConfigList().Match(
                    "segment_0_level_0/summary/offset", "NOMATCH"));
}
