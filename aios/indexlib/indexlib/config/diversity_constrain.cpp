...
IE_LOG_SETUP(config, DiversityConstrain);
...
void DiversityConstrain::ParseDefaultFilterParam(KVMap& paramMap)
{
    ...
    if (!filterMask.empty() && !StringUtil::hexStrToUint64(
                    filterMask.c_str(), mFilterMask))
    {
        IE_CONFIG_ASSERT_PARAMETER_VALID(false, 
                "parse filterMask fail in Default FilterType");
    }

    if (!filterMin.empty() && !StringUtil::fromString(
                    filterMin.c_str(), mFilterMinValue))
    {
        IE_CONFIG_ASSERT_PARAMETER_VALID(false, 
                "parse filterMinValue fail in Default FilterType");
    }

    if (!filterMax.empty() && !StringUtil::fromString(
                    filterMax.c_str(), mFilterMaxValue))
    {
        IE_CONFIG_ASSERT_PARAMETER_VALID(false, 
                "parse filterMask fail in Default FilterType");
    }
...
void DiversityConstrain::ParseTimeStampFilter(
...
void DiversityConstrain::ParseFilterByTimeStampParam(
        KVMap &paramMap, int64_t beginTime, int64_t baseTime)
{
    ...
    {
        stringstream ss;
        ss << "beginTime:" << beginTimeStr 
           << ", adviseEndTime:" << adviseEndTime
    ...
    IE_CONFIG_ASSERT_PARAMETER_VALID(mFilterMinValue > 0, 
            "beginTime should greater than 0");
    int64_t maxTime = 0;
    ...
    IE_LOG(INFO, "filterByTimestamp baseTime %ld, beginTime %ld, endTime %ld",
           baseTime, mFilterMinValue, mFilterMaxValue);
}
...
void DiversityConstrain::ParseFilterParam()
{
    ...
    if (mFilterType == "Default")
    {
        ...
        {
            stringstream ss;
            ss << "FilterType: " << mFilterType << 
                ", FilterParameter: " << mFilterParameter;
            IE_CONFIG_ASSERT_PARAMETER_VALID(false, ss.str());
        }
...
DiversityConstrain::KVMap 
DiversityConstrain::GetKVMap(const string& paramsStr)
{
    KVMap paramMap;
...
void DiversityConstrain::Jsonize(autil::legacy::Jsonizable::JsonWrapper& json)
{
    ...
    if (FROM_JSON == json.GetMode())
    {
        ...
        if (!filterMaskStr.empty()
            && !autil::StringUtil::hexStrToUint64(
                filterMaskStr.c_str(), mFilterMask))
        {
            mFilterMask = 0;
        }
    }
    else
    {
        ...
        autil::StringUtil::uint64ToHexStr(mFilterMask, filterMaskBuff, 
                sizeof(filterMaskBuff));
...
{ 
    ...
    {
        valid = mDistCount > 0 
                && mDistExpandLimit >= mDistCount;

        IE_CONFIG_ASSERT_PARAMETER_VALID(valid, 
                "invalid distinct config for DiversityConstrain");
    }
    
    if (NeedFilter())
    {
        valid = mFilterMinValue <= mFilterMaxValue
                && mFilterMask > 0
                && !(mFilterByTimeStamp && mFilterByMeta);
        IE_CONFIG_ASSERT_PARAMETER_VALID(valid, 
                "invalid filter config for DiversityConstrain");
    }
}

bool DiversityConstrain::operator==(const DiversityConstrain& other) const
{
    return mDistCount == other.mDistCount
        && mDistExpandLimit == other.mDistExpandLimit
        && mDistField == other.mDistField
        && mFilterField == other.mFilterField
        && mFilterMaxValue == other.mFilterMaxValue
        && mFilterMinValue == other.mFilterMinValue
        && mFilterMask == other.mFilterMask
        && mFilterByTimeStamp == other.mFilterByTimeStamp
