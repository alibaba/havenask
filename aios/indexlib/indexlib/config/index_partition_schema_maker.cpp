...
void IndexPartitionSchemaMaker::MakeSchema(
...
IndexPartitionSchemaMaker::StringVector 
IndexPartitionSchemaMaker::SpliteToStringVector(const string& names)
{
    StringVector sv;
...
void IndexPartitionSchemaMaker::MakeFieldConfigSchema(
        IndexPartitionSchemaPtr& schema,
...
void IndexPartitionSchemaMaker::MakeIndexConfigSchema(
        IndexPartitionSchemaPtr& schema,
        const StringVector& fieldNames)
{
    // make indexSchema
    for (size_t i = 0; i < fieldNames.size(); ++i)
    {
        ...
        IndexType indexType = IndexConfig::StrToIndexType(st[1], schema->GetTableType());
            ...
            && indexType != it_expack && indexType != it_customized)
        {
            SingleFieldIndexConfigPtr indexConfig = 
                IndexConfigCreator::CreateSingleFieldIndexConfig(indexName, 
                        indexType, st2[0], "", false, schema->GetFieldSchema());
...
void IndexPartitionSchemaMaker::MakeAttributeConfigSchema(
        IndexPartitionSchemaPtr& schema,
        const StringVector& fieldNames)
{
    for (size_t i = 0; i < fieldNames.size(); ++i)
...
void IndexPartitionSchemaMaker::MakeSummaryConfigSchema(
        IndexPartitionSchemaPtr& schema,
        const StringVector& fieldNames)
{
    // make SummarySchema
...
void IndexPartitionSchemaMaker::MakeTruncateProfiles(
    IndexPartitionSchemaPtr& schema,
    const string& truncateProfileStr)
{
