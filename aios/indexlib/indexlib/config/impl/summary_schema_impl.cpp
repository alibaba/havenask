...
SummarySchemaImpl::SummarySchemaImpl()
    : mNeedStoreSummary(true)
{
    ...
    assert(defaultGroupId == DEFAULT_SUMMARYGROUPID); (void)defaultGroupId;
}

SummaryConfigPtr SummarySchemaImpl::GetSummaryConfig(fieldid_t fieldId) const
{
    if (!IsInSummary(fieldId))
    {
        return SummaryConfigPtr();
    }
    size_t pos = mFieldId2SummaryVec[fieldId];
    return mSummaryConfigs[pos];
}

void SummarySchemaImpl::AddSummaryConfig(const SummaryConfigPtr& summaryConfig,
                                         summarygroupid_t groupId)
{
    ...
    {
        INDEXLIB_FATAL_ERROR(Schema, "invalid summary groupId[%d], expect[%d]",
                             groupId, (summarygroupid_t)mSummaryGroups.size() - 1);
    }
...
void SummarySchemaImpl::AddSummaryPosition(fieldid_t fieldId, size_t pos,
                                           summarygroupid_t groupId)
{
...
{
    IE_CONFIG_ASSERT_EQUAL(mFieldId2SummaryVec, other.mFieldId2SummaryVec, 
                           "FieldId2SummaryMap not equal");
    ...
    IE_CONFIG_ASSERT_EQUAL(mSummaryGroups.size(), other.mSummaryGroups.size(), 
                           "mSummaryGroups's size not equal");
...
void SummarySchemaImpl::AssertCompatible(const SummarySchemaImpl& other) const
{
    ...
    IE_CONFIG_ASSERT(mSummaryGroups.size() <= other.mSummaryGroups.size(), 
                     "mSummaryGroups's size not compatible");
    for (size_t i = 0; i < mSummaryGroups.size(); ++i)
    {
...
void SummarySchemaImpl::Jsonize(autil::legacy::Jsonizable::JsonWrapper& json) 
{
    if (json.GetMode() == TO_JSON)
    {
        ...
        if (defaultGroup->IsCompress())
        {
            ...
            {
                summaryMap[SUMMARY_COMPRESS_TYPE] = defaultGroup->GetCompressType();
            }
        }
...
SummaryConfigPtr SummarySchemaImpl::GetSummaryConfig(const string &fieldName) const
{
    for (size_t i = 0; i < mSummaryConfigs.size(); i++)
    {
...
summarygroupid_t SummarySchemaImpl::CreateSummaryGroup(const string& groupName)
{
    ...
    {
        INDEXLIB_FATAL_ERROR(Schema, "summary groupName[%s] already exists",
                             groupName.c_str());
    }
    summarygroupid_t groupId = (summarygroupid_t)mSummaryGroups.size();
    mSummaryGroups.push_back(SummaryGroupConfigPtr(new SummaryGroupConfig(
                            groupName, groupId, mSummaryConfigs.size())));
