...
schema_opid_t SchemaModifyOperationImpl::GetOpId() const
{
    return mOpId;
}
...
void SchemaModifyOperationImpl::LoadDeleteOperation(
        const Any &any, IndexPartitionSchemaImpl& schema)
{
    ...
    for (auto& id : mDeleteAttrs) {
        if (!attrSchema) {
            INDEXLIB_FATAL_ERROR(Schema, "fail to delete attribute [%s], no attribute schema",
                    id.c_str());
        }
        if (!fieldSchema) {
        ...
        if (truncateSortFields.find(id) != truncateSortFields.end()) {
            INDEXLIB_FATAL_ERROR(Schema, "delete attribute [%s] fail, "
                    "which is sort field in truncate_profile!", id.c_str());
        }
    ...
    for (auto& id : mDeleteIndexs) {
        if (!indexSchema) {
            INDEXLIB_FATAL_ERROR(Schema, "fail to delete index [%s], no index schema",
                    id.c_str());
        }
...
void SchemaModifyOperationImpl::LoadAddOperation(
        const Any &any, IndexPartitionSchemaImpl& schema)
{
    ...
    if (it != jsonMap.end())
    {
        ...
        for (JsonArray::iterator iter = indexs.begin(); iter != indexs.end(); ++iter) 
        {
            IndexConfigPtr indexConfig = IndexConfigCreator::Create(
                    schema.GetFieldSchema(), schema.GetDictSchema(), 
                    schema.GetAdaptiveDictSchema(), *iter, schema.GetTableType());
    ...
    if (it != jsonMap.end())
    {
        ...
        for (JsonArray::iterator iter = attrs.begin(); iter != attrs.end(); ++iter)
        {
            ...
            {
                INDEXLIB_FATAL_ERROR(UnSupported, "not support add attribute with jsonMap format");
            }
    ...
    if (regionSchema)
    {
        vector<AttributeConfigPtr> indexAccessoryAttrs =
            regionSchema->EnsureSpatialIndexWithAttribute();
        if (!indexAccessoryAttrs.empty())
        {
            mAddAttrs.insert(mAddAttrs.end(),
                    indexAccessoryAttrs.begin(), indexAccessoryAttrs.end());
        }
    }
}
...
void SchemaModifyOperationImpl::CollectEffectiveModifyItem(
        ModifyItemVector& indexs, ModifyItemVector& attrs)
{
    ...
    for (auto& index: mAddIndexs)
    {
        if (index->IsNormal())
        {
            ModifyItemPtr item(new ModifyItem(
                            index->GetIndexName(),
                            IndexConfig::IndexTypeToStr(index->GetIndexType()),
                            mOpId));
    ...
    for (auto& attr : mAddAttrs)
    {
        if (attr->IsNormal())
        {
            ModifyItemPtr item(new ModifyItem(
                            attr->GetAttrName(),
                            FieldConfig::FieldTypeToStr(attr->GetFieldType()),
                            mOpId));
...
{
    IE_CONFIG_ASSERT_EQUAL(mDeleteFields, other.mDeleteFields,
                           "mDeleteFields not equal");
    IE_CONFIG_ASSERT_EQUAL(mDeleteAttrs, other.mDeleteAttrs,
                           "mDeleteAttrs not equal");
    IE_CONFIG_ASSERT_EQUAL(mDeleteIndexs, other.mDeleteIndexs,
                           "mDeleteIndexs not equal");

    IE_CONFIG_ASSERT_EQUAL(mAddFields.size(), other.mAddFields.size(),
                           "mAddFields size not equal");
    for (size_t i = 0; i < mAddFields.size(); i++)
    {
        mAddFields[i]->AssertEqual(*(other.mAddFields[i]));
    }

    IE_CONFIG_ASSERT_EQUAL(mAddAttrs.size(), other.mAddAttrs.size(),
                           "mAddAttrs size not equal");
    for (size_t i = 0; i < mAddAttrs.size(); i++)
    {
        mAddAttrs[i]->AssertEqual(*(other.mAddAttrs[i]));
    }

    IE_CONFIG_ASSERT_EQUAL(mAddIndexs.size(), other.mAddIndexs.size(),
                           "mAddIndexs size not equal");
...
void SchemaModifyOperationImpl::EnsureAccompanyAttributeDeleteForSpatialIndex(
        const string& indexName, IndexPartitionSchemaImpl& schema)
{
    const SummarySchemaPtr& summarySchema = schema.GetSummarySchema();
    const AttributeSchemaPtr& attrSchema = schema.GetAttributeSchema();
    const FieldSchemaPtr& fieldSchema = schema.GetFieldSchema();
    ...
    if (!indexConfig || indexConfig->GetIndexType() != it_spatial)
    {
        return;
    }
...
void SchemaModifyOperationImpl::InitTruncateSortFieldSet(
        const IndexPartitionSchemaImpl& schema,
        set<string>& truncateSortFields)
{
    TruncateProfileSchemaPtr truncProfileSchema =
        schema.GetTruncateProfileSchema(DEFAULT_REGIONID);
    if (!truncProfileSchema)
    {
        return;
    }

    for (auto iter = truncProfileSchema->Begin();
         iter != truncProfileSchema->End(); iter++)
    {
