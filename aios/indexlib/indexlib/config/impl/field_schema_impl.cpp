...
FieldSchemaImpl::FieldSchemaImpl(size_t capacity)
    : mBaseFieldCount(-1)
                                     ...
                                     vector<FieldConfigPtr>& fields)
{
    ...
    {
        INDEXLIB_FATAL_ERROR(UnSupported, "add field [%s] fail, "
    ...
    nameMap.insert(make_pair(fieldConfig->GetFieldName(), 
...
bool FieldSchemaImpl::IsFieldTypeSortable(const std::string& fieldName,
...
{
    IE_CONFIG_ASSERT(mNameToIdMap.size() <= other.mNameToIdMap.size(), 
                     "mNameToIdMap's size not compatible");
    IE_CONFIG_ASSERT(mFields.size() <= other.mFields.size(), 
                     "mFields' size not compatible");
...
void FieldSchemaImpl::SetModifySchemaMutable()
{
    mOnlyAddVirtual = false;
}
...
void FieldSchemaImpl::Jsonize(autil::legacy::Jsonizable::JsonWrapper& json) 
{
    if (json.GetMode() == TO_JSON) 
    {
        ...
        int32_t fieldCount = (mBaseFieldCount >= 0) ?
                             mBaseFieldCount : (int32_t)mFields.size();
        for (int32_t id = 0; id < fieldCount; id++)
        {
            ...
            {
                anyVec.push_back(
                        ToJson(*(dynamic_pointer_cast<EnumFieldConfig>(field))));
            }
            else
            {
...
FieldConfigIteratorPtr FieldSchemaImpl::CreateIterator(ConfigIteratorType type) const
{
