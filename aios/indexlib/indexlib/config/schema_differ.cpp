...
bool SchemaDiffer::CheckAlterFields(const IndexPartitionSchemaPtr& oldSchema,
                                    ...
                                    string& errorMsg)
{
    ...
    if(!CalculateAlterFields(oldSchema, newSchema,
                                alterAttributes, alterIndexes, errorMsg) ||
       !CalculateDeleteFields(oldSchema, newSchema,
                              deleteAttributes, deleteIndexes, errorMsg))
    {
        return false;
    }
    return true;
...
bool SchemaDiffer::CalculateDeleteFields(
    ...
    std::string& errorMsg)
{
    ...
    for (size_t i = 0; i < deleteAttributes.size(); i++)
    {
        ...
        {
            stringstream ss;
            ss << "not support delete pack attribute:"
               << deleteAttributes[i]->GetAttrName();
            errorMsg = ss.str();
        ...
        if (oldSummarySchema && oldSummarySchema->GetSummaryConfig(deleteAttrName) &&
            newSummarySchema && newSummarySchema->GetSummaryConfig(deleteAttrName))
        {
            //attribute in summary, only delete attrbute, not delete summary
            stringstream ss;
            ss << "not support delete attribute in summary:"
               << deleteAttributes[i]->GetAttrName();
            errorMsg = ss.str();
            IE_LOG(ERROR, "%s", errorMsg.c_str());
            return false;
    ...
    {
        IndexType type = deleteIndexes[i]->GetIndexType();
        if (type == it_primarykey64 ||
            type == it_primarykey128 || type == it_trie)
        {
            stringstream ss;
            ss << "not support delete index " << deleteIndexes[i]->GetIndexName()
               << "type [" << IndexConfig::IndexTypeToStr(type) << "]";
            errorMsg = ss.str();
            IE_LOG(ERROR, "%s", errorMsg.c_str());
            return false;
        }
    ...
    vector<SummaryConfigPtr> deleteSummarys;
    GetAlterFields(newSchema->GetSummarySchema(),
                   oldSchema->GetSummarySchema(), deleteSummarys);
    for (size_t i = 0; i < deleteSummarys.size(); i++)
    {
        if (oldAttrSchema && oldAttrSchema->GetAttributeConfig(
                deleteSummarys[i]->GetSummaryName()))
        {
            continue;
        }
...
bool SchemaDiffer::CalculateAlterFields(
        const IndexPartitionSchemaPtr& oldSchema,
        const IndexPartitionSchemaPtr& newSchema,
        vector<AttributeConfigPtr>& alterAttributes,
        vector<IndexConfigPtr>& alterIndexes,
        string& errorMsg)
{
    ...
    auto oldIndexSchema = oldSchema->GetIndexSchema();
    auto newIndexSchema = newSchema->GetIndexSchema();
    vector<SummaryConfigPtr> alterSummarys;
    GetAlterFields(oldAttrSchema, newAttrSchema, alterAttributes);
    GetAlterFields(oldIndexSchema, newIndexSchema, alterIndexes);
    GetAlterFields(oldSchema->GetSummarySchema(),
                   newSchema->GetSummarySchema(), alterSummarys);

    if (!CheckAlterFields(alterIndexes, alterAttributes, alterSummarys,
                          newSchema, errorMsg))
    {
        IE_LOG(WARN, "%s", errorMsg.c_str());
        return false;
...
size_t SchemaDiffer::GetAlterFields(const AttributeSchemaPtr& oldSchema,
...
size_t SchemaDiffer::GetAlterFields(const IndexSchemaPtr& oldSchema,
...
size_t SchemaDiffer::GetAlterFields(const SummarySchemaPtr& oldSchema,
...
bool SchemaDiffer::CheckSchemaMeta(
        const IndexPartitionSchemaPtr& oldSchema,
        const IndexPartitionSchemaPtr& newSchema,
        string& errorMsg)
{
    ...
    if (oldSchema->GetSchemaVersionId() >= newSchema->GetSchemaVersionId())
    {
        errorMsg = "not support update schema with oldSchema versionId [" +
                   StringUtil::toString(oldSchema->GetSchemaVersionId()) +
                   "] >= newSchema versionId [" +
    ...
    if (oldSchema->GetTableType() != newSchema->GetTableType())
    {
        errorMsg = "not support update schema with different table type."
                   "old schema [" +
                   IndexPartitionSchema::TableType2Str(oldSchema->GetTableType()) +
                   "], new schema [" +
    ...
    if (oldSchema->GetSchemaName() != newSchema->GetSchemaName())
    {
        errorMsg = "not support update schema with different table name."
                   "old schema [" + oldSchema->GetSchemaName() + "], new schema [" +
                   newSchema->GetSchemaName() + "]";
        return false;
    }

    if (oldSchema->GetTableType() != tt_index)
    {
        errorMsg = "not support update schema for [" +
                   IndexPartitionSchema::TableType2Str(oldSchema->GetTableType()) +
                   "] table";
        return false;
    }
    ...
    if (!newSchema->HasModifyOperations()) {
        if (oldSchema->GetSubIndexPartitionSchema() || newSchema->GetSubIndexPartitionSchema()) {
            errorMsg = "not support update schema for sub doc schema, table [" +
        ...
        if (newSubSchema && newSubSchema->HasModifyOperations()) {
            errorMsg = "not support modify sub schema, table [" +
                oldSchema->GetSchemaName() + "]";
            return false;
        }
...
bool SchemaDiffer::CheckAlterFields(const vector<IndexConfigPtr>& alterIndexes,
                                    ...
                                    const vector<SummaryConfigPtr>& alterSummarys,
                                    const IndexPartitionSchemaPtr& newSchema,
                                    string& errorMsg)
{
    auto newAttrSchema = newSchema->GetAttributeSchema();
    for (auto index : alterIndexes) {
        ...
        {
            errorMsg = "not support add new index [" + index->GetIndexName() +
                       "] which is primary key index!";
            return false;
        }
    ...
    for (auto attr : alterAttributes) {
        if (attr->GetPackAttributeConfig()) {
            errorMsg = "not support add new attribute [" + attr->GetAttrName() +
                       "] which in pack attribute!";
            return false;
        }
    }
    for (auto summary : alterSummarys) {
        if (!newAttrSchema || !newAttrSchema->GetAttributeConfig(summary->GetSummaryName())) {
            errorMsg = "not support add new summary field [" +
