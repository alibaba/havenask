...
void MergeConfigBase::Jsonize(Jsonizable::JsonWrapper& json)
{
    ...
    json.Jsonize("merge_strategy_params", 
                 mergeStrategyParameter, mergeStrategyParameter);
    json.Jsonize("document_reclaim_config_path", docReclaimConfigPath,
                 docReclaimConfigPath);
    ...
    json.Jsonize("uniq_pack_attribute_merge_buffer_size", uniqPackAttrMergeBufferSize,
                 uniqPackAttrMergeBufferSize);
    json.Jsonize("merge_io_config", mergeIOConfig, mergeIOConfig);
    json.Jsonize("enable_merge_item_check_point",
                 enableMergeItemCheckPoint, enableMergeItemCheckPoint);
    json.Jsonize("enable_reclaim_expired_doc",
                 enableReclaimExpiredDoc, enableReclaimExpiredDoc);
...
void MergeConfigBase::Check() const
{
    if (maxMemUseForMerge == 0)
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             "max_merge_memory_use should be greater than 0.");
    }

    if (uniqPackAttrMergeBufferSize == 0)
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             "uniq_pack_attribute_merge_buffer_size"
    ...
    if (mergeThreadCount == 0 || mergeThreadCount > MAX_MERGE_THREAD_COUNT)
    {
        INDEXLIB_FATAL_ERROR(BadParameter,
                             ...
                             " and no more than %d.", MAX_MERGE_THREAD_COUNT);
    }
