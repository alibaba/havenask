#ifndef __INDEXLIB_DIVERSITY_CONSTRAIN_H
#define __INDEXLIB_DIVERSITY_CONSTRAIN_H
...
public:
    //for test
    void SetDistField(const std::string& distField) 
    { 
        mDistField = distField; 
    }
    
    void SetDistCount(uint64_t distCount) 
    { 
        mDistCount = distCount;
    }

    void SetDistExpandLimit(uint64_t distExpandLimit) 
    { 
        mDistExpandLimit = distExpandLimit;
    }

    void SetFilterField(const std::string filterField)
    {
        mFilterField = filterField;
    }

    void SetFilterMaxValue(int64_t filterMaxValue)
    {
        mFilterMaxValue = filterMaxValue;
    }

    void SetFilterMinValue(int64_t filterMinValue)
    {
        mFilterMinValue = filterMinValue;
    }

    void SetFilterMask(uint64_t filterMask)
    {
        mFilterMask = filterMask;
    }

    void SetFilterByMeta(bool flag)
    {
        mFilterByMeta = flag;
    }

    void SetFilterByTimeStamp(bool flag)
    {
        mFilterByTimeStamp = flag;
    }
    ...
    {
        return mFilterByMeta;
    }

    bool IsFilterByTimeStamp() const
    {
        return mFilterByTimeStamp;
    }
...
private:
    ...
    void ParseFilterByTimeStampParam(KVMap &paramMap,
...
DEFINE_SHARED_PTR(DiversityConstrain);

IE_NAMESPACE_END(config);

#endif //__INDEXLIB_DIVERSITY_CONSTRAIN_H
