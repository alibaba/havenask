...
string TablePathDefine::constructIndexPath(const string &indexRoot,
                                           const std::string &tableName,
                                           uint32_t generationId,
                                           const std::string &rangeName) {
    string genDir = GENERATION_PREFIX + StringUtil::toString(generationId);
    string partDir = PARTITION_PREFIX + rangeName;
    return PathDefine::join(indexRoot, PathDefine::join(tableName, PathDefine::join(genDir, partDir)));
}
