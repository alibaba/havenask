...
LogReplicatorCreator::create(const DrcConfig &config,
    ...
    if (!source) {
        AUTIL_LOG(ERROR, "create source from %s failed", FastToJsonString(sourceConfig).c_str());
        return nullptr;
    }
...
std::unique_ptr<LogWriter> LogReplicatorCreator::createWriter(const SwiftClientCreatorPtr &swiftClientCreator,
    ...
    if (!sink) {
        AUTIL_LOG(ERROR, "create sink from %s failed", FastToJsonString(sinkConfig).c_str());
        return nullptr;
    }
