...
std::string KVTableSearcher::constructJsonQuery(const std::vector<std::string> &keys,
    ...
    jsonMap["indexName"] = autil::legacy::ToJson(_indexName);
    jsonMap["pk"] = autil::legacy::ToJson(keys);
    jsonMap["attrs"] = autil::legacy::ToJson(attrs);
    return autil::legacy::FastToJsonString(jsonMap);
}
