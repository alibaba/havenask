...
void SyncVersion::run() {
    if (_event & SyncEvent::SE_ROLLBACK) {
        if (!syncRollbackVersion()) {
            AUTIL_LOG(WARN, "[%s]: sync rollback  version failed", ToJsonString(getPartitionId(), true).c_str());
            return;
        }
    }
...
bool SyncVersion::syncRollbackVersion() {
    ...
    if (!_versionSync->getVersionList(pid, versions)) {
        AUTIL_LOG(ERROR, "[%s]: get version list  failed  ", ToJsonString(pid, true).c_str());
        return false;
    }
    auto currentVersionId = _table->getPartitionMeta().getIncVersion();
    if (currentVersionId == INVALID_VERSION) {
        AUTIL_LOG(ERROR, "[%s]: current version id invalid  ", ToJsonString(pid, true).c_str());
        return false;
    }
    ...
    if (!_versionSync->updateVersionList(pid, versions)) {
        AUTIL_LOG(ERROR, "[%s]: update version list failed  ", ToJsonString(pid, true).c_str());
    }
    AUTIL_LOG(INFO,
              "[%s]: update version list to [%s] success",
              ToJsonString(pid, true).c_str(),
              ToJsonString(versions, true).c_str());
...
void SyncVersion::syncCommitVersion() {
    ...
    if (version.publishedVersion.isFinished()) {
        ...
        AUTIL_LOG(INFO, "[%s]: published version is sealed, stop leader election", ToJsonString(pid, true).c_str());
...
bool SyncVersion::updateVersionList(const TableVersion &newVersion) {
    ...
    std::vector<TableVersion> versions;
    if (!_versionSync->getVersionList(pid, versions)) {
        AUTIL_LOG(WARN, "[%s]: get list failed, retry later", ToJsonString(pid, true).c_str());
        return false;
    }
    ...
    if (!status.IsOK()) {
        AUTIL_LOG(WARN, "[%s]: list index root [%s] failed", ToJsonString(pid, true).c_str(), partitionRoot.c_str());
        return false;
    }
    ...
    if (!_versionSync->updateVersionList(pid, newVersions)) {
        AUTIL_LOG(WARN,
                  ...
                  ToJsonString(pid, true).c_str(),
                  ToJsonString(newVersion, true).c_str());
        return false;
    }
    AUTIL_LOG(INFO,
    ...
    if (version.localVersion.getVersionId() > version.leaderVersion.getVersionId()) {
        ...
        if (version.localVersion.isLeaderVersion() &&
            ...
            AUTIL_LOG(INFO,
                      ...
                      ToJsonString(pid, true).c_str(),
                      ToJsonString(version.leaderVersion, true).c_str(),
                      ToJsonString(version.localVersion, true).c_str());
            ...
            if (!updateVersionList(version.localVersion)) {
                AUTIL_LOG(INFO, "[%s]: update version list failed, retry later", ToJsonString(pid, true).c_str());
                return;
            }
        }
    }
    if (version.leaderVersion.getVersionId() > version.publishedVersion.getVersionId()) {
        if (_versionPublisher->publish(_target.getRawIndexRoot(), pid, version.leaderVersion)) {
            AUTIL_LOG(INFO,
                      ...
                      ToJsonString(pid, true).c_str(),
                      ToJsonString(version.publishedVersion, true).c_str(),
                      ToJsonString(version.leaderVersion, true).c_str());
    ...
    if (!version.leaderVersion.isFinished() && _versionSync->supportSyncFromPersist(_target)) {
        ...
        if (_versionSync->syncFromPersist(pid, _target.getConfigPath(), leaderVersion) &&
            ...
            AUTIL_LOG(INFO,
                      ...
                      ToJsonString(pid, true).c_str(),
                      ToJsonString(version.leaderVersion, true).c_str(),
                      ToJsonString(leaderVersion, true).c_str());
...
bool SyncVersion::recoverVersion(const PartitionId &pid,
    ...
    if (!versionSync->supportSyncFromPersist(target)) {
        AUTIL_LOG(INFO, "[%s] no need sync from remote", ToJsonString(pid, true).c_str());
        return true;
    }
    if (!versionSync->syncFromPersist(pid, target.getConfigPath(), leaderVersion)) {
        AUTIL_LOG(ERROR, "[%s]: recover leader version failed", ToJsonString(pid, true).c_str());
        return false;
    }
