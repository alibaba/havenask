...
bool TabletAdapter::cleanUnreferencedIndexFiles(const std::set<std::string> &toKeepFiles) {
    ...
    // TODO: support cleanUnreferencedIndexFiles with keepFile list
    AUTIL_LOG(ERROR, "this interface is NOT supported yet");
    return false;
}
...
TableBuilder *TabletAdapter::createBuilder(const build_service::proto::PartitionId &pid,
    ...
    if (directWrite) {
        AUTIL_LOG(INFO, "create DirectBuilder for %s", pid.ShortDebugString().c_str());
        auto schema = _tablet->GetTabletSchema();
        std::string schemaConfigPath;
        if (schema != nullptr && schema->GetValueFromUserDefinedParam(CONFIG_PATH_KEY, schemaConfigPath)) {
            AUTIL_LOG(INFO, "restore schema config path: %s", schemaConfigPath.c_str());
            auto localConfigPath = TablePathDefine::constructTempConfigPath(
                pid.clusternames(0), pid.range().from(), pid.range().to(), schemaConfigPath);
            if (localConfigPath.empty()) {
                AUTIL_LOG(ERROR, "schema config path invalid: %s", schemaConfigPath.c_str());
                return nullptr;
            } else {
                ...
                if (errorCode == build_service::common::ConfigDownloader::DEC_NORMAL_ERROR ||
                    ...
                    AUTIL_LOG(ERROR,
                              "download schema config from [%s] to [%s] failed",
                              schemaConfigPath.c_str(),
        ...
        return new DirectBuilder(pid, _tablet, rtResource, configPath, properties.walConfig);
    }
