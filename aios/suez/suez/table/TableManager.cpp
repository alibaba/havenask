...
void TableManager::generateKeepList(const HeartbeatTarget &target, bool isFinal, TodoList &result) {
    ...
    for (const auto &it : tableMetas) {
        ...
        if (tables.count(pid) == 0) {
            AUTIL_LOG(WARN, "create table for %s failed, need retry", ToJsonString(pid, true).c_str());
            continue;
        }
            ...
            if (table->getPartitionMeta().isTableInMemory()) {
                if (todoList.remove(OP_LOAD, pid)) {
                    AUTIL_LOG(INFO,
                              ...
                              ToJsonString(table->getPid(), true).c_str());
...
bool TableManager::getTableReader(const HeartbeatTarget &target, MultiTableReader &tableReader) const {
    ...
    for (const auto &it : target.getTableMetas()) {
        ...
        if (_tableMap.end() == tit) {
            AUTIL_LOG(DEBUG, "table not found, %s", ToJsonString(pid, true).c_str());
            ret = false;
            continue;
        }
        auto data = tit->second->getSuezPartitionData(_scheduleConfig.allowForceLoad, meta);
        if (!data) {
            AUTIL_LOG(DEBUG, "table is not available, %s", ToJsonString(pid, true).c_str());
            ret = false;
            continue;
        }
        tableReader.addTableReader(tit->second->getPid(), SingleTableReaderPtr(new SingleTableReader(data)));
    }
    return ret;
}
...
bool TableManager::initGlobalTableResource(const InitParam &param) {
    ...
    _globalIndexResource = indexlib::partition::PartitionGroupResource::Create(
        getAllowedMem(), _globalMetricsReporter, getenv("RS_BLOCK_CACHE"), getenv("RS_SEARCH_CACHE"));
