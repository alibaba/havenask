...
void TableServiceImpl::queryVersion(google::protobuf::RpcController *controller,
    ...
    if (indexPartition) {
        ...
        if (!tablet) {
            ...
            buildLocatorProto->set_offset(buildLocator.GetOffset());
            ...
            commitLocatorProto->set_offset(commitLocator.GetOffset());
        }
    }
}
...
Status TableServiceImpl::queryTablet(shared_ptr<indexlibv2::framework::ITablet> tablet,
    ...
    if (tableType == indexlib::table::TABLE_TYPE_NORMAL) {
        return indexlibv2::tools::IndexTableExecutorV2::QueryIndex(tabletReader, query, partitionResponse);
    } else if (tableType == indexlib::table::TABLE_TYPE_KV || tableType == indexlib::table::TABLE_TYPE_AGGREGATION) {
        auto kvConfigVec = schema->GetIndexConfigs("kv");
        if (kvConfigVec.size() != 1) {
            return Status::InvalidArgs("kv config size[%lu] != 1", kvConfigVec.size());
        }
        return indexlibv2::tools::KvTableExecutorV2::QueryKVTable(
            tabletReader, kvConfigVec[0], query, partitionResponse);
    } else if (tableType == indexlib::table::TABLE_TYPE_KKV) {
        auto kkvConfigVec = schema->GetIndexConfigs("kkv");
        if (kkvConfigVec.size() != 1) {
            return Status::InvalidArgs("kkv config size[%lu] != 1", kkvConfigVec.size());
        }
        return indexlibv2::tools::KkvTableExecutorV2::QueryKKVTable(
            tabletReader, kkvConfigVec[0], query, partitionResponse);
    } else if (tableType == indexlib::table::TABLE_TYPE_ORC) {
        return indexlibv2::tools::OrcTableExecutor::QueryIndex(tabletReader, query, partitionResponse);
    }
...
void TableServiceImpl::aggregate(google::protobuf::RpcController *controller,
                                 const AggRequest *request,
                                 AggResponse *response,
                                 google::protobuf::Closure *done) {
    autil::ScopedTime2 t;
    COMMON_CHECK(tablename);
    const auto &tableName = request->tablename();
    auto tableReader = indexProvider->multiTableReader.getTableReader(tableName, request->hashid());
    if (!tableReader) {
        ERROR_THEN_RETURN(
            done, TBS_ERROR_NO_TABLE, "cannot find table", kQueryTable, tableName, _metricsReporter.get());
    }
    ...
    if (!tablet) {
        ERROR_THEN_RETURN(
            done, TBS_ERROR_NOT_SUPPORTED, "table type unsupported", kQueryTable, tableName, _metricsReporter.get());
    }
    auto tabletReader = tablet->GetTabletReader();
    const auto &indexName = request->indexname();
    auto indexReader = tabletReader->GetIndexReader(indexlibv2::index::AGGREGATION_INDEX_TYPE_STR, indexName);
    auto aggReader = std::dynamic_pointer_cast<indexlibv2::index::AggregationReader>(indexReader);
    if (!aggReader) {
        ERROR_THEN_RETURN(done, TBS_ERROR_OTHERS, "index not found", kQueryTable, tableName, _metricsReporter.get());
    }

    autil::mem_pool::UnsafePool pool(1024);
    indexlibv2::index::Aggregator agg(aggReader.get(), &pool);
    if (request->has_key()) {
        agg.Key(request->key());
    } else {
        std::vector<autil::StringView> rawKeys;
        for (int i = 0; i < request->rawkeys_size(); ++i) {
            rawKeys.emplace_back(autil::StringView(request->rawkeys(i)));
        }
        agg.Keys(rawKeys);
    }

    autil::Result<uint64_t> r;
    if (!request->has_distinctfield()) {
        r = agg.Count();
    } else {
        r = agg.DistinctCount(request->distinctfield());
    }
    if (r.is_ok()) {
        response->set_value(r.get());
        response->set_latencyinms(t.done_ms());
    } else {
        ERROR_THEN_RETURN(
            done, TBS_ERROR_OTHERS, r.get_error().message(), kQueryTable, tableName, _metricsReporter.get());
    }
}
...
void TableServiceImpl::convertPartitionResponseToResponse(const TableQueryRequest *const request,
    ...
    for (int64_t i = 0; i < partitionResponse.rows_size(); ++i) {
        ...
        for (int j = 0; j < attrInfo.fields_size(); ++j) {
            ...
            setSingleAttr(request, singleAttrValue, row.attrvalues(j));
        }
    }
...
void TableServiceImpl::setSingleAttr(const TableQueryRequest *const request,
