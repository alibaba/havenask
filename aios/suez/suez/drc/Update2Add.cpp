...
bool Update2Add::init(indexlibv2::framework::ITablet *index) {
    ...
    _decoder = std::make_unique<BinaryRecordDecoder>();
    if (!_decoder->init(*kvIndexConfig)) {
        return false;
    }
...
bool Update2Add::createSnapshot() {
    ...
    _readerSnapshot = std::dynamic_pointer_cast<indexlibv2::index::KVIndexReader>(indexReader);
    return _readerSnapshot.get() != nullptr;
}

void Update2Add::releaseSnapshot() { _readerSnapshot.reset(); }
