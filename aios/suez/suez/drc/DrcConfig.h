...
inline std::string getParameter(const std::map<std::string, std::string> &params, const std::string &key) {
...
template <typename T>
inline bool getParameterT(const std::map<std::string, std::string> &params, const std::string &key, T &val) {
    auto it = params.find(key);
    if (it == params.end()) {
        return false;
    }
