...
SearchManagerUpdater::SearchManagerUpdater(bool enableLocalCatalog, bool allowPartialTableReady)
    : _allowPartialTableReady(allowPartialTableReady)
    , _configManager(new ConfigManager())
    , _counterReporter(new CounterReporter())
    , _searchMetas(new SearchMetas()) {
    _builtinService = std::make_unique<BuiltinSearchManagerList>(enableLocalCatalog);
}
...
bool SearchManagerUpdater::init(const InitParam &initParam) {
    ...
    searchInitParam.catalogClient = initParam.catalogClient;
    ...
    if (!_allowPartialTableReady && !indexProvider->allTableReady) {
        ...
        for (const auto &it : target.getTableMetas()) {
            ...
            if (!indexProvider->multiTableReader.hasTableReader(pid)) {
                notReadyTables.push_back(ToJsonString(pid, /*isCompact=*/true));
            }
        }
        AUTIL_LOG(INFO,
...
std::unique_ptr<iquan::CatalogInfo> SearchManagerUpdater::getCatalogInfo() const {
    if (!_searchManager) {
        return nullptr;
    }
    return _searchManager->getCatalogInfo();
}
...
bool SearchManagerUpdater::needUpdate(const UpdateArgs &args) const {
    return (*(_searchMetas->_indexProviderSnapshot) != *(args.indexProvider)) ||
           ...
           ToJsonString(args.customAppInfo) != ToJsonString(_searchMetas->_customAppInfoSnapshot) ||
