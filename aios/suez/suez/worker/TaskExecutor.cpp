...
TaskExecutor::TaskExecutor(RpcServer *rpcServer, const KMonitorMetaInfo &metaInfo)
    ...
    , _enableLocalCatalog(false)
    , _enableUpdateCatalog(false) {}
...
bool TaskExecutor::start(HeartbeatManager *hbManager, const std::string &installRoot, const EnvParam &param) {
    ...
    _enableLocalCatalog = param.enableLocalCatalog;
    _enableUpdateCatalog = param.enableUpdateCatalog;
    _iquanCatalogBridge = std::make_unique<IquanCatalogBridge>();
    if (!_iquanCatalogBridge->init()) {
        AUTIL_LOG(ERROR, "init IquanCatalogBridge failed");
        return false;
    }
    ...
    initParam.catalogClient = _iquanCatalogBridge->getCatalogClient();
...
bool TaskExecutor::initSearchManager(const InitParam &initParam) {
    _searchManagerUpdater = std::make_unique<SearchManagerUpdater>(_enableLocalCatalog, _allowPartialTableReady);
...
void TaskExecutor::stopDecision() {
    ...
    if (_iquanCatalogBridge) {
        _iquanCatalogBridge->stop();
    }
}
...
bool TaskExecutor::doWorkloop(HeartbeatTarget &target,
    ...
    // TODO: catalog address use name service (such as zk/cm2/vip), then remove this
    _iquanCatalogBridge->updateCatalogAddress(target.getCatalogAddress());
    ...
    if (UR_NEED_RETRY != searchUpdateResult) {
        if (UR_ERROR == searchUpdateResult) {
            ...
            if (UR_REACH_TARGET == searchUpdateResult && _enableUpdateCatalog) {
                auto catalogInfo = _searchManagerUpdater->getCatalogInfo();
                if (catalogInfo.get() != nullptr) {
                    AUTIL_LOG(DEBUG, "commit catalog info, version[%ld]", target.getTargetVersion());
                    reachTarget = _iquanCatalogBridge->commitCatalogInfo(target.getTargetVersion(), *catalogInfo);
                }
            }
        }
