        ...
        if (collector != nullptr) {
            ...
            if (_reporter) {
                kmonitor::MetricsTags tags{
                    {{"table_name", _pid->clusternames(0)}, {"role_type", RoleType_Name(_roleType)}}};
...
void TableWriter::maybeInitWALLocked() {
    ...
    if (!wal) {
        AUTIL_LOG(ERROR,
                  ...
                  ToJsonString(*_walConfig, true).c_str());
        return;
    }
    _wal = std::move(wal);
}
...
void TableWriter::fillWriteResult(WriteResult &result, const WriteMetricsCollector *collector) const {
    ...
    if (locator.IsValid()) {
        result.watermark.buildLocatorOffset = locator.GetOffset();
    }
