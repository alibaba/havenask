...
bool SingleTableReader::getDataTimestamp(int64_t &dataTimestamp) const {
    if (SPT_INDEXLIB == _partitionData->type()) {
        auto indexPartitionData = static_pointer_cast<SuezIndexPartitionData>(_partitionData);
        return indexPartitionData->getDataTimestamp(dataTimestamp);
        ...
        return tabletPartitionData->getDataTimestamp(dataTimestamp);
    } else {
        return false;
    }
}
...
indexlib::partition::IndexPartitionPtr SingleTableReader::getIndexPartition() const {
    if (SPT_INDEXLIB == _partitionData->type()) {
        ...
        AUTIL_LOG(
            DEBUG, "table[%s] is not indexlib table", FastToJsonString(_partitionData->getPartitionId(), true).c_str());
        return nullptr;
    }
}
...
std::shared_ptr<indexlibv2::framework::ITablet> SingleTableReader::getTablet() const {
    if (SPT_TABLET == _partitionData->type()) {
        ...
        AUTIL_LOG(
            DEBUG, "table [%s] is not tablet table", FastToJsonString(_partitionData->getPartitionId(), true).c_str());
        return nullptr;
    }
}
...
string SingleTableReader::getFilePath() const {
    if (SPT_RAWFILE == _partitionData->type()) {
        ...
        AUTIL_LOG(
            DEBUG, "table[%s] is not rawfile table", FastToJsonString(_partitionData->getPartitionId(), true).c_str());
        return string();
    }
}
...
bool SingleTableReader::getPartitionIndex(uint32_t &partId) const {
    if (SPT_INDEXLIB == _partitionData->type()) {
        auto indexPartitionData = static_pointer_cast<SuezIndexPartitionData>(_partitionData);
        return indexPartitionData->getPartitionIndex(partId);
    } else if (SPT_TABLET == _partitionData->type()) {
        auto tabletPartitionData = static_pointer_cast<SuezTabletPartitionData>(_partitionData);
        return tabletPartitionData->getPartitionIndex(partId);
    } else {
        partId = 0;
        return true;
    }
...
SingleTableReaderPtr MultiTableReader::getTableReader(const PartitionId &pid) const {
    ...
    if (_tableReaders.end() == it) {
        AUTIL_LOG(ERROR, "table reader for [%s] is not found", FastToJsonString(pid, true).c_str());
        return nullptr;
    }
    return it->second;
}
...
bool MultiTableReader::getDataTimestamp(const string &tableName, int64_t &dataTimestamp) const {
    auto it = _namedTableReaders.find(tableName);
    ...
    for (const auto &pit : tablePartitions) {
        return pit.second->getDataTimestamp(dataTimestamp);
    }
    return false;
}
...
vector<string> MultiTableReader::getTableNames() const {
    ...
    for (; it != _tableReaders.end(); it++) {
        tableNames.push_back(FastToJsonString(it->first, true).c_str());
    }
    return tableNames;
}
    ...
    for (; it1 != left._tableReaders.end() && it2 != right._tableReaders.end(); ++it1, ++it2) {
        if (it1->first != it2->first) {
            AUTIL_LOG(INFO,
                      ...
                      FastToJsonString(left.getTableNames(), true).c_str(),
                      FastToJsonString(right.getTableNames(), true).c_str());
            return false;
        }
        if (*(it1->second) != *(it2->second)) {
            AUTIL_LOG(INFO, "[%s] table changed", FastToJsonString(it1->first, true).c_str());
            return false;
        }
    }
    return true;
