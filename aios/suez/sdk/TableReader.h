...
public:
    SingleTableReader(const SuezPartitionDataPtr &partitionData);
...
public:
    ...
    void setTableDefConfig(const TableDefConfig &config) { _tableDefConfig = config; }
    const TableDefConfig &getTableDefConfig() const { return _tableDefConfig; }

private:
    SuezPartitionDataPtr _partitionData;
    TableDefConfig _tableDefConfig;
};
...
public:
    ...
    bool getDataTimestamp(const std::string &tableName, int64_t &dataTimestamp) const;
    void clear();
