...
bool ZkVersionSynchronizer::persistToZk(const PartitionId &pid,
    ...
    if (state.get() == nullptr) {
        AUTIL_LOG(ERROR, "create version state for %s failed", FastToJsonString(pid, true).c_str());
        return false;
    }
    std::string writeCtx = autil::legacy::FastToJsonString(ctx, true);
    auto ret = state->write(writeCtx);
    if (ret == VersionState::EC_FAIL) {
        AUTIL_LOG(
            ERROR, "partition[%s]: sync version %s failed", FastToJsonString(pid, true).c_str(), writeCtx.c_str());
        return false;
    } else {
...
bool ZkVersionSynchronizer::doSyncFromPersist(const PartitionId &pid,
    ...
    auto fillCtx = [&](const std::string ctx) {
        try {
            autil::legacy::FastFromJsonString(version, ctx);
            return true;
