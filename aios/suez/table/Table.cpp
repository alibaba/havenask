...
#define SUEZ_PREFIX FastToJsonString(_tableResource.pid, true).c_str()
...
std::string Table::getIdentifier() const { return FastToJsonString(_tableResource.pid, true); }
...
void Table::deploy(const TargetPartitionMeta &target, bool distDeploy) {
    ...
    if (status.status == DS_DEPLOYDONE && _leaderElectionMgr != nullptr) {
        AUTIL_LOG(INFO, "[%s] deploy done, allow to campagin leader.", FastToJsonString(getPid(), true).c_str());
    ...
    AUTIL_LOG(INFO,
              ...
              FastToJsonString(getPid(), true).c_str(),
...
void Table::unload() {
    ...
    if (_suezPartition && _suezPartition->isInUse()) {
        AUTIL_LOG(INFO,
                  ...
                  autil::legacy::FastToJsonString(getPid(), true).c_str());
        return;
    }
...
void Table::doRoleSwitch(const TargetPartitionMeta &target) {
    std::shared_ptr<SwitchState> state;
    {
        ...
        if (writer) {
            ...
            writer->setRoleType(RoleType_Name(target.getRoleType()));
...
bool Table::targetReached(const TargetPartitionMeta &target, bool allowForceLoad) const {
    ...
    return targetReached(target, current, allowForceLoad, isRecovered());
}

bool Table::targetReached(const TargetPartitionMeta &target,
                          const CurrentPartitionMeta &current,
                          bool allowForceLoad,
                          bool isRecovered) {
    auto dpStatus = current.getDeployStatus(target.getIncVersion());
    return isRecovered && dpStatus == DS_DEPLOYDONE && current.isReady(allowForceLoad) &&
