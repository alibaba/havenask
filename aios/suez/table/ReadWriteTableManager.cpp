...
void ReadWriteTableManager::handleCommitResult(const PartitionId &pid, const std::pair<bool, TableVersion> &result) {
    if (result.first && result.second.isLeaderVersion()) {
        ...
        AUTIL_LOG(
            INFO, "partition[%s] commit version: %s", FastToJsonString(pid, true).c_str(), version.toString().c_str());
...
TablePtr ReadWriteTableManager::createTable(const PartitionId &pid, const PartitionMeta &meta) {
    if (meta.getTableMode() == TM_READ_WRITE) {
        ...
        if (!SyncVersion::recoverVersion(
            ...
            AUTIL_LOG(WARN, "recover version for %s failed", FastToJsonString(pid, true).c_str());
            return TablePtr();
        }
        ...
        if (!publishedVersion.isFinished() && !_leaderElectionMgr->registerPartition(pid, std::move(fn))) {
            AUTIL_LOG(WARN, "start leader election for %s failed", FastToJsonString(pid, true).c_str());
            return TablePtr();
        }
...
void ReadWriteTableManager::removeTable(const PartitionId &pid) {
    AUTIL_LOG(INFO, "remove table %s", FastToJsonString(pid, true).c_str());
...
void ReadWriteTableManager::leaderChange(const PartitionId &pid, bool isLeader) {
    ...
    if (table->getLeaderInfoPublisher()->publish()) {
        AUTIL_LOG(
            INFO, "[%s]: update leader info success, isLeader: [%d]", FastToJsonString(pid, true).c_str(), isLeader);
        ...
        if (writer) {
            ...
            writer->setRoleType(RoleType_Name(isLeader ? RT_LEADER : RT_FOLLOWER));
        }
    } else {
        AUTIL_LOG(
            INFO, "[%s]: update leader info failed, isLeader: [%d]", FastToJsonString(pid, true).c_str(), isLeader);
    }
}
...
void ReadWriteTableManager::processTableIndications(const set<PartitionId> &releaseLeaderTables,
    ...
    for (const auto &pid : releaseLeaderTables) {
        AUTIL_LOG(INFO, "release leader for pid[%s]", FastToJsonString(pid, true).c_str());
        _leaderElectionMgr->releaseLeader(pid);
    }
    for (const auto &pid : campaginLeaderTables) {
        AUTIL_LOG(INFO, "set allow campagin leader for pid[%s]", FastToJsonString(pid, true).c_str());
    ...
    for (const auto &pid : publishLeaderTables) {
        auto table = getTable(pid);
        if (table) {
            AUTIL_LOG(INFO, "set allow publish leader for pid[%s]", FastToJsonString(pid, true).c_str());
    ...
    for (const auto &kv : tablesWeight) {
        ...
        if (table) {
            AUTIL_LOG(INFO, "set weight[%d] for pid[%s]", kv.second, FastToJsonString(kv.first, true).c_str());
