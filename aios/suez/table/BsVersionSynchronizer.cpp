...
bool BsVersionSynchronizer::persistVersion(const PartitionId &pid,
    ...
    if (!committer->Commit(version.getVersionMeta()).IsOK()) {
        AUTIL_LOG(ERROR,
                  ...
                  autil::legacy::FastToJsonString(pid, true).c_str(),
                  autil::legacy::FastToJsonString(version.getVersionMeta(), true).c_str());
        return false;
    }
    AUTIL_LOG(INFO,
...
BsVersionSynchronizer::createCommitter(const PartitionId &pid,
    ...
    if (!committer->Init(initParam).IsOK()) {
        AUTIL_LOG(ERROR, "init version committer for pid [%s] failed", FastToJsonString(pid, true).c_str());
        return nullptr;
    }
    return committer;
}
