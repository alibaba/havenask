...
bool HeartbeatTarget::parseFrom(const std::string &targetStr, const std::string &signature, bool preload) {
    ...
    try {
        FastFromJsonString(_target, targetStr);
    ...
    for (const auto &t : _tableMetas) {
        if (!t.second.validate()) {
            AUTIL_LOG(ERROR,
                      ...
                      FastToJsonString(t.first).c_str(),
                      FastToJsonString(t.second).c_str());
            ret = false;
        }
    }
    if (!ret) {
        AUTIL_LOG(ERROR, "invalid target [%s]", FastToJsonString(_target).c_str());
    }
    return ret;
}
