...
IE_LOG_SETUP(config, SourceSchema);
...
bool SourceSchema::GetModule(const string& moduleName, ModuleInfo& ret) const
{
    return mImpl->GetModule(moduleName, ret);
}

void SourceSchema::AddGroupConfig(const SourceGroupConfigPtr& groupConfig) { mImpl->AddGroupConfig(groupConfig); }

const SourceGroupConfigPtr& SourceSchema::GetGroupConfig(index::groupid_t groupId) const
{
...
void SourceSchema::DisableFieldGroup(index::groupid_t groupId) { mImpl->DisableFieldGroup(groupId); }

void SourceSchema::GetDisableGroupIds(std::vector<index::groupid_t>& ids) const { mImpl->GetDisableGroupIds(ids); }
