...
{
public:
    MockMergePolicy() : MergePolicy() {}
    ~MockMergePolicy() {}
    MOCK_METHOD(vector<TableMergePlanPtr>, CreateMergePlansForIncrementalMerge,
                (const string&, const MergeStrategyParameter&, const vector<SegmentMetaPtr>&, const PartitionRange&),
                (const, override));

    MOCK_METHOD(TableMergePlanResourcePtr, CreateMergePlanResource, (), (const, override));

    MOCK_METHOD(MergeTaskDescriptions, CreateMergeTaskDescriptions,
                (const index_base::Version&, const TableMergePlanPtr&, const TableMergePlanResourcePtr&,
                 const vector<SegmentMetaPtr>&, MergeSegmentDescription& segmentDescription),
                (const, override));

    MOCK_METHOD(bool, ReduceMergeTasks,
                (const TableMergePlanPtr&, const vector<MergeTaskDescription>&, const vector<DirectoryPtr>&,
...
public:
    bool Init(const TableMergePlanPtr& mergePlan, const std::vector<SegmentMetaPtr>& segmentMetas) override
    {
        return true;
    }
    ...
    {
        directory->Store("custom_resource_file", "73", WriterOption());
        return true;
    }
    ...
    size_t EstimateMemoryUse() const override { return 0; }
};
...
void TableMergeMetaTest::TestSimpleProcess()
{
    ...
    vector<TaskExecuteMetas> instanceExecMetas = dispatcher.DispatchMergeTasks(meta->mergeTaskDescriptions, 2);
    meta->instanceExecMetas = instanceExecMetas;
    meta->SetTargetVersion(targetVersion);

    MergeMetaPtr mergeMeta(meta);
    string metaRootDir = GET_TEMP_DATA_PATH("merge_meta");
    ...
    EXPECT_CALL(*mockMergePolicy, CreateMergePlanResource()).Times(0);
...
void TableMergeMetaTest::TestLoadWithoutPlanResource()
{
    ...
    MergeMetaPtr mergeMeta(meta);
    string metaRootDir = GET_TEMP_DATA_PATH();
...
void TableMergeMetaTest::CompareMergeMeta(const MergeMetaPtr& expectMeta, const MergeMetaPtr& actualMeta,
                                          bool loadResource)
{
