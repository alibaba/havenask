...
void MultiPartitionMergerInteTest::MakeOnePartitionData(const IndexPartitionSchemaPtr& schema,
...
void MultiPartitionMergerInteTest::TestMergeWithSubDocDeleteAndUpdate(bool withPackage)
{
    ...
    IndexPartitionSchemaPtr subSchema =
        SchemaMaker::MakeSchema("subpkstr:string;sub_long:uint32;", "sub_pk:primarykey64:subpkstr", "sub_long;", "");
    ...
    MultiPartitionMerger multiPartMerger(options, metricProvider, "", CommonBranchHinterOption::Test());
    MetricPtr metric = metricProvider->DeclareMetric("indexlib/merge/progress", kmonitor::STATUS);

    ASSERT_TRUE(metric != nullptr);
    ASSERT_DOUBLE_EQ(std::numeric_limits<double>::min(), metric->TEST_GetValue());
    multiPartMerger.Merge(mergeSrcs, mergePartPath);
    ASSERT_DOUBLE_EQ(100.0, metric->TEST_GetValue());

    PartitionStateMachine psm;
    psm.Init(schema, options, mergePartPath);
    // check main doc
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello0", "docid=0,main_join=1"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello1", "docid=1,main_join=2"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello2", "docid=2,main_join=2"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello3", "docid=3,main_join=3"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello4", "docid=4,main_join=4"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "pk:hello5", "docid=5,main_join=4"));
}
...
{
    DoTestMerge(false, false, BALANCE_TREE_MERGE_STRATEGY_STR,
                "conflict-segment-number=2;base-doc-count=2;max-doc-count=8");
}

void MultiPartitionMergerInteTest::TestIncPriorityQueueMerge()
{
    DoTestMerge(
        false, false, PRIORITY_QUEUE_MERGE_STRATEGY_STR,
...
{
    DoTestMerge(false, false, REALTIME_MERGE_STRATEGY_STR,
                "max-small-segment-count=10;merge-size-upperbound=0;merge-size-lowerbound=0");
}
...
void MultiPartitionMergerInteTest::TestMergeWithEmptyPartitions()
{
    ...
    auto partDir1 = GET_TEMP_DATA_PATH("part1");
    ASSERT_NO_THROW(versionLoader.GetVersionS(partDir1, version, versionid_t(0)));
    EXPECT_EQ(size_t(0), version.GetSegmentCount());

    auto partDir2 = GET_TEMP_DATA_PATH("part2");
    ASSERT_NO_THROW(versionLoader.GetVersionS(partDir2, version, versionid_t(0)));
    EXPECT_EQ(size_t(0), version.GetSegmentCount());
    ...
    auto mergePartDir = GET_TEMP_DATA_PATH("mergePart");
    ASSERT_NO_THROW(versionLoader.GetVersionS(mergePartDir, version, versionid_t(0)));
    EXPECT_EQ(size_t(0), version.GetSegmentCount());
}
...
void MultiPartitionMergerInteTest::DoTestMerge(bool isFullMerge, bool isMultiPart, const string& mergeStrategy,
                                               const string& mergeStrategyParam, bool isMergeTwice)
{
    ...
    if (isMultiPart) {
        ...
        merger.reset(PartitionMergerCreator::TEST_CreateSinglePartitionMerger(mergePartPath, options, NULL, ""));
    }
...
void MultiPartitionMergerInteTest::TestMergerMetrics()
{
    ...
    MultiPartitionMerger multiPartMerger(options, metricProvider, "", CommonBranchHinterOption::Test());
    MetricPtr progress = metricProvider->DeclareMetric("indexlib/merge/progress", kmonitor::STATUS);
    MetricPtr leftItemCount = metricProvider->DeclareMetric("indexlib/merge/leftItemCount", kmonitor::STATUS);
    ASSERT_TRUE(progress != nullptr);
    ASSERT_TRUE(leftItemCount != nullptr);
    ASSERT_DOUBLE_EQ(std::numeric_limits<double>::min(), progress->TEST_GetValue());
    ASSERT_DOUBLE_EQ(std::numeric_limits<double>::min(), leftItemCount->TEST_GetValue());
    multiPartMerger.Merge(mergeSrcs, mergePartPath);
    ASSERT_DOUBLE_EQ(100.0, progress->TEST_GetValue());
    ASSERT_DOUBLE_EQ(0, leftItemCount->TEST_GetValue());
}
...
void MultiPartitionMergerInteTest::TestMergeCounters()
{
    ...
    auto OverWriteCounterFile = [this](const string& partPath, segmentid_t segId, int64_t val1, int64_t val2) {
        string partRoot = GET_TEMP_DATA_PATH(partPath);
        auto fs = FileSystemCreator::CreateForWrite("tmp", partRoot).GetOrThrow();
        ASSERT_EQ(FSEC_OK, fs->MountVersion(partRoot, 0, "", FSMT_READ_ONLY, nullptr));
        DirectoryPtr segDir =
            Directory::Get(fs)->GetDirectory("segment_" + StringUtil::toString(segId) + "_level_0", true);
        ...
        segDir->Store(COUNTER_FILE_NAME, offlineCounterMap.ToJsonString(), WriterOption());
    ...
    auto counterFilePath = GET_TEMP_DATA_PATH("mergePart/segment_0_level_0/" + COUNTER_FILE_NAME);
    ASSERT_TRUE(FslibWrapper::IsExist(counterFilePath).GetOrThrow());

    util::CounterMap counterMap;
    string counterMapContent;
    ASSERT_EQ(FSEC_OK, FslibWrapper::Load(counterFilePath, counterMapContent).Code());
...
void MultiPartitionMergerInteTest::TestMergeLocatorAndTimestamp()
{
    ...
    auto OverWriteSegmentInfoFile = [this](const string& partPath, segmentid_t segId, int64_t timestamp, int64_t src,
                                           int64_t offset) {
        string segName = "segment_" + StringUtil::toString(segId) + "_level_0";
        string segInfoPath = GET_TEMP_DATA_PATH(PathUtil::JoinPath(partPath, segName, SEGMENT_INFO_FILE_NAME));
        SegmentInfo segInfo;
        segInfo.TEST_Load(segInfoPath);
        file_system::FslibWrapper::DeleteFileE(segInfoPath, DeleteOption::NoFence(false));
        segInfo.timestamp = timestamp;
        segInfo.SetLocator(indexlibv2::framework::Locator(src, offset));
        segInfo.TEST_Store(segInfoPath);
    };
    ...
    SegmentInfo segInfo;
    segInfo.TEST_Load(GET_TEMP_DATA_PATH("mergePart/segment_0_level_0/" + SEGMENT_INFO_FILE_NAME));
    EXPECT_EQ((int64_t)20, segInfo.timestamp);

    IndexLocator indexLocator;
    indexLocator.fromString(segInfo.GetLocator().Serialize());
