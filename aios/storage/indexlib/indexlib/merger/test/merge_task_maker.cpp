...
void MergeTaskMaker::CreateMergeTask(const string& mergeTaskStr, MergeTask& mergeTask,
                                     const index_base::SegmentMergeInfos& segMergeInfos)
{
    StringTokenizer st(mergeTaskStr, ";", StringTokenizer::TOKEN_TRIM | StringTokenizer::TOKEN_IGNORE_EMPTY);
    for (size_t i = 0; i < st.getNumTokens(); ++i) {
        ...
        for (uint32_t j = 0; j < st2.getNumTokens(); j++) {
            ...
            auto iter = std::find_if(
                segMergeInfos.begin(), segMergeInfos.end(),
                [segId](const index_base::SegmentMergeInfo& segMergeInfo) { return segMergeInfo.segmentId == segId; });
