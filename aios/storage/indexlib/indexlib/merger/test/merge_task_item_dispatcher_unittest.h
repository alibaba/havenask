...
private:
    void CheckMergeTaskItemCost(MergeTaskItems& allItems, const std::string& itemName, double expectCost);

    void MakeDataForDir(const std::string& dirPath, const std::string& fileDespStr);

    void CreateDirAndFile(const std::string& dirPath, const std::vector<std::string>& fileNames,
                          std::vector<uint32_t> fileListSize);

    void CreateDirAndFileInPackageFile(const std::string& segmentPath, const std::vector<std::string>& fileNames,
                                       std::vector<uint32_t> fileListSize);
