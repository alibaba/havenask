...
public:
    ...
    void CreateMergeTaskData(MergeTask& task, uint32_t planItemCount, uint32_t segmentCountPerPlanItem)
    {
        for (uint32_t k = 0; k < planItemCount; ++k) {
            MergePlan item;
            for (uint32_t i = k * segmentCountPerPlanItem; i < (k + 1) * segmentCountPerPlanItem; ++i) {
