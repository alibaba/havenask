...
public:
    bool Init(const std::string& joinFieldName, bool isSubField, config::IndexPartitionSchemaPtr mainPartSchema,
              partition::IndexPartition* auxPart, versionid_t auxReaderIncVersionId);

    common::AttributeValueInitializerPtr Create(const index_base::PartitionDataPtr& partitionData) override;

    // for segment build (local segment data)
    /* override*/ common::AttributeValueInitializerPtr
    Create(const index_base::InMemorySegmentReaderPtr& inMemSegmentReader) override;

private:
    config::AttributeConfigPtr mJoinFieldAttrConfig;
    partition::IndexPartition* mAuxPart;
