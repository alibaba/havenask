...
TableReaderContainerUpdater::TableReaderContainerUpdater(std::function<void()> updateCallBackFunc)
    : mUpdateCallBackFunc(updateCallBackFunc)
{
}
...
TableReaderContainerUpdater::~TableReaderContainerUpdater()
{
    ...
    for (size_t i = 0; i < mIndexPartitions.size(); i++) {
        ...
        IndexPartition* indexPart = mIndexPartitions[i];
        assert(indexPart);
        auto iter = mTableToVirutalAttributes.find(tableName);
        if (iter != mTableToVirutalAttributes.end() && !iter->second.empty()) {
            vector<AttributeConfigPtr> mainAttributeConfigs;
            vector<AttributeConfigPtr> subAttributeConfigs;
            indexPart->AddVirtualAttributes(mainAttributeConfigs, subAttributeConfigs);
        }
    }
    UnLockAll();
}
...
bool TableReaderContainerUpdater::Init(const std::vector<IndexPartitionPtr>& indexPartitionVec,
                                       const JoinRelationMap& joinRelations, TableReaderContainer* readerContainer)
{
    ...
    if (!ReserveAllVirtualAttributeMemory(reserverVec) || !AddAllVirtualAttributes()) {
        UnLockAll();
        return false;
    }
    for (size_t i = 0; i < mIndexPartitions.size(); i++) {
        mIndexPartitions[i]->AddReaderUpdater(this);
        mReaderContainer->UpdateReader(mIndexPartitions[i]->GetSchema()->GetSchemaName(),
...
bool TableReaderContainerUpdater::Update(const IndexPartitionReaderPtr& indexPartReader)
{
    ...
    for (size_t i = 0; i < mainTableIdxVec.size(); i++) {
        ...
        if (!ReserveVirtualMemoryForOneTable(mainTableName, partMemoryReservers)) {
            IE_LOG(ERROR, "reserve virtual mem for table[%s] failed", mainTableName.c_str());
    ...
    try {
        ...
        for (; iter != mTableToVirutalAttributes.end(); iter++) {
            if (!iter->second.empty()) {
                IndexPartition* mainPartition = GetIndexPartition(iter->first);
                assert(mainPartition);
                AddVirtualAttributes(mainPartition);
            }
        }
...
bool TableReaderContainerUpdater::ReserveAllVirtualAttributeMemory(vector<MemoryReserverPtr>& reserverVec)
{
    auto iter = mTableToVirutalAttributes.begin();
    for (; iter != mTableToVirutalAttributes.end(); iter++) {
...
bool TableReaderContainerUpdater::ReserveVirtualMemoryForOneTable(const std::string& mainTableName,
                                                                  vector<MemoryReserverPtr>& reserverVec)
{
    ...
    IndexPartition* indexPart = GetIndexPartition(mainTableName);
    assert(indexPart);
    MemoryReserverPtr reserver = indexPart->ReserveVirtualAttributesResource(mainAttrConfigs, subAttrConfigs);
    if (!reserver) {
        reserverVec.clear();
        return false;
    }
                                                                         ...
                                                                         const vector<JoinField>& joinFields)
{
    for (size_t i = 0; i < joinFields.size(); i++) {
        ...
        AttributeConfigPtr attrConfig = CreateAttributeConfig(indexPartition, joinTableName, joinFieldName, isSub);
        mTableToVirutalAttributes[mainTableName].push_back(VirtualAttributeInfo(joinTableName, isSub, attrConfig));
    }
}

bool TableReaderContainerUpdater::CollectToUpdateMainTables(const IndexPartitionReaderPtr& indexPartReader,
    ...
    for (; joinFieldMapIter != iter->second.end(); joinFieldMapIter++) {
        if (joinFieldMapIter->second.genJoinCache) {
            ...
            if (mIndexPartitions[mainTableId]->GetSchema()->GetTableType() == TableType::tt_customized) {
                ...
                IE_LOG(ERROR, "customized table[%s] can't be update by join table[%s]", mainTableName.c_str(),
                       tableName.c_str());
                return false;
            }
...
AttributeConfigPtr TableReaderContainerUpdater::CreateAttributeConfig(IndexPartition* mainPart,
                                                                      const string& auxTableName,
                                                                      const string& joinField, bool& isSub)
{
    HasAttribute(mainPart, joinField, isSub);
    IndexPartition* auxPart = GetIndexPartition(auxTableName);
    assert(auxPart);
    ...
    string virtualAttributeName = JoinField::GenerateVirtualAttributeName(
        isSub ? BUILD_IN_SUBJOIN_DOCID_VIRTUAL_ATTR_NAME_PREFIX : BUILD_IN_JOIN_DOCID_VIRTUAL_ATTR_NAME_PREFIX,
    ...
    AttributeConfigPtr attrConfig = VirtualAttributeConfigCreator::Create(virtualAttributeName, ft_int32, false,
                                                                          StringUtil::toString(INVALID_DOCID), creator);
    return attrConfig;
}
...
void TableReaderContainerUpdater::UnLock(const vector<int32_t>& mainTableIdxVec)
{
    for (int32_t i = mainTableIdxVec.size() - 1; i >= 0; i--) {
        IndexPartition* indexPart = mIndexPartitions[mainTableIdxVec[i]];
        if (indexPart) {
            indexPart->UnLock();
        }
    }
...
void TableReaderContainerUpdater::UpdateVirtualAttrConfigs(IndexPartitionReaderPtr auxPartReader,
                                                           const vector<int32_t>& mainTableIdxVec)
{
    ...
    for (auto& idx : mainTableIdxVec) {
        ...
        bool isSub;
        IndexPartition* indexPartition = mIndexPartitions[idx];
        assert(indexPartition);
        AttributeConfigPtr attrConfig = CreateAttributeConfig(indexPartition, auxTableName, joinFieldName, isSub);
...
void TableReaderContainerUpdater::FillVirtualAttributeConfigs(const string& tableName,
                                                              ...
                                                              vector<AttributeConfigPtr>& subAttributeConfigs)
{
    partition::IndexPartition* partition = GetIndexPartition(tableName);
    assert(partition);
    partition->Lock();
...
void TableReaderContainerUpdater::AddVirtualAttributes(IndexPartition* mainPartition)
{
...
bool TableReaderContainerUpdater::InitPartitions(const std::vector<IndexPartitionPtr>& indexPartitionVec,
                                                 const JoinRelationMap& joinRelations)
{
    ...
    for (size_t i = 0; i < indexPartitionVec.size(); i++) {
        ...
        if (joinRelations.find(tableName) != joinRelations.end()) {
            if (indexPartitionVec[i]->GetSchema()->GetTableType() == TableType::tt_customized) {
                IE_LOG(ERROR, "customized table[%s] join failed, action not support.", tableName.c_str());
                return false;
            }
    ...
    for (size_t i = 0; i < mIndexPartitions.size(); i++) {
        ...
        if (subSchema && mIndexPartitions[i]->GetSchema()->GetTableType() != TableType::tt_customized) {
...
bool TableReaderContainerUpdater::ParseJoinRelations(const JoinRelationMap& joinRelations)
{
    JoinRelationMap::const_iterator iter = joinRelations.begin();
    for (; iter != joinRelations.end(); iter++) {
        ...
        if (partition->GetSchema()->GetTableType() == TableType::tt_customized) {
            IE_LOG(ERROR, "do not support JoinRelation for customized table[%s]", mainTableName.c_str());
            return false;
        }

        if (!ParseJoinRelation(partition, iter->second)) {
            IE_LOG(ERROR, "parse join relation for table[%s] failed", iter->first.c_str());
            return false;
        }
    }
    return true;
...
bool TableReaderContainerUpdater::HasAttribute(IndexPartition* partition, const string& fieldName, bool& isSub)
{
...
bool TableReaderContainerUpdater::ParseJoinRelation(IndexPartition* mainPartition, const vector<JoinField>& joinFields)
{
    ...
    for (size_t i = 0; i < joinFields.size(); i++) {
        ...
        if (!joinPartition) {
            IE_LOG(ERROR, "join table [%s] not exist", joinTableName.c_str());
            return false;
        }
        const string& joinFieldName = joinFields[i].fieldName;
        bool isSub = false;
        if (!HasAttribute(mainPartition, joinFieldName, isSub)) {
            IE_LOG(ERROR, "join field[%s] is not found in in main table [%s]", joinFieldName.c_str(),
                   mainTableName.c_str());
        ...
        if (isSub) {
            string subMainTableName = schema->GetSubIndexPartitionSchema()->GetSchemaName();
