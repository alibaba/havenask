...
OperationIterator::OperationIterator(const PartitionDataPtr& partitionData, const IndexPartitionSchemaPtr& schema)
    : mCurrentSegIdx(-1)
...
bool OperationIterator::LocateStartLoadPosition(const OperationCursor& skipCursor, int& segIdx, int& offset)
{
...
bool OperationIterator::LocateStartLoadPositionInNormalSegment(const OperationCursor& skipCursor, int& segIdx,
                                                               int& offset)
{
    for (size_t i = 0; i < mSegDataVec.size(); i++) {
        if (mSegDataVec[i].GetSegmentId() == skipCursor.segId) {
            ...
            if (skipCursor.pos >= (int32_t)opCount) {
                IE_LOG(ERROR,
                       "invalid skip cursor: [segid:%d, pos:%d], "
                       "totalOpCount [%lu]",
...
bool OperationIterator::LocateStartLoadPositionInDumpingSegment(const OperationCursor& skipCursor, int& segIdx,
                                                                int& offset)
{
    ...
    for (size_t i = 0; i < mDumpingSegments.size(); i++) {
        ...
        OperationWriterPtr opWriter = DYNAMIC_POINTER_CAST(OperationWriter, mDumpingSegments[i]->GetOperationWriter());
        if (!opWriter) {
...
bool OperationIterator::LocateStartLoadPositionInBuildingSegment(const OperationCursor& skipCursor, int& segIdx,
                                                                 int& offset)
{
    if (!mBuildingSegment) {
        return false;
...
SegmentOperationIteratorPtr OperationIterator::LoadSegIterator(int segmentIdx, int offset)
{
    ...
    if (segmentIdx == (int)(mSegDataVec.size() + mDumpingSegments.size())) {
        ...
        iter = operationWriter->CreateSegmentOperationIterator(mBuildingSegment->GetSegmentId(), offset, mTimestamp);
        ...
        iter = operationWriter->CreateSegmentOperationIterator(dumpingSegment->GetSegmentId(), offset, mTimestamp);
    } else {
        NormalSegmentOperationIteratorPtr normalSegIter(new NormalSegmentOperationIterator(
            mSchema, mSegMetaVec[segmentIdx], mSegDataVec[segmentIdx].GetSegmentId(), offset, mTimestamp));
...
segmentid_t OperationIterator::GetCurrentSegment() const
{
    if (!mCurrentSegIter) {
        return INVALID_SEGMENTID;
    }
    return mCurrentSegIter->GetSegmentId();
}

void OperationIterator::ExtractUnObsolteRtSegmentDatas(const PartitionDataPtr& partitionData, int64_t timestamp,
                                                       SegmentDataVector& segDatas)
{
    SegmentIteratorPtr builtSegIter = partitionData->CreateSegmentIterator()->CreateIterator(SIT_BUILT);
    assert(builtSegIter);
...
size_t OperationIterator::GetMaxUnObsoleteSegmentOperationSize(const PartitionDataPtr& partitionData,
...
size_t OperationIterator::GetTotalBuildingOperationCntFromCursor(const index_base::PartitionDataPtr& partitionData,
                                                                 const OperationCursor& cursor)
{
    size_t totalOperationCnt = 0;
    ...
    while (buildingSegIter->IsValid()) {
        ...
        segmentid_t inMemSegId = -1;
        InMemorySegmentPtr inMemSegment = buildingSegIter->GetInMemSegment();
        if (inMemSegment != nullptr) {
            opWriter = DYNAMIC_POINTER_CAST(OperationWriter, inMemSegment->GetOperationWriter());
            inMemSegId = inMemSegment->GetSegmentId();
        }
        if (opWriter != nullptr) {
            uint64_t cnt = opWriter->GetOperationMeta().GetOperationCount();
            if (inMemSegId == cursor.segId) {
                if (cursor.pos == -1) {
                    totalOperationCnt += cnt;
                } else {
                    totalOperationCnt += cnt - cursor.pos;
                }
            } else if (inMemSegId > cursor.segId) {
                totalOperationCnt += cnt;
            }
        }
        buildingSegIter->MoveToNext();
    }
    return totalOperationCnt;
}

void OperationIterator::LoadOperationMeta(const SegmentDataVector& segDataVec, OperationMetaVector& segMetaVec)
{
...
void OperationIterator::AppendInMemOperationMeta(const PartitionDataPtr& partitionData, OperationMetaVector& segMetaVec)
{
    SegmentIteratorPtr buildingSegIter = partitionData->CreateSegmentIterator()->CreateIterator(SIT_BUILDING);
    assert(buildingSegIter);
    while (buildingSegIter->IsValid()) {
...
void OperationIterator::InitBuiltSegments(const PartitionDataPtr& partitionData, const OperationCursor& skipCursor)
{
    ...
    while (builtSegIter->IsValid()) {
        const SegmentData& segData = builtSegIter->GetSegmentData();
        if (segData.GetSegmentInfo()->timestamp < mTimestamp || segData.GetSegmentId() < skipCursor.segId) {
            builtSegIter->MoveToNext();
            continue;
        }
        DirectoryPtr opDir = segData.GetOperationDirectory(false);
...
void OperationIterator::InitInMemSegments(const PartitionDataPtr& partitionData, const OperationCursor& skipCursor)
{
    PartitionSegmentIteratorPtr segIter = partitionData->CreateSegmentIterator();
    ...
    while (buildingSegIter->IsValid()) {
        ...
        if (inMemSegment->GetStatus() == InMemorySegment::BUILDING) {
            ...
            IE_LOG(INFO, "load building segment [%d] for redo!", inMemSegment->GetSegmentData().GetSegmentId());
            break;
        }
        // dumping segment
