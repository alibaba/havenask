...
class ReclaimRtIndexExecutorTest : public INDEXLIB_TESTBASE_WITH_PARAM<int>
{
public:
    ReclaimRtIndexExecutorTest();
    ~ReclaimRtIndexExecutorTest();

    DECLARE_CLASS_NAME(ReclaimRtIndexExecutorTest);
...
INSTANTIATE_TEST_CASE_P(BuildMode, ReclaimRtIndexExecutorTest, testing::Values(0, 1, 2));

INDEXLIB_UNIT_TEST_CASE_WITH_PARAM(ReclaimRtIndexExecutorTest, TestSimpleProcess);
IE_LOG_SETUP(partition, ReclaimRtIndexExecutorTest);

ReclaimRtIndexExecutorTest::ReclaimRtIndexExecutorTest() {}

ReclaimRtIndexExecutorTest::~ReclaimRtIndexExecutorTest() {}

void ReclaimRtIndexExecutorTest::CaseSetUp()
{
...
void ReclaimRtIndexExecutorTest::CaseTearDown() {}

void ReclaimRtIndexExecutorTest::TestSimpleProcess()
{
    ...
    string incDocString = "cmd=add,string1=pk5,ts=7,long1=2;"
                          "cmd=add,string1=pk6,ts=8,long1=2;"
                          "cmd=add,string1=pk7,ts=9,long1=2;";

    string rtDocString = "cmd=add,string1=pk5,ts=5,long1=3;"
                         "cmd=add,string1=pk6,ts=5,long1=3;"
                         "cmd=add,string1=pk7,ts=5,long1=3;";

    INDEXLIB_TEST_TRUE(psm.Transfer(PE_BUILD_INC, incDocString, "", ""));
    INDEXLIB_TEST_TRUE(psm.Transfer(PE_BUILD_RT, rtDocString, "pk:pk5", "long1=3"));
    ...
    // old reader will seek rt value
    INDEXLIB_TEST_TRUE(psm.Transfer(QUERY, "", "pk:pk5", "long1=3"));
    ...
    {
        QueryPtr query = QueryParser::Parse("pk:pk5", oldReader);
        ASSERT_TRUE(query);
    ...
    // branch info is delete, use inc value
    {
        ...
        ResultPtr expectResult = DocumentParser::ParseResult("long1=2");
        ASSERT_TRUE(ResultChecker::Check(result, expectResult));
    }
}
