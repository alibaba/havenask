...
{
public:
    PrejoinExecutor(const JoinSegmentWriterPtr& joinSegWriter) : mJoinSegWriter(joinSegWriter) {}
    ~PrejoinExecutor() {}

    bool Execute(ExecutorResource& resource) override;

    void Drop(ExecutorResource& resource) override { mJoinSegWriter.reset(); }
