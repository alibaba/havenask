...
void SpatialIndexInteTest::CaseSetUp()
{
    ...
    mEnableFileCompress = GET_PARAM_VALUE(0);
    size_t configIdx = GET_PARAM_VALUE(1);
    string loadConfig;
    if (configIdx == 0) {
        loadConfig = R"(
        {
    ...
    string jsonStr = loadConfig;
    IndexPartitionOptions options;
    FromJsonString(options.GetOnlineConfig().loadConfigList, jsonStr);
    mOptions = options;
    if (mEnableFileCompress) {
        string compressorStr = "compressor:zstd;";
        string indexCompressStr = "spatial_index:compressor";
        string attrCompressStr = "coordinate:compressor";
...
void SpatialIndexInteTest::TestPolygonSearch()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 100000000000.0, 5.0, "polygon_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "polygon_index:polygon(0.0 40.0,40.0 40.0,40.0 0.0,0.0 0.0,0 40.0)",
                             "docid=0|polygon=0.1 30.1,30.1 30.1,30.1 0.1,0.1 0.1,0.1 30.1"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
    ASSERT_TRUE(
        psm.Transfer(QUERY, "", "polygon_index:polygon(0.0 0.0,0.0 45.0,45.0 45.0,45.0 0.0,0.0 0.0)", "docid=0"));

    ASSERT_TRUE(
        psm.Transfer(QUERY, "", "polygon_index:polygon(0.0 17.0,0.0 21.0,11.0 21.0,11.0 17.0,0.0 17.0)", "docid=0"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "polygon_index:point(11.0 21.0)", "docid=0"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "polygon_index:point(29.0 21.0)", "docid=0"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "polygon_index:point(40.0 50.0)", ""));

    ASSERT_TRUE(
        psm.Transfer(QUERY, "", "polygon_index:polygon(0.0 17.0,0.0 21.0,11.0 21.0,11.0 17.0,0.0 17.0)", "docid=0"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "polygon_index:line(20.0 50.0,10.0 17.0,35.0 40.0,45.0 17.0)", "docid=0"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "polygon_index:line(20.0 50.0,40.0 41.0,45.0 50.0)", ""));
}

void SpatialIndexInteTest::TestPolygonLeafSearch()
{
    string docString =
        "cmd=add|pk=1|polygon=97.39374 33.844938,97.780536 33.367,97.44659 32.979707,97.543289 32.563474,98.338709 "
        "32.354629,98.736419 31.653433,98.803438 30.924593,99.012729 29.760153,99.205265 28.209051,99.60915 "
        "28.858578,100.294491 28.295606,100.241654 27.818837,100.742579 27.929667,101.212768 26.932207,101.711534 "
        "26.154332,102.664043 26.355593,103.008461 26.456092,102.785162 27.329166,103.332692 27.685353,103.562512 "
        "28.057092,103.897149 28.281188,103.836277 28.736268,104.399484 28.402617,104.505306 28.002845,105.217396 "
        "28.112932,105.573442 27.74094,106.234863 27.865706,106.301901 28.083233,106.027804 28.11434,105.671028 "
        "28.362164,105.976039 28.717356,106.743778 28.470421,107.215539 28.732788,107.53931 29.017556,107.920922 "
        "29.198015,108.551181 28.82716,108.910256 28.293549,109.353465 28.451598,109.267453 29.031248,108.960774 "
        "29.511261,108.499818 29.902915,108.401176 30.288086,108.835253 30.517971,109.579636 30.670516,110.127608 "
        "30.935354,110.357649 31.405671,109.725599 31.639951,108.486723 32.241353,108.350683 31.908584,108.326608 "
        "31.479771,107.8092 30.888241,108.407429 32.166764,107.43213 32.46634,106.812644 32.874488,105.797651 "
        "32.764923,105.129252 32.658835,104.399544 32.975172,104.122581 33.500145,103.676483 33.724904,103.101871 "
        "33.69093,102.990346 34.293401,102.40724 34.048622,102.55514 33.450618,101.838075 33.150057,101.307246 "
        "33.313787,100.952198 32.444823,100.17967 32.868609,99.758704 32.774594,98.80927 33.338687,98.431125 "
        "34.117722,97.850617 34.140671,97.39374 33.844938";
    mSchema = RewriteSpatialConfig(mSchema, 100000, 1000, "polygon_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));

    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "polygon_index:point(103.542207 30.212242)", "docid=0"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
}
...
void SpatialIndexInteTest::TestLazyloadSpatialAttribute()
{
    ...
    offlinePartition.Open(GET_TEMP_DATA_PATH(), "", mSchema, mOptions);
...
void SpatialIndexInteTest::InnerCheckIterator(RawDocumentFieldExtractor& extractor, docid_t docId,
...
void SpatialIndexInteTest::TestLineSearch()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 100000000000.0, 500000.0, "line_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "line_index:polygon(0.0 40.0,40.0 40.0,40.0 0.0,0.0 0.0,0 40.0)",
                             "docid=0|line=0.1 30.1,30.1 30.1,30.1 0.1,0.1 0.1"));
    ASSERT_TRUE(
        psm.Transfer(QUERY, "", "line_index:point(30.1 30.1)", "docid=0|line=0.1 30.1,30.1 30.1,30.1 0.1,0.1 0.1"));
    ASSERT_TRUE(
        psm.Transfer(QUERY, "", "line_index:point(15.1 30.1)", "docid=0|line=0.1 30.1,30.1 30.1,30.1 0.1,0.1 0.1"));
}
...
void SpatialIndexInteTest::TestAttributeInputIllegal()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 100000.0, 50.0, "spatial_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
...
void SpatialIndexInteTest::TestSimpleProcess()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 6000000.0, 50.0, "spatial_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "spatial_index:point(116.3906 39.92324)",
    ...
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:point(116.3906 39.92325)", "docid=0;docid=3"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(116.00 39.00, 117.00 40.00)",
                             "docid=0,coordinate=116.3906 39.92324 116.3906 39.92325;"
                             "docid=1;docid=2;docid=3;docid=4;docid=5"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:polygon(-45 0, 0 0, 0 -45, -45 -45,-45 0)",
                             "docid=6;docid=7;docid=8;docid=9"));

    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:line(116.3906 39.92324,116.3906 39.92325,-22.5 -0.1)",
...
void SpatialIndexInteTest::TestSearchTopLevel()
{
    ...
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "", ""));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(0.00 0.00, 114.00 24.00)", "docid=0"));
}

IndexPartitionSchemaPtr SpatialIndexInteTest::RewriteSpatialConfig(const IndexPartitionSchemaPtr& schema,
                                                                   double maxSearchDist, double maxDistError,
                                                                   const string& indexName)
{
    IndexSchemaPtr indexSchema = schema->GetIndexSchema();
    SpatialIndexConfigPtr spatialIndexConfig =
        DYNAMIC_POINTER_CAST(SpatialIndexConfig, indexSchema->GetIndexConfig(indexName));
...
void SpatialIndexInteTest::TestFindPointNear180DateLine()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 100000000000.0, 200000.0, "spatial_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "spatial_index:rectangle(-170 30, 180 40)", "docid=0;docid=2"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());

    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(-170 30, -180 40)", "docid=0;docid=2"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(170 30, -170 40)", "docid=0;docid=1"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:rectangle(-170 30, 170 40)", "docid=2"));
}
...
void SpatialIndexInteTest::TestLocationOnCellRightBoundary()
{
    ...
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "spatial_index:rectangle(45 0, 90 40)", "docid=0;docid=1;"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
}
...
void SpatialIndexInteTest::TestFindPointsInCircle()
{
    ...
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "spatial_index:circle(45 29, 200000)", "docid=1;"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
}
...
void SpatialIndexInteTest::TestFindNearPole()
{
    ...
    mSchema = RewriteSpatialConfig(mSchema, 100000000000.0, 1, "spatial_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "", ""));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:point(180 90)", "docid=0;"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:point(-180 90)", "docid=1;"));
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:circle(179 90,12)", "docid=0;docid=1;"));
}
...
void SpatialIndexInteTest::TestSeekAndFilterIterator()
{
    ...
    for (double lon = -0.5; lon < 0.5; lon += 0.01) {
        for (double lat = -0.5; lat < 0.5; lat += 0.01) {
            ss << "cmd=add,pk=" << count++ << ",coordinate=" << pLon + lon << " " << pLat + lat << ";";
        }
    }

    string docString = ss.str();
    mSchema = RewriteSpatialConfig(mSchema, 10000000.0, 100000.0, "spatial_index");
    PartitionStateMachine psm;
    ASSERT_TRUE(psm.Init(mSchema, mOptions, GET_TEMP_DATA_PATH()));
    ASSERT_TRUE(psm.Transfer(BUILD_FULL, docString, "spatial_index:point(0.6 -0.2)", "docid=0"));
    CheckSpatialIndexCompressInfo(psm.GetFileSystem());
    ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:point(0.600001 -0.2)", ""));

    partition::OnlinePartitionPtr onlinePart = DYNAMIC_POINTER_CAST(OnlinePartition, psm.GetIndexPartition());
    std::shared_ptr<InvertedIndexReader> indexReader = onlinePart->GetReader()->GetInvertedIndexReader("spatial_index");
    ASSERT_TRUE(indexReader);

    Term term("circle(1.1 0.3, 22134)", "spatial_index");
    Pool pool;
    Pool* poolPtr = &pool;
    PostingIterator* iter = indexReader->Lookup(term, 1000, pt_default, poolPtr).ValueOrThrow();
    ...
    POOL_COMPATIBLE_DELETE_CLASS(poolPtr, iter);
    POOL_COMPATIBLE_DELETE_CLASS(poolPtr, cloneIter);
}

void SpatialIndexInteTest::TestLookupAsync()
{
    double pLon = 1.1;
    double pLat = 0.3;
    ...
    auto executor = util::FutureExecutor::CreateExecutor(1, 32);
    Term term("circle(1.1 0.3, 22134)", "spatial_index");
    Pool pool;
    Pool* poolPtr = &pool;
...
{
    {
        ...
        mSchema = RewriteSpatialConfig(mSchema, 6000000.0, 50.0, "spatial_index");
        PartitionStateMachine psm;
        std::string mergeConfigStr = "{\"class_name\":\"test\",\"parameters\":{\"segment_count\":\"7\"}}";
        autil::legacy::FromJsonString(mOptions.GetMergeConfig().GetSplitSegmentConfig(), mergeConfigStr);
        ...
        ASSERT_TRUE(psm.Transfer(QUERY, "", "spatial_index:point(116.3906 39.92325)", "docid=0;docid=6"));
...
void SpatialIndexInteTest::CheckSpatialIndexCompressInfo(const file_system::IFileSystemPtr& fileSystem)
{
    auto partData = OnDiskPartitionData::CreateOnDiskPartitionData(fileSystem);
    if (mEnableFileCompress) {
        CheckFileCompressInfo(partData, "index/spatial_index/dictionary", "zstd");
        CheckFileCompressInfo(partData, "index/spatial_index/posting", "zstd");
        CheckFileCompressInfo(partData, "attribute/coordinate/offset", "zstd");
        CheckFileCompressInfo(partData, "attribute/coordinate/data", "zstd");
    } else {
        CheckFileCompressInfo(partData, "index/spatial_index/dictionary", "null");
        CheckFileCompressInfo(partData, "index/spatial_index/posting", "null");
        CheckFileCompressInfo(partData, "attribute/coordinate/offset", "null");
        CheckFileCompressInfo(partData, "attribute/coordinate/data", "null");
    }
}

void SpatialIndexInteTest::CheckFileCompressInfo(const PartitionDataPtr& partitionData, const string& filePath,
