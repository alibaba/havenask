 ...
 */
#include "indexlib/index/normal/inverted_index/builtin_index/dynamic/dynamic_search_tree_node.h"
...
string InternalNode::DebugString() const noexcept
{
    string res;
...
string LeafNode::DebugString() const noexcept
{
    string res;
    for (slotid_t i = 0; i < _keySlotUse; ++i) {
        if (!Key(i).IsDelete()) {
