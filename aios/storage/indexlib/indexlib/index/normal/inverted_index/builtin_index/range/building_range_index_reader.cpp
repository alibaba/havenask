                                      ...
                                      mem_pool::Pool* sessionPool)
{
    for (size_t i = 0; i < _innerSegReaders.size(); ++i) {
        shared_ptr<SegmentPostings> segPosting(new SegmentPostings);
        RangeInMemSegmentReader* innerReader = (RangeInMemSegmentReader*)_innerSegReaders[i].second.get();
        if (innerReader->Lookup(leftTerm, rightTerm, _innerSegReaders[i].first, segPosting, sessionPool) &&
