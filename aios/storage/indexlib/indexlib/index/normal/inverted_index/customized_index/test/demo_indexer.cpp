...
bool DemoIndexer::Dump(const file_system::DirectoryPtr& indexDir)
{
    ...
    try {
        ...
        IE_LOG(ERROR, "fail to store meta file [%s], error: [%s]", META_FILE_NAME.c_str(), e.what());
        return false;
    }
    return true;
}
...
{
    return DemoInMemSegmentRetrieverPtr(new DemoInMemSegmentRetriever(&mDataMap, mParameters));
}
