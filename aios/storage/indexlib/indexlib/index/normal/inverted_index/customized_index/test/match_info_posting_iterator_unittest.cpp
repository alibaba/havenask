...
vector<SegmentMatchInfo> MatchInfoPostingIteratorTest::MakeSegMatchInfos(const vector<MatchInfoTuple>& matchInfos,
...
void MatchInfoPostingIteratorTest::TestSeekOneSegment()
{
    vector<SegmentMatchInfo> segMatchInfos = MakeSegMatchInfos(
        ...
        &mPool);
    std::shared_ptr<PostingIterator> iter(new MatchInfoPostingIterator(segMatchInfos, &mPool));
...
void MatchInfoPostingIteratorTest::TestSeekMultiSegments()
{
    vector<SegmentMatchInfo> segMatchInfos = MakeSegMatchInfos(
        ...
        &mPool);
    std::shared_ptr<PostingIterator> iter(new MatchInfoPostingIterator(segMatchInfos, &mPool));
    InnerTestSeekDoc(iter, INVALID_DOCID, {0, 1, 4, 99, 100, 101, 104, 199, 200, 201, 204, 299},
...
void MatchInfoPostingIteratorTest::InnerTestSeekDoc(const std::shared_ptr<PostingIterator>& iter, docid_t startDocId,
