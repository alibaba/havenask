...
void TrieIndexInteTest::TestLookup()
{
    ...
    provider.Init(GET_TEMP_DATA_PATH(), "string", SFP_TRIE);
...
bool TrieIndexInteTest::CheckResult(TrieIndexReader::Iterator* iter, const string& expectStr)
{
...
bool TrieIndexInteTest::CheckPrefixSearch(const TrieIndexReader& reader, const string& key, const string& expectStr,
                                          int32_t maxMatches)
{
    Pool pool;
    TrieIndexReader::Iterator* iter = NULL;
    if (maxMatches >= 0) {
        iter = reader.PrefixSearch(StringView(key), &pool, maxMatches);
        if (iter->Size() > (size_t)maxMatches) {
            EXPECT_TRUE(iter->Size() <= (size_t)maxMatches);
            return false;
        }
    } else {
        iter = reader.PrefixSearch(StringView(key), &pool);
    }
    return CheckResult(iter, expectStr);
}

bool TrieIndexInteTest::CheckInfixSearch(const TrieIndexReader& reader, const string& key, const string& expectStr,
                                         int32_t maxMatches)
{
    Pool pool;
    TrieIndexReader::Iterator* iter = NULL;
    if (maxMatches >= 0) {
        iter = reader.InfixSearch(StringView(key), &pool, maxMatches);
        if (iter->Size() > (size_t)maxMatches) {
            EXPECT_TRUE(iter->Size() <= (size_t)maxMatches);
            return false;
        }
    } else {
        iter = reader.InfixSearch(StringView(key), &pool);
    }
    return CheckResult(iter, expectStr);
}
...
void TrieIndexInteTest::TestDuplicateAdd()
{
    ...
    DeletionMapReaderPtr deleteReader = provider.GetPartitionData()->GetDeletionMapReader();
