...
void DoubleArrayTrie::FindChildNodes(const KVPairVector& sortedKVPairVector, const Node& parentNode,
                                     NodeVector& childNodes)
{
    ...
    for (size_t i = parentNode.left; i < parentNode.right; ++i) {
        const StringView& key = sortedKVPairVector[i].first;
...
size_t DoubleArrayTrie::FindBegin(const KVPairVector& sortedKVPairVector, const NodeVector& childNodes)
{
    size_t begin = 0;
    size_t nonZeroNum = 0;
    bool first = false;
...
size_t DoubleArrayTrie::Insert(const KVPairVector& sortedKVPairVector, const NodeVector& childNodes)
{
    if (mError) {
        return 0;
    }
...
autil::StringView DoubleArrayTrie::Data() const
{
    return autil::StringView((const char*)mStateArray.data(), mSize * sizeof(State));
}
