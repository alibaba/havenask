...
void IndexDocumentMaker::UpdateAnswer(const string& key, docid_t docId, pos_t globalPos, pospayload_t posPayload,
...
void IndexDocumentMaker::CreateSections(uint32_t sectionNum, vector<Section*>& sections, docid_t docId, Answer* answer,
...
IndexTokenizeField* IndexDocumentMaker::CreateField(vector<Section*>& sectionVec, autil::mem_pool::Pool* pool)
{
...
IndexDocumentPtr IndexDocumentMaker::MakeIndexDocument(Pool* pool, const IndexDocument::FieldVector& fieldVec,
                                                       docid_t docId, Answer* answer,
                                                       const HashKeyToStrMap& hashKeyToStrMap, TokenType tokenType,
                                                       uint32_t* basePos)
{
...
IndexDocumentPtr IndexDocumentMaker::MakeIndexDocument(Pool* pool, const Field* field, docid_t docId,
                                                       TokenType tokenType)
{
...
void IndexDocumentMaker::MakeIndexDocuments(Pool* pool, vector<IndexDocumentPtr>& indexDocs, uint32_t docNum,
...
void IndexDocumentMaker::RewriteSectionAttributeInIndexDocuments(vector<document::IndexDocumentPtr>& indexDocs,
...
void IndexDocumentMaker::AddDocsToWriter(vector<IndexDocumentPtr>& indexDocs, IndexWriter& writer)
{
...
void IndexDocumentMaker::AddFieldsToWriter(IndexDocument::FieldVector& fieldVec, IndexWriter& writer, docid_t begDocId,
                                           bool empty)
{
    Pool pool;
...
void IndexDocumentMaker::CleanDir(const string& dir)
{
    ...
    if (FslibWrapper::IsExist(sectionPath).GetOrThrow()) {
        FslibWrapper::DeleteDirE(sectionPath, DeleteOption::NoFence(false));
    }
}
...
void IndexDocumentMaker::UpdateFieldMapInAnswer(const IndexDocument::FieldVector& fieldVec, docid_t docId,
                                                PackageIndexConfigPtr& indexConfig, Answer* answer)
{
...
void IndexDocumentMaker::CreatePackageIndexConfig(std::shared_ptr<PackageIndexConfig>& indexConf,
                                                  IndexPartitionSchemaPtr& schema, uint32_t fieldNum,
                                                  InvertedIndexType indexType, const string& indexName)
{
...
void IndexDocumentMaker::CreatePackageIndexConfig(std::shared_ptr<indexlibv2::config::PackageIndexConfig>& packConf,
                                                  IndexPartitionSchemaPtr& schema, uint32_t fieldNum,
                                                  InvertedIndexType indexType, const string& indexName)
{
    schema.reset(new IndexPartitionSchema);
    ...
    auto indexConf = make_shared<PackageIndexConfig>(indexName, indexType);
    indexConf->SetFieldSchema(schema->GetFieldSchema());
    for (uint32_t i = 0; i < fieldNum; i++) {
        stringstream ss;
        ss << "field" << i;
    ...
    schema->AddIndexConfig(indexConf);
    auto configV2 = indexConf->ConstructConfigV2();
    packConf = std::shared_ptr<indexlibv2::config::PackageIndexConfig>(
        dynamic_cast<indexlibv2::config::PackageIndexConfig*>(configV2.release()));
}

void IndexDocumentMaker::CreateSingleFieldIndexConfig(SingleFieldIndexConfigPtr& indexConfig, const string& indexName,
                                                      InvertedIndexType indexType, FieldType type,
                                                      optionflag_t optionFlag)
{
    ...
    if (indexType == it_number) {
        InvertedIndexType indexType = config::NumberIndexTypeTransformor::TransformFieldTypeToIndexType(type);
        indexConfig->SetInvertedIndexType(indexType);
    }
}

void IndexDocumentMaker::CreateSingleFieldIndexConfig(
    std::shared_ptr<indexlibv2::config::SingleFieldIndexConfig>& indexConfig, const string& indexName,
    InvertedIndexType indexType, FieldType type, optionflag_t optionFlag)
{
    indexConfig.reset(new indexlibv2::config::SingleFieldIndexConfig(indexName, indexType));
    FieldConfigPtr fieldConfig(new FieldConfig("field0", type, false));
    fieldConfig->SetFieldId(0);
    indexConfig->SetOptionFlag(optionFlag);
    [[maybe_unused]] auto status = indexConfig->SetFieldConfig(fieldConfig);
    assert(status.IsOK());
