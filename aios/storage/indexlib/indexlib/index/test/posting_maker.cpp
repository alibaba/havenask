...
PostingMaker::PostingList PostingMaker::MakePostingData(const vector<uint32_t>& numDocsInRecords, docid_t& maxDocId,
                                                        tf_t& totalTf)
{
    vector<string> strs;
    docid_t oldDocId = 0;
...
PostingMaker::PostingList PostingMaker::SortByWeightMergePostingLists(vector<PostingMaker::PostingList>& postLists,
                                                                      const ReclaimMap& reclaimMap)
{
    ...
    for (size_t i = 0; i < recordNum; i++) {
        ...
        for (size_t j = 0; j < MAX_DOC_PER_RECORD; j++) {
            ...
            DocKey docKey = make_pair(docInfos[index].mNewDocId, docInfos[index].mDocPayload);
...
PostingMaker::PostingList PostingMaker::MergePostingLists(vector<PostingMaker::PostingList>& postLists)
{
    PostingList mergedList;
    for (vector<PostingList>::const_iterator it = postLists.begin(); it != postLists.end(); ++it) {
    ...
    if (mergedList.second.empty()) {
        ...
        for (; it != postList.second.end(); ++it) {
            ...
            for (DocRecord::const_iterator recordIt = it->begin(); recordIt != it->end(); ++recordIt) {
                record.insert(
                    make_pair(make_pair(recordIt->first.first + newBaseId, recordIt->first.second), recordIt->second));
            }
            mergedList.second.push_back(record);
        }
    } else {
...
PostingMaker::PostingList PostingMaker::MakePositionData(const vector<uint32_t>& numDocsInRecords, docid_t& maxDocId,
                                                         uint32_t& totalTF)
{
    vector<string> strs;
    docid_t oldDocId = 0;
...
/////////////////////////////////////////////////////////////////
// inner functions
void PostingMaker::ParseKeyValue(const string& str, const string& splite, string& key, string& value)
{
