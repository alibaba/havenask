...
IE_LOG_SETUP(index, SortCollectInfoProcessor);

SortCollectInfoProcessor::SortCollectInfoProcessor() {}

SortCollectInfoProcessor::~SortCollectInfoProcessor() {}

bool SortCollectInfoProcessor::Init(const KKVIndexConfigPtr& kkvConfig)
{
...
uint32_t SortCollectInfoProcessor::Process(SKeyCollectInfos& skeyInfos, uint32_t validSkeyCount)
{
    uint32_t flushCount = (uint32_t)skeyInfos.size();
    ...
    sort(beginIter, beginIter + flushCount, mCollectComp);
    if (skipFirstNode) {
        ++flushCount;
    }
    return flushCount;
