...
{
    {
        IndexPartitionSchemaPtr schema = SchemaMaker::MakeSchema("pk:int32;", "pk:primarykey64:pk", "", "");
    ...
    {
        IndexPartitionSchemaPtr schema = SchemaMaker::MakeKVSchema("key:string;value:uint64;", "key", "value");
        DocumentFactoryWrapper wrapper(schema);
        wrapper.Init();
        DocumentParser* parser = wrapper.CreateDocumentParser();
    ...
    }
    {
        IndexPartitionSchemaPtr schema =
            SchemaMaker::MakeKKVSchema("pkey:uint64;skey:uint32;value:uint32;", "pkey", "skey", "value");
        DocumentFactoryWrapper wrapper(schema);
        wrapper.Init();
        DocumentParser* parser = wrapper.CreateDocumentParser();
...
void DocumentFactoryWrapperTest::TestInitPluginInFactory()
{
    ...
    string pluginPath = GET_PRIVATE_TEST_DATA_PATH() + "/document/plugins";
    DocumentFactoryWrapperPtr wrapper1 =
        DocumentFactoryWrapper::CreateDocumentFactoryWrapper(schema, CUSTOMIZED_DOCUMENT_CONFIG_PARSER, pluginPath);
    DocumentFactoryWrapperPtr wrapper2 = DocumentFactoryWrapper::CreateDocumentFactoryWrapper(
