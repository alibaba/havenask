...
TEST_F(KeyMapManagerTest, testSimple)
{
    ...
    hashMapPtr0->insert(StringView(string("key0")));
    hashMapPtr0->insert(StringView(string("key1")));
    hashMapPtr1->insert(StringView(string("key1")));
    ...
    for (size_t i = 0; i < maxKeySize; ++i) {
        hashMapPtr0->insert(StringView(string("key") + StringUtil::toString(i + 2)));
    }
    ...
    hashMapPtr1->insert(StringView(string("newKey0")));
    hashMapManager.updatePrimary(hashMapPtr1);
    ASSERT_FALSE(hashMapManager._fastUpdateable);
