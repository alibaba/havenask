...
{
    IndexPartitionSchemaPtr schema = test::SchemaMaker::MakeSchema("string1:string;", "index1:string:string1;", "", "");

    fieldid_t tsFieldId = 1;
    TimestampDocumentRewriter rewriter(tsFieldId);
    NormalDocumentPtr doc = DocumentCreator::CreateNormalDocument(schema, "cmd=add,string1=1,ts=1,long1=0;");
