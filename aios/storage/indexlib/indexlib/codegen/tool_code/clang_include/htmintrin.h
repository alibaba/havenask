...
static __inline int __attribute__((__always_inline__, __nodebug__)) __builtin_tbegin_retry_null(int __retry)
{
    int cc, i = 0;
...
static __inline int __attribute__((__always_inline__, __nodebug__)) __builtin_tbegin_retry_tdb(void* __tdb, int __retry)
{
    int cc, i = 0;

    while ((cc = __builtin_tbegin(__tdb)) == _HTM_TBEGIN_TRANSIENT && i++ < __retry)
        __builtin_tx_assist(i);
...
#define __builtin_tbegin_retry(tdb, retry)                                                                             \
    (__builtin_constant_p(tdb == 0) && tdb == 0 ? __builtin_tbegin_retry_null(retry)                                   \
                                                : __builtin_tbegin_retry_tdb(tdb, retry))

static __inline int __attribute__((__always_inline__, __nodebug__)) __builtin_tbegin_retry_nofloat_null(int __retry)
{
    int cc, i = 0;

    while ((cc = __builtin_tbegin_nofloat(0)) == _HTM_TBEGIN_TRANSIENT && i++ < __retry)
        __builtin_tx_assist(i);
...
__builtin_tbegin_retry_nofloat_tdb(void* __tdb, int __retry)
{
    ...
    while ((cc = __builtin_tbegin_nofloat(__tdb)) == _HTM_TBEGIN_TRANSIENT && i++ < __retry)
        __builtin_tx_assist(i);
...
#define __builtin_tbegin_retry_nofloat(tdb, retry)                                                                     \
    (__builtin_constant_p(tdb == 0) && tdb == 0 ? __builtin_tbegin_retry_nofloat_null(retry)                           \
