...
void PartitionStateMachineTest::TestSimpleProcess()
{
    ...
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_FULL, docString, "pk:pk1", "long1=2,docid=0"));
    ...
    INDEXLIB_TEST_TRUE(
        psm.Transfer(BUILD_INC, "cmd=add,string1=pk3,long1=3,ts=103,locator=0:105", "pk:pk2", "long1=4,docid=1"));

    RESET_FILE_SYSTEM();
    directory = GET_PARTITION_DIRECTORY();
    ...
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_RT, "cmd=add,string1=pk3,long1=30,ts=200", "pk:pk3", "long1=30,docid=4"));
}
...
void PartitionStateMachineTest::TestInvertedIndex()
{
    ...
    PartitionStateMachine psm;
    INDEXLIB_TEST_TRUE(psm.Init(schema, options, GET_TEMP_DATA_PATH()));
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_FULL, docString, "pack1:t1", "docid=0,pos=0#2;"));
}
...
void PartitionStateMachineTest::TestDocIdQuery()
{
    ...
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_FULL, docString, "__docid:0", "docid=0,long1=1,text1=t1;"));
}
...
void PartitionStateMachineTest::TestBuildWithBinaryString()
{
    ...
    vector<NormalDocumentPtr> docs = DocumentCreator::CreateNormalDocuments(schema, fullDocStr, true);
    ...
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_FULL, docBinaryStr, "pk:pk1", "long1=2,docid=0"));

    string incDocStr = "cmd=add,string1=pk3,long1=3";
    vector<NormalDocumentPtr> incDocs = DocumentCreator::CreateNormalDocuments(schema, incDocStr, true);
    string incDocBinaryStr = PartitionStateMachine::SerializeDocuments(incDocs);

    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_INC, incDocBinaryStr, "pk:pk2", "long1=4,docid=1"));

    string rtDocStr = "cmd=add,string1=pk3,long1=30,ts=200";
    vector<NormalDocumentPtr> rtDocs = DocumentCreator::CreateNormalDocuments(schema, rtDocStr, true);
    string rtDocBinStr = PartitionStateMachine::SerializeDocuments(rtDocs);

    // not support inc reopen
    INDEXLIB_TEST_TRUE(psm.Transfer(BUILD_RT, rtDocBinStr, "pk:pk3", "long1=30,docid=3"));
}
