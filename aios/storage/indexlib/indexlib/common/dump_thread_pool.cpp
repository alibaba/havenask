...
DumpThreadPool::~DumpThreadPool() { stop(STOP_AND_CLEAR_QUEUE_IGNORE_EXCEPTION); }
...
bool DumpThreadPool::createThreads(const string& name)
{
    ...
    while (num--) {
        autil::ThreadPtr tp = autil::Thread::createThread(std::bind(&DumpThreadPool::workerLoop, this), name);
