...
#define IE_ROOT_LOG_SETLEVEL(level) alog::Logger::getRootLogger()->setLevel(alog::LOG_LEVEL_##level)
#define IE_LOG_DECLARE() static alog::Logger* _logger
#define IE_LOG_SETUP(n, c) alog::Logger* c::_logger = alog::Logger::getLogger("indexlib." #n "." #c)
#define IE_LOG_TEST()                                                                                                  \
    do {                                                                                                               \
        std::cout << "*** in log test of alog ***" << endl;                                                            \
    } while (0)

#define IE_LOG_SETUP_TEMPLATE(n, c)                                                                                    \
    template <typename T>                                                                                              \
    alog::Logger* c<T>::_logger = alog::Logger::getLogger("indexlib." #n "." #c)

#define IE_LOG_SETUP_TEMPLATE2(n, c)                                                                                   \
    template <typename T1, typename T2>                                                                                \
    alog::Logger* c<T1, T2>::_logger = alog::Logger::getLogger("indexlib." #n "." #c)

#define IE_LOG_SETUP_TEMPLATE3(n, c)                                                                                   \
    template <typename T1, typename T2, typename T3>                                                                   \
    alog::Logger* c<T1, T2, T3>::_logger = alog::Logger::getLogger("indexlib." #n "." #c)

#define IE_LOG_SETUP_TEMPLATE_CUSTOM(TT1, n, c)                                                                        \
    template <TT1 T1>                                                                                                  \
    alog::Logger* c<T1>::_logger = alog::Logger::getLogger("indexlib." #n "." #c)
...
#define IE_DECLARE_AND_SETUP_LOGGER(n) static alog::Logger* _logger = alog::Logger::getLogger("indexlib." #n)
...
{
public:
    ScopedLogLevel(uint32_t logLevel, const std::string& loggerName = "indexlib") : mLoggerName(loggerName)
    {
        mOriginalLevel = alog::Logger::getLogger(mLoggerName.c_str())->getLevel();
        alog::Logger::getLogger(mLoggerName.c_str())->setLevel(logLevel);
    }
    ~ScopedLogLevel() { alog::Logger::getLogger(mLoggerName.c_str())->setLevel(mOriginalLevel); }
