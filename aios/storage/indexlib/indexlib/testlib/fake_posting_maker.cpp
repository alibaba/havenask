...
IE_LOG_SETUP(testlib, FakePostingMaker);

FakePostingMaker::FakePostingMaker() {}

FakePostingMaker::~FakePostingMaker() {}

void FakePostingMaker::parseOnePostingDetail(const string& postingStr, FakePostingIterator::Posting& posting)
{
    ...
    if (docPayloadPos != string::npos) {
        posting.docPayload = StringUtil::fromString<docpayload_t>(docInfo.substr(docPayloadPos + 1));
    }

    StringTokenizer st(posInfo, ",", StringTokenizer::TOKEN_IGNORE_EMPTY | StringTokenizer::TOKEN_TRIM);
    for (size_t i = 0; i < st.getNumTokens(); ++i) {
        ...
        if (posPayloadPos != string::npos) {
            posting.fieldBitArray.push_back(StringUtil::fromString<int32_t>(onePos.substr(posPayloadPos + 1)));
        }
    }
}

void FakePostingMaker::makeFakePostingsDetail(const string& str, FakePostingIterator::Map& postingMap)
{
    ...
    for (size_t i = 0; i < st.getNumTokens(); ++i) {
        ...
        StringTokenizer st1(postingsStr, ";", StringTokenizer::TOKEN_IGNORE_EMPTY | StringTokenizer::TOKEN_TRIM);
        FakePostingIterator::Postings pts;
        for (size_t j = 0; j < st1.getNumTokens(); ++j) {
            const string& postingStr = st1[j];
            FakePostingIterator::Posting pt;
...
void FakePostingMaker::makeFakePostingsSection(const std::string& str, FakePostingIterator::Map& postingMap)
{
    size_t startPos = 0;
    while (true) {
...
void FakePostingMaker::parseOnePostingSection(const std::string& postingStr, FakePostingIterator::Posting& posting)
{
