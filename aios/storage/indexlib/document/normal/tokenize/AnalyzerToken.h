...
public:
    struct TokenProperty {
        ...
        inline uint8_t& getValue() { return *(uint8_t*)this; }
        inline uint8_t getValue() const { return *(uint8_t*)this; }

        void serialize(autil::DataBuffer& dataBuffer) const { dataBuffer.write(getValue()); }
        void deserialize(autil::DataBuffer& dataBuffer) { dataBuffer.read(getValue()); }

        bool operator==(const TokenProperty& other) const { return getValue() == other.getValue(); }
    };

public:
    AnalyzerToken();
    AnalyzerToken(const std::string& text, uint32_t pos);
    AnalyzerToken(const std::string& text, uint32_t pos, const std::string& normalizedText, bool isStopWord = false);
...
public:
    ...
    void setNormalizedText(const std::string& normalizedText) { _normalizedText = normalizedText; }
...
{
    return left._text == right._text && left._normalizedText == right._normalizedText && left._pos == right._pos &&
           left._tokenProperty == right._tokenProperty && left._posPayLoad == right._posPayLoad;
}
} // namespace document
} // namespace indexlib
