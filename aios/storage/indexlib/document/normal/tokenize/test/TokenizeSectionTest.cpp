...
void TokenizeSectionTest::CaseSetUp() { _tokenNodeAllocatorPtr.reset(new TokenNodeAllocator); }
...
TEST_F(TokenizeSectionTest, testAllocator)
{
    ...
    autil::ThreadPtr threads[THREAD_COUNT];

    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
        threads[i] = autil::Thread::createThread(bind(&(TokenizeSectionTest::allocate)));
    }
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
        threads[i]->join();
    }
...
{
    std::shared_ptr<TokenNodeAllocator> allocator = TokenNodeAllocatorPool::getAllocator();
