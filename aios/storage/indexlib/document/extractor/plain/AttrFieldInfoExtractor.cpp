...
AUTIL_LOG_SETUP(indexlib.document, AttrFieldInfoExtractor);
...
Status AttrFieldInfoExtractor::ExtractField(IDocument* doc, void** field)
{
    ...
    if (!fieldInfo) {
        AUTIL_LOG(ERROR, "can not extract attribute field information from fieldId [%d].", _fieldId);
        return Status::Unknown("can not extract attribute field information from fieldId [%d].", _fieldId);
    }
