...
AUTIL_LOG_SETUP(indexlib.config, IndexPartitionSchemaImpl);
...
{
    if (GetRegionCount() > 1) {
        AUTIL_LOG(ERROR, "not support multi region schema");
...
void IndexPartitionSchemaImpl::Jsonize(autil::legacy::Jsonizable::JsonWrapper& json)
{
    if (json.GetMode() == TO_JSON) {
        ...
        if (iter != jsonMap.end()) {
            ...
            AUTIL_LOG(INFO, "auto_update_preference will be set false"
...
void IndexPartitionSchemaImpl::AssertCompatible(const IndexPartitionSchemaImpl& other) const
{
    try {
        ...
        AUTIL_LOG(ERROR, "Schema is not compatible: %s", e.GetMessage().c_str());
        if (mThrowAssertCompatibleException) {
            throw e;
        }
    } catch (const ExceptionBase& e) {
        AUTIL_LOG(ERROR, "Exception: %s", e.GetMessage().c_str());
        if (mThrowAssertCompatibleException) {
            throw e;
        }
    }
...
void IndexPartitionSchemaImpl::DoAssertCompatible(const IndexPartitionSchemaImpl& other) const
{
    ...
    if (other.mModifyOperations.size() != mModifyOperations.size()) {
        ...
        try {
            ...
            AUTIL_LOG(ERROR, "base schema not equal for modify operation, ExceptionBase: %s", e.GetMessage().c_str());
...
bool IndexPartitionSchemaImpl::Str2TableType(const std::string& str, TableType& tableType)
{
    if (str == "" || str == TABLE_TYPE_INDEX) {
        ...
        AUTIL_LOG(ERROR, "unrecognized table type [%s]", str.c_str());
        return false;
    }
    return true;
}
...
bool IndexPartitionSchemaImpl::GetValueFromUserDefinedParam(const string& key, string& value) const
{
    ...
    if (jsonMap.end() == it) {
        AUTIL_LOG(INFO, "[%s] not found in schema", key.c_str());
        return false;
    }
    try {
        value = AnyCast<string>(it->second);
    } catch (const exception& e) {
        AUTIL_LOG(ERROR, "cast [%s] failed, exception happened [%s]", key.c_str(), e.what());
        return false;
    }
    return true;
}
...
void IndexPartitionSchemaImpl::AddSummaryConfig(const string& fieldName, index::summarygroupid_t summaryGroupId,
                                                regionid_t id)
{
    RegionSchemaPtr region = GetRegionSchema(id);
    if (!region && id == DEFAULT_REGIONID) {
...
{
    if (regionId < 0 || regionId >= (regionid_t)mRegionVector.size()) {
        AUTIL_LOG(ERROR, "regionId [%d] out of range!", regionId);
        return;
    }
...
bool IndexPartitionSchemaImpl::MarkOngoingModifyOperation(schema_opid_t opId)
{
    ...
    if (!op) {
        AUTIL_LOG(ERROR, "get schema modify operation [id:%d] fail!", opId);
        return false;
    }
    op->MarkNotReady();
    return true;
