...
AUTIL_LOG_SETUP(indexlib.config, SummarySchemaImpl);
...
SummarySchemaImpl::SummarySchemaImpl() : mNeedStoreSummary(true), mIsAllFieldsDisabled(false)
{
    ...
    index::summarygroupid_t defaultGroupId = CreateSummaryGroup(index::DEFAULT_SUMMARYGROUPNAME);
...
void SummarySchemaImpl::AddSummaryConfig(const std::shared_ptr<SummaryConfig>& summaryConfig,
                                         index::summarygroupid_t groupId)
{
    ...
    if (groupId + 1 != (index::summarygroupid_t)mSummaryGroups.size()) {
        INDEXLIB_FATAL_ERROR(Schema, "invalid summary groupId[%d], expect[%d]", groupId,
                             (index::summarygroupid_t)mSummaryGroups.size() - 1);
    }
...
void SummarySchemaImpl::AddSummaryPosition(fieldid_t fieldId, size_t pos, index::summarygroupid_t groupId)
{
    if (fieldId >= (fieldid_t)mFieldId2SummaryVec.size()) {
        ...
        mFieldId2GroupIdVec.resize(fieldId + 1, (size_t)index::INVALID_SUMMARYGROUPID);
    }
...
void SummarySchemaImpl::Jsonize(autil::legacy::Jsonizable::JsonWrapper& json)
{
    if (json.GetMode() == TO_JSON) {
        JsonMap summaryMap;
        // group 0
        assert(index::DEFAULT_SUMMARYGROUPID == 0);
...
void SummarySchemaImpl::SetNeedStoreSummary(fieldid_t fieldId)
{
    ...
    index::summarygroupid_t gropuId = FieldIdToSummaryGroupId(fieldId);
    mSummaryGroups[gropuId]->SetNeedStoreSummary(true);
}

index::summarygroupid_t SummarySchemaImpl::CreateSummaryGroup(const string& groupName)
{
    ...
    index::summarygroupid_t groupId = (index::summarygroupid_t)mSummaryGroups.size();
...
index::summarygroupid_t SummarySchemaImpl::FieldIdToSummaryGroupId(fieldid_t fieldId) const
{
    assert(!mIsAllFieldsDisabled);
    ...
    return (index::summaryfieldid_t)mFieldId2GroupIdVec[fieldId];
}

index::summarygroupid_t SummarySchemaImpl::GetSummaryGroupId(const std::string& groupName) const
{
...
{
    index::summarygroupid_t groupId = GetSummaryGroupId(groupName);
    assert(groupId != index::INVALID_SUMMARYGROUPID);
    return GetSummaryGroupConfig(groupId);
}

const SummaryGroupConfigPtr& SummarySchemaImpl::GetSummaryGroupConfig(index::summarygroupid_t groupId) const
{
    assert(groupId >= 0 && groupId < (index::summarygroupid_t)mSummaryGroups.size());
    return mSummaryGroups[groupId];
}

index::summarygroupid_t SummarySchemaImpl::GetSummaryGroupConfigCount() const
{
    return (index::summarygroupid_t)mSummaryGroups.size();
}
