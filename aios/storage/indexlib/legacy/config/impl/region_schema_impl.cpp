...
AUTIL_LOG_SETUP(indexlib.config, RegionSchemaImpl);
...
void RegionSchemaImpl::AddSummaryConfig(const string& fieldName, index::summarygroupid_t summaryGroupId)
{
...
void RegionSchemaImpl::SetSummaryCompress(bool compress, const string& compressType,
                                          index::summarygroupid_t summaryGroupId)
{
    if (!mSummarySchema) {
        mSummarySchema.reset(new SummarySchema);
    }
...
index::summarygroupid_t RegionSchemaImpl::CreateSummaryGroup(const string& summaryGroupName)
{
...
void RegionSchemaImpl::LoadValueConfig()
{
    ...
    if (tableType == tt_kkv) {
        KKVIndexConfigPtr kkvIndexConfig = std::dynamic_pointer_cast<KKVIndexConfig>(singleConfig);
        if (!kkvIndexConfig) {
    ...
    KVIndexConfigPtr kvIndexConfig = std::dynamic_pointer_cast<KVIndexConfig>(singleConfig);
    if (!kvIndexConfig) {
...
void RegionSchemaImpl::FromJson(Jsonizable::JsonWrapper& json)
{
    ...
    if (iter != jsonMap.end()) {
        ...
        AUTIL_LOG(DEBUG, "no indexs section defined");
    }
    ...
    if (iter != jsonMap.end()) {
        ...
        AUTIL_LOG(DEBUG, "no attributes section defined");
    }
...
bool RegionSchemaImpl::LoadSummaryGroup(const Any& any, index::summarygroupid_t summaryGroupId)
{
    JsonMap summary = AnyCast<JsonMap>(any);
    JsonMap::const_iterator it = summary.find(SUMMARY_FIELDS);
    if (it == summary.end()) {
        AUTIL_LOG(WARN, "summary can not be empty");
        return false;
    }

    JsonArray summaryFields = AnyCast<JsonArray>(it->second);
    if (summaryFields.size() < 1) {
        AUTIL_LOG(WARN, "summarys can not be empty");
        return false;
    }
    ...
    if (it == summary.end()) {
        ...
        AUTIL_LOG(INFO, "compress is not set, default set to : false");
    } else {
...
void RegionSchemaImpl::LoadSourceSchema(const Any& any)
{
    ...
    for (index::sourcegroupid_t id = 0; id < mSourceSchema->GetSourceGroupCount(); id++) {
...
{
    if (!LoadSummaryGroup(any, index::DEFAULT_SUMMARYGROUPID)) {
        return;
    }
    ...
    if (it != summary.end()) {
        ...
        for (JsonArray::iterator iter = summaryGroups.begin(); iter != summaryGroups.end(); ++iter) {
            ...
            index::summarygroupid_t groupId = CreateSummaryGroup(groupName);
...
void RegionSchemaImpl::SetEnableOrderPreserving()
{
    ...
    if (tableType != tt_index) {
        AUTIL_LOG(ERROR, "not index table cannot support order preserving");
    ...
    if (!mAttributeSchema || !mAttributeSchema->GetAttributeConfig(mOrderPreservingField)) {
        AUTIL_LOG(ERROR, "order preserving field [%s] not in attribute config", mOrderPreservingField.c_str());
    ...
    if (!fieldConfig) {
        AUTIL_LOG(ERROR, "order preserving field [%s] not in field config", mOrderPreservingField.c_str());
...
#undef MACRO
    default: {
        AUTIL_LOG(ERROR, "order preserving field [%s] should be number type", mOrderPreservingField.c_str());
...
void RegionSchemaImpl::SetEnableTTL(bool enableTTL, const string& fieldName)
{
    ...
    if (tableType == tt_kv || tableType == tt_kkv) {
        ...
        auto kvConfig = std::dynamic_pointer_cast<KVIndexConfig>(mIndexSchema->GetPrimaryKeyIndexConfig());
        assert(kvConfig);
...
void RegionSchemaImpl::CheckSpatialIndexConfig(const IndexConfigPtr& indexConfig) const
{
    ...
    SpatialIndexConfigPtr spatialIndexConf = std::dynamic_pointer_cast<SpatialIndexConfig>(indexConfig);
...
vector<AttributeConfigPtr> RegionSchemaImpl::EnsureSpatialIndexWithAttribute()
{
    ...
    for (; it != mIndexSchema->End(); ++it) {
        ...
        if (indexType == it_spatial) {
            ...
            std::string fieldName = fieldConfig->GetFieldName();
            if (!mAttributeSchema || !mAttributeSchema->GetAttributeConfig(fieldName)) {
                AUTIL_LOG(INFO, "inner add attribute [%s] to ensure spatial index precision", fieldName.c_str());
                AddAttributeConfig(fieldName);
                AttributeConfigPtr attrConfig = mAttributeSchema->GetAttributeConfig(fieldName);
                assert(attrConfig);
...
void RegionSchemaImpl::SetSummaryGroupDataParam(const GroupDataParameter& param, index::summarygroupid_t summaryGroupId)
{
    if (!mSummarySchema) {
        mSummarySchema.reset(new SummarySchema);
    }
...
void RegionSchemaImpl::SetAdaptiveOffset(bool adaptiveOffset, index::summarygroupid_t summaryGroupId)
{
    if (!mSummarySchema) {
        mSummarySchema.reset(new SummarySchema);
    }
