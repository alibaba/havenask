...
size_t AttributeConfig::EstimateAttributeExpandSize(size_t docCount)
{
    ...
    if (GetFieldConfig()->IsMultiValue() || fieldType == ft_string || fieldType == ft_raw) {
        INDEXLIB_FATAL_ERROR(UnSupported,
                             "can not estimate the size of multivalue attribute or raw attribute, attribute name [%s]",
                             GetAttrName().c_str());
    }
...
bool AttributeConfig::IsLegacyAttributeConfig() const
{
    if (autil::EnvUtil::hasEnv("INDEXLIB_FORCE_USE_LEGACY_ATTRIBUTE_CONFIG")) {
        AUTIL_LOG(WARN,
                  "INDEXLIB_FORCE_USE_LEGACY_ATTRIBUTE_CONFIG is set, will serialize legacy attribute config, some "
