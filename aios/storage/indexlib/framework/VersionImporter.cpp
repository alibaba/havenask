...
AUTIL_LOG_SETUP(indexlib.framework, VersionImporter);

Status VersionImporter::Check(const std::vector<Version>& versions, const Version* baseVersion,
                              const ImportOptions& options, std::vector<Version>& validVersions)
{
    ...
    for (const auto& version : versions) {
        ...
        if (!locator.IsSameSrc(baseVersion->GetLocator(), false)) {
            ...
            if (importStrategy == ImportOptions::NOT_SUPPORT) {
                AUTIL_LOG(ERROR, "version src not same, import failed");
                return Status::InvalidArgs();
            }
            assert(importStrategy == ImportOptions::KEEP_SEGMENT_IGNORE_LOCATOR ||
                   importStrategy == ImportOptions::KEEP_SEGMENT_OVERWRITE_LOCATOR);
            validVersions.emplace_back(version);
            continue;
        }

        if (baseVersion->GetLocator().IsFasterThan(locator, false) != Locator::LocatorCompareResult::LCR_FULLY_FASTER) {
            validVersions.emplace_back(version);
        }
    }
...
std::pair<Status, Locator>
VersionImporter::GetSegmentLocator(const std::shared_ptr<indexlib::file_system::IDirectory>& segDir)
{
    Locator segLocator;
    if (!segDir) {
...
Status VersionImporter::Import(const std::vector<Version>& versions, const Fence* fence, const ImportOptions& options,
                               Version* baseVersion)
{
    if (fence == nullptr) {
    ...
    Locator resultLocator = baseVersion->GetLocator();
    ...
    for (const auto& version : validVersions) {
        ...
        if (!resultLocator.IsSameSrc(versionLocator, false)) {
            if (options.GetImportStrategy() == ImportOptions::KEEP_SEGMENT_OVERWRITE_LOCATOR) {
    ...
    std::vector<std::shared_ptr<SegmentDescriptions>> otherSegDescs;
    ...
    bool removeVersionRet = true;
    {
        ...
        for (const auto& validVersion : validVersions) {
            ...
            indexlib::file_system::MountDirOption mountOption(indexlib::file_system::FSMT_READ_ONLY);
            ...
            for (auto [segmentId, schemaId] : validVersion) {
                ...
                Locator baseLocator = baseVersion->GetLocator();
                if (!baseLocator.IsSameSrc(segLocator, false)) {
                    auto importStrategy = options.GetImportStrategy();
                    if (importStrategy == ImportOptions::KEEP_SEGMENT_IGNORE_LOCATOR ||
                        importStrategy == ImportOptions::KEEP_SEGMENT_OVERWRITE_LOCATOR) {
