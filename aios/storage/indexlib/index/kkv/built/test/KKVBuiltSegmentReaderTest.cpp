...
class KKVBuiltSegmentReaderTest : public TESTBASE
{
public:
    void setUp() override;
    void PrepareSchema();
...
AUTIL_LOG_SETUP(indexlib.index, KKVBuiltSegmentReaderTest);

void KKVBuiltSegmentReaderTest::setUp()
{
...
void KKVBuiltSegmentReaderTest::PrepareSchema()
{
...
TEST_F(KKVBuiltSegmentReaderTest, TestSimpleProcess)
{
    PrepareSchema();
    ...
    {
        auto status = reader.Open(_indexConfig, indexDirectory);
        ASSERT_TRUE(status.IsOK()) << status.ToString();
    }
    // Found key
    {
