...
class NormalKKVDocSorter : public KKVDocSorterBase
{
public:
    NormalKKVDocSorter(const std::shared_ptr<indexlibv2::config::KKVIndexConfig>& kkvIndexConfig)
        : KKVDocSorterBase(kkvIndexConfig)
        , _skeyCountLimits(0)
    {
    }
    virtual ~NormalKKVDocSorter() = default;

public:
    bool Init() override;
    std::vector<std::pair<bool, KKVDoc>> Sort() override;
    bool KeepSkeySorted() const override { return true; }
