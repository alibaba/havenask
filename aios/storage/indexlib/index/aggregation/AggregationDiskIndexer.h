...
class AggregationDiskIndexer : public IDiskIndexer
{
public:
...
public:
    const std::shared_ptr<ISegmentReader>& GetReader() const { return _reader; }
    const std::shared_ptr<ISegmentReader>& GetDeleteReader() const { return _deleteReader; }

private:
    std::shared_ptr<ISegmentReader> _reader;
    std::shared_ptr<ISegmentReader> _deleteReader;
};
