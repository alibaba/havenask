...
class NormalSegmentSearcher : public SegmentSearcher
{
public:
    NormalSegmentSearcher(const AithetaIndexConfig& indexConfig, const MetricReporterPtr& metricReporter)
        : SegmentSearcher(indexConfig, metricReporter)
    {
    }
    ~NormalSegmentSearcher() = default;
...
private:
    bool DoSearch(const AithetaQueries& query, const std::shared_ptr<AithetaAuxSearchInfoBase>& searchInfo,
                  ResultHolder& holder) override;

protected:
    IndexSearcherMap _indexSearcherMap;

    AUTIL_LOG_DECLARE();
};

typedef std::shared_ptr<NormalSegmentSearcher> NormalSegmentSearcherPtr;
} // namespace indexlibv2::index::ann
