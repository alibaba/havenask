...
FixedLenCuckooHashTableFileReaderCreator<KeyType, ValueType, useCompactBucket>::CreateHashTable() noexcept
{
    static constexpr bool HasSpecialKey = ClosedHashTableTraits<KeyType, ValueType, useCompactBucket>::HasSpecialKey;
    using HashTableType = CuckooHashTableFileReader<KeyType, ValueType, HasSpecialKey, useCompactBucket>;
    return std::make_unique<HashTableType>();
}

template <typename KeyType, typename ValueType, bool useCompactBucket>
std::unique_ptr<HashTableFileIterator>
FixedLenCuckooHashTableFileReaderCreator<KeyType, ValueType, useCompactBucket>::CreateHashTableFileIterator() noexcept
{
    static constexpr bool HasSpecialKey = ClosedHashTableTraits<KeyType, ValueType, useCompactBucket>::HasSpecialKey;
    using HashTableType = CuckooHashTableFileReader<KeyType, ValueType, HasSpecialKey, useCompactBucket>;
...
#define INDEXLIB_KV_HASHTABLE_INSTANTIATION_VALUETYPE(ValueType)                                                       \
    template class indexlibv2::index::FixedLenCuckooHashTableFileReaderCreator<uint32_t, ValueType, true>;             \
    template class indexlibv2::index::FixedLenCuckooHashTableFileReaderCreator<uint32_t, ValueType, false>;            \
    template class indexlibv2::index::FixedLenCuckooHashTableFileReaderCreator<uint64_t, ValueType, true>;             \
    template class indexlibv2::index::FixedLenCuckooHashTableFileReaderCreator<uint64_t, ValueType, false>;
