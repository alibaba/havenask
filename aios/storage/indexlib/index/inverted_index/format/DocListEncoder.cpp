...
AUTIL_LOG_SETUP(indexlib.index, DocListEncoder);

DocListEncoder::DocListEncoder(const DocListFormatOption& docListFormatOption, util::SimplePool* simplePool,
                               autil::mem_pool::Pool* byteSlicePool, autil::mem_pool::RecyclePool* bufferPool,
                               DocListFormat* docListFormat, index::CompressMode compressMode,
                               format_versionid_t formatVersion)
    ...
    , _byteSlicePool(byteSlicePool)
{
    ...
    if (docListFormatOption.HasTfBitmap()) {
        _tfBitmapWriter = IE_POOL_COMPATIBLE_NEW_CLASS(byteSlicePool, index::PositionBitmapWriter, simplePool);
    }
...
void DocListEncoder::Dump(const std::shared_ptr<file_system::FileWriter>& file)
{
    Flush();
...
InMemDocListDecoder* DocListEncoder::GetInMemDocListDecoder(autil::mem_pool::Pool* sessionPool) const
{
    // copy sequence
