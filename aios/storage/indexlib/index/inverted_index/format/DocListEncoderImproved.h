...
class DocListEncoderImproved
{
public:
    DocListEncoderImproved(
        const DocListFormatOption& docListFormatOption, util::SimplePool* simplePool,
        autil::mem_pool::Pool* byteSlicePool, autil::mem_pool::RecyclePool* bufferPool,
        DocListFormat* docListFormat = nullptr, CompressMode compressMode = PFOR_DELTA_COMPRESS_MODE,
    ...
    PositionBitmapWriter* GetPositionBitmapWriter() const { return _tfBitmapWriter; }
...
private:
    ...
    friend class DocListEncoderImprovedTest;
    AUTIL_LOG_DECLARE();
};
