...
{
public:
    typedef indexlibv2::index::SingleValueAttributeReader<T> AttributeReaderType;
    typedef indexlibv2::index::AttributeIteratorTyped<T> AttributeIterator;
...
#define DEFINE_MULTI_NUMBER_DOC_VALUE_TRAITS(T)                                                                        \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        typedef indexlibv2::index::MultiValueAttributeReader<T> AttributeReaderType;                                   \
        typedef indexlibv2::index::AttributeIteratorTyped<autil::MultiValueType<T>> AttributeIterator;                 \
...
public:
    ...
    typedef std::shared_ptr<AttributeReaderType> AttributeReaderTypePtr;
    typedef typename NumberDocValueTraits<T>::AttributeIterator AttributeIterator;

    NumberDocValueFilterTyped(indexlibv2::index::AttributeReader* attrReader, int64_t left, int64_t right,
        ...
        , _attrIter(NULL)
    {
        _attrReader = dynamic_cast<AttributeReaderType*>(attrReader);
        if (_attrReader) {
    ...
    {
        if (_attrIter) {
            IE_POOL_COMPATIBLE_DELETE_CLASS(_sessionPool, _attrIter);
        }
    }
...
private:
    int64_t _left;
    int64_t _right;
    AttributeReaderType* _attrReader;
