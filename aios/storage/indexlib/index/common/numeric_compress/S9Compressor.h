...
public:
    template <typename T>
    static std::pair<Status, size_t> Encode(uint32_t* dest, size_t destLen, const T* src, size_t srcLen);
...
template <typename T>
inline std::pair<Status, size_t> S9Compressor::Encode(uint32_t* dest, size_t destLen, const T* src, size_t srcLen)
{
    const static uint32_t BIT_LENGTH[] = {1, 2, 3, 4, 5, 7, 9, 14, 28};
    const static uint32_t CODE_NUM[] = {28, 14, 9, 7, 5, 4, 3, 2, 1};
    ...
    while (curPos < srcLen) {
        for (uint32_t k = 0; k < 9; k++) {
            ...
            if (k >= 8) {
                RETURN2_IF_STATUS_ERROR(Status::OutOfRange(), 0,
                                        "Encode posting-list "
                                        "FAILED: [%u] exceeds 28 bit",
                                        (uint32_t)src[curPos]);
            }
        }
    }
    ...
    return std::make_pair(Status::OK(), resNum);
}
