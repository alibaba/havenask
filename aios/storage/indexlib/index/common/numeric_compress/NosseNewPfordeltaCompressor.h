...
#include "indexlib/index/common/numeric_compress/Pack.h"
#include "indexlib/index/common/numeric_compress/S9Compressor.h"
#include "indexlib/index/common/numeric_compress/UnalignedUnpack.h"
#include "indexlib/index/common/numeric_compress/Unpack.h"
...
public:
     ...
     */
    std::pair<Status, size_t> CompressInt32(uint32_t* dest, size_t destLen, const uint32_t* src, size_t srcLen,
     ...
     * @return compressed length in int32 size
     */
    std::pair<Status, size_t> CompressInt16(uint32_t* dest, size_t destLen, const uint16_t* src, size_t srcLen,
                                            bool enableBlockOpt = false) const;
     ...
     * @return compressed length in int32 size
     */
    std::pair<Status, size_t> CompressInt8(uint32_t* dest, size_t destLen, const uint8_t* src, size_t srcLen,
                                           bool enableBlockOpt = false) const;
...
public:
    ...
    template <typename T>
    static std::pair<Status, size_t> Compress(uint32_t* dest, size_t destLen, const T* src, size_t srcLen,
...
protected:
    template <typename T>
    static std::pair<Status, size_t> CompressBlock(uint32_t* dest, size_t destLen, const T* src, size_t srcLen,
...
protected:
    ...
    template <typename T>
    static std::pair<Status, size_t> TransformToFrame(uint32_t* dest, size_t destLen, uint32_t frameBits,
...
protected:
    ...
    template <typename T>
    static std::pair<Status, size_t> S9Encode(uint32_t* dest, size_t destLen, T* src, size_t srcLen);
...
template <typename T>
std::pair<Status, size_t> NosseNewPForDeltaCompressor::Compress(uint32_t* dest, size_t destLen, const T* src,
                                                                size_t srcLen, bool enableBlockOpt)
{
...
template <typename T>
inline uint32_t NosseNewPForDeltaCompressor::SelectCompressBits(const T* block, size_t blkLen, bool enableBlockOpt,
...
template <typename T>
std::pair<Status, size_t> NosseNewPForDeltaCompressor::CompressBlock(uint32_t* dest, size_t destLen, const T* src,
                                                                     ...
                                                                     uint32_t numExceptions, bool isLastBlock)
{
    ...
    if (numExceptions == SPECIAL_EXCEPTION_POS_FOR_SAME_VALUE_ALONE ||
        ...
        return std::make_pair(Status::OK(),
                              TransformToSpecialFrame(dest, destLen, src, srcLen, numExceptions, isLastBlock));
    }
...
template <typename T>
inline size_t NosseNewPForDeltaCompressor::TransformToSpecialFrame(uint32_t* dest, size_t destLen, const T* code,
...
template <typename T>
inline std::pair<Status, size_t> NosseNewPForDeltaCompressor::TransformToFrame(
    uint32_t* dest, size_t destLen, uint32_t frameBits, size_t firstExceptionPos, const T* code, size_t codeLen,
    uint32_t* exception, uint32_t* exceptionOffset, size_t numExceptions, bool isLastBlock)
{
    ...
    if (firstExceptionPos == 0) {
        ...
        *dest++ = MakeHeader(frameBits, firstExceptionPos, codeLen, 0, 0, isLastBlock);
        ...
        return std::make_pair(Status::OK(), exceptionOffsetIntSize);
    }
    ...
    auto [status, exceptionRange] = S9Encode(dest + exceptionOffsetIntSize, destLen - exceptionOffsetIntSize,
                                             exceptionDatum, exceptionOffsetNum + numExceptions);
    RETURN2_IF_STATUS_ERROR(status, 0, "s9 encode fail");
    ...
    *dest++ = MakeHeader(frameBits, firstExceptionPos, codeLen, numExceptions, exceptionRange, isLastBlock);
    ...
    return std::make_pair(Status::OK(), intDataSize);
}
...
template <typename T>
inline bool
NosseNewPForDeltaCompressor::TryDecompressSpecialBlock(T* dest, uint32_t dataNum, uint32_t firstExceptionPos,
                                                       uint32_t exceptionIntRange, uint32_t*& src, size_t& srcLen)
{
...
template <typename T>
size_t NosseNewPForDeltaCompressor::DecompressBlock(T* dest, size_t destLen, uint32_t*& src, size_t& srcLen,
                                                    bool& isLastBlock)
{
...
inline uint32_t NosseNewPForDeltaCompressor::MakeHeader(uint32_t frameBits, size_t firstExceptionPos, size_t codeLen,
                                                        uint32_t numExceptions, uint32_t exceptionIntRange,
                                                        bool isLastBlock)
{
...
inline void NosseNewPForDeltaCompressor::PackException(uint32_t* dest, size_t exceptionOffsetIntSize,
...
template <typename T>
inline std::pair<Status, size_t> NosseNewPForDeltaCompressor::S9Encode(uint32_t* dest, size_t destLen, T* src,
                                                                       size_t srcLen)
{
...
template <typename T>
inline void NosseNewPForDeltaCompressor::FastMemSet(T* dest, size_t destLen, T value)
{
...
typedef std::shared_ptr<NosseNewPForDeltaCompressor> NosseNewPForDeltaCompressorPtr;
...
inline bool NosseNewPForDeltaCompressor::IsUniqValueStoreInline(uint32_t value)
{
