...
class DenseHashTableTest : public indexlib::INDEXLIB_TESTBASE
{
public:
    DenseHashTableTest() {}
    ~DenseHashTableTest() {}
...
public:
        ...
        std::string loadConfigStr = R"(
    {
        ...
        if (!_rootDir->IsExist("test_segment").GetOrThrow()) {
            _segmentDir =
                _rootDir->MakeDirectory("test_segment", indexlib::file_system::DirectoryOption()).GetOrThrow();
        } else {
            _segmentDir = _rootDir->GetDirectory("test_segment").GetOrThrow();
        }
        _counter.Reset();
    }
    void CaseTearDown() override
    {
        _rootDir->RemoveDirectory("test_segment", indexlib::file_system::RemoveOption()).GetOrThrow();
    }

private:
    indexlib::util::BlockAccessCounter _counter;
    std::shared_ptr<indexlib::file_system::IDirectory> _rootDir;
    std::shared_ptr<indexlib::file_system::IDirectory> _segmentDir;

private:
    AUTIL_LOG_DECLARE();
};

AUTIL_LOG_SETUP(indexlib.index, DenseHashTableTest);
...
#define CheckHashTable(hashTable, args...)                                                                             \
    ...
    _CheckFileIterator(__FILE__, __LINE__, _segmentDir, hashTable, ##args);                                            \
    _CheckBufferedFileIterator(__FILE__, __LINE__, _segmentDir, hashTable, ##args);                                    \
    _CheckFileReader(__FILE__, __LINE__, _segmentDir, hashTable, &_counter, ##args);
...
{
    size_t me_size = hashTable.DoCapacityToTableMemory(maxKeyCount, 50);
    ASSERT_LE(me_size, size);
    ASSERT_TRUE(hashTable.MountForWrite(buffer, me_size, 50));
                       ...
                       bool expectFound, bool expectIsDeleted = false, const _VT& expectValue = _VT())
{
    ...
    indexlib::util::Status st = hashTable.Find(key, value);
    if (expectFound) {
        ASSERT_TRUE(value) << msg;
        if (expectIsDeleted) {
            ASSERT_EQ(indexlib::util::DELETED, st) << msg;
        } else {
            ASSERT_EQ(indexlib::util::OK, st) << msg;
        ...
        ASSERT_EQ(indexlib::util::NOT_FOUND, st) << msg;
    }
}
...
template <typename _KT, typename _VT>
static void
_CheckFileIterator(const char* file, int32_t line, const std::shared_ptr<indexlib::file_system::IDirectory>& directory,
                   DenseHashTable<_KT, _VT>& hashTable, uint64_t maxKeyCount, vector<Action<_KT, _VT>> actions,
                   vector<_KT> expectKeys, vector<_KT> notExistKeys = vector<_KT>())
{
    string msg = string(file) + ":" + StringUtil::toString(line) + ": Location\n";
    ...
    if (directory->IsExist("file").GetOrThrow()) {
        directory->RemoveFile("file", indexlib::file_system::RemoveOption()).GetOrThrow();
    }
    ...
    indexlib::file_system::FileWriterPtr fileWriter =
        directory->CreateFileWriter("file", indexlib::file_system::WriterOption()).GetOrThrow();
    ...
    indexlib::file_system::FileReaderPtr fileReader =
        directory->CreateFileReader("file", indexlib::file_system::FSOT_MEM).GetOrThrow();
    ...
    Iterator iterV;
    fileReader = directory->CreateFileReader("file", indexlib::file_system::FSOT_MEM).GetOrThrow();
...
template <typename _KT, typename _VT>
static void _CheckBufferedFileIterator(const char* file, int32_t line,
                                       ...
                                       vector<_KT> notExistKeys = vector<_KT>())
{
    ...
    std::shared_ptr<Iterator> iter(new Iterator);
    indexlib::file_system::FileReaderPtr fileReader =
        directory->CreateFileReader("file", indexlib::file_system::FSOT_MEM).GetOrThrow();
                            ...
                            vector<_KT> notExistKeys = vector<_KT>())
{
    ...
    for (const auto& expectKV : expectMap) {
        const Action<_KT, _VT>& expectAct = expectKV.second;
        const _VT* value = NULL;
        indexlib::util::Status st = hashTable.Find(expectAct.key, value);
...
template <typename _KT, typename _VT>
static void
_CheckFileReader(const char* file, int32_t line, const std::shared_ptr<indexlib::file_system::IDirectory>& directory,
                 DenseHashTable<_KT, _VT>& hashTable, indexlib::util::BlockAccessCounter* counter, uint64_t maxKeyCount,
                 vector<Action<_KT, _VT>> actions, vector<_KT> expectKeys, vector<_KT> notExistKeys = vector<_KT>())
{
    string msg = string(file) + ":" + StringUtil::toString(line) + ": Location\n";
    ...
    CreateHashTable(hashTable, buffer, sizeof(buffer), maxKeyCount, actions);
    indexlib::file_system::FileWriterPtr fileWriter =
        directory->CreateFileWriter(fileName, indexlib::file_system::WriterOption()).GetOrThrow();
    fileWriter->Write(buffer, hashTable.MemoryUse()).GetOrThrow();
    ASSERT_EQ(FSEC_OK, fileWriter->Close());

    indexlib::file_system::FileReaderPtr fileReader =
        directory->CreateFileReader(fileName, indexlib::file_system::FSOT_CACHE).GetOrThrow();
...
//////////////////////////////////////////////////////////////////////
TEST_F(DenseHashTableTest, TestSpecialKeyBucketInsert)
{
...
TEST_F(DenseHashTableTest, TestCalculateDeleteCount)
{
    typedef DenseHashTable<uint64_t, double> HashTable;
    HashTable hashTable;
    // init
...
TEST_F(DenseHashTableTest, TestSpecialKeyBucketDelete)
{
    typedef DenseHashTable<uint64_t, double> HashTable;
    HashTable hashTable;
    // init
...
TEST_F(DenseHashTableTest, TestSpecialKeyBucketForSpecialKey)
{
    typedef DenseHashTable<uint64_t, double> HashTable;
    HashTable hashTable;
...
TEST_F(DenseHashTableTest, TestSpecialKeyBucket)
{
    typedef double VT;
...
TEST_F(DenseHashTableTest, TestTimestampBucket)
{
    // Simple Case
...
TEST_F(DenseHashTableTest, TestOffsetBucket)
{
    // Simple Case
...
TEST_F(DenseHashTableTest, TestOffsetBucketSpecial)
{
...
TEST_F(DenseHashTableTest, TestSpecialKeyBucketFull)
{
...
TEST_F(DenseHashTableTest, TestSpecialKeyBucketInteTest)
{
    typedef int64_t VT;
    typedef DenseHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
    ...
    ASSERT_EQ(_counter.blockCacheHitCount, 13);
    // each key hit only once
    ASSERT_EQ(_counter.blockCacheMissCount, 0);
...
TEST_F(DenseHashTableTest, TestTimestamp0BucketInteTest)
{
...
TEST_F(DenseHashTableTest, TestTimestampBucketShrink)
{
...
TEST_F(DenseHashTableTest, TestTimestampBucketInteTest)
{
    typedef TimestampValue<uint64_t> V;
    typedef DenseHashTable<uint64_t, V> HashTable;
    typedef InsertAction<uint64_t, V> I;
...
TEST_F(DenseHashTableTest, TestOffsetBucketInteTest)
{
...
TEST_F(DenseHashTableTest, TestSimpleSpecialValueIterator)
{
...
TEST_F(DenseHashTableTest, TestOccupancyPct)
{
...
static void CheckCompressForOffsetValue(bool shouldCompress)
{
    ...
    std::shared_ptr<BucketCompressor> bucketCompressor(new index::BucketOffsetCompressor<Bucket>());
...
TEST_F(DenseHashTableTest, TestCompress)
{
...
class FakeFileReader : public indexlib::file_system::FileReader
{
public:
    FakeFileReader(indexlib::file_system::FileReaderPtr fileReader) noexcept : _fileReader(fileReader) {}
    FSResult<void> Open() noexcept override { return _fileReader->Open(); }
    ...
    void* GetBaseAddress() const noexcept override { return _fileReader->GetBaseAddress(); }
    size_t GetLength() const noexcept override { return _fileReader->GetLength(); }
    const std::string& GetLogicalPath() const noexcept override { return _fileReader->GetLogicalPath(); }
    const std::string& GetPhysicalPath() const noexcept override { return _fileReader->GetPhysicalPath(); }
    FSResult<void> Close() noexcept override { return _fileReader->Close(); }
    FSOpenType GetOpenType() const noexcept override { return _fileReader->GetOpenType(); }
    FSFileType GetType() const noexcept override { return _fileReader->GetType(); }
    FileNodePtr GetFileNode() const noexcept override { return _fileReader->GetFileNode(); }

private:
    indexlib::file_system::FileReaderPtr _fileReader;
};

TEST_F(DenseHashTableTest, TestInitNoIO)
{
    typedef int64_t VT;
    typedef DenseHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
    auto dir = _segmentDir;

    string testFileName = "test_file";
    size_t hashTableSize = HashTable::DoBuildMemoryToTableMemory(1024 * 1024, 80);
    auto hashFile = dir->CreateFileWriter(testFileName, WriterOption::Mem(hashTableSize)).GetOrThrow();
    ...
    indexlib::file_system::FileReaderPtr fileReader =
        dir->CreateFileReader(testFileName, indexlib::file_system::FSOpenType::FSOT_LOAD_CONFIG).GetOrThrow();
    ...
    ASSERT_EQ(hashTableReader1._bucketCount, hashTableReader2._bucketCount);
    ASSERT_EQ(hashTableReader1._keyCount, hashTableReader2._keyCount);
}
