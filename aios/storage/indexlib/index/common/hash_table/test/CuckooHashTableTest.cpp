...
class CuckooHashTableTest : public indexlib::INDEXLIB_TESTBASE
{
public:
    CuckooHashTableTest();
    ~CuckooHashTableTest();
...
AUTIL_LOG_SETUP(indexlib.index, CuckooHashTableTest);
...
void CuckooHashTableTest::CaseSetUp()
{
    ...
    if (!_rootDir->IsExist("test_segment").GetOrThrow()) {
        _segmentDir =
            _rootDir->MakeDirectory("test_segment", indexlib::file_system::DirectoryOption::Mem()).GetOrThrow();
    } else {
...
TEST_F(CuckooHashTableTest, TestCalculateDeleteCount)
{
...
TEST_F(CuckooHashTableTest, TestSimpleProcess)
{
    typedef CuckooHashTable<uint64_t, int32_t> HashTable;
    HashTable hashTable;
    constexpr size_t buffLen = 1024;
...
TEST_F(CuckooHashTableTest, TestCuckooKick)
{
    typedef CuckooHashTable<uint64_t, int32_t> HashTable;
    ...
    for (uint32_t num = 0; num < 100; ++num) {
        ...
        uint64_t blockId = hashTable.GetFirstBucketIdInBlock(hash, hashTable._blockCount) / 4;
        if (blockId == 0) {
            uint64_t hash1 = hashTable.CuckooHash(num, 1);
            uint64_t blockId1 = hashTable.GetFirstBucketIdInBlock(hash1, hashTable._blockCount) / 4;
            if (blockId1 == 1) {
                tester = num;
            } else {
                zeros.push_back(num);
    ...
    const int32_t* value;
    indexlib::util::Status status = hashTable.Find(zeros[4], value);
...
TEST_F(CuckooHashTableTest, TestTableDeleteFull)
{
    typedef CuckooHashTable<uint64_t, int32_t> HashTable;
...
TEST_F(CuckooHashTableTest, TestTableInsertMany)
{
    typedef CuckooHashTable<uint64_t, int32_t> HashTable;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        key = key * 3 - 1;
        indexlib::util::Status status = hashTable.Find(key, value);
        if (i % 7 <= 2) {
...
TEST_F(CuckooHashTableTest, TestTimestmapValue)
{
...
TEST_F(CuckooHashTableTest, TestOffsetValue)
{
...
TEST_F(CuckooHashTableTest, TestFindForReadWrite)
{
    typedef TimestampValue<int64_t> VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
...
TEST_F(CuckooHashTableTest, TestInsertSpecialKey)
{
...
TEST_F(CuckooHashTableTest, TestIterator)
{
    typedef SpecialValue<uint64_t> VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
...
TEST_F(CuckooHashTableTest, TestFileIterator)
{
    ...
    const std::shared_ptr<indexlib::file_system::IDirectory>& directory = _segmentDir;
    const char* filename = "file";
    if (directory->IsExist(filename).GetOrThrow()) {
        directory->RemoveFile(filename, indexlib::file_system::RemoveOption()).GetOrThrow();
    }
    ...
    indexlib::file_system::FileReaderPtr fileReader =
        directory->CreateFileReader(filename, indexlib::file_system::FSOT_MEM).GetOrThrow();
    ...
    std::shared_ptr<BufferedIterator> bufferedIter(new BufferedIterator);
...
TEST_F(CuckooHashTableTest, TestShrink)
{
    typedef TimestampValue<int64_t> VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
...
TEST_F(CuckooHashTableTest, TestIncHashFunc)
{
    typedef TimestampValue<int64_t> VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
    ...
    ASSERT_EQ(2, hashTable._nu_hashFunc);
    ...
    hashTable.PrintHashTable();
    ASSERT_EQ(3, hashTable._nu_hashFunc);
...
TEST_F(CuckooHashTableTest, TestShrinkFail)
{
    typedef TimestampValue<int64_t> VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
...
TEST_F(CuckooHashTableTest, TestStretch)
{
    typedef int64_t VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    HashTable hashTable;
...
TEST_F(CuckooHashTableTest, TestBuildMemoryToCapacity)
{
...
TEST_F(CuckooHashTableTest, TestCompress)
{
...
public:
    ...
    std::shared_ptr<FileNode> GetFileNode() const noexcept override { return _fileReader->GetFileNode(); }

private:
    indexlib::file_system::FileReaderPtr _fileReader;
};

TEST_F(CuckooHashTableTest, TestInitNoIO)
{
    typedef int64_t VT;
    typedef CuckooHashTable<uint64_t, VT> HashTable;
    ...
    ASSERT_EQ(hashTableReader1._nu_hashFunc, hashTableReader2._nu_hashFunc);
}

TEST_F(CuckooHashTableTest, TestTableInsertManyMod16)
{
    typedef CuckooHashTable<uint64_t, int32_t> HashTable;
    static const uint32_t TOTAL = 1000000;
    HashTable hashTable;
