...
{
public:
    RangeFieldEncoder(const std::vector<std::shared_ptr<indexlibv2::config::IIndexConfig>>& indexConfigs);
    ~RangeFieldEncoder();
...
private:
    std::vector<RangeFieldType> _fieldVec;
...
{
    RangeFieldType type = _fieldVec[fieldId];
    if (type == INT) {
        int64_t num = 0;
        if (!autil::StringUtil::numberFromString(fieldValue, num)) {
            AUTIL_LOG(DEBUG, "field value [%s] is not date number.", fieldValue.c_str());
            return false;
        }
        value = (1UL << 63) + num;
        return true;
...
{
    if (fieldId >= (fieldid_t)_fieldVec.size()) {
        return false;
    }
    return _fieldVec[fieldId] != UNKOWN;
}
} // namespace indexlib::index
