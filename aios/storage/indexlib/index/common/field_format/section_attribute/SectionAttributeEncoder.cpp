...
AUTIL_LOG_SETUP(indexlib.index, SectionAttributeEncoder);
...
const uint8_t SectionAttributeEncoder::_itemNumPerUnit[16] = {28, 21, 21, 21, 14, 9, 8, 7, 6, 6, 5, 5, 4, 3, 2, 1};

std::pair<Status, uint32_t> SectionAttributeEncoder::Encode(section_len_t* lengths, section_fid_t* fids,
                                                            ...
                                                            uint8_t* buf, uint32_t bufLen)
{
    ...
    auto st = EncodeSectionLens(lengths, sectionCount, cursor, bufEnd);
    RETURN2_IF_STATUS_ERROR(st, 0, "encode section lens failed.");
    st = EncodeFieldIds(fids, sectionCount, cursor, bufEnd);
    RETURN2_IF_STATUS_ERROR(st, 0, "encode field ids failed.");
    st = EncodeSectionWeights(weights, sectionCount, cursor, bufEnd);
    RETURN2_IF_STATUS_ERROR(st, 0, "encode section weight failed");

    return std::make_pair(Status::OK(), cursor - buf);
}

Status SectionAttributeEncoder::EncodeSectionWeights(const section_weight_t* srcWeights, uint32_t sectionCount,
                                                     uint8_t*& cursor, uint8_t* bufEnd)
{
    ...
    while (leftSectionCount > 0) {
        ...
        auto st = EncodeWeightUnit(weightUnit, cursor, bufEnd);
        RETURN_IF_STATUS_ERROR(st, "encode weight unit failed.");
    }
    return Status::OK();
}

Status SectionAttributeEncoder::EncodeWeightUnit(const section_weight_t* weightUnit, uint8_t*& cursor, uint8_t* bufEnd)
{
    auto st = CheckBufferLen(cursor, bufEnd, 1);
    ...
    for (uint32_t i = 0; i < 4; i++) {
        ...
        if (weightUnit[i] <= 255) {
            st = CheckBufferLen(cursor, bufEnd, 1);
            RETURN_IF_STATUS_ERROR(st, "check buffer failed.");
        ...
        } else // if (weightUnit[i] <= 65535)
        {
            ...
            st = CheckBufferLen(cursor, bufEnd, 2);
            RETURN_IF_STATUS_ERROR(st, "check buffer failed.");
...
Status SectionAttributeEncoder::EncodeFieldIds(const section_fid_t* srcFids, uint32_t sectionCount, uint8_t*& cursor,
                                               uint8_t* bufEnd)
{
    auto [st, unitCount] = S16Compressor::Encode((uint32_t*)cursor, (uint32_t*)bufEnd, srcFids, sectionCount);
    RETURN_IF_STATUS_ERROR(st, "s16 compress encode failed");
    cursor += sizeof(uint32_t) * unitCount;
    return Status::OK();
}

Status SectionAttributeEncoder::EncodeSectionLens(const section_len_t* srcLengths, uint32_t sectionCount,
                                                  uint8_t*& cursor, uint8_t* bufEnd)
{
    ...
    auto [st, unitCount] =
        S16Compressor::Encode((uint32_t*)cursor, (uint32_t*)bufEnd, bufferToEncode, sectionCount + 1);
    RETURN_IF_STATUS_ERROR(st, "s16 compress failed.");
    cursor += sizeof(uint32_t) * unitCount;
    return Status::OK();
}

Status SectionAttributeEncoder::Decode(const uint8_t* srcBuf, uint32_t srcBufLen, uint8_t* destBuf, uint32_t destBufLen)
{
    const uint8_t* srcCursor = (const uint8_t*)srcBuf;
    uint8_t* destCursor = (uint8_t*)destBuf;
    uint8_t* destEnd = destBuf + destBufLen;
    auto st = DecodeSectionLens(srcCursor, destCursor, destEnd);
    RETURN_IF_STATUS_ERROR(st, "decode section len failed.");

    uint32_t sectionCount = *((section_len_t*)destBuf);
    st = DecodeSectionFids(srcCursor, sectionCount, destCursor, destEnd);
    RETURN_IF_STATUS_ERROR(st, "decode section field ids failed.");

    const uint8_t* srcEnd = srcBuf + srcBufLen;
    st = DecodeSectionWeights(srcCursor, srcEnd, sectionCount, destCursor, destEnd);
    RETURN_IF_STATUS_ERROR(st, "decode section weight failed");
    return Status::OK();
}
...
Status SectionAttributeEncoder::DecodeSectionFids(const uint8_t*& srcCursor, uint32_t sectionCount,
                                                  uint8_t*& destCursor, uint8_t* destEnd)
{
    int32_t leftCount = (int32_t)sectionCount;
    section_fid_t* fidCursor = (section_fid_t*)destCursor;
    auto st = CheckBufferLen(fidCursor, (section_fid_t*)destEnd, sectionCount);
    RETURN_IF_STATUS_ERROR(st, "check buffer failed.");
    ...
    while (leftCount > 0) {
        ...
        leftCount -= _itemNumPerUnit[k];
        fidCursor += _itemNumPerUnit[k];
        srcCursor += sizeof(uint32_t);
    }
...
Status SectionAttributeEncoder::DecodeSectionWeights(const uint8_t*& srcCursor, const uint8_t* srcEnd,
                                                     uint32_t sectionCount, uint8_t*& destCursor, uint8_t* destEnd)
{
    ...
    auto st = CheckBufferLen(weightsCursor, (section_weight_t*)destEnd, sectionCount);
    RETURN_IF_STATUS_ERROR(st, "check buffer len failed");
