...
{
public:
    BlockFileNodeCreator(FileBlockCacheContainerPtr container, const std::string& fileSystemIdentifier);
...
public:
    ...
    FSOpenType GetDefaultOpenType() const override { return FSOT_CACHE; }
    size_t EstimateFileLockMemoryUse(size_t fileLength) const noexcept override { return 0; }
    // If dictionary is loaded with block cache, then block index is always locked.

private:
    LoadConfig _loadConfig;
    FileBlockCachePtr _fileBlockCache;
    bool _useDirectIO;
    bool _cacheDecompressFile;
    bool _useHighPriority;
    FileBlockCacheContainerPtr _fileBlockCacheContainer;
    std::string _fileSystemIdentifier;
...
typedef std::shared_ptr<BlockFileNodeCreator> BlockFileNodeCreatorPtr;
