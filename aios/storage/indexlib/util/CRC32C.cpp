...
#include "indexlib/util/CRC32C.h"
...
AUTIL_LOG_SETUP(indexlib.util, CRC32C);
...
{
    return Coding::DecodeFixed32(reinterpret_cast<const char*>(buffer));
}
...
{
    return reinterpret_cast<uint8_t*>((reinterpret_cast<uintptr_t>(pointer) + (N - 1)) &
...
uint32_t CRC32C::Extend(uint32_t crc, const char* data, size_t n)
{
...
#define STEP4(s)                                                                                                       \
    do {                                                                                                               \
        crc##s = ReadUint32LE(p + s * 4) ^ kStrideExtensionTable3[crc##s & 0xff] ^                                     \
                 kStrideExtensionTable2[(crc##s >> 8) & 0xff] ^ kStrideExtensionTable1[(crc##s >> 16) & 0xff] ^        \
