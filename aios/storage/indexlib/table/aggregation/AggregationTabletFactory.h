...
class AggregationTabletFactory : public framework::ITabletFactory
{
public:
    AggregationTabletFactory();
    ~AggregationTabletFactory();
...
public:
    ...
    std::unique_ptr<framework::ITabletLoader> CreateTabletLoader(const std::string& fenceName) override;
...
private:
    std::unique_ptr<config::TabletOptions> _tabletOptions;
