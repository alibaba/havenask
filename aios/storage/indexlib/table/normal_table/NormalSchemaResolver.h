...
private:
    ...
    Status ResolveSummary(config::UnresolvedSchema* schema);
    ...
    Status LoadAdaptiveBitmapTerms(const std::shared_ptr<indexlib::file_system::IDirectory>& rootDir,
                                   config::UnresolvedSchema* schema);
    Status ResolvePKIndexConfig(config::UnresolvedSchema* schema, config::TabletOptions* option);
    Status ResolveAliasTypes(config::UnresolvedSchema* schema);
    void SetNeedStoreSummary(config::UnresolvedSchema* schema);
