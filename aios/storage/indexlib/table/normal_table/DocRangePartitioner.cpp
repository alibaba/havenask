...
AUTIL_LOG_SETUP(indexlib.index, DocRangePartitioner);

bool DocRangePartitioner::GetPartedDocIdRanges(const DocIdRangeVector& rangeHint, size_t incDocCount,
                                               size_t totalWayCount, size_t wayIdx, DocIdRangeVector& ranges)
{
    vector<DocIdRangeVector> rangeVectors;
    if (!GetPartedDocIdRanges(rangeHint, incDocCount, totalWayCount, rangeVectors)) {
        return false;
    }
...
{
    if (totalWayCount == 0) {
        AUTIL_LOG(ERROR, "totalWayCount is zero");
        return false;
    }
