...
{
public:
    AndIndexReclaimer(const std::shared_ptr<kmonitor::MetricsReporter>& reporter, const IndexReclaimerParam& param,
                      std::map<segmentid_t, docid_t> segmentId2BaseDocId,
                      std::map<segmentid_t, size_t> segmentId2DocCount);
    ~AndIndexReclaimer() = default;
...
private:
    ...
    INDEXLIB_FM_DECLARE_METRIC(andReclaim);
