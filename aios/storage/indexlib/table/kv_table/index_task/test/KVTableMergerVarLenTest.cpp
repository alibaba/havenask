...
class KVTableMergerVarLenTest : public TESTBASE
{
public:
    KVTableMergerVarLenTest() {}
    ~KVTableMergerVarLenTest() {}

public:
    void setUp() override
    {
        // lead to a small value of kv_key_value_mem_ratio in SegmentMetrics
        _valueSuffix = string(1000, '0');

        std::string field = "key:string;value:string";
        _tabletSchema = table::KVTabletSchemaMaker::Make(field, "key", "value");
        _docs1 = "cmd=add,key=1,value=s1,ts=1,locator=0:1;"
                 "cmd=add,key=2,value=s2,ts=2,locator=0:2;"
                 "cmd=add,key=3,value=s3,ts=3,locator=0:3;"
                 "cmd=add,key=4,value=s4,ts=4,locator=0:4;";
        _docs2 = "cmd=add,key=5,value=s5,ts=5,locator=0:5;"
                 "cmd=add,key=6,value=s6,ts=6,locator=0:6;"
                 "cmd=add,key=7,value=s7,ts=7,locator=0:7;"
                 "cmd=add,key=8,value=s8,ts=8,locator=0:8;"
                 "cmd=add,key=9,value=s9,ts=9,locator=0:9;"
                 "cmd=add,key=3,value=s103,ts=3,locator=0:10;"
                 "cmd=add,key=2,value=s102,ts=3,locator=0:11;"
                 "cmd=add,key=1,value=s101,ts=3,locator=0:12;"
...
AUTIL_LOG_SETUP(indexlib.index, KVTableMergerVarLenTest);

std::unique_ptr<config::TabletOptions> KVTableMergerVarLenTest::CreateMultiShardOptions()
{
...
void KVTableMergerVarLenTest::CheckDoc(KVTableTestHelper& mainHelper, const set<int>& deletedDoc)
{
    for (int i = 1; i <= DOC_COUNT; ++i) {
        ...
        if (deletedDoc.end() == deletedDoc.find(i)) {
            string valueStr;
            if (i <= 3) {
                valueStr = "s" + to_string(i + 100);
            } else if (i <= 9) {
                valueStr = "s" + to_string(i);
            } else {
                valueStr = "s" + to_string(i) + _valueSuffix;
            }
...
TEST_F(KVTableMergerVarLenTest, TestMergeDifferentLevel)
{
    ...
    kvIndexFactory->TEST_SetMemoryFactor(256);
    ...
    // merge to bottom level，VarLenKVMerger::_dropDeleteKey = true
    {
    ...
    // not merge to bottom level，VarLenKVMerger::_dropDeleteKey = false
    // merge is not expected, because of key/value size of level 0 is very small
    {
    ...
    kvIndexFactory->TEST_SetMemoryFactor(2);
}

TEST_F(KVTableMergerVarLenTest, TestTTLFilter)
{
    framework::IndexRoot indexRoot(GET_TEMP_DATA_PATH(), GET_TEMP_DATA_PATH());
    std::string field = "key:string;value:string";
    ...
    KVTableTestHelper mainHelper;
    ASSERT_TRUE(mainHelper.Open(indexRoot, tabletSchema, CreateMultiShardOptions()).IsOK());
    ASSERT_TRUE(mainHelper.BuildSegment(_docs1).IsOK());
    ASSERT_TRUE(mainHelper.BuildSegment(_docs2).IsOK());
    ASSERT_TRUE(mainHelper.BuildSegment(_docs3).IsOK());
    ASSERT_TRUE(mainHelper.Merge(TableTestHelper::MergeOption::MergeAutoReadOption(true)).IsOK());
...
TEST_F(KVTableMergerVarLenTest, TestEmptyTarget)
{
    framework::IndexRoot indexRoot(GET_TEMP_DATA_PATH(), GET_TEMP_DATA_PATH());
...
TEST_F(KVTableMergerVarLenTest, TestNotShortOffset)
{
    framework::IndexRoot indexRoot(GET_TEMP_DATA_PATH(), GET_TEMP_DATA_PATH());
    std::string field = "key:string;value:string";
    auto tabletSchema = table::KVTabletSchemaMaker::Make(field, "key", "value");
