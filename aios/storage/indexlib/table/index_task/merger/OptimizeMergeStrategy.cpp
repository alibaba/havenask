...
OptimizeMergeStrategy::GetSegmentValidDocCount(const std::shared_ptr<framework::Segment>& segment) const
{
    return {Status::OK(), segment->GetSegmentInfo()->docCount};
}
...
OptimizeMergeStrategy::SplitDocCount(const vector<std::shared_ptr<framework::Segment>>& srcSegments) const
{
    ...
    for (size_t i = 0; i < srcSegments.size(); i++) {
        auto result = GetSegmentValidDocCount(srcSegments[i]);
        if (!result.first.IsOK()) {
            return std::make_pair(result.first, 0);
        }
        totalDocCount += result.second;
    }
                                         ...
                                         const vector<std::shared_ptr<framework::Segment>>& srcSegments)
{
    ...
    uint64_t maxSingleSegmentDocCount = indexlib::MAX_SEGMENT_DOC_COUNT;
    size_t segmentIdx = 0;
    while (segmentIdx < srcSegments.size()) {
        SegmentMergePlan plan;
        uint64_t docCount = 0;
        do {
            auto result = GetSegmentValidDocCount(srcSegments[segmentIdx]);
            if (!result.first.IsOK()) {
                return std::make_pair(result.first, nullptr);
            }
            uint32_t acutalSegmentDocCount = result.second;
            if (docCount + (uint64_t)acutalSegmentDocCount > maxSingleSegmentDocCount && docCount > 0) {
                break;
            }
