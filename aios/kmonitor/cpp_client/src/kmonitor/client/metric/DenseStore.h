...
class DenseStore
{
 public:
    DenseStore(int32_t arrayLengthGrowthIncrement = DEFAULT_ARRAY_LENGTH_GROWTH_INCREMENT,
               int32_t arrayLengthOverhead = int32_t(DEFAULT_ARRAY_LENGTH_GROWTH_INCREMENT * DEFAULT_ARRAY_LENGTH_OVERHEAD_RATIO));
    ~DenseStore();
    ...
    int32_t getArrayLengthGrowthIncrement() {
        return arrayLengthGrowthIncrement_;
    }

    int32_t getArrayLengthOverhead() {
        return arrayLengthOverhead_;
    }
    ...
    int64_t getTotalCount() {
        return getTotalCount(minIndex_, maxIndex_);
    }

    void extendRange(int32_t index) {
