...
const string &KMonitor::Name() { return name_; }
...
MutableMetric *KMonitor::RegisterMetric(const std::string &name, MetricType metric_type, MetricLevel level) {
    ...
    MutableMetric *mutable_metric =
        manager_->RegisterMetric(target_name, metric_type, level, tags_manager_, metricsCache_);
...
void KMonitor::Report(const string &name, double value) { Report(name, NULL, value); }
...
Metric *KMonitor::declareMetric(const string &name, const MetricsTags *tags) {
    ...
    if (metric == NULL) {
        AUTIL_LOG(WARN, "metics[%s] tags[%s] can't match", target_name.c_str(), tags->ToString().c_str());
        return NULL;
    }
...
void KMonitor::registerBuildInMetrics(MetricsTags *tags, MetricLevel level, const string &metric_name_prefix) {
    ...
    AUTIL_LOG(INFO, "register build metrics prefix [%s] tags [%s]", metric_name.c_str(), tagsPtr->ToString().c_str());
