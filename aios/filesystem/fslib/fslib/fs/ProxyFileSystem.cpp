...
ProxyFileSystem::ProxyFileSystem(AbstractFileSystem* fs)
    : _fs(fs)
{
}
...
File* ProxyFileSystem::openFile(const string& fileName, Flag flag)
{
    return openFile(fileName, flag, -1);
}
...
File* ProxyFileSystem::openFile(const string& fileName, Flag flag, ssize_t fileLength)
{
    ...
    if (flag == READ) {
        ...
        if (FSCacheModule::getInstance()->getFileDataFromCache(normPath, data)) {
            AUTIL_LOG(INFO, "fslib openFile [%s] with READ flag from innerDataCache, len [%lu]",
                      normPath.c_str(), data.size());
    ...
    if (!file || (file->getLastError() != EC_OK && file->getLastError() != EC_NOENT)) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib openFile [%s][%d] failed.",
                         fileName.c_str(), flag);
    }
    if (file) {
...
MMapFile* ProxyFileSystem::mmapFile(const string& fileName, Flag openMode,
...
ErrorCode ProxyFileSystem::rename(const string& oldFileName,
                                  const string& newFileName)
{
    ...
    STACK_TRACER_LOG(INFO, "FSLIB_FileSystem rename [%s] to [%s]",
                     oldNormPath.c_str(), newNormPath.c_str());
    ErrorCode ret;
    {
    ...
    if (ret != EC_OK && ret != EC_NOENT && ret != EC_EXIST) {
        ...
        REPORT_OPERATION(ERROR, oldNormPath, "fslib rename [%s] to [%s] failed, ec[%d].",
                         oldFileName.c_str(), newFileName.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::getFileMeta(const string& fileName, FileMeta& fileMeta)
{
    ...
    if (ret != EC_OK && ret != EC_NOENT) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib getfilemeta [%s] failed, ec[%d].",
                         fileName.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::getPathMeta(const string &path, PathMeta &pathMeta)
{
    ...
    FileSystem::reportQpsMetric(normPath, FSLIB_METRIC_TAGS_OPTYPE_GETFILEMETA);
    if (ret == EC_OK) {
        FSCacheModule::getInstance()->addPathMeta(normPath, pathMeta.length,
                pathMeta.createTime, pathMeta.lastModifyTime, !pathMeta.isFile);
    } else if (ret != EC_NOENT) {
        FileSystem::reportErrorMetric(normPath, FSLIB_METRIC_TAGS_OPTYPE_GETFILEMETA);
        REPORT_OPERATION(ERROR, normPath, "fslib getpathmeta [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::mkDir(const string& dirName, bool recursive)
{
    ...
    if (ret == EC_OK) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib mkdir [%s] failed, ec[%d].",
                         dirName.c_str(), ret);
    }
    return ret;
}

ErrorCode ProxyFileSystem::listDir(const string& dirName,
                                   FileList& fileList)
{
    assert(_fs);
    string normPath = PathUtil::normalizePath(dirName);
    vector<pair<string, PathMeta>> subPathMetas;
    if (FSCacheModule::getInstance()->listInImmutablePath(
                    normPath, subPathMetas, false))
    {
    ...
    if (ret != EC_OK && ret != EC_NOENT)
    {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib listDir [%s] failed, ec[%d].",
                         dirName.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::isDirectory(const string& path)
{
    ...
    {
        FileSystem::reportErrorMetric(normPath, FSLIB_METRIC_TAGS_OPTYPE_GETFILEMETA);
        REPORT_OPERATION(ERROR, normPath, "fslib isDir [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::remove(const string& path)
{
    ...
    if (ret != EC_OK && ret != EC_NOENT) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib remove [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    ...
    if (ret == EC_OK) {
        REPORT_REMOVE_SUCCESS_OPERATION(normPath, "fslib remove [%s] success.",
                path.c_str());
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::removeFile(const string& path)
{
    ...
    if (ret != EC_OK && ret != EC_NOENT) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib remove file [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    ...
    if (ret == EC_OK) {
        REPORT_REMOVE_SUCCESS_OPERATION(normPath, "fslib removeFile [%s] success.",
                path.c_str());
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::removeDir(const string& path)
{
    ...
    if (ret != EC_OK && ret != EC_NOENT) {
        ...
        REPORT_OPERATION(ERROR, normPath, "fslib remove dir [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    ...
    if (ret == EC_OK) {
        REPORT_REMOVE_SUCCESS_OPERATION(normPath, "fslib removeDir [%s] success.",
                path.c_str());
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::isExist(const string& path)
{
    ...
    {
        FileSystem::reportErrorMetric(normPath, FSLIB_METRIC_TAGS_OPTYPE_GETFILEMETA);
        REPORT_OPERATION(ERROR, normPath, "fslib isExist [%s] failed, ec[%d].",
                         path.c_str(), ret);
    }
    return ret;
}
...
ErrorCode ProxyFileSystem::forward(const string &command, const string &path,
...
AbstractFileSystem* ProxyFileSystem::getFs() const
{
    return _fs;
}
