...
{
    if (_flag == WRITE) {
        char* tmp = getenv("FSLIB_ENABLE_PRINT_FILE_WRITE_INFO");
        if (tmp && string(tmp) == "true") {
            int64_t createTimestamp = autil::TimeUtility::currentTime();
            _md5Stream.reset(new Md5Stream);
            AUTIL_LOG(INFO, "[FSLIB_INFO] operate_type=[CREATE] path=[%s] timestamp=[%ld] ip=[%s] host=[%s]",
                      _filePath.c_str(), createTimestamp, GetIp().c_str(), GetHostName().c_str());
        } 
    }
}
...
void ProxyFile::pread(IOController* controller,
...
#if (__cplusplus >= 201703L)
void ProxyFile::preadv(IOController* controller,
...
#if (__cplusplus >= 201703L)
void ProxyFile::pwrite(IOController* controller,
                      void* buffer, size_t length, off_t offset,
                      function<void()> callback)
{
    assert(_file);
...
void ProxyFile::pwritev(IOController* controller,
                       const iovec* iov, int iovcnt, off_t offset,
                       function<void()> callback)
{
    //TODO add metric
...
ErrorCode ProxyFile::ioctlImpl(IOCtlRequest request, va_list args) {
    return _file->ioctlImpl(request, args);
}
...
ErrorCode ProxyFile::close()
{
    ...
    if (_md5Stream && ec == EC_OK) {
        ...
        AUTIL_LOG(INFO, "[FSLIB_INFO] operate_type=[CLOSE] path=[%s] timestamp=[%ld] md5=[%s] ip=[%s] host=[%s]",
                  _filePath.c_str(), modifyTimestamp, GetMD5String().c_str(), GetIp().c_str(), GetHostName().c_str());
        _md5Stream.reset();
    }
