...
bool LocalFileSystem::normalizeFilePath(const string& filePath,
...
File* LocalFileSystem::openFile(const string& rawFileName, Flag flag)
{
    ...
    if (!normalizeFilePath(rawFileName, fileName)) {
        AUTIL_LOG(ERROR, "openFile %s fail, normalize path fail",
                  rawFileName.c_str());
        return NULL;
    }
    ...
    if (useDirectIO) {
        ...
        case READ: {
            if (isExistInternal(fileName) != EC_TRUE) {
                AUTIL_LOG(WARN, "direct openFile %s fail, file not exist",
                        fileName.c_str());
                return new LocalDirectFile(fileName, -1, EC_NOENT);
            }
            if (isDirectoryInternal(fileName) == EC_TRUE) {
                AUTIL_LOG(ERROR, "direct openFile %s fail, cannot open dir",
                        fileName.c_str());
        ...
        case WRITE: {
            ...
            if (parentDir != "" && isExistInternal(parentDir) != EC_TRUE) {
                ...
                if (ret != EC_OK && ret != EC_EXIST) {
                    AUTIL_LOG(ERROR, "openFile %s fail, cannot create parent dir",
                            fileName.c_str());
                    return new LocalDirectFile(fileName, -1, ret);
                }
            }
            fd = ::open(fileName.c_str(),
                        O_CREAT | O_DIRECT | O_WRONLY | O_TRUNC,
        ...
        case APPEND: {
            ...
            fd = ::open(fileName.c_str(),
                        O_CREAT | O_DIRECT | O_WRONLY | O_APPEND,
                        S_IRWXU | S_IRGRP | S_IROTH);
            break;
        }
        default: {
            AUTIL_LOG(ERROR, "direct openFile %s fail, flag %d not support",
                      fileName.c_str(), flag);
        ...
        AUTIL_LOG(ERROR, "open file %s fail, %s",
        ...
        case READ: {
            ...
            if (isExistInternal(fileName) != EC_TRUE) {
                AUTIL_LOG(INFO, "openFile %s fail, which does not exist",
                        fileName.c_str());
                return new LocalFile(fileName, NULL, EC_NOENT);
            }
            if (isDirectoryInternal(fileName) == EC_TRUE) {
                AUTIL_LOG(ERROR, "openFile %s fail, cannot open dir",
                        fileName.c_str());
        ...
        default: {
            AUTIL_LOG(ERROR, "openFile %s fail, flag %d [%d] not support",
                      fileName.c_str(), flag, flag & FLAG_CMD_MASK);
...
MMapFile* LocalFileSystem::mmapFile(const std::string& rawFileName,
                                    Flag openMode,
                                    ...
                                    int mapFlag, off_t offset)
{
    ...
    case WRITE:
        ...
        break;
    default:
        AUTIL_LOG(ERROR, "mmap file %s fail, open mode not support.",
                  fileName.c_str());
    ...
    if (fd == -1) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "mmap file %s fail, %s.", fileName.c_str(),
                  strerror(errno));
    ...
    if (stat(fileName.c_str(), &statBuf) < 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "mmap file %s fail, get file length fail, %s.",
                  fileName.c_str(), strerror(errno));
        return new MMapFile(fileName, fd, NULL, 0, 0, convertErrno(curErrno));;
    }
    ...
    if (truncRet == -1) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "mmap file %s fail, truncate fail, %s.",
                  fileName.c_str(), strerror(errno));
        return new MMapFile(fileName, fd, NULL, 0, 0, convertErrno(curErrno));
    }
    ...
    if (base == MAP_FAILED) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "mmap file %s fail, %s.",
                  fileName.c_str(), strerror(errno));
        return new MMapFile(fileName, fd, NULL, 0, 0, convertErrno(curErrno));
    }
...
ErrorCode LocalFileSystem::rename(const string& oldRawFileName,
                                  const string& newRawFileName)
{
    string oldFileName("");
    if (!normalizeFilePath(oldRawFileName, oldFileName)) {
        AUTIL_LOG(ERROR, "openFile %s fail, normalize path fail",
                  oldRawFileName.c_str());
        return EC_PARSEFAIL;
    }
    string newFileName("");
    if (!normalizeFilePath(newRawFileName, newFileName)) {
        AUTIL_LOG(ERROR, "openFile %s fail, normalize path fail",
                  newRawFileName.c_str());
        return EC_PARSEFAIL;
    }

    if (isExistInternal(oldFileName) != EC_TRUE) {
        AUTIL_LOG(WARN, "renameFile %s to %s fail, oldFile does not exist.",
                  oldFileName.c_str(), newFileName.c_str());
        return EC_NOENT;
    }

    if (isExistInternal(newFileName) == EC_TRUE) {
        AUTIL_LOG(ERROR, "renameFile %s fail, newFileName %s exist.",
                  oldFileName.c_str(), newFileName.c_str());
        return EC_EXIST;
    }

    if (isFileInternal(oldFileName) == EC_TRUE
        && newFileName[newFileName.length() -1] == '/')
    {
        AUTIL_LOG(ERROR, "renameFile %s to %s fail, oldFileName is not dir.",
                  oldFileName.c_str(), newFileName.c_str());
        return EC_NOTDIR;
    }
    ...
    if (::rename(oldFileName.c_str(), newFileName.c_str()) != 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "renameFile %s fail, %s",
...
ErrorCode LocalFileSystem::getFileMeta(const string& rawFileName,
                                       FileMeta& meta)
{
    string fileName("");
    if (!normalizeFilePath(rawFileName, fileName)) {
    ...
    if (stat(fileName.c_str(), &statBuf) < 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(INFO, "getFileMeta %s fail, %s",
               fileName.c_str(), strerror(errno));
        return convertErrno(curErrno);
    }
...
ErrorCode LocalFileSystem::isFileInternal(const string& path) {
    ...
    if (stat(path.c_str(), &buf) < 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(INFO, "isFile for path %s fail, %s",
...
ErrorCode LocalFileSystem::isLinkInternal(const string& path) {
    ...
    if (lstat(path.c_str(), &buf) < 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "isLink for path %s fail, %s",
                  path.c_str(), strerror(errno));
        return convertErrno(curErrno);
    }
...
FileChecksum LocalFileSystem::getFileChecksum(const string& fileName)
{
    return 0;
}
...
ErrorCode LocalFileSystem::mkNestDir(const string& dirName) {
    ...
    if (pos == string::npos) {
        if (mkdir(dirName.c_str(), 0755) < 0) {
            if (errno != EEXIST) {
                int32_t curErrno = errno;
                AUTIL_LOG(ERROR, "create directory %s fail, %s",
...
ErrorCode LocalFileSystem::mkDir(const string& rawDirName,
                                 bool recursive)
{
    string dirName("");
...
ErrorCode LocalFileSystem::mkDirInternal(const string& dirName,
                                 bool recursive)
{
    string dir = dirName;
    size_t len = dir.size();
    ...
    if (pos == string::npos) {
        if (mkdir(dir.c_str(), 0755) < 0) {
            int32_t curErrno = errno;
            AUTIL_LOG(ERROR, "create directory %s fail, %s", dir.c_str(),
                      strerror(errno));
            return convertErrno(curErrno);;
        }
        return EC_OK;
    ...
    if (!parentDir.empty() && access(parentDir.c_str(), F_OK) != 0) {
        if (recursive) {
            ErrorCode ret = mkNestDir(parentDir);
            if (ret != EC_OK) {
                AUTIL_LOG(ERROR, "create directory %s fail, fail to create "
                        "parent dir", dir.c_str());
                return ret;
            }
        } else {
            int32_t curErrno = errno;
            AUTIL_LOG(ERROR, "create directory %s fail, parameter -p is needed",
                      dir.c_str());
            return convertErrno(curErrno);
        }
    }
...
ErrorCode LocalFileSystem::listDir(const string& rawDirName,
                                   FileList& fileList)
{
    string dirName("");
    if (!normalizeFilePath(rawDirName, dirName)) {
    ...
    if (dp == NULL) {
        int32_t curErrno = errno;
        AUTIL_LOG(WARN, "open dir %s fail, %s",
               dirName.c_str(), strerror(errno));
        return convertErrno(curErrno);;
    }
    ...
    if (closedir(dp) < 0) {
        int32_t curErrno = errno;
        AUTIL_LOG(ERROR, "close dir %s fail, %s",
               dirName.c_str(), strerror(errno));
        return convertErrno(curErrno);
    }
                                   ...
                                   EntryList& entryList)
{
    ...
    for (size_t i = 0; i < fileList.size(); ++i) {
        ...
        if (ec == EC_TRUE) {
            ...
            AUTIL_LOG(ERROR, "isDirectory %s failed, errorCode: %d.",
                      (dirName + "/" + fileList[i]).c_str(), ec);
            continue;
        }
...
ErrorCode LocalFileSystem::removeTypeInternal(const string& path, const char* type) {
    if (::remove(path.c_str()) != 0) {
        if (errno != ENOENT) {
            int32_t curErrno = errno;
            AUTIL_LOG(ERROR, "remove %s %s fail, %s",
                      type, path.c_str(), strerror(errno));
            return convertErrno(curErrno);
        }
    }
    return EC_OK;
...
ErrorCode LocalFileSystem::removeDirInternal(const string& path) {
    ...
    while ((ent = readdir(pDir)) != NULL) {
        ...
        if (ent->d_type == DT_UNKNOWN) {
            ...
            if (stat(childName.c_str(), &sb) == -1) {
                int32_t curErrno = errno;
                AUTIL_LOG(ERROR, "stat dir %s fail, %s",
                        childName.c_str(), strerror(errno));
                return convertErrno(curErrno);
            }
        ...
        if (isDir) {
            ...
            if (removeInternal(childName) != EC_OK) {
                AUTIL_LOG(ERROR, "delete dir %s fail, %s",
                        childName.c_str(), strerror(errno));
                closedir(pDir);
                return convertErrno(errno);
            }
            ...
            if (::remove(childName.c_str()) != 0) {
                if (errno != ENOENT) {
                    int32_t curErrno = errno;
                    AUTIL_LOG(ERROR, "delete file %s fail, %s",
                            childName.c_str(), strerror(errno));
                    closedir(pDir);
                    return convertErrno(curErrno);
                }
    ...
    if (::remove(path.c_str()) != 0) {
        if (errno != ENOENT) {
            int32_t curErrno = errno;
            AUTIL_LOG(ERROR, "remove dir %s fail, %s",
                      path.c_str(), strerror(errno));
            return convertErrno(curErrno);
        }
    }
...
FileReadWriteLock* LocalFileSystem::createFileReadWriteLock(
        const string& fileName)
{
    return new LocalFileReadWriteLock(fileName);
}

FileLock* LocalFileSystem::createFileLock(const string& fileName)
{
