...
public:
    ...
    MMapFile *mmapFile(const std::string &rawFileName,
                       Flag openMode,
                       char *start,
                       size_t length,
                       int prot,
    ...
    ErrorCode rename(const std::string &oldRawFileName, const std::string &newRawFileName) override;

    ErrorCode link(const std::string &srcPath, const std::string &dstPath) override;

    ErrorCode getFileMeta(const std::string &rawFileName, FileMeta &fileMeta) override;
    ...
    FileSystemCapability getCapability() const override { return FSC_ALL_CAPABILITY; }
...
private:
    static bool normalizeFilePath(const std::string &filePath, std::string &normalizedFilePath);
    static std::string getParentDir(const std::string &dirName);
    ErrorCode mkDirInternal(const std::string &dirName, bool recursive = false);
