...
public:
    ...
    /*override*/ ErrorCode getPathMeta(const std::string& path,
                                       PathMeta& pathMeta);
...
public:
    static bool parsePath(const std::string& path, std::string& filePath,
                          HdfsFsConfig &config);
    HdfsConnectionPtr getHdfsConnection(const std::string& name);    
private:
    static bool eliminateSeqSlash(const char* src, size_t len,
                                  std::string& dst, bool keepLastSlash);
    static bool parseConfig(const std::string& configStr,
                            HdfsFsConfig &config);
    ...
    ErrorCode connectServer(const std::string& server,
                            const HdfsFsConfig& fsConfig,
                            HdfsConnectionPtr& hdfsFs);
    ...
    ErrorCode internalIsExist(HdfsConnectionPtr hdfsConnection,
                              const std::string &dstPath);

    void initClassPath();
    bool getHadoopClassPath(std::string *hadoopClassPath);
    void getHadoopHome(std::string *hadoopHome);
    void genHadoopClassPath(const std::string &hadoopHome, 
                            std::string *hadoopClassPath);
    void setHadoopClassPath(const std::string &hadoopClassPath);
    static std::string getHadoopClassPathPattern(
            const std::string &hadoopHome);
    static void extendHadoopClassPathPattern(const std::string &pattern, 
            std::string *hadoopClassPath);
    ...
    static void globPath(const char *pathPattern, 
