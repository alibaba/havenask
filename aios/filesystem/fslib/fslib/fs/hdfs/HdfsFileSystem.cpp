...
{
    setenv("LIBHDFS_OPTS", "-Xrs", 0);
    setenv("JAVA_TOOL_OPTIONS", "-Djdk.lang.processReaperUseDefaultStackSize=true", 0);
    initClassPath();
}
...
bool HdfsFileSystem::eliminateSeqSlash(const char* src, size_t len,
                                       string& dst, bool keepLastSlash) 
{
    int slashNum = 0;
    bool flag = false;
...
bool HdfsFileSystem::parsePath(const string& fullPath, string& filePath,
                               HdfsFsConfig &config)
{
    ...
    if (beginPosForFileName == string::npos) {
        AUTIL_LOG(ERROR, "parse hdfs config fail for path %s which "
                  "is an invalid path", fullPath.c_str());
        return false;
    }
    ...
    size_t beginPosForConfig = configStr.find(
            HdfsFileSystem::CONFIG_SEPARATOR.c_str(), prefixLen);
    configStr.resize(0);
    if (beginPosForConfig != string::npos) {
        configStr.append(fullPath.c_str() + beginPosForConfig + 1, 
                         beginPosForFileName - beginPosForConfig - 1);
        filePath.append(fullPath.c_str() + prefixLen,
                        beginPosForConfig - prefixLen);
    } else {
        filePath.append(fullPath.c_str() + prefixLen,
                        beginPosForFileName - prefixLen);
    }

    if (!eliminateSeqSlash(fullPath.c_str() + beginPosForFileName, 
                           fullPath.size() - beginPosForFileName,
                           filePath, true))
    {
        AUTIL_LOG(ERROR, "parse hdfs config fail for path %s which "
                  "is an invalid path", fullPath.c_str());
...
bool HdfsFileSystem::parseConfig(const string& configStr, HdfsFsConfig &config) {
    vector<string> confVec = StringUtil::split(configStr,
            HdfsFileSystem::KV_PAIR_SEPARATOR);
    for (uint32_t i = 0; i < confVec.size(); i++) {
        vector<string> subConfVec = StringUtil::split(confVec[i], 
                HdfsFileSystem::KV_SEPARATOR);
        if (subConfVec.size() != 2) {
            AUTIL_LOG(ERROR, "parse HdfsFsConfig string <%s> fail! "
                      "parse key value pair error", configStr.c_str());
            return false;
        }
        switch (subConfVec[0][0]) {
        case 'b' : {
            if (strcmp(subConfVec[0].c_str(),
                       HdfsFileSystem::HDFS_BUFFER_SIZE.c_str()) == 0) 
            {
                if (!StringUtil::strToInt32(subConfVec[1].c_str(), 
                                config._bufferSize)) 
                {
                    AUTIL_LOG(ERROR, "parse HdfsFsConfig string <%s> fail "
                            "while parsing buffer size, convert string to "
                            "int32 error", configStr.c_str());
                    return false;
                } else {
                    continue;
                }
            } else if (strcmp(subConfVec[0].c_str(),
                            HdfsFileSystem::HDFS_BLOCK_SIZE.c_str()) == 0)
            {
                if (!StringUtil::strToInt32(subConfVec[1].c_str(), 
                                config._blockSize)) 
                {
        ...
        case 'r' : {
            if (strcmp(subConfVec[0].c_str(),
                       HdfsFileSystem::HDFS_REPLICA.c_str()) == 0) {
                if (!StringUtil::strToInt16(subConfVec[1].c_str(), 
                                config._replica)) 
                {
                    AUTIL_LOG(ERROR, "parse HdfsFsConfig string <%s> fail "
        ...
        case 'u' : {
            if (strcmp(subConfVec[0].c_str(),
                       HdfsFileSystem::HDFS_USER.c_str()) == 0) {
        ...
        case 'g' : {
            if (strcmp(subConfVec[0].c_str(),
                       HdfsFileSystem::HDFS_GROUPS.c_str()) == 0) {
                vector<string> groupVec = StringUtil::split(subConfVec[1], 
                        HdfsFileSystem::HDFS_GROUP_SEPARATOR);
        ...
        AUTIL_LOG(WARN, "parse HadoopFsConfig string <%s> fail! "
                  "not support config: %s", configStr.c_str(), 
                  subConfVec[0].c_str());
    } // for
    ...
    if ((config._user && config_groups && config._groupNum > 0) ||
        (config._user == NULL && config._groups == NULL &&
         config._groupNum == 0))
    {
        return true;
    }

    AUTIL_LOG(ERROR, "parse HdfsFsConfig string <%s> fail! user and groups "
...
File* HdfsFileSystem::openFile(const string& fileName, Flag flag)
{
    ...
    ErrorCode ec = internalGetConnectionAndPath(fileName, fsConfig, 
            hdfsConnection, dstPath);
    if (ec != EC_OK) {
    ...
    case APPEND: {
        ...
        if (ec == EC_TRUE) {
            ...
            AUTIL_LOG(ERROR, "check file exist fail, file: %s", 
                      fileName.c_str());
            return new HdfsFile(fileName, dstPath, hdfsConnection, NULL, NULL, 0, ec);
        }
#else
        AUTIL_LOG(TRACE1, "not defined HADOOP_CDH");
        AUTIL_LOG(ERROR, "openFile <%s> fail, hdfs currently does not "
                  "support append", fileName.c_str());
        return new HdfsFile(fileName, dstPath, hdfsConnection, NULL, NULL, 0, EC_NOTSUP);
#endif
        break;
    }
    default:
        AUTIL_LOG(ERROR, "openFile <%s> fail, flag %d not support", 
                  fileName.c_str(), flag);
        return new HdfsFile(fileName, dstPath, hdfsConnection, NULL, NULL, 0, EC_NOTSUP);
    }
    ...
    if (flag == APPEND || !file) {
        ...
        hdfsFileInfo *fileInfo = hdfsGetPathInfo(hdfsConnection->getHdfsFs(), 
                                                 dstPath.c_str());
        if (fileInfo) {
        ...
        if (!fileInfoExist) {
            AUTIL_LOG(ERROR, "get info of file <%s> fail, %s, %d", 
                      fileName.c_str(), strerror(errno), errno);
            return new HdfsFile(fileName, dstPath, hdfsConnection, 
                    NULL, NULL, 0, convertErrno(errno));
        }
    
        if (isDir) {
            AUTIL_LOG(ERROR, "openFile <%s> fail, cannot open directory", 
                      fileName.c_str());
    ...
    if (!file) {
        AUTIL_LOG(ERROR, "hdfsOpenFile <%s> fail, %s, %d", fileName.c_str(), 
                  strerror(errno), errno);
        if (errno == EINTERNAL) {
            hdfsConnection->setError(true);
        }
        return new HdfsFile(fileName, dstPath, hdfsConnection, NULL, NULL,
                            0, convertErrno(errno));
    }
    return new HdfsFile(fileName, dstPath, hdfsConnection, file, &_panguMonitor, curPos);
}
    
MMapFile* HdfsFileSystem::mmapFile(const string& fileName, Flag openMode, 
...
ErrorCode HdfsFileSystem::rename(const string& oldPath,
                                 const string& newPath)
{
    ...
    ErrorCode ec = splitServerAndPath(oldPathWithoutConfig, oldFsConfig._prefix, 
				      oldServer, oldDstPath);
    if (ec != EC_OK) {
        return ec;
    }
    ...
    ec = splitServerAndPath(newPathWithoutConfig, newFsConfig._prefix, 
			    newServer, newDstPath);
    if (ec != EC_OK) {
        return ec;
    }
    ...
    hdfsFileInfo *fileInfo = hdfsGetPathInfo(oldConnection->getHdfsFs(), 
            oldDstPath.c_str());
    if (!fileInfo) {
        AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, fail to get"
                  "src path info, %s", oldPath.c_str(), newPath.c_str(),
                  strerror(errno));
        if (errno == EINTERNAL) {
    ...
    if (hdfsExists(newConnection->getHdfsFs(), newDstPath.c_str()) == 0) {
        AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, src path"
                  "already exist", oldPath.c_str(), newPath.c_str());
        return EC_EXIST;
    }
    if (errno == EINTERNAL) {
    ...
    if (newDstPath[newDstPath.size() - 1] == '/' && isFile == true) {
        AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, src path is"
                  "not a directory", oldPath.c_str(), newPath.c_str());
        return EC_NOTDIR;
    }
    ...
    if (pos != string::npos) {
        if (pos == newDstPath.size() - 1) {
            if (pos == 0) {
                AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail,"
                        "dst path should not be /", oldPath.c_str(), 
                        newPath.c_str());
                return EC_NOTSUP;
            }
        ...
        if (pos != 0) {
            ...
            if (hdfsExists(newConnection->getHdfsFs(), parentDir.c_str()) != 0 && 
                hdfsCreateDirectory(newConnection->getHdfsFs(), 
                        parentDir.c_str()) < 0) 
            {
                AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, create"
                        " parent dir of src path fail, %s.", oldPath.c_str(), 
                        newPath.c_str(), strerror(errno));
                return convertErrno(errno);
            }
        }
    } else {
        AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, invalid src path",
                  oldPath.c_str(), newPath.c_str());
        return EC_NOENT;
    }

    if (isSameHdfs) {
        if (hdfsRename(oldConnection->getHdfsFs(), oldDstPath.c_str(), 
                       newDstPath.c_str()) < 0) {
            AUTIL_LOG(ERROR, "hdfs rename path <%s> to path <%s> fail, %s", 
        ...
        if (hdfsMove(oldConnection->getHdfsFs(), oldDstPath.c_str(), 
                     newConnection->getHdfsFs(), newDstPath.c_str()) < 0) 
        {
            AUTIL_LOG(ERROR, "hdfsMove path <%s> to path <%s> fail, %s", 
                      oldPath.c_str(), newPath.c_str(), strerror(errno));
            ec = convertErrno(errno);
        }
...
ErrorCode HdfsFileSystem::getFileMeta(const string& fileName,
                                      FileMeta& fileMeta)
{
    PathMeta pathMeta;
...
ErrorCode HdfsFileSystem::getPathMeta(const std::string& path,
                                      PathMeta& pathMeta)
{
    HdfsConnectionPtr hdfsConnection;
    string dstPath;
    HdfsFsConfig fsConfig;
    ErrorCode ec = internalGetConnectionAndPath(path, fsConfig,
            hdfsConnection, dstPath);
    if (ec != EC_OK) {
        return ec;
    }
    ...
    hdfsFileInfo *fileInfo = hdfsGetPathInfo(hdfsConnection->getHdfsFs(), 
            dstPath.c_str());
    if (!fileInfo) {
        AUTIL_LOG(ERROR, "hdfsGetPathInfo of <%s> fail, %s", 
...
ErrorCode HdfsFileSystem::isFile(const string& path) {
    ...
    hdfsFileInfo *fileInfo = hdfsGetPathInfo(hdfsConnection->getHdfsFs(), 
            dstPath.c_str());
    if (!fileInfo) {
        AUTIL_LOG(DEBUG, "hdfsGetPathInfo of file <%s> fail, %s", 
                  path.c_str(), strerror(errno));
        if (errno == EINTERNAL) {
            hdfsConnection->setError(true);
        }
...
FileChecksum HdfsFileSystem::getFileChecksum(const string& fileName) {
    return 0;
}

ErrorCode HdfsFileSystem::mkDir(const string& dirName,
                                bool recursive) 
{
    HdfsConnectionPtr hdfsConnection;
    string dstDir;
    HdfsFsConfig fsConfig;
    ErrorCode ec = internalGetConnectionAndPath(dirName, fsConfig,
            hdfsConnection, dstDir);
    if (ec != EC_OK) {
        return ec;
    }

    if (hdfsExists(hdfsConnection->getHdfsFs(), dstDir.c_str()) == 0) {    
        AUTIL_LOG(ERROR, "hdfsCreateDirectory of dir <%s> fail, path with the " 
                  "same name already exist. ", dstDir.c_str());
        return EC_EXIST;
    }
    ...
    if (ret < 0) {
        AUTIL_LOG(ERROR, "hdfsCreateDirectory of dir <%s> fail, %s, address %p", 
                  dirName.c_str(), strerror(errno), hdfsConnection->getHdfsFs());
        ec = convertErrno(errno);
    } 
...
ErrorCode HdfsFileSystem::listDir(const string& dirName,
                                  FileList& fileList) 
{
    EntryList entryList;
                                  ...
                                  EntryList& entryList)
{
    ...
    hdfsFileInfo *fileInfo = hdfsGetPathInfo(hdfsConnection->getHdfsFs(), 
            dstDir.c_str());
    if (!fileInfo) {
        AUTIL_LOG(ERROR, "hdfsListDirectory of dir <%s> fail, %s", 
    ...
    hdfsFreeFileInfo(fileInfo, 1);
    if (!isDir) {
        AUTIL_LOG(ERROR, "hdfsListDirectory of path <%s> fail, path is not"
                  " a directory", dirName.c_str());
        return EC_NOTDIR;
    }

    int fileNum = -1;
    hdfsFileInfo *fileInfos = hdfsListDirectory(hdfsConnection->getHdfsFs(), 
            dstDir.c_str(), &fileNum);
...
ErrorCode HdfsFileSystem::remove(const string& path) {
    ...
    if (ret == 0) {
        ...
        AUTIL_LOG(ERROR, "hdfsDelete path <%s> fail, path does not exist", 
                  path.c_str());
        ec = EC_NOENT;
    } else {
        AUTIL_LOG(ERROR, "hdfsDelete path <%s> fail, %s", 
                  path.c_str(), strerror(errno));
        ec = convertErrno(errno);
    }
...
ErrorCode HdfsFileSystem::internalIsExist(HdfsConnectionPtr hdfsConnection,
        const string &dstPath) 
{
    ...
    if (ret == 0) {
        ...
        AUTIL_LOG(ERROR, "path exist <%s> fail, %s", 
...
ErrorCode HdfsFileSystem::splitServerAndPath(const string& srcPath, 
					     const string& prefix, 
					     string& server, 
					     string& dstPath)
{
    ...
    if (pos == string::npos) {
        AUTIL_LOG(ERROR, "cannot find / between servername and pathname for %s",
                  srcPath.c_str());
        return EC_PARSEFAIL;
    }
...
ErrorCode HdfsFileSystem::connectServer(const string& server,
                                        const HdfsFsConfig& fsConfig,
                                        HdfsConnectionPtr& hdfsConnection) 
{
    string username;
    ...
    if (pos != string::npos) {
        if (!StringUtil::strToUInt16(server.substr(pos + 1).c_str(), port)) {
            AUTIL_LOG(ERROR, "cannot convert port %s to int16_t value fail", 
                      server.substr(pos + 1).c_str());
            return EC_PARSEFAIL;
        }
    }
    hdfsConnection = getHdfsConnection(username);
...
#if defined HADOOP_CDH
    if (fsConfig._user) {
        hdfsFs = hdfsConnectAsUserNewInstance(server.substr(0, pos).c_str(), (tPort)port,
                fsConfig._user);
#else
    const char** groups = fsConfig._groups;
    if (fsConfig._user && fsConfig._groups
        && fsConfig._groupNum > 0)
    {
        hdfsFs = hdfsConnectAsUserNewInstance(server.substr(0, pos).c_str(), (tPort)port,
                    fsConfig._user, groups, fsConfig._groupNum);
#endif
    } else {
    ...
    if (_connectionMap.size() >= MAX_CONNECTION) {
        ...
        while (it != _connectionMap.end()) {
            if (it->second.use_count() == 1) {
                itToDel = it;
                it++;
                AUTIL_LOG(DEBUG, "erase connection with key %s from pool, address %p", 
                        itToDel->first.c_str(), this);
    ...
    AUTIL_LOG(DEBUG, "Add connection %p with key %s into pool with address %p",
              hdfsConnection->getHdfsFs(), username.c_str(), this);
    return EC_OK;
}

ErrorCode HdfsFileSystem::internalGetConnectionAndPath(
        const string& srcPath, HdfsFsConfig& config,
        HdfsConnectionPtr& hdfsConnection, string& dstPath)
{
    string server;
...
void HdfsFileSystem::getConnectUser(const HdfsFsConfig& fsConfig,
                                    string& username)
{
    if (fsConfig._user) {
...
bool HdfsFileSystem::getHadoopClassPath(string *hadoopClassPath) {
    return getEnv(HADOOP_CLASSPATH_ENV, hadoopClassPath);
}

void HdfsFileSystem::getHadoopHome(string *hadoopHome) {
    if (!getEnv(HADOOP_HOME_ENV, hadoopHome)) {
        *hadoopHome = "";
    }
}

void HdfsFileSystem::genHadoopClassPath(const string &hadoopHome, 
                                        string *hadoopClassPath)
{
...
void HdfsFileSystem::setHadoopClassPath(const string &hadoopClassPath) {
    ...
    string classPath;
    if (!getEnv(CLASSPATH_ENV, &classPath)) {
        classPath = hadoopClassPath;
    } else {
        if (classPath.empty()) {
            classPath = hadoopClassPath;
        } else {
    ...
    setEnv(CLASSPATH_ENV, classPath);
}

bool HdfsFileSystem::getEnv(const string &name, string *value) {
...
bool HdfsFileSystem::setEnv(const string &name, const string &value) {
...
string HdfsFileSystem::getHadoopClassPathPattern(
        const string &hadoopHome) 
{
    string hadoopBin = hadoopHome + "/bin/hadoop";
    if (access(hadoopBin.c_str(), F_OK) != 0) {
        AUTIL_LOG(ERROR, "hadoop bin not exist. path:[%s]", 
                  hadoopBin.c_str());
        return "";
    }
    ...
    if (stream == NULL) {
        AUTIL_LOG(ERROR, "execute hadoop cmd [%s] failed. "
                  "errorno:[%d], errormsg:[%s].",
                  hadoopCmd.c_str(), errno, strerror(errno));
        return "";
    }
...
void HdfsFileSystem::readStream(FILE* stream, string *pattern) {
    ...
    while (true) {
        ...
        if (ferror(stream)) {
            AUTIL_LOG(ERROR, "read hadoop cmd stream failed. error:[%s]",
                      strerror(errno));
            *pattern = "";
            return;
        }
...
void HdfsFileSystem::extendHadoopClassPathPattern(const string &pattern, 
        string *hadoopClassPath)
{
...
void HdfsFileSystem::globPath(const char *pathPattern, 
                              vector<string> *paths) 
{
    glob_t pglob;
