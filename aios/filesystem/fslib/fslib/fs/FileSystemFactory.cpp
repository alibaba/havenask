...
void FileSystemFactory::destroyFs(const FsType& fsType, AbstractFileSystem* fs)
{
    ...
    if (isLocalFileSystem(fsType)) {
        delete fs;
        return;
    } else if (_fsModuleInfo.find(fsType) == _fsModuleInfo.end() ||
               _fsModuleInfo[fsType].dllWrapper == NULL ) {
        return;
    }
    DestroyFileSystemFun destroyFsFunction =
        (DestroyFileSystemFun)_fsModuleInfo[fsType].dllWrapper->dlsym(
                FSLIB_FS_DLL_DESTROY_FUNCTION_NAME);
...
bool FileSystemFactory::isLocalFileSystem(const FsType& type) {
    return FSLIB_FS_LOCAL_FILESYSTEM_NAME == type;
}


bool FileSystemFactory::isZookeeperFileSystem(const FsType& type) {
    return FSLIB_FS_ZFS_FILESYSTEM_NAME == type;
}

bool FileSystemFactory::isHttpFileSystem(const FsType& type) {
    return FSLIB_FS_HTTP_FILESYSTEM_NAME == type;
}

bool FileSystemFactory::isDfsFileSystem(const FsType& type) {
    return FSLIB_FS_DFS_FILESYSTEM_NAME == type;
}
...
bool FileSystemFactory::isFslibPlugin(const string &fileName) {
    ...
    return StringUtil::startsWith(fileName, FSLIB_FS_LIBSO_NAME_PREFIX) &&
        (StringUtil::endsWith(fileName, ".so") ||
         fileName.find(".so.") != string::npos);
}

bool FileSystemFactory::parsePlugin(const string &fileName,
                                    FsType &fileType,
                                    uint32_t &majorVersion)
{
    ...
    fileType = fileName.substr(strlen(FSLIB_FS_LIBSO_NAME_PREFIX),
                               found - strlen(FSLIB_FS_LIBSO_NAME_PREFIX));
    ...
    if (version == "") {
        ...
        if (!StringUtil::startsWith(version, ".")) {
            ...
            // +1 to ignore "."
            return StringUtil::strToUInt32(version.c_str() + 1,
                    majorVersion);
        }
    }
}
    ...
    if (n < 0) {
        ...
        while(n--) {
            ...
            if (parsePlugin(soFileName, type, version)) {
                ...
                AUTIL_LOG(INFO, "find fslib plugin[%s]:[%s], %u",
                        type.c_str(), _fsModuleInfo[type].path.c_str(), version);
            }
        }
        free(nameList);
    }
...
bool FileSystemFactory::init() {
    ...
    env = getenv(FSLIB_FS_LIBSO_PATH_ENV_NAME);
    // add "./"
    if (NULL == env) {
        pathes = "./:./aios/filesystem/fslib/";
    } else {
        pathes = "./:./aios/filesystem/fslib/:" + string(env);
    }
    ...
    env = getenv(FSLIB_CREATE_FS_RETRY_INTERVAL);
    if (env == nullptr) {
        _retryInterval = 10;
    } else {
        _retryInterval = atoi(env);
...
AbstractFileSystem* FileSystemFactory::createFileSystem(const FsType& type) {
    ...
    if (_fsModuleInfo[type].dllWrapper == NULL) {
        _fsModuleInfo[type].dllWrapper = new DllWrapper(
                _fsModuleInfo[type].path);
    }

    if (!_fsModuleInfo[type].dllWrapper->dlopen()) {
        AUTIL_LOG(ERROR, "dlopen [%s] fail, error :[%s]",
    ...
    CreateFileSystemFun createFsFunction =
        (CreateFileSystemFun)_fsModuleInfo[type].dllWrapper->dlsym(
                FSLIB_FS_DLL_CREATE_FUNCTION_NAME);
    if (createFsFunction == NULL) {
        AUTIL_LOG(ERROR, "dlsym 'create' from [%s] fail, error :[%s]",
                  _fsModuleInfo[type].path.c_str(),
                  _fsModuleInfo[type].dllWrapper->dlerror().c_str());
        #ifdef ENABLE_BEEPER
        BEEPER_FORMAT_REPORT_WITHOUT_TAGS(
...
AbstractFileSystem* FileSystemFactory::doGetFs(const FsType& type) {
    ...
    if (unlikely(it == _fsMap.end())) {
        ...
        if (fs) {
            ...
            if (!getEnvBool("FSLIB_DISABLE_PROXY_FILE_SYSTEM", false)) {
...
AbstractFileSystem* FileSystemFactory::getFs(const FsType& type) {
    ...
    if (unlikely(!fs)) {
        ...
        BEEPER_FORMAT_REPORT_WITHOUT_TAGS(
                FSLIB_ERROR_COLLECTOR_NAME,
                "not support file system of type [%s]", type.c_str());
        #endif
    } else {
...
bool FileSystemFactory::getEnvBool(const char* name, bool defaultValue)
{
    const char* env = getenv(name);
    if (env) {
        bool ret = defaultValue;
        if (autil::StringUtil::fromString(env, ret)) {
            return ret;
        }
    }
    return defaultValue;
...
AbstractFileSystem* FileSystemFactory::createDfsFileSystem() {
    ...
    const char* mode = getenv("FSLIB_DFS_STORAGE_MODE");
    if (mode) {
        bool useHdfs = (strcmp(mode, "hdfs") == 0);
        AUTIL_LOG(INFO, "env FSLIB_DFS_STORAGE_MODE set to [%s], useHdfs[%d]", mode, useHdfs);
    ...
    bool pov = false;
    bool dir = true;
    const char* linkEnv = getenv("FSLIB_DFS_STORAGE_LINKS");
    if (!linkEnv) {
        linkEnv = getenv("HIPPO_ENV_STORAGE_LINKS");
    }
    if (linkEnv) {
