...
public:
    ...
    static std::string
    appendParam(const std::string &rawPath, const std::string &key, const std::string &value, bool overwrite = false);
    static std::string eraseParam(const std::string &rawPath, const std::string &key);

private:
    static std::string assemblePath(const Slice &scheme, const Slice &netloc, const Slice &params, const Slice &path);
