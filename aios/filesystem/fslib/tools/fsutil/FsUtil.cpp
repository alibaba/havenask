...
public:
    static bool parse(std::unique_ptr<T> &reader)
    {
        ...
        if ((gzip_header.magic1 != 31) || (gzip_header.magic2 != 139) ||
...
void FsUtil::timeToStr(uint64_t time, char* timestr) {
    ...
    sprintf(timestr, "%d-%d-%d %d:%d:%d", result.tm_year + 1900, result.tm_mon + 1, 
            result.tm_mday, result.tm_hour, result.tm_min, result.tm_sec);
}

ErrorCode FsUtil::runCopy(const char* srcPath, const char* dstPath,
                     bool recursive) 
{
    if (strcmp(srcPath, dstPath) == 0) {
        cerr << "copy src <" << srcPath << "> to dst <" << dstPath
             << "> fail. cannot copy path to itself." << endl
             << endl << "operation fail! " << endl;
        return EC_NOTSUP;
    }
    ...
    if (FileSystem::isZkLikeFileSystem(srcFs) &&
        !FileSystem::isZkLikeFileSystem(dstFs))
    {
...
ErrorCode FsUtil::runEncrypt(const char* srcPath, const char* dstPath,
                             const char* cipherOption, bool optionUseBase64)
{
    ...
    if (ret != EC_OK) {
        cerr << "fail to get length of file <" << srcPath
             << ">. " << FileSystem::getErrorString(ret) 
             << endl;
        return ret;
    }
    unique_ptr<File> rFile(FileSystem::openFile(srcPath, READ));
    if (!rFile->isOpened()) {
        cerr << "open file <" << srcPath << "> for read fail. "
             << FileSystem::getErrorString(rFile->getLastError()) << endl;
    ...
    if (!wFile->isOpened()) {
        cerr << "open file <" << dstPath << "> for write fail. "
    ...
    auto encrypter = autil::cipher::AESCipherCreator::createStreamEncrypter(
            option, 2 * 1024 * 1024 /* 2 MB */);
    ...
    std::thread writeThread([&]() {
        ...
        wCode = wFile->close();
        if (wCode != EC_OK) {
            cerr << "close dst file <" << dstPath << "> fail.  "
                 << FileSystem::getErrorString(wFile->getLastError()) << endl;
            hasError = true;
        }
    });
    ...
    cout << "plain file length:" << totalEncryptLen
         << ", cipher file length:" << totalWriteLen << endl;
    cout << "=================================================" << endl;    
...
ErrorCode FsUtil::runDecrypt(const char* srcPath, const char* dstPath,
                             const char* cipherOption, bool optionUseBase64)
{
    autil::cipher::CipherOption option(autil::cipher::CT_UNKNOWN);
    if (cipherOption == nullptr) {
    ...
    volatile bool hasError = false;
    ErrorCode rCode = EC_OK;
    auto decrypter = autil::cipher::AESCipherCreator::createStreamDecrypter(
            option, 2 * 1024 * 1024 /* 2 MB */);
    ...
    cout << "cipher file length:" << totalDecryptLen
         << ", plain file length:" << totalWriteLen << endl;
    cout << "=================================================" << endl;
...
public:
    AESCipherDataReader(
            std::unique_ptr<autil::cipher::AESCipherStreamDecrypter>& decrypter, volatile bool& hasError)
        : _hasError(hasError)
...
ErrorCode FsUtil::runDecryptCat(const char* srcPath, 
                                const char* cipherOption, bool optionUseBase64)
{
    autil::cipher::CipherOption option(autil::cipher::CT_UNKNOWN);
    if (cipherOption == nullptr) {
                                 ...
                                 size_t maxInputTimes, T& value)
{
    ...
    while (true)
    {
        ...
        if (!validInputs.empty() &&
            validInputs.find(inputStr) == validInputs.end())
        {
            std::cout << "invalid input: [" << inputStr << "], candidates : "
                      << autil::StringUtil::toString(validInputs, "|") << endl;
            continue;
        }
...
bool FsUtil::getCipherOption(autil::cipher::CipherOption& option)
{
    ...
    if (!getUserConsoleInput("\n2. select cipher mode [passwd|key], enter for default[passwd] :",
                             { "passwd", "key" }, "passwd", maxInputTimes, mode))
    {
        return false;
    }
    ...
    if (mode == "passwd") {
        ...
        if (!getUserConsoleInput("\n4. select digist [md5|sha256], enter for default[md5] :",
                        { "md5", "sha256"}, "md5", maxInputTimes, digist))
        {
            return false;
        }
        autil::cipher::DigistType dgstType =
            autil::cipher::CipherOption::parseDigistType(digist);
        if (dgstType == autil::cipher::DT_UNKNOWN) {
            std::cout << "invalid digist [" << digist << "]" << endl;
            return false;
        }
        std::string usePbkdf2;
        if (!getUserConsoleInput("\n5. use pbkdf2 ? yes or no, enter for default [no] :",
                        { "yes", "no" }, "no", maxInputTimes, usePbkdf2))
        {
            return false;
        }
        ...
        if (!getUserConsoleInput("\n6. need salt ? yes or no, enter for default [yes] :",
                        { "yes", "no" }, "yes", maxInputTimes, needSaltStr))
        {
            return false;
        }
        if (needSaltStr == "no") {
        ...
        if (!getUserConsoleInput("\n7. use random salt ? yes or no, enter for default [yes] :",
                        { "yes", "no" }, "yes", maxInputTimes, useRandomSaltStr))
        {
            return false;
        }
        if (useRandomSaltStr == "yes") {
            option = autil::cipher::CipherOption::passwordWithRandomSalt(
                    type, dgstType, passwd, usePbkdf2 == "yes");
            return true;
        }
        std::string useDefineSalt;
        getUserConsoleInput("\n8. input salt hex string:", {}, "", maxInputTimes, useDefineSalt);
        option = autil::cipher::CipherOption::passwordWithSalt(
                type, dgstType, passwd, useDefineSalt, usePbkdf2 == "yes");
        return true;
    }
...
ErrorCode FsUtil::runListDirWithRecursive(const char* dirname, 
...
ErrorCode FsUtil::runIsExist(const char* pathname) {
    ...
    if (ret == EC_TRUE) {
        ...
        cerr << FileSystem::getErrorString(ret) << endl
             << "operation fail!" << endl;
    }
...
ErrorCode FsUtil::runMd5Sum(const char* fileName) {
    ...
    if (ret != EC_OK) {
        cerr << "fail to get length of file <" << fileName
             << ">. " << FileSystem::getErrorString(ret) 
             << endl;
        return ret;
    }
    ...
    if (!file->isOpened()) {
        cerr << "open file <" << fileName << "> for read fail. "
             << FileSystem::getErrorString(file->getLastError()) << endl;
...
ErrorCode FsUtil::getPathMeta(const string& pathName, FileMeta& meta) {
    ...
    if (ret != EC_OK) {
        cerr << "get metainfo of file <" << pathName << "> fail!"
             << FileSystem::getErrorString(ret) << endl;
    }
    return ret;
}

ErrorCode FsUtil::getDirMeta(const string& pathName, uint32_t& fileCount,
                        uint32_t& dirCount, uint64_t& size) 
{
    FileList fileList;
    ErrorCode ret = FileSystem::listDir(pathName, fileList);
    if (ret != EC_OK) {
        cerr << "list dir <" << pathName << "> fail!"
             << FileSystem::getErrorString(ret) << endl;
        return ret;
    }
    ...
    for (size_t i = 0; i < fileList.size(); i++) {
        ...
        if (ret == EC_TRUE) {
            ...
            if (ret == EC_TRUE) {
                ...
                cerr << "path <" << subPath << "> is neither a file nor directory" 
                     << endl;
                return EC_NOTSUP;
            } else {
                cerr << "get path <" << subPath << "> meta fail, " 
                     << FileSystem::getErrorString(ret) << endl;
                return ret;
            }
        } else {
...
ErrorCode FsUtil::runGetMeta(const char* pathName) {
    ...
    if (ret == EC_TRUE) {
        ...
        if (ret == EC_TRUE) {
            ...
            cerr << "path <" << pathName << "> is neither a file or directory" 
                 << endl << "operation fail!" << endl;
            return EC_NOTSUP;
        } else {
            cerr << "fail to get meta of path <" << pathName << ">, "
                 << FileSystem::getErrorString(ret)
                 << endl << "operation fail!" << endl;
            return ret;
        }
...
ErrorCode FsUtil::getZookeeperNodeMeta(const string& pathName,
