...
public:
    ColumnDataBase(matchdoc::ReferenceBase *ref, std::vector<Row> *rows) : _ref(ref), _rows(rows) {}
...
public:
    ColumnData(matchdoc::Reference<T> *ref, std::vector<Row> *rows) : ColumnDataBase(ref, rows), _ref(ref) {}
    ~ColumnData() {}
