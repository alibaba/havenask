...
Table::Table(const std::shared_ptr<autil::mem_pool::Pool> &poolPtr)
    : _allocator(new MatchDocAllocator(poolPtr)), _dependentPools({poolPtr}) {}

Table::Table(const vector<MatchDoc> &matchDocs, const MatchDocAllocatorPtr &allocator, uint8_t level)
    : _rows(matchDocs), _allocator(allocator), _dependentPools({allocator->getPoolPtr()}) {
...
void Table::init() {
    ReferenceVector refs = _allocator->getRefBySerializeLevel(_serializeInfo.serializeLevel);
    const ReferenceVector &subRefs = _allocator->getAllSubNeedSerializeReferences(_serializeInfo.serializeLevel);
...
void Table::compact() {
    if (_allocator->getDocSize() > 0 &&
        _allocator->getAllocatedCount() > NEED_COMPACT_MEM_SIZE / _allocator->getDocSize() + getRowCount()) {
...
Column *Table::declareColumn(const std::string &name, matchdoc::BuiltinType bt, bool isMulti, bool endGroup) {
...
Column *Table::declareColumnWithConstructFlag(
    const std::string &name, matchdoc::BuiltinType bt, bool isMulti, bool needConstruct, bool endGroup) {
    auto iter = _columnMap.find(name);
    if (iter != _columnMap.end()) {
        if (iter->second != nullptr) {
            return iter->second;
    ...
    auto func = [&](auto a) {
        ...
        matchdoc::Reference<T> *ref =
            _allocator->declareWithConstructFlagDefaultGroup<T>(name, needConstruct, SL_ATTRIBUTE);
        if (!ref) {
            AUTIL_LOG(WARN, "can not declare column [%s]", name.c_str());
            return false;
        }
...
Column *Table::getColumn(size_t col) const { return col < getColumnCount() ? _columnVec[col] : nullptr; }
...
void Table::mergeDependentPools(const TablePtr &other) { mergeDependentPools(*other); }
...
void Table::dependPool(const autil::mem_pool::PoolPtr &pool) { _dependentPools.insert(pool); }

const std::set<std::shared_ptr<autil::mem_pool::Pool>> &Table::getDependentPools() const { return _dependentPools; }
...
size_t Table::estimateUsedMemory() const { return getRowCount() * _allocator->getDocSize(); }
