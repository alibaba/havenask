...
class VectorStorage : public std::enable_shared_from_this<VectorStorage> {
public:
    struct Chunk {
        Chunk() = default;
...
public:
    ...
    const std::shared_ptr<autil::mem_pool::Pool> &getPoolPtr() const { return _pool; }
...
public:
    template <typename T>
    static std::shared_ptr<VectorStorage>
