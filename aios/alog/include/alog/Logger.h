 ...
 *
 *The Logger class is the handle to output log information, one logger may have many output destinations called Appender
 *class,
 *@n and you can call Logger's method to change output destination, log level etc.
 *@n There is a root Logger object initialized in getRootLogger() function, and its default output destination is
 *console, default log
 *@n level is LOG_LEVEL_INFO, if you do not want to change the logger's action in multi-module, you had better use the
 *root Logger
...
public:
     ...
     */
    void log(uint32_t level, const char *filename, int line, const char *function, const char *format, ...)
     ...
     *@param name the logger name which will be outputed. default NULL, then name will be revalued to
     *this->m_loggerName.
     *@param isLevel whether need to compare log level. default true. if this method is called from it's child,this
     ...
     *
     *this method will remove all the appenders belong to the logger before, then set the appender as the logger's
     *output destination. If you want to add a appender to logger's appender list, please call addAppender() function.
     ...
     */
    inline bool isLevelEnabled(uint32_t level) { return (level <= m_loggerLevel) ? true : false; }
    /**
     ...
     */
    const Logger *getParentLogger() const { return m_parent; }
...
private:
     ...
     */
    void _log(LoggingEvent &event, int depth = 0);
    /**
    ...
    std::unique_ptr<RWMutex> m_appenderMutex;
    /* Logger's name.*/
...
template <typename T>
static char *getTLSBufferStorage() {
    static thread_local bool valid = true;
    class BufferStorage {
    public:
        BufferStorage(size_t size) : _size(size), _buffer(new char[size]) {}
        ~BufferStorage() {
            delete[] _buffer;
            _buffer = nullptr;
            _size = -1;
    ...
    static thread_local BufferStorage storage(Logger::MAX_MESSAGE_LENGTH);
    // check TLS storage deallocated
    if (!valid) {
        return nullptr;
    }
    if (storage.size() != Logger::MAX_MESSAGE_LENGTH) {
        storage.resize(Logger::MAX_MESSAGE_LENGTH);
    }
