...
class HapePackTextIndexTest(unittest.TestCase, HapeTestBase):
    ...
    def _run_direct_table_create(self, table):
        self.assertTrue(self.hape_domain.create_table(table, 1, self.testdata_root + "/pack_schema.json", None))
        self.assertTrue(self.hape_domain.suez_cluster.wait_cluster_ready())
    ...
    def _update_load_strategy_1(self, table):
        ...
        self.assertTrue(self.hape_domain.create_or_update_load_strategy(table, json.dumps(online_index_config)))
        self.assertTrue(self.hape_domain.suez_cluster.wait_cluster_ready())
    ...
    def _check_max_realtime_memory_use(self, mem_str):# mem_str equal to max_realtime_memory_use/3
        searcher_pid = self.getSearcherPid()
        sql_log_file_path = os.path.join(self.workdir, 'havenask-sql-proc_database.database_partition_0/ha_sql/logs/sql.log')
        ...
        indexlib_log_file_path = os.path.join(self.workdir, 'havenask-sql-proc_database.database_partition_0/ha_sql/logs/index_bs.log')
        def check_update_load_strategy_ready():
