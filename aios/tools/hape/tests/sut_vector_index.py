...
class HapePackTextIndexTest(unittest.TestCase, HapeTestBase):
    def setUp(self):
        self.init(test_conf_dir="hape_conf/vector-proc")
    ...
    def _run_offline_table_rw(self, table):
        ...
        #     "vector": vector
        # }
        # write_swift_message_bydict(self.self.domain_config.global_config.get_service_appmaster_zk_address(HapeCommon.SWIFT_KEY), table, 1, "id", data)
        response = self.search_sql(address, query)
        self.assert_result(response, ["id"], ["uint32"], [[1]])
    
    def _run_direct_table_create(self, table):
        self.assertTrue(self.hape_domain.create_table(table, 1, self.testdata_root + "/vector/vector_schema.json", None))
        self.assertTrue(self.hape_domain.suez_cluster.wait_cluster_ready())
        
    def _run_offline_table_create(self, table):
        self.assertTrue(self.hape_domain.create_table(table, 1, self.testdata_root + "/vector/vector_schema.json", self.testdata_root + "/vector/vector.data"))
