...
class ClusterConfig():
    ...
    def get_extra_envs_and_args(self, key):
        if self.has_domain_config_param(key, "extraEnvs"):
            envs = self.get_domain_config_param(key, "extraEnvs").split(";")
        else:
            envs = []
        if self.has_domain_config_param(key, "extraArgs"):
            args = self.get_domain_config_param(key, "extraArgs").split(";")
        else:
            args = []
        return envs, args
