...
#define REPORT_USED_TIME(nodePath, metricName)                          \
    carbon::monitor::ReportUsedTime                                     \
    __reportUsedTime(nodePath, carbon::monitor::metricName)
...
{
public:
    ReportUsedTime(const std::string &nodePath, const std::string &metricName)
        : _nodePath(nodePath), _metricName(metricName)
    {
    ...
    ~ReportUsedTime() {
        ...
        REPORT_METRIC_RAW(_nodePath, _metricName, usedTimeMS);
    }
private:
