        ...
        GroupPlan *groupPlan)
{
    ...
    string skylineGroup;
    string skylineAppStage;
    transferServiceConfigs(groupDesc.services, &serviceConfigsMap, skylineGroup, skylineAppStage);
    ...
    for (size_t i = 0; i < groupDesc.roles.size(); i++) {
        ...
        RolePlan rolePlan;
        transferRoleDescToRolePlan(roleDesc, skylineGroup, skylineAppStage, &rolePlan);
    ...
    groupPlan->schedulerConfig.name = groupDesc.schedulerName;
    groupPlan->schedulerConfig.configStr = groupDesc.schedulerConfig;
    groupPlan->minHealthCapacity = minHealthCapacity;
    groupPlan->extraRatio = groupDesc.stepPercent;
}

void TransferUtil::transferConstraintConfig(
        const proto::ConstraintConfig& pbConstraintConfig,
        ConstraintConfig& constraintConfig)
{
    if (pbConstraintConfig.has_spreadlevel()) {
        constraintConfig.level = hippo::ConstraintConfig::SpreadLevel(pbConstraintConfig.spreadlevel());
    }
    if (pbConstraintConfig.has_strictly()){
        constraintConfig.strictly = pbConstraintConfig.strictly();
    }
    if (pbConstraintConfig.has_usehostworkdir()) {
        constraintConfig.useHostWorkDir = pbConstraintConfig.usehostworkdir();
    }
    if (pbConstraintConfig.has_maxinstanceperhost()) {
        constraintConfig.maxInstancePerHost = pbConstraintConfig.maxinstanceperhost();
    }
    if (pbConstraintConfig.has_maxinstanceperframe()) {
        constraintConfig.maxInstancePerFrame = pbConstraintConfig.maxinstanceperframe();
    }
    if (pbConstraintConfig.has_maxinstanceperrack()) {
        constraintConfig.maxInstancePerRack = pbConstraintConfig.maxinstanceperrack();
    }
    if (pbConstraintConfig.has_maxinstanceperasw()) {
        constraintConfig.maxInstancePerASW = pbConstraintConfig.maxinstanceperasw();
    }
    if (pbConstraintConfig.has_maxinstanceperpsw()) {
        constraintConfig.maxInstancePerPSW = pbConstraintConfig.maxinstanceperpsw();
    }
    for (auto i = 0; i < pbConstraintConfig.specifiedips_size(); ++i) {
        constraintConfig.specifiedIps.push_back(pbConstraintConfig.specifiedips(i));
    }
    for (auto i = 0; i < pbConstraintConfig.prohibitedips_size(); ++i) {
        constraintConfig.prohibitedIps.push_back(pbConstraintConfig.prohibitedips(i));
    }
}
        ...
        RolePlan *rolePlan)
{
    ...
    global.minHealthCapacity = roleDesc.minhealthcapacity();
    if (roleDesc.properties_size() > 0) {
        global.properties = transferKVs(roleDesc.properties());
    }
    transferHealthCheckerConfig(roleDesc, &global.healthCheckerConfig);
    global.brokenRecoverQuotaConfig.maxFailedCount = roleDesc.brokenrecoverquota();
    ...
    versionedPlan.recoverResourceNotMatchNode = roleDesc.recoverresourcenotmatchnode();
    ...
    if (roleDesc.metatags_size() > 0) {
        resourcePlan.metaTags = transferKVs(roleDesc.metatags());
    }
    if (!skylineGroup.empty()){
        resourcePlan.metaTags[METATAGS_INSTANCE_GROUP_KEY] = skylineGroup; 
    }
    if (!skylineAppStage.empty()){
        resourcePlan.metaTags[METATAGS_APP_STAGE_KEY] = skylineAppStage; 
    }
    ...
    if (roleDesc.has_constraints()) {
        transferConstraintConfig(roleDesc.constraints(), 
                                 resourcePlan.constraints);
    }
}
        ...
        std::string &skylineAppStage)
{
    for (size_t i = 0; i < serviceVec.size(); i++) {
        ...
        for (int32_t i = 0; i < serviceRequest.serviceconfigs_size(); i++) {
            ...
            if (serviceConfig.type == ST_SKYLINE) {
                SkylineServiceConfig _skylineServiceConfig;
                try {
                    FromJsonString(_skylineServiceConfig, serviceConfig.configStr);
                } catch (const autil::legacy::ExceptionBase &e) {
                    CARBON_LOG(ERROR, "skyline service config from json fail, exception[%s]", e.what());
                }
                skylineGroup = _skylineServiceConfig.group;
                skylineAppStage = _skylineServiceConfig.appUseType;
            }
...
std::map<std::string, std::string> TransferUtil::transferKVs(const ::google::protobuf::RepeatedPtrField<proto::KVPair> &kvpairs)
{
    map<std::string, std::string> result;
    int count = kvpairs.size();
    for (int i = 0; i < count; i++) {
        result[kvpairs.Get(i).key()] = kvpairs.Get(i).value();
    }
    return result;
}
                                      ...
                                      proto::RoleStatus *roleStatusProto)
{
    ...
    roleStatusProto->set_adjustedcount(roleStatus.adjustedCount);
    roleStatusProto->set_adjustedminhealthcapacity(roleStatus.adjustedMinHealthCapacity);
        ...
        proto::ReplicaNode *replicaNodeProto)
{
    ...
    replicaNodeProto->set_workermode(workerNodeStatus.workerMode);
