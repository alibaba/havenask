...
public:
    ...
    {
        if (!_executor && force) {
            FL_LOG(ERROR, "executor is nullptr, can not set force schedule continaution");
            return;
        }
        _forceSched = force;
    }
...
private:
    void scheduleContinuation(bool triggerByContinuation)
    {
        ...
        if (!_forceSched && (!_executor || triggerByContinuation || currentThreadInExecutor())) {
            ...
            try {
                ...
                if (Executor::NULLCTX == _context) {
                    ret = _executor->schedule([fsRef = std::move(guard)]() mutable {
                    ...
                    ret = _executor->checkin([fsRef = std::move(guard)]() mutable {
                        auto ref = std::move(fsRef);
                        auto fs = ref.getFutureState();
                        fs->_continuation(std::move(fs->_try));
                    }, _context, opts);
