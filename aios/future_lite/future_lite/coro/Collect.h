...
//std::vector<Try<int>> = co_await collectAllWindowed(maxConcurrency, yield, std::vector<intLazy>);
//std::vector<Try<int>> = co_await collectAllWindowedPara(maxConcurrency, yield, std::vector<intLazy>);
...
struct CollectAnyAwaiter {
    ...
    void await_suspend(std::coroutine_handle<> continuation) {
        ...
        auto parent = promise_type._storage;
        ...
        for (i = 0; i < input.size() && (result->_idx == -1); ++i) {
            ...
            auto callback = [i = i, size = input.size(), r = result,
                             c = continuation,
            ...
            input[i].template start<decltype(callback), false>(std::move(callback), parent);
        }  // end for
    }
    auto await_resume() {
...
struct CollectAllAwaiter {
    ...
    inline void await_suspend(
        ...
        for (size_t i = 0; i < _input.size(); ++i) {
            ...
            auto&& func = [this, i, parent]() {
                auto callback = [this, i](Try<ValueType>&& result) {
                ...
                _input[i].template start<decltype(callback), false>(std::move(callback), parent);
            };
