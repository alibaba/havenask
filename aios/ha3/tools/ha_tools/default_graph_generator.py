...
class Ha3GraphGenerator(object):
    ...
    def _genSearcherGraph(self, summaryWithExtra = False):
        ...
        with g.as_default():
            ...
            run_model = turing_module.run_model_op(request, seek_res[0], name="run_model")
            srcCount = tf.constant(1, dtype=tf.uint16)
            with tf.control_dependencies([kvpair, run_model, code_compile]):
                final_score = turing_module.cava_scorer_op(kvpair, run_model,
                                                           name="final_score",
