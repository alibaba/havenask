...
examples:
    ...
    def _runHttpSearch(self):
        ...
        allQuery = ''
        if self.qrsQueueName or self.timeout or self.eagleeyeTraceId or self.eagleeyeUserdata or self.bizName:
            queryMap = {}
            queryMap["assemblyQuery"] = self.query
            if self.qrsQueueName:
                queryMap["taskQueueName"] = self.qrsQueueName
            if self.timeout:
                queryMap["timeout"] = self.timeout
            if self.eagleeyeUserdata:
                queryMap["userData"] = self.eagleeyeUserdata
            if self.eagleeyeTraceId:
                queryMap["traceId"] = self.eagleeyeTraceId
            if self.bizName:
                queryMap["bizName"] = self.bizName
            allQuery = json.dumps(queryMap)
        else:
            allQuery = self.query
        tmp.write(allQuery)
        tmp.flush()
    ...
    def _runArpcSearch(self):
        ...
        callMethod = 'search'
        if self.useSql:
            callMethod = 'searchSql'
        reply = stubWrapper.asyncCall(callMethod, request)
