...
HA3_LOG_SETUP(index, FakeMetaMaker);
using namespace std;
...
void FakeMetaMaker::makeFakeMeta(std::string str,
    ...
    while (true) {
        ...
        FakeTextIndexReader::DocSectionMap::iterator iter =
            docSecMetaMap.find((docid_t)docId);        
...
void FakeMetaMaker::parseDocMeta(std::string str, FakeInDocSectionMeta::DocSectionMeta
        &docSecMeta) {
    size_t startPos = 0;
    ...
    while (true) {
        ...
        if (iter == docSecMeta.fieldLength.end()) {
            docSecMeta.fieldLength.insert(std::make_pair((int32_t)fieldPosition,
                (field_len_t)fieldLen));
...
void FakeMetaMaker::parseFieldMeta(std::string str, int32_t fieldPosition,
    ...
    while (true) {
        ...
        std::pair<int32_t, sectionid_t> p = std::make_pair((int32_t)fieldPosition,
            (sectionid_t)secId);                
        FakeTextIndexReader::FieldAndSectionMap::iterator iter = 
            fieldAndSecMap.find(p);
