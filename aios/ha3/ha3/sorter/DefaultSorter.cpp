...
HA3_LOG_SETUP(sorter, DefaultSorter);
    ...
    if (finalSortClause) {
        ...
        if (iter != kvMap.end()) {
            if (!initSortExpressionsFromSortKey(iter->second, sortExpressions, provider)) {
                HA3_LOG(WARN, "init sortExpression from SortKey[%s] failed.", iter->second.c_str());
                return false;
            }
            return true;
        }
        ...
        SorterProvider *provider)
{
    ...
    for (uint32_t i  = 0; i < fieldNum; i++) {
        ...
        if (attributeExpr->isMultiValue()) {
            HA3_LOG(WARN, "%s is multi value attribute, cannot be used in sort key!", 
                    sortField.c_str());
            return false;
        }
