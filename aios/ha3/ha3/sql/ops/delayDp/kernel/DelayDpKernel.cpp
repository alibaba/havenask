 ...
 */
#include "ha3/sql/ops/delayDp/DelayDpKernel.h"
...
AUTIL_LOG_SETUP(sql, DelayDpKernel);
...
bool DelayDpKernel::config(navi::KernelConfigContext &ctx) {
            ...
            try {
                ...
                if (tableDist.partCnt > 1) {
                    if (_partCnt == 1) {
                        ...
                        SQL_LOG(WARN, "invalid table distCnd, prev:%d, cur:%d",
                            _partCnt, tableDist.partCnt);
                        return false;
                    }
                }
    ...
    string debugStr;
    if (initStringAttr(ctx, DELAY_DP_DEBUG_ATTR, debugStr) &&
        debugStr == "1")
    {
        _debug = true;
    }
    ...
    if (inputSize != distSize) {
        SQL_LOG(WARN, "input_names size[%ld] not equal to input_dists size[%ld]",
            inputSize, distSize);
        return false;
    }
...
bool DelayDpKernel::initStringAttr(
...
bool DelayDpKernel::initVectorAttr(
    navi::KernelConfigContext &ctx,
    const std::string& key,
...
bool DelayDpKernel::mergeInputData(navi::GroupDatas &datas) {
    ...
    if (dataSize != inputSize) {
        SQL_LOG(ERROR, "group data size[%ld] not equal to config size[%ld]",
            dataSize, inputSize);
        return false;
    }
    for (size_t i = 0; i < dataSize; ++i) {
        ...
        auto inputTable = KernelUtil::getTable(streamData.data, _memoryPoolResource);
...
void DelayDpKernel::addOverrideDatas(
    int partId,
    std::vector<navi::OverrideData> &overrideDatas)
{
    for (size_t i = 0; i < _inputInfos.size(); ++i) {
        ...
        if (info.partRows.empty()) {
            ...
            table.reset(new Table(
                info.partRows[partId],
                info.table->getMatchDocAllocatorPtr()));
        }
        ...
        SQL_LOG(DEBUG, "override data, partId:%d, row count:%ld",
            overrideData.partId, table->getRowCount());
...
navi::ErrorCode DelayDpKernel::forkGraph(navi::KernelComputeContext &runContext) {
    ...
    std::vector<navi::OverrideData> overrideDatas;
    if (!redirectGraphInput(overrideDatas)) {
    ...
    if (_metaInfoResource) {
        auto p = _metaInfoResource->getOverwriteInfoCollector();
        if (p) {
            DelayDpInfo info;
            ...
            p->overwriteDelayDpInfo(info);
            p->setSqlRunForkGraphBeginTime(TimeUtility::currentTime());
        }
    }
    return runContext.fork(_graphDef.release(), overrideDatas);
}
