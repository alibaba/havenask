...
class MergeKernelTest : public OpTestBase {
public:
    MergeKernelTest();
    ~MergeKernelTest();
public:
...
MergeKernelTest::MergeKernelTest() {
}

MergeKernelTest::~MergeKernelTest() {
}

TEST_F(MergeKernelTest, testSimpleProcess) {
...
TEST_F(MergeKernelTest, testBatchInput) {
    KernelTesterBuilder builder;
    auto testerPtr = buildTester(builder);
    ASSERT_TRUE(testerPtr.get());
    auto &tester = *testerPtr;
...
TEST_F(MergeKernelTest, testNoInput) {
    KernelTesterBuilder builder;
    auto testerPtr = buildTester(builder);
    ASSERT_TRUE(testerPtr.get());
    auto &tester = *testerPtr;
...
TEST_F(MergeKernelTest, testKernelDoComputeFailed) {
    KernelTesterBuilder builder;
    auto testerPtr = buildTester(builder);
    ASSERT_TRUE(testerPtr.get());
    auto &tester = *testerPtr;
...
TEST_F(MergeKernelTest, testDoCompute) {
...
TEST_F(MergeKernelTest, testDoComputeInvalidInput) {
    navi::NaviLoggerProvider provider;
    MergeKernel kernel;
    prepareTable();
    ASSERT_TRUE(kernel.doCompute(_table));
...
TEST_F(MergeKernelTest, testDoComputeSchemaMismatch) {
    navi::NaviLoggerProvider provider;
    MergeKernel kernel;
    prepareTable();
    ASSERT_TRUE(kernel.doCompute(_table));
