...
bool JoinBase::getColumnInfo(const table::TablePtr &table, const string &field,
...
{
    size_t outputSize = min(_joinBaseParam._outputFields.size(),
...
bool JoinBase::evaluateLeftTableColumns(const vector<size_t> &rowIndexes,
                                        ...
                                        table::TablePtr &outputTable)
{
    size_t leftSize = min(_joinBaseParam._leftInputFields.size(),
                          _joinBaseParam._outputFields.size());
    for (size_t i = 0; i < leftSize; i++) {
        const string &inputField = _joinBaseParam._leftInputFields[i];
        const string &outputField = _joinBaseParam._outputFields[i];
        if (!evaluateTableColumn(inputTable, outputTable, inputField, outputField,
                        joinIndexStart, rowIndexes))
        {
            return false;
        }
    }
...
bool JoinBase::evaluateRightTableColumns(const vector<size_t> &rowIndexes,
        const table::TablePtr &inputTable, size_t joinIndexStart,
        table::TablePtr &outputTable)
{
    size_t outputSize = _joinBaseParam._outputFields.size();
    size_t leftSize = min(_joinBaseParam._leftInputFields.size(), outputSize);
...
bool JoinBase::evaluateTableColumn(const table::TablePtr &inputTable,
                                   table::TablePtr &outputTable,
                                   const std::string &inputField,
                                   const std::string &outputField, size_t joinIndex,
...
bool JoinBase::evaluateEmptyColumns(size_t fieldIndex, size_t joinIndex,
                                    table::TablePtr &outputTable)
{
    ...
    auto func = [&](auto a) {
         ...
         const std::string &type =
             ExprUtil::transVariableTypeToSqlType(vt.getBuiltinType());
         ...
         if (!type.empty() && iter != _joinBaseParam._defaultValue.end()) {
             if (!StringUtil::fromString<T>(iter->second, val)) {
                 SQL_LOG(ERROR, "field [%s] type [%s] cast default value [%s] failed",
                         _joinBaseParam._outputFields[index].c_str(), type.c_str(),
                         iter->second.c_str());
                 return false;
             }
         }
    ...
    auto strFunc = [&]() {
         ...
         auto iter = _joinBaseParam._defaultValue.find(type);
         if (!type.empty() && iter != _joinBaseParam._defaultValue.end()) {
             MultiChar val(MultiValueCreator::createMultiValueBuffer(iter->second.data(),
                             iter->second.size(), _joinBaseParam._pool));
