...
LookupJoinKernel::LookupJoinKernel()
    ...
    _matchedInfo = new search::MatchDataCollectMatchedInfo(
...
bool LookupJoinKernel::config(navi::KernelConfigContext &ctx) {
    ...
    ctx.Jsonize("left_is_build", _leftTableIndexed, _leftTableIndexed);
    ctx.Jsonize("lookup_batch_size", _lookupBatchSize, _lookupBatchSize);
    if (_lookupBatchSize == 0) {
        _lookupBatchSize = LOOKUP_BATCH_SIZE;
    }
    auto subCtx = ctx.enter("build_node");
    if (!_initParam.initFromJson(subCtx)) {
        return false;
    }
    ctx.Jsonize("left_table_meta", _leftTableMeta, _leftTableMeta);
    ctx.Jsonize("right_table_meta", _rightTableMeta, _rightTableMeta);
...
navi::ErrorCode LookupJoinKernel::init(navi::KernelInitContext &context) {
    ...
    if (_initParam.isRemoteScan()) {
        if (_initParam.tableType != SCAN_KV_TABLE_TYPE) {
            SQL_LOG(ERROR, "remote table only support kv table now");
            return navi::EC_ABORT;
        }
        _scanBase.reset(new RemoteScan(_initParam, _naviQuerySessionR));
    } else {
        if (_initParam.tableType == SCAN_NORMAL_TABLE_TYPE) {
            ...
            _scanBase.reset(new SummaryScan(_initParam, _asyncPipe, false));
            ...
            _scanBase.reset(new KVScan(_initParam, _asyncPipe, false));
        } else if (_initParam.tableType == SCAN_KKV_TABLE_TYPE) {
            _scanBase.reset(new KKVScan(_initParam, false));
            ...
            _scanBase.reset(new Ha3SqlRemoteScan(_initParam, _asyncPipe, false));
            _asyncPipe->setData(ActivationData::inst()); // mock activation data for first compute
        } else {
    ...
    std::string field;
    if (_disableMatchRowIndexOptimize || _scanBase->getUseSub() ||
    ...
    if (_useMatchedRow) {
        _scanBase->setMatchDataCollectorBaseConfig(_matchedInfo);
                                 ...
                                 TablePtr &outputTable)
{
    ...
    } else if (_useMatchedRow && _initParam.tableType == SCAN_NORMAL_TABLE_TYPE) {
...
bool LookupJoinKernel::docIdsJoin(const LookupJoinBatch &batch,
                                  const TablePtr &streamOutput)
{
    SQL_LOG(DEBUG, "use docIds join optimize");
    if (batch.count != streamOutput->getRowCount()) {
        SQL_LOG(ERROR, "unexpected input table size [%lu] != joined table size [%lu]",
                batch.count, streamOutput->getRowCount());
        return false;
    }
...
bool LookupJoinKernel::doHashJoin(const LookupJoinBatch &batch,
...
StreamQueryPtr
LookupJoinKernel::genFinalStreamQuery(const LookupJoinBatch &batch) {
    StreamQueryPtr streamQuery;
    if (_initParam.tableType == SCAN_KV_TABLE_TYPE || _initParam.tableType == SCAN_KKV_TABLE_TYPE
        || _initParam.tableType == SCAN_SUMMARY_TABLE_TYPE
        || _initParam.tableType == SCAN_EXTERNAL_TABLE_TYPE) {
        vector<string> pks;
        if (genStreamKeys(batch, pks)) {
            streamQuery.reset(new StreamQuery());
            streamQuery->primaryKeys = std::move(pks);
        }
    } else if (_initParam.tableType == SCAN_NORMAL_TABLE_TYPE) {
        QueryPtr query;
        std::string field;
...
bool LookupJoinKernel::genStreamKeys(const LookupJoinBatch &batch,
                                     vector<string> &pks) {
    string triggerField;
...
void LookupJoinKernel::fillTerms(const LookupJoinBatch &batch,
    ...
    for (uint16_t i = 0; i < _joinColumns->size(); i++) {
        ...
        ColumnTerm *term = makeColumnTerm(batch,
                                          (*_joinColumns)[i],
                                          (*_lookupColumns)[i],
                                          returnEmptyQuery);
        if (term) {
...
void LookupJoinKernel::fillTermsWithRowOptimized(const LookupJoinBatch &batch,
    ...
    if (genRowGroupKey(batch, singleTermCols, rowGroupKey)) {
        ...
        if (genMultiTermColumns(batch.table, multiTermCols, multiTermID, rowGroups, rowCount, terms)) {
            genSingleTermColumns(batch.table, singleTermCols, singleTermID, rowGroups, rowCount, terms);
        }
    }
...
bool LookupJoinKernel::isDocIdsOptimize() {
    auto normalScan = dynamic_cast<NormalScan *>(_scanBase.get());
