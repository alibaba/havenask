 ...
 */
#include "ha3/sql/ops/join/LeftMultiJoinKernel.h"
...
void LeftMultiJoinKernel::def(navi::KernelDefBuilder &builder) const {
    ...
    builder.name("LeftMultiJoinKernel")
...
navi::ErrorCode LeftMultiJoinKernel::init(navi::KernelInitContext &context) {
    ...
    if (_initParam.tableType == SCAN_KV_TABLE_TYPE){
        _scanBase.reset(new KVScan(_initParam, {}, false));
    } else {
    ...
    if (_leftJoinColumns.size() != 1 || _rightJoinColumns.size() != 1) {
        SQL_LOG(ERROR, "only support join one column, left join columns [%s], right join columns [%s]",
...
bool LeftMultiJoinKernel::doMultiJoin(const table::TablePtr &inputTable,
...
bool setExpandColumnData(table::Column *oldColumn, table::Column *newColumn,
    ...
    for (size_t j = 0; j < rowCount; j++) {
        if constexpr (ft == bt_string) {
            ...
            char *buf = autil::MultiValueCreator::createMultiStringBuffer(
                    values, pool);
            ...
            char *buf = autil::MultiValueCreator::createMultiValueBuffer<T>(
                    values, pool);
            ...
            case ft: {                                                  \
                if (isMulti) {                                          \
                    ...
                    if (!setColumnData<T>(tableColumn, newColumn)) {    \
                        SQL_LOG(ERROR, "set column data failed, input [%s]", \
                                inputField.c_str());                    \
                    }                                                   \
                } else {                                                \
...
#undef CASE_MACRO
    ...
    for (size_t i = leftSize; i < outputSize; i++) {
        ...
        if (isMultiValue) {
            SQL_LOG(ERROR, "right table dont support multi value, input field [%s], output field [%s], schema [%s]",
                    inputField.c_str(), outputField.c_str(), tableColumn->getColumnSchema()->toString().c_str());
            return false;
        }
...
#define CASE_MACRO(ft)                                                  \
            case ft: {                                                  \
                if (!setExpandColumnData<ft>(tableColumn, newColumn, resultIndexes, \
                                pool,                                   \
...
bool LeftMultiJoinKernel::initJoinedTable(const table::TablePtr &leftTable,
        ...
        table::TablePtr &outputTable)
{
    outputTable.reset(new table::Table(_memoryPoolResource->getPool()));
        ...
        bool needExpand) const
{
    ...
    Column *column = outputTable->declareColumn(outputField, vt.getBuiltinType(), vt.isMultiValue() | needExpand);
    if (column == nullptr) {
...
bool LeftMultiJoinKernel::getColumnInfo(const table::TablePtr &table, const string &field,
                                        table::Column *&tableColumn, ValueType &vt) const
{
...
StreamQueryPtr LeftMultiJoinKernel::genStreamQuery(const table::TablePtr &input, const std::string &joinColumnName) {
    ...
    if (!vt.isMultiValue()) {
        SQL_LOG(ERROR, "left join column must be multi value, schema [%s]", schema->toString().c_str());
        return nullptr;
    }
