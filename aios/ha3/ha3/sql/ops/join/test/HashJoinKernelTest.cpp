...
private:
    ...
    void prepareAttributeSemiJoin() {
        ...
        _attributeMap["output_fields_internal"] =
            ParseJson(R"json(["$uid", "$cid", "$cid0"])json");
        _attributeMap["equi_condition"] = _attributeMap["condition"] =
            ParseJson(R"json({"op":"=", "type":"OTHER", "params":["$cid0", "$cid"]})json");
    }
    ...
    void prepareTable1(const vector<int32_t> &uid, const vector<int32_t> &cid,
                       bool eof, bool isLeftTable = true)
    {
        std::string port = isLeftTable ? "input0" : "input1";
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(_allocator, uid.size()));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator,
                        leftDocs, "uid", uid));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator,
                        leftDocs, "cid", cid));
        auto leftTable = createTable(_allocator, leftDocs);
        ASSERT_TRUE(_testerPtr->setInput(port, leftTable));
        if (eof) {
            ASSERT_TRUE(_testerPtr->setInputEof(port));
        }
    }

    void prepareTable2(const vector<int32_t> &cid, const vector<string>&group,
                       bool eof, bool isLeftTable = false)
    {
        std::string port = isLeftTable ? "input0" : "input1";
        ...
        ASSERT_TRUE(_testerPtr->setInput(port, rightTable));
        if (eof) {
            ASSERT_TRUE(_testerPtr->setInputEof(port));
        }
    }

    void prepareTable3(const vector<int32_t> &uid, const vector<vector<int32_t>> &cid,
                       bool eof, bool isLeftTable = true)
    {
        std::string port = isLeftTable ? "input0" : "input1";
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(_allocator, uid.size()));
        ...
        for (auto &id : cid) {
            char* buf1 = MultiValueCreator::createMultiValueBuffer(
                    id.data(), id.size(), &_pool);
            cidValue.emplace_back(buf1);
        }
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<MultiValueType<int32_t>>(
...
TEST_F(HashJoinKernelTest, testSimpleProcess) {
    ...
    ASSERT_FALSE(testerPtr->hasError());
    {
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "uid", {0, 0}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "cid", {1, 3}));
        auto leftTable = createTable(_allocator, leftDocs);
        ASSERT_TRUE(testerPtr->setInput("input0", leftTable));
        ASSERT_TRUE(testerPtr->setInputEof("input0"));

        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 5));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {0, 1, 2, 3, 4}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name",
                        {"groupA", "groupB", "groupC", "groupD", "groupE"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
        ASSERT_TRUE(testerPtr->setInputEof("input1"));
    }
...
TEST_F(HashJoinKernelTest, testJoinWithFilter) {
    ...
    _attributeMap["remaining_condition"] =
        ParseJson(R"json({"op":"!=", "type":"OTHER", "params":["$group_name0", "$group_name"]})json");
    ...
    ASSERT_FALSE(testerPtr->hasError());
    {
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "uid", {0, 1, 2}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "cid", {1, 3, 4}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, leftDocs, "group_name", {"groupB", "groupB", "groupB"}));
        auto leftTable = createTable(_allocator, leftDocs);
        ASSERT_TRUE(testerPtr->setInput("input0", leftTable));
        ASSERT_TRUE(testerPtr->setInputEof("input0"));
...
TEST_F(HashJoinKernelTest, testJoinMultiField) {
    ...
    {
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(_allocator, 2));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "uid", {0, 1}));
        int32_t dataArray1[] = {0, 1};
        char* buf1 = MultiValueCreator::createMultiValueBuffer(dataArray1, 2, &_pool);
        MultiValueType<int32_t> multiValue1(buf1);
        int32_t dataArray2[] = {3, 4};
        char* buf2 = MultiValueCreator::createMultiValueBuffer(dataArray2, 2, &_pool);
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {0, 1, 2, 3}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name",
    ...
    ASSERT_NO_FATAL_FAILURE(checkOutputColumn(outputTable, "group_name0", {"groupA", "groupB", "groupD"}));
}
...
TEST_F(HashJoinKernelTest, testJoinMultiFieldWithEmpty) {
    ...
    ASSERT_FALSE(testerPtr->hasError());
    {
        ...
        MultiValueType<int32_t> multiValue1(buf1);
        int32_t dataArray2[] = {};
        char* buf2 = MultiValueCreator::createMultiValueBuffer(dataArray2, 0, &_pool);
        MultiValueType<int32_t> multiValue2(buf2);
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<MultiValueType<int32_t>>(
                        _allocator, leftDocs, "cid", {multiValue1, multiValue2}));
        auto leftTable = createTable(_allocator, leftDocs);
        ...
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 1));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {0}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name",
                        {"groupA"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
...
TEST_F(HashJoinKernelTest, testExchangeTable) {
    ...
    ASSERT_FALSE(testerPtr->hasError());
    {
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(_allocator, 5));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "cid", {0, 1, 2, 3, 4}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, leftDocs, "group_name",
                        {"groupA", "groupB", "groupC", "groupD", "groupE"}));
        auto leftTable = createTable(_allocator, leftDocs);
        ASSERT_TRUE(testerPtr->setInput("input0", leftTable));
        ASSERT_TRUE(testerPtr->setInputEof("input0"));
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "uid", {0, 0}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {1, 3}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
        ASSERT_TRUE(testerPtr->setInputEof("input1"));
    }
...
TEST_F(HashJoinKernelTest, testBatchInput) {
    ...
    bool eof = false;
    {
        ...
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 1));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {0}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name", {"groupA"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
    }
    ASSERT_TRUE(testerPtr->compute());
    ...
    {
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 2));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {1, 2}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name", {"groupB", "groupC"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
    }
    ASSERT_TRUE(testerPtr->compute());
    ...
    {
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 2));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {3, 4}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name", {"groupD", "groupE"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
        ASSERT_TRUE(testerPtr->setInputEof("input1"));
    }
...
TEST_F(HashJoinKernelTest, testInputExceedBufferLimit) {
    ...
    ASSERT_EQ("input buffers exceed limit, cannot make hash join",
...
TEST_F(HashJoinKernelTest, testBufferLimitBlockInput) {
...

    {
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(_allocator, 2));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "uid", {0, 0}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, leftDocs, "cid", {1, 4}));
        auto leftTable = createTable(_allocator, leftDocs);
        ASSERT_TRUE(testerPtr->setInput("input0", leftTable));
        ASSERT_TRUE(testerPtr->setInputEof("input0"));
    ...
    {
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(_allocator, 1));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(_allocator, rightDocs, "cid", {4}));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(_allocator, rightDocs, "group_name", {"groupE"}));
        auto rightTable = createTable(_allocator, rightDocs);
        ASSERT_TRUE(testerPtr->setInput("input1", rightTable));
        ASSERT_TRUE(testerPtr->setInputEof("input1"));
    }
...
TEST_F(HashJoinKernelTest, testMakeHashJoin) {
    HashJoinKernel kernel;
    {
        ...
        int64_t leftSize = 1000000;
        vector<MatchDoc> leftDocs = std::move(getMatchDocs(lallocator, leftSize));
        vector<vector<int64_t>> leftIds;
        vector<MultiString> leftGroups;
        for (int64_t i = 0; i < leftSize; ++i) {
            leftIds.push_back({i, i+1, i+2});
            vector<string> subs = {StringUtil::toString(i),
                                   StringUtil::toString(i+1),
                                   StringUtil::toString(i+2)};
            char* buf = MultiValueCreator::createMultiStringBuffer(subs, &_pool);
            MultiString multiGroup;
            multiGroup.init(buf);
            leftGroups.push_back(multiGroup);
        }
        ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int64_t>(lallocator, leftDocs, "lid", leftIds));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<MultiString>(lallocator, leftDocs, "lgroup", leftGroups));
        kernel._leftBuffer = dynamic_pointer_cast<Table>(createTable(lallocator, leftDocs));
    }
    {
        MatchDocAllocatorPtr rallocator;
        int64_t rightSize = 500;
        vector<MatchDoc> rightDocs = std::move(getMatchDocs(rallocator, rightSize));
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int64_t>(rallocator, rightDocs, "rid", rightIds));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(rallocator, rightDocs, "rgroup", rightGroups));
        kernel._rightBuffer = dynamic_pointer_cast<Table>(createTable(rallocator, rightDocs));
    }
    ...
    kernel._rightJoinColumns = {"rid"};
    {
        ...
        ASSERT_TRUE(kernel.createHashMap(kernel._rightBuffer, 0,
                        kernel._rightBuffer->getRowCount(),  kernel._hashLeftTable));
        int64_t mid0 = TimeUtility::currentTime();
        HashJoinKernel::HashValues largeTableValues;
        ASSERT_TRUE(kernel.getHashValues(
                        kernel._leftBuffer, 0, kernel._leftBuffer->getRowCount(),
                        kernel._leftJoinColumns, largeTableValues));
        ...
        cout << "[hash right id] create:" << mid0-st
             << ", hash:" << mid1-mid0
             << ", join:" << end-mid1
             << ", count:" << count << endl;
    }
    ...
    kernel._rightJoinColumns = {"rgroup"};
    {
        ...
        auto count = kernel.makeHashJoin(largeTableValues);
        int64_t end = TimeUtility::currentTime();
        cout << "[hash right group] create:" << mid0-st
             << ", hash:" << mid1-mid0
             << ", join:" << end-mid1
             << ", count:" << count << endl;
    }

    {
        ...
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int64_t>(lallocator, leftDocs, "lid", leftIds));
        ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator(lallocator, leftDocs, "lgroup", leftGroups));
        kernel._leftBuffer = dynamic_pointer_cast<Table>(createTable(lallocator, leftDocs));
    }
    kernel._leftJoinColumns = {"lid"};
    kernel._rightJoinColumns = {"rid"};
...
TEST_F(HashJoinKernelTest, testLeftJoinWithDefaultValue) {
    _attributeMap["hints"] = ParseJson(
            R"json({"JOIN_ATTR":{"defaultValue":"INTEGER:10,VARCHAR:aa"}})json");
    _attributeMap["join_type"] = string("LEFT");
    _attributeMap["remaining_condition"] =
        ParseJson(R"json({"op":">", "type":"OTHER", "params":["$uid", 1]})json");
