...
bool AntiJoin::generateResultTable(const std::vector<size_t> &leftTableIndexes,
    ...
    if (_joinBaseParam._calcTable != nullptr
        && !_joinBaseParam._calcTable->filterTable(_joinedTable, 0,
                _joinedTable->getRowCount(), true))
    {
...
bool AntiJoin::finish(const table::TablePtr &leftTable, size_t offset, table::TablePtr &outputTable) {
