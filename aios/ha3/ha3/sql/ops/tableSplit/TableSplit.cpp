...
bool TableSplit::collectHashValues(table::Column& col, std::vector<std::string>& hashValues) {
    ...
    auto notSupportFunc = [&](auto a) -> bool {
        return false;
    };
...
void TableSplit::splitByHashValues(
    const std::vector<std::vector<std::string>>& multiHashValues,
    table::Table& table,
    std::vector<std::vector<table::Row>>& partRows)
{
    const auto valueSize = multiHashValues.size();
    const auto fieldSize = _hashMode._hashFields.size();
    splitByHashRange(multiHashValues, table, partRows);
}

void TableSplit::splitByHashRange(
    const std::vector<std::vector<std::string>>& multiHashValues,
    table::Table& table,
    std::vector<std::vector<table::Row>>& partRows)
{
    ...
    default:
        ...
        splitByHashRange(
            ...
            table,
            partRows,
            split_table::CollectCommonValues(n));
        return;
    }
}
    ...
    Func func)
{
    ...
    for (size_t i = 0; i < rowCount; ++i) {
        ...
        for (const auto& range : ranges) {
            ...
            if (unlikely(partIds.first < 0 ||
                    partIds.second < 0 ||
                    partIds.first >= _partCnt ||
