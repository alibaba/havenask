...
class mycomparison {
    ...
    bool reverse;
public:
    explicit mycomparison(const table::ComboComparatorPtr& cmp,
            const bool revParam = false) : comparator(cmp), reverse(revParam) {
    }
...
protected:
    ...
    bool convertHashValue(const std::string& str,
                          ...
                          size_t* pHashValue);
    bool makeInputHash(const table::TablePtr& table,
    ...
    bool makeGroup(const table::TablePtr& table,
                   std::vector<std::vector<table::Row> >* pGroups);
