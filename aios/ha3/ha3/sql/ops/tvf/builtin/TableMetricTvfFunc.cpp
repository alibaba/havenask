...
bool TableMetricTvfFunc::compute(const TablePtr &input, bool eof, TablePtr &output) {
    ...
    if (_metricReporter && !_tagName.empty()) {
        auto reporter = _metricReporter->getSubReporter("sql.user_define", {{{"table_name", _tagName}}});
...
void TableMetricTvfFuncCreator::def(navi::ResourceDefBuilder &builder) const {
    builder.name("METRICREPORT_TVF_FUNC");
}
