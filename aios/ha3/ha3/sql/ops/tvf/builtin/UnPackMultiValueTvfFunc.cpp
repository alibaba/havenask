...
HA3_LOG_SETUP(sql, UnPackMultiValueTvfFunc);
HA3_LOG_SETUP(sql, UnPackMultiValueTvfFuncCreator);
...
bool UnPackMultiValueTvfFunc::compute(const TablePtr &input, bool eof, TablePtr &output) {
    ...
    output = input;
    if (output) {
        SQL_LOG(DEBUG, "before unpack row count [%ld], after unpack row count [%ld], pool used [%ld]",
                rawRowCount, output->getRowCount(), _queryPool->getTotalBytes() - poolUsedSize);
    }
    return true;
}

bool UnPackMultiValueTvfFunc::unpackMultiValue(const ColumnPtr &column, TablePtr table) {
...
bool UnPackMultiValueTvfFunc::unpackTable(
        ...
        TablePtr &unpackTable)
{
    ...
    const vector<Row> &allRow = unpackTable->getRows();
    for (size_t i = 0; i < columnCount; i++) {
        ColumnPtr column = unpackTable->getColumn(i);
        if (column == nullptr) {
            return false;
        }
        auto schema = column->getColumnSchema();
...
#define CASE_MACRO(ft)                                                  \
        case ft: {                                                      \
            ...
            for (int32_t i = allRow.size() - 1; i >= 0; i--) {          \
                ...
                if (mvSize == 0) {                                      \
                    ...
                    if (unpackVec[i].second >= mvSize) {                \
                        SQL_LOG(ERROR, "multi value offset [%d] larger than expect [%d]",\
                                unpackVec[i].second, mvSize);           \
                        return false;                                   \
                    }                                                   \
            ...
            SQL_LOG(INFO, "column [%s] unpack uniq val count [%ld].", colName.c_str(), mvValCache.size()); \
            break;                                                      \
        }
...
#undef CASE_MACRO
    case bt_string: {
            ...
            vector<ConstString> valVec;
            map<ConstString, char*> mvValCache;
            for (int32_t i = allRow.size() - 1; i >= 0; i--) {
                const auto &mv = columnData->get(allRow[unpackVec[i].first]);
                int32_t mvSize = mv.size();
                if (mvSize == 0) {
                    ...
                    ConstString constVal(val.data(), val.size());
...
bool UnPackMultiValueTvfFunc::genColumnUnpackOffsetVec(const ColumnPtr& column,
...
UnPackMultiValueTvfFuncCreator::UnPackMultiValueTvfFuncCreator()
    : TvfFuncCreator(unpackMultiValueTvfDef, unpackMultiValueTvfInfo)
{}

UnPackMultiValueTvfFuncCreator::~UnPackMultiValueTvfFuncCreator() {
}

TvfFunc *UnPackMultiValueTvfFuncCreator::createFunction(
