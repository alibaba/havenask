...
SimpleRerankTvfFunc::SimpleRerankTvfFunc() : _needMakeUp(false), _needSort(false) {
}
...
bool SimpleRerankTvfFunc::init(TvfFuncInitContext &context) {
...

    {
        ...
        if (_quotaNums.size() == 0) {
            ...
            SQL_LOG(ERROR, "quota num [%s] must less or equal to all values size [%lu]",
...
bool SimpleRerankTvfFunc::computeTable(const table::TablePtr& table) {
    ...
    for (size_t i = 0; i < groups.size(); i++) {
        ...
        if (groups[i].size() >= _quotaNums[i]) {
            target.insert(target.end(),
                    groups[i].begin(), groups[i].begin() + _quotaNums[i]);
            extra.insert(extra.end(),
                    groups[i].begin() + _quotaNums[i], groups[i].end());
        } else {
            target.insert(target.end(),
                    groups[i].begin(), groups[i].begin() + groups[i].size());
        }
    }
    if (_needMakeUp) {
        makeUp(target, extra);
