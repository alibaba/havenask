...
HA3_LOG_SETUP(sql, UnPackMultiValueTvfFuncTest);
...
TEST_F(UnPackMultiValueTvfFuncTest, testComputeInputTableRowCount0) {
    ...
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 0));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {}));
...
TEST_F(UnPackMultiValueTvfFuncTest, testComputeUnpackFieldEmpty) {
    // unpack field empty
    MatchDocAllocatorPtr allocator;
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 1));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {{1}}));
    TablePtr input(new Table(leftDocs, allocator));
    UnPackMultiValueTvfFunc func;
...
TEST_F(UnPackMultiValueTvfFuncTest, testComputeUnpackOneField) {
    ...
    MatchDocAllocatorPtr allocator;
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 1));
    ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(allocator, leftDocs, "uid", {1}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {{1, 2, 3}}));
    TablePtr input(new Table(leftDocs, allocator));
    UnPackMultiValueTvfFunc func;
...
TEST_F(UnPackMultiValueTvfFuncTest, testComputeUnpackMultiField) {
    MatchDocAllocatorPtr allocator;
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 6));
    ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(allocator, leftDocs, "uid", {0, 1, 2, 3, 4, 5}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {{1}, {}, {}, {2, 4}, {2, 3}, {5}}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<string>(allocator,
                    leftDocs, "msuid", {{}, {"1"}, {"2", "4"}, {"2", "3"}, {}, {"5"}}));
    TablePtr input(new Table(leftDocs, allocator));
    UnPackMultiValueTvfFunc func;
    ...
    checkOutputMultiColumn<int32_t>(output, "muid", expectMuid);
    vector<vector<string> > expectMsuid = {{}, {"1"}, {"2"} ,{"2"}, {}, {"5"}, {}, {"2"}, {"4"}, {"3"}, {"3"}};
    checkOutputMultiColumn<string>(output, "msuid", expectMsuid);
}

TEST_F(UnPackMultiValueTvfFuncTest, testMultiValue) {
    MatchDocAllocatorPtr allocator;
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 5));
    ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<int32_t>(allocator, leftDocs, "uid", {0, 1, 2, 3, 4}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {{1}, {}, {2, 4}, {1, 2, 3}, {}}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<string>(allocator,
                    leftDocs, "msuid", {{"1"}, {}, {"2", "4"}, {"1", "2", "3"}, {}}));
    TablePtr table(new Table(leftDocs, allocator));
    UnPackMultiValueTvfFunc func;
    func._queryPool = _poolPtr.get();
    ColumnPtr col = table->getColumn("muid");
    ...
    vector<vector<string> > expectMsuid = {{"1"},{}, {"2", "4"}, {"1", "2", "3"}, {}, {"1", "2", "3"}, {"1", "2", "3"}, {"2", "4"}};
...
TEST_F(UnPackMultiValueTvfFuncTest, testUnpackTable) {
    ...
    UnPackMultiValueTvfFunc func;
    func._queryPool = _poolPtr.get();
    vector<pair<int32_t, int32_t> > unpackVec = {{0, 0}, {1, 0}, {2, 0}, {3, 0}, {4, 0}, {3, 1}, {3, 2}, {2, 1}};
...
TEST_F(UnPackMultiValueTvfFuncTest, testCopyUnpackCol) {
    ...
    {
        ColumnPtr col = table->getColumn("msuid");
        ASSERT_TRUE(col != nullptr);
        ASSERT_TRUE(func.copyUnpackCol(5, table->getRows(), unpackVec, col));
    ...
    {
        ColumnPtr col = table->getColumn("uid");
        ASSERT_TRUE(col != nullptr);
...
TEST_F(UnPackMultiValueTvfFuncTest, testCopyNormalCol) {
    ...
    table->batchAllocateRow(unpackVec.size()  - 5);
    {
        ...
        vector<vector<int32_t> > expect = {{1},{}, {2, 4}, {1, 2, 3}, {}, {1, 2, 3}, {1, 2 ,3}, {2, 4}};
        checkOutputMultiColumn<int32_t>(table, "muid", expect);
    }
    {
        ColumnPtr col = table->getColumn("uid");
...
TEST_F(UnPackMultiValueTvfFuncTest, testGenColumnUnpackOffsetVec) {
    MatchDocAllocatorPtr allocator;
    vector<MatchDoc> leftDocs = std::move(getMatchDocs(allocator, 5));
    ASSERT_NO_FATAL_FAILURE(extendMatchDocAllocator<uint32_t>(allocator, leftDocs, "uid", {0, 1, 2, 3, 4}));
    ASSERT_NO_FATAL_FAILURE(extendMultiValueMatchDocAllocator<int32_t>(allocator,
                    leftDocs, "muid", {{1}, {}, {2, 4}, {1, 2, 3}, {}}));
    TablePtr table(new Table(leftDocs, allocator));
    {// not multi col
    ...
    {// multi col
        ...
        vector<pair<int32_t, int32_t> > expect = {{0, 0}, {1, 0}, {2, 0}, {3, 0}, {4, 0}, {3, 1}, {3, 2}, {2, 1}};
