...
bool InputTableTvfFunc::init(TvfFuncInitContext &context) {
    ...
    if (rowSep.size() != 1u ||
        colSep.size() != 1u ||
        multiValueSep.size() != 1u)
    {
        SQL_LOG(ERROR, "invalid sep row[%s],col[%s],multi_value[%s]",
                rowSep.data(), colSep.data(), multiValueSep.data());
        return false;
    }
    _rowSep = rowSep[0];
    _colSep = colSep[0];
...
bool InputTableTvfFunc::prepareInputTableInfo(const string &input,
    ...
    vector<StringView> rows = StringTokenizer::constTokenize(
        constInput, _rowSep, StringTokenizer::TOKEN_LEAVE_AS);
