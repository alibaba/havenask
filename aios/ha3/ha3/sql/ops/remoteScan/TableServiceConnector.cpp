...
public:
    ...
    void report(const kmonitor::MetricsTags *tags, TableServiceConnector::MetricCollector *metrics) {
...
bool TableServiceConnector::batchScan(const Connector::ScanOptions &options, TablePtr &table) {
    ...
    if (pks.empty()) {
        ...
        auto pkMap = groupByPartition(pks, options.tableDist.hashMode._hashFunction, options.tableDist.partCnt);
...
suez::KVBatchGetGenerator::KVBatchGetOptions TableServiceConnector::prepareKVOptions(const Connector::ScanOptions &options) {
...
bool TableServiceConnector::tryReportMetrics(kmonitor::MetricsReporter &opMetricsReporter) {
    ...
    SQL_LOG(TRACE3, "lookup time [%ld], convert time [%ld]", _metricsCollector.lookupTime,
...
int32_t TableServiceConnector::getPartId(uint32_t hashid, uint32_t partCount) {
    const autil::RangeVec& vec = autil::RangeUtil::splitRange(0, autil::RangeUtil::MAX_PARTITION_RANGE, partCount);
...
unordered_map<int32_t, set<string>> TableServiceConnector::groupByPartition(const vector<string> &pks, const string &hashType, uint32_t totalPartCount) {
...
bool TableServiceConnector::fillTableResult(const multi_call::ReplyPtr &reply, table::TablePtr &table,
    ...
    for (const auto &gigResponse : responseVec) {
        ...
        if (gigResponse->isFailed()) {
            SQL_LOG(ERROR, "batch get failed, partId [%d], ec [%s]", partId, multi_call::translateErrorCode(gigResponse->getErrorCode()));
            return false;
        }
        ...
        if (pbResponse->errorinfo().errorcode() != suez::TBS_ERROR_NONE) {
            SQL_LOG(ERROR, "batch get failed, error msg: %s", pbResponse->errorinfo().errormsg().c_str());
            return false;
        }
        ...
        SQL_LOG(TRACE3, "after merge %d table is %s", partId, TableUtil::toString(table, 20).c_str());
    }

    SQL_LOG(TRACE3, "lookup count: %d, not found count: %d, failed count: %d",
...
bool TableServiceConnector::mergeTableResult(table::TablePtr &table, const std::string &result,
        const autil::mem_pool::PoolPtr &pool) {
    ...
    if (table == nullptr) {
        ...
        if (!table->merge(partTable)) {
            SQL_LOG(ERROR, "merge table failed, old table: %s, new table: %s", TableUtil::toString(table, 20).c_str(), TableUtil::toString(partTable, 20).c_str());
            return false;
        }
    }
    return true;
