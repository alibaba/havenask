...
GraphTransform::Config::Config(const ExecConfig &execConfig,
...
static autil::legacy::RapidValue *getJsonValue(PlanOp &op, const string &key) {
...
static string getJsonStringValue(PlanOp &op, const string &key) {
...
std::string GraphTransform::getQrsBizName() {
    if (_config.qrsBizName.empty()) {
        return DEFAULT_QRS_SQL_BIZ_NAME;
    } else {
        return _config.qrsBizName;
    }
}

std::string GraphTransform::getRemoteBizName(PlanOp &op) {
    if (_config.searcherBizName.empty()) {
        ...
        if (name.empty()) {
            ...
            if (name == SQL_DEFAULT_EXTERNAL_DATABASE_NAME) {
                return std::string("qrs") + "." + DEFAULT_SQL_BIZ_NAME;
            }
        }
        return name + "." + DEFAULT_SQL_BIZ_NAME;
    } else {
        return _config.searcherBizName;
    }
}

std::string GraphTransform::getJsonSegment(PlanOp &op, const char *key) {
    auto p = getJsonValue(op, key);
    if (p == nullptr) {
        return EMPTY_STRING;
    }
...
size_t GraphTransform::getScanParallel(PlanOp &op) {
...
void GraphTransform::preprocessOp(PlanOp &op) {
    preprocessOpName(op);
    preprocessOpInputs(op);
}

void GraphTransform::preprocessOpName(PlanOp &op) {
    op.patchInt64Attrs[OP_ID] = BASE_OP_ID + op.id;
    ...
    if (_config.logicTableOps.count(opName) > 0) {
        op.opName = "ScanKernel";
        op.patchStrAttrs["table_type"] = "logical";
    }
}

void GraphTransform::preprocessOpInputs(PlanOp &op) {
...
plan::PlanNode *GraphTransform::createPlanNode(PlanOp &op) {
...
void GraphTransform::addExchangeBorder(navi::P buildOutput,
    ...
    if (sameGraph) {
        if (exchangeNode.root->getRoot()->getGraphId() == _rootGraphId) {
            ...
            buildInput.from(innerBuildOutput).require(true).merge("TableMergeKernel");
        ...
        buildEdge.merge("TableMergeKernel");
        buildEdge.split("TableSplitKernel")
