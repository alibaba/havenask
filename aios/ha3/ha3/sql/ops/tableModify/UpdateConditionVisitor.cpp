...
bool UpdateConditionVisitor::hasSameFields(
...
bool UpdateConditionVisitor::isIncludeFields(
        const std::map<std::string, std::string> &a,
        const std::map<std::string, std::string> &b)
{
    for (auto pair : b) {
...
bool UpdateConditionVisitor::isSameFields(
        const std::map<std::string, std::string> &a,
        const std::map<std::string, std::string> &b)
{
    if (a.size() != b.size()) {
...
bool UpdateConditionVisitor::parseKey(const SimpleValue &condition) {
    ...
    if (op == SQL_IN_OP) {
        ...
        if ((!param.IsArray()) || param.Size() != 2) {
            SQL_LOG(ERROR, "[%s] op `=` params size != 2",
        ...
        SQL_LOG(ERROR, "Update only supported `IN` OR `=`: %s",
                RapidJsonHelper::SimpleValue2Str(condition).c_str());
        return false;
    }
}
...
bool UpdateConditionVisitor::parseEqual(const SimpleValue &left, const SimpleValue &right) {
    ...
    if (leftIsCast && rightIsCast) {
        ...
        if ((!paramLeft.IsArray()) || paramLeft.Size() == 0) {
            SQL_LOG(ERROR, "[%s] params is not array",
        ...
        const SimpleValue &left, const SimpleValue &right)
{
    ...
    if (leftHasColumn && rightHasColumn) {
        SQL_LOG(ERROR, "not found column, [%s] [%s]",
                RapidJsonHelper::SimpleValue2Str(left).c_str(),
                RapidJsonHelper::SimpleValue2Str(right).c_str());
        return false;
        ...
        SQL_LOG(ERROR, "unexpected both column, [%s] [%s]",
                RapidJsonHelper::SimpleValue2Str(left).c_str(),
                RapidJsonHelper::SimpleValue2Str(right).c_str());
        return false;
    }
...
bool UpdateConditionVisitor::tryAddKeyValue(
        const SimpleValue &attr, const SimpleValue &value)
{
    ...
    if (valueStr.empty()) {
        SQL_LOG(ERROR, "fetch value [%s] failed",
...
bool UpdateConditionVisitor::fetchValues(
        std::vector<std::map<std::string, std::string>> &values)
{
    if (isError()) {
...
bool UpdateConditionVisitor::checkFetchValues() {
    ...
    for (size_t i = 0; i < _values.size(); ++i) {
        if (!isSameFields(_values[i], fetchFields)) {
            SQL_LOG(ERROR, "UPDATE: fetch values [%s] not same as expected [%s]",
