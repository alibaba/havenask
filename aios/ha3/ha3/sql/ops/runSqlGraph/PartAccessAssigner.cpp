...
PartAccessAssigner::PartAccessAssigner(navi::GigClientResource *gigClientResource, HashFunctionCacheR *hashFunctionR)
    : _gigClientResource(gigClientResource) // can be null if no remote sub graphs
...
bool PartAccessAssigner::convertTableDistToPartIds(const std::string &bizName,
    ...
    if (!_gigClientResource) {
        SQL_LOG(ERROR,
                "gig client resource is null, can not get fullPartCount for biz [%s]",
                bizName.c_str());
        return false;
    }
    auto *searchService = _gigClientResource->getSearchService().get();
...
autil::HashFunctionBasePtr PartAccessAssigner::createHashFunc(const HashMode &hashMode) {
    ...
    if (_hashFunctionCacheR && hashFunc == "ROUTING_HASH") {
        ...
        if (hashFunctionBasePtr == nullptr) {
            hashFunctionBasePtr = HashFuncFactory::createHashFunc(hashFunc, hashMode._hashParams, MAX_PARTITION_COUNT);
            if (hashFunctionBasePtr != nullptr) {
...
uint64_t PartAccessAssigner::calHashKey(const std::string &hashName, const std::map<std::string, std::string> &hashParams) {
