...
namespace isearch {
namespace sql {
AUTIL_LOG_SETUP(sql, Aggregator);
...
bool Aggregator::init(const AggFuncManager *aggFuncManager,
...
bool Aggregator::createAggFunc(const AggFuncManager *aggFuncManager,
                               const TablePtr &table,
...
{
    auto dataPool = _memoryPoolResource->getPool();
    ...
    auto func = aggFuncManager->createAggFunction(funcName, types, inputs, outputs, _mode);
    if (func == nullptr) {
