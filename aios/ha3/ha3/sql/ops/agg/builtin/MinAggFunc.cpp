...
HA3_LOG_SETUP(sql, MinAggFuncCreator);
AggFunc *MinAggFuncCreator::createLocalFunction(
...
#define CREATE_MIN_AGG_FUN_HELPER(ft)                                   \
        case ft: {                                                      \
            if (inputTypes[0].isMultiValue()) {                         \
                ...
                return new MinAggFunc<T>(inputFields, outputFields, true); \
            } else {                                                    \
...
#undef CREATE_MIN_AGG_FUN_HELPER
    default: {
        SQL_LOG(WARN, "min func input type only support number type, not support %s",
...
AggFunc *MinAggFuncCreator::createGlobalFunction(
        const vector<ValueType> &inputTypes,
        const vector<string> &inputFields,
        const vector<string> &outputFields)
{
