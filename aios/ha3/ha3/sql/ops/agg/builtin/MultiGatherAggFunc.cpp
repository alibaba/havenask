...
namespace isearch {
namespace sql {
AUTIL_LOG_SETUP(sql, MultiGatherAggFuncCreator);
template <>
bool MultiGatherAggFunc<autil::MultiChar>::collect(Row inputRow, Accumulator *acc) {
    MultiGatherAccumulator<autil::MultiChar> *gatherAcc
        = static_cast<MultiGatherAccumulator<autil::MultiChar> *>(acc);
    const MultiValueType<MultiChar> &multiVals = _inputColumn->get(inputRow);
    for (size_t i = 0; i < multiVals.size(); ++i) {
        gatherAcc->poolVector.push_back(StringView(multiVals[i].data(), multiVals[i].size()));
    }
    return true;
}
...
navi::ErrorCode MultiGatherAggFuncCreator::init(navi::ResourceInitContext &ctx) {
...
AggFunc *MultiGatherAggFuncCreator::createFirstStageFunction(const vector<ValueType> &inputTypes,
    ...
    if (!inputTypes[0].isMultiValue()) {
        SQL_LOG(ERROR, "local gather func input type not support single value type");
        return nullptr;
    }
...
#define CREATE_GATHER_AGG_FUN_HELPER(ft)                                                           \
    case ft: {                                                                                     \
        ...
        return new MultiGatherAggFunc<T>(inputFields, outputFields, mode);                         \
    }
...
AggFunc *MultiGatherAggFuncCreator::createGlobalFunction(const vector<ValueType> &inputTypes,
                                                         const vector<string> &inputFields,
                                                         const vector<string> &outputFields) {
    if (inputFields.size() != 1) {
        SQL_LOG(ERROR, "impossible input length, expect 1, actual %lu", inputFields.size());
...
#define CREATE_GATHER_AGG_FUN_HELPER(ft)                                                           \
    case ft: {                                                                                     \
        typedef MatchDocBuiltinType2CppType<ft, false>::CppType T;                                 \
        return new MultiGatherAggFunc<T>(                                                          \
...
REGISTER_RESOURCE(MultiGatherAggFuncCreator);
