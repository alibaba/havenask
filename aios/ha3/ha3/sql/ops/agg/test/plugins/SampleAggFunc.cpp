...
HA3_LOG_SETUP(sql, SampleAggFuncCreator);

AggFunc *SampleAggFuncCreator::createLocalFunction(
...
#define CREATE_SUM_AGG_FUN_HELPER(ft, V)                                \
        case ft: {                                                      \
            ...
            return new SampleAggFunc<T, V>(inputFields, outputFields, true); \
        }
...
AggFunc *SampleAggFuncCreator::createGlobalFunction(
        const vector<ValueType> &inputTypes,
        const vector<string> &inputFields,
        const vector<string> &outputFields)
{
