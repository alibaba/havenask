...
#include "ha3/sql/ops/scan/DocIdRangesReduceOptimize.hpp"
...
DocIdRangesReduceOptimize::DocIdRangesReduceOptimize(
        const std::vector<turing::Ha3SortDesc> &sortDescs,
...
void DocIdRangesReduceOptimize::parseArithmeticOp(
        const SimpleValue &param, const std::string &op)
{
    if ((!param.IsArray()) || param.Size() != 2) {
        SQL_LOG(TRACE2, "param [%s] is not 2 args array, skip",
    ...
    if (leftHasColumn && rightHasColumn) {
        SQL_LOG(TRACE2, "param [%s] args are both column, skip",
                RapidJsonHelper::SimpleValue2Str(param).c_str());
        return;
        ...
        SQL_LOG(TRACE2, "param [%s] args are not column, skip",
                RapidJsonHelper::SimpleValue2Str(param).c_str());
        return;
    }
}

void DocIdRangesReduceOptimize::tryAddKeyRange(
        const SimpleValue &attr, const SimpleValue &value, const std::string &op)
{
    if (!SqlJsonUtil::isColumn(attr)) {
        SQL_LOG(TRACE3, "attr [%s] not column",
...
#define KEY_RANGE_HELPER(ft, R, func)                                   \
            case ft: {                                                  \
                ...
                _key2keyRange[attrName] = KeyRangeBasePtr(genKeyRange<T>( \
                                attrName, op, value));                  \
                break;                                                  \
            }
...
search::LayerMetaPtr DocIdRangesReduceOptimize::reduceDocIdRange(
        const search::LayerMetaPtr &lastRange, autil::mem_pool::Pool *pool,
        search::IndexPartitionReaderWrapperPtr &readerPtr)
{
    ...
    search::LayerMetaPtr layerMeta(new search::LayerMeta(pool));
    ...
    for (size_t i = 0; i < lastRange->size(); ++i) {
        if ((*lastRange)[i].ordered != search::DocIdRangeMeta::OT_ORDERED) {
        ...
        if (!readerPtr->getSortedDocIdRanges(
                        dimens, rangeLimit, resultRanges))
        {
            return lastRange;
        }
        for (size_t j = 0; j < resultRanges.size(); ++j) {
            search::DocIdRangeMeta rangeMeta(
                    resultRanges[j].first, resultRanges[j].second - 1,
                    (*lastRange)[i].ordered);
