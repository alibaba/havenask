...
QueryExecutorExpressionWrapper::QueryExecutorExpressionWrapper(common::Query *query)
    : _query(query)
    , _initialized(false)
{}
        ...
        const std::vector<search::LayerMetaPtr> &layerMetas)
{
    ...
    for (auto layerMeta : layerMetas) {
        if (layerMeta->empty()) {
            SQL_LOG(ERROR, "unexpected layer meta is empty, query [%s]",
                    _query ? _query->toString().c_str() : "");
            return false;
        }
        _docIdRanges.push_back(make_pair((*layerMeta)[0].begin, (*layerMeta)[0].end));
        search::QueryExecutor *queryExecutor = createQueryExecutor(_query,
                ...
                layerMeta.get());
        if (!queryExecutor) {
            SQL_LOG(ERROR, "table [%s], create query executor failed, query [%s]",
                    mainTableName.c_str(), _query ? _query->toString().c_str() : "");
            return false;
        }
...
search::QueryExecutor *QueryExecutorExpressionWrapper::createQueryExecutor(
        ...
        search::LayerMeta *layerMeta)
{
    ...
    search::QueryExecutor *queryExecutor = nullptr;
    try {
        search::QueryExecutorCreator qeCreator(
        ...
        SQL_LOG(WARN, "unknown exception with create query executor");
    }
