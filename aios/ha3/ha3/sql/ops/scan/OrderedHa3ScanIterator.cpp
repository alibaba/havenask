...
OrderedHa3ScanIterator::OrderedHa3ScanIterator(const Ha3ScanIteratorParam &param,
    ...
    , _comp(nullptr)
{
    ...
    for (size_t i = 0; i < _layerMetas.size(); ++i) {
        SingleLayerSearcherPtr singleLayerSearcher(new SingleLayerSearcher(
                        _queryExecutors[i].get(),
                        _layerMetas[i].get(),
                        _filterWrapper.get(),
                        _delMapReader.get(),
    ...
    SQL_LOG(TRACE2, "order single layer searcher [%lu], unordered single layer searcher [%lu]",
            _orderedSingleLayerSearcher.size(), _unorderedSingleLayerSearcher.size());
}
...
bool OrderedHa3ScanIterator::seekAndPushQueue(
        size_t i, RangePriorityQueueType &rangePriorityQueue)
{
    MatchDoc doc;
