...
public:
    ...
    void report(const kmonitor::MetricsTags *tags, AsyncSummaryLookupMetricsCollector *summaryMetrics) {
...
void AsyncSummaryLookupCallbackCtx::prepareDocs(std::vector<docid_t> docIds,
                                                size_t fieldCount) {
...
void AsyncSummaryLookupCallbackCtx::processErrorCodes(const indexlib::index::ErrorCodeVec &errorCodes) {
    ...
    if (!_errorDocIds.empty()) {
        NAVI_LOG(WARN, "summary reader has error doc ids: [%s]", autil::StringUtil::toString(_errorDocIds).c_str());
    }
    _metricsCollector.failedDocsCount = _errorDocIds.size();
}

void AsyncSummaryLookupCallbackCtx::onSessionCallback(const future_lite::Try<indexlib::index::ErrorCodeVec> &errorCodeTry) {
    _metricsCollector.lookupTime = incCallbackVersion();
...
ErrorCode CountedAsyncPipe::done() {
    ...
    if (ret == 1) {
        return _asyncPipe->setData(ActivationData::inst());
    }
    return EC_NONE;
}
