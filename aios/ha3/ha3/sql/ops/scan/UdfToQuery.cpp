...
Query *
UdfToQuery::tokenizeQuery(common::Query *query,
                          QueryOperator qp,
    ...
    common::QueryTokenizer queryTokenizer(analyzerFactory);
...
Query *UdfToQuery::cleanStopWords(common::Query *query) {
    if (query == nullptr) {
        return nullptr;
    }
    ...
    if (!ret) {
        SQL_LOG(WARN,
                ...
                haErrorCodeToString(stopWordsCleaner.getErrorCode()).c_str());
        return nullptr;
    }
