...
public:
    ...
    void insertAndTrim(KeyRangeBase *others) override {
        ...
        _ranges.insert(_ranges.end(),
                       otherRanges->_ranges.begin(),
                       otherRanges->_ranges.end());
        trimRange();
    }
    ...
    std::shared_ptr<indexlib::table::DimensionDescription> convertDimenDescription() override {
        ...
        for (const auto &range : _ranges) {
            ...
            if (end < begin + DISCRETE_POINT_LIMIT) {
                ...
                dimen->ranges.push_back({autil::StringUtil::toString(begin),
                                        autil::StringUtil::toString(end)});
            }
        }
        return dimen;
    }
