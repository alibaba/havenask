...
void NaviConfigAdapter::setGigClientConfig(
    ...
    SQL_LOG(INFO, "set gig client config mcConfig[%s] mcFlowConfigMap[%s]",
            ToJsonString(mcConfig).c_str(),
            ToJsonString(mcFlowConfigMap).c_str());
    _hasMcConfig = true;
    _mcConfig = mcConfig;
...
bool NaviConfigAdapter::parseExternalTableConfig(const ExternalTableConfig &externalTableConfig) {
    std::vector<multi_call::Cm2Config> cm2Configs;
    if (!externalTableConfig.gigConfig.subscribeConfig.getCm2Configs(cm2Configs)) {
        SQL_LOG(ERROR, "get cm2 configs failed");
        return false;
    }
    auto &cm2Configs_ = _mcConfig.subConf.cm2Configs;
    cm2Configs_.insert(cm2Configs_.end(), cm2Configs.begin(), cm2Configs.end());

    auto &flowConfigMap = externalTableConfig.gigConfig.flowConfigMap;
    _mcFlowConfigMap.insert(flowConfigMap.begin(), flowConfigMap.end());
    bool cm2Enabled, vipEnabled, localEnabled, istioEnabled;
    if (!_mcConfig.subConf.validate(cm2Enabled, vipEnabled, localEnabled, istioEnabled)) {
        SQL_LOG(ERROR, "validate merged subscribe config failed");
        return false;
    }
    SQL_LOG(INFO, "end parse external table config");
    return true;
}
...
bool NaviConfigAdapter::createNaviConfig(const std::string &bizName, navi::NaviConfig &naviConfig) {
    ...
    bizConfig.partIds = _partIds;
    if (_addResource) {
        bizConfig.initResources.insert(TABLET_MANAGER_RESOURCE_ID);
        ...
        bizConfig.initResources.insert("HashFunctionCacheR");
    }
...
bool NaviConfigAdapter::parseExtraTaskQueueConfig() {
    ...
    if (!initExtraTaskQueueFromStr(configStr)) {
        SQL_LOG(ERROR, "parse extra task queue config failed [%s]",
                configStr.c_str());
        return false;
    }
    return true;
...
void NaviConfigAdapter::parseGrpcConfig() {
    _grpcProtocolThreadNum = EnvUtil::getEnv<size_t>(GRPC_PROTOCOL_THREAD_NUM, DEFAULT_GRPC_PROTOCOL_THREAD_NUM);
    _grpcProtocolKeepAliveInterval = EnvUtil::getEnv<int64_t>(GRPC_PROTOCOL_KEEP_ALIVE_INTERVAL,
                                                             DEFAULT_GRPC_PROTOCOL_KEEP_ALIVE_INTERVAL);
...
void NaviConfigAdapter::addLogConfig(navi::NaviConfig &naviConfig) {
    ...
    SQL_LOG(INFO, "navi log config [%s]",
            ToJsonString(fileConfig).c_str());
}
...
bool NaviConfigAdapter::initExtraTaskQueueFromStr(const std::string &configStr)
{
    ...
    for (const auto &str : vecStr) {
        ...
        if (_extraTaskQueueMap.end() != _extraTaskQueueMap.find(queueName)) {
            SQL_LOG(ERROR, "error in task extra queue config, duplicated name[%s]", queueName.c_str());
            return false;
        }
        int threadNum;
        size_t queueSize;
        ...
        if (!(StringUtil::fromString<size_t>(paramVec[3], processingSize) &&
            ...
            SQL_LOG(ERROR, "error in task extra queue config, processingSize error[%s]", str.c_str());
            return false;
        }
...
bool NaviConfigAdapter::addEngineConfig(navi::NaviConfig &naviConfig) {
    ...
    SQL_LOG(INFO,
            "navi engine config[%s]",
            ToJsonString(naviConfig.engineConfig).c_str());
    return true;
}
...
bool NaviConfigAdapter::addGigClientResourceConfig(navi::NaviConfig &naviConfig) {
    ...
    StringUtil::replaceLast(configStr, "$$MCCONFIG", ToJsonString(_mcConfig));
    StringUtil::replaceLast(configStr, "$$FLOW_CONFIG", ToJsonString(_mcFlowConfigMap));

    NaviRegistryConfig registryConfig;
    if (!registryConfig.loadConfig(navi::GIG_CLIENT_RESOURCE_ID, configStr)) {
        SQL_LOG(ERROR, "create navi resource [%s] config registry failed, content [%s]",
                navi::GIG_CLIENT_RESOURCE_ID.c_str(),
                configStr.c_str());
        return false;
    }
    SQL_LOG(INFO, "add gig client resource config, name[%s], level[snapshot]", registryConfig.name.c_str());
    naviConfig.buildinResourceConfigVec.emplace_back(std::move(registryConfig));
    return true;
}
