...
bool SwiftMessageWriterManager::init(const std::string& config) {
    isearch::sql::SwiftWriteConfig swiftWriteConfig;
    try {
        FromJsonString(swiftWriteConfig, config);
    } catch (...) {
    ...
    if (swift::protocol::ERROR_NONE != ec) {
        SQL_LOG(WARN, "init swift client failed, config is[%s], ec[%d]",
                  swiftWriteConfig.getSwiftClientConfig().c_str(), ec);
        reset();
        return false;
    }
    ...
    for (const auto & iter : readWriteConfigMap) {
        ...
        std::unique_ptr<SwiftWriter> swiftWriter(_swiftClient->createWriter(readWriteConfig.swiftTopicName, readWriteConfig.swiftWriterConfig));
        if (!swiftWriter) {
            SQL_LOG(WARN, "init swift writer failed, table is [%s], config is [%s]",
                    iter.first.c_str(),
                    ToJsonString(readWriteConfig).c_str());
            reset();
            return false;
        }
        auto swiftWriterAsyncHelper = make_shared<SwiftWriterAsyncHelper>();
        if (!swiftWriterAsyncHelper->init(std::move(swiftWriter))) {
            SQL_LOG(WARN, "init swift writer async helper failed, table is [%s]", iter.first.c_str());
            reset();
            return false;
        }
...
MessageWriter *SwiftMessageWriterManager::getMessageWriter(const std::string &dbName, const std::string &tableName) {
