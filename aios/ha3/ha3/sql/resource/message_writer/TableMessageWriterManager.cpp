...
bool TableMessageWriterManager::init(const std::string& config) {
    isearch::sql::TableWriteConfig tableWriteConfig;
    try {
        FromJsonString(tableWriteConfig, config);
    } catch (...) {
        SQL_LOG(WARN, "parse table write config failed [%s].", config.c_str());
        return false;
    }
    for (const auto &zoneName : tableWriteConfig.zoneNames) {
        suez::RemoteTableWriterPtr tableWriter(
                new suez::RemoteTableWriter(zoneName, _searchService.get(), tableWriteConfig.allowFollowWrite));
        _remoteTableWriterMap.emplace(zoneName, std::move(tableWriter));
    }
    SQL_LOG(INFO, "init table message writer manager success, config is [%s]",
              config.c_str());
    return true;
}

MessageWriter *TableMessageWriterManager::getMessageWriter(const std::string &dbName, const std::string &tableName) {
