...
private:
    ...
    std::priority_queue<CallbackItemPtr, std::vector<CallbackItemPtr>, CallbackItemPtrCmpTargetTs> _targetTsQueue;
    std::priority_queue<CallbackItemPtr, std::vector<CallbackItemPtr>, CallbackItemPtrCmpExpireTime> _timeoutQueue;
...
template <typename T>
GenericWaiter<T>::GenericWaiter() : _notifier(1) {}
...
bool GenericWaiter<T>::init() {
    ...
    _workerThread = autil::Thread::createThread(std::bind(&GenericWaiter::workerCheckLoop, this), "WmCb");
    if (!_workerThread) {
        AUTIL_LOG(ERROR, "[%p] init targetTs callback thread failed, desc[%s]", this, getDesc().c_str());
        return false;
    }
    return true;
}
...
template <typename T>
void GenericWaiter<T>::wait(int64_t targetTs, typename CallbackItem::CallbackFunc callback, int64_t timeoutInUs) {
    assert(!_stopped);
    ...
    AUTIL_LOG(TRACE1, "[%p] end emplace targetTs[%ld] pending[%lu]", this, targetTs, pendingItemCount);
}
...
void GenericWaiter<T>::checkCallback() {
    ...
    for (auto &itemPtr : targetTsList) {
        AUTIL_LOG(TRACE1, "[%p] onDone for checkpoint[%ld] targetTs[%ld]", this, checkpoint, itemPtr->targetTs);
        if (itemPtr->onDone(param)) {
            ++doneCnt;
        }
    }
    for (auto &itemPtr : timeoutList) {
        AUTIL_LOG(TRACE1, "[%p] onTimeout for checkpoint[%ld] targetTs[%ld]", this, checkpoint, itemPtr->targetTs);
