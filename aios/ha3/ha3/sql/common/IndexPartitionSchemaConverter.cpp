...
bool IndexPartitionSchemaConverter::convert(
    ...
    if (tableType == IQUAN_TABLE_TYPE_CUSTOMIZED) {
        return convertCustomizedTable(schema, tableType, tableDef);
...
bool IndexPartitionSchemaConverter::convertV2(
    ...
    if (!convertAggregationIndex(schema, tableDef)) {
        AUTIL_LOG(ERROR, "convert aggregation index failed");
        return false;
    }
...
bool IndexPartitionSchemaConverter::convertAggregationIndex(
        const std::shared_ptr<indexlibv2::config::ITabletSchema> &schema,
        TableDef &tableDef) {
    const auto &indexConfigs = schema->GetIndexConfigs(indexlibv2::index::AGGREGATION_INDEX_TYPE_STR);
    std::vector<IndexDef> indexList;
    for (const auto &indexConfig : indexConfigs) {
        IndexDef index;
        index.indexName = indexConfig->GetIndexName();
        index.indexType = indexConfig->GetIndexType();
        for (const auto &field : indexConfig->GetFieldConfigs()) {
            index.indexFields.emplace_back(field->GetFieldName());
        }
        auto aggIndexConfig = dynamic_pointer_cast<indexlibv2::config::AggregationIndexConfig>(indexConfig);
        if (aggIndexConfig == nullptr) {
            return false;
        }
        index.valueFields = aggIndexConfig->GetValueFields();
        indexList.push_back(index);
    }
    if (!indexList.empty()) {
        tableDef.indexes.emplace(indexlibv2::index::AGGREGATION_INDEX_TYPE_STR, indexList);
    }
    return true;
}
...
bool IndexPartitionSchemaConverter::convertCustomizedTable(const IndexPartitionSchemaPtr &schema,
                                                           const std::string &tableType,
                                                           TableDef &tableDef) {
    auto &fieldSchema = schema->GetFieldSchema();
    auto &indexSchema = schema->GetIndexSchema();
...
void IndexPartitionSchemaConverter::convertIndexFieldConfig(const IndexSchemaPtr &indexSchema,
    ...
    if (tableType == IQUAN_TABLE_TYPE_NORMAL || tableType == IQUAN_TABLE_TYPE_SUMMARY
        ...
        if (indexConfig) {
            ...
            fieldDef.indexType = IndexConfig::InvertedIndexTypeToStr(indexConfig->GetInvertedIndexType());
        }
        ...
        if (pkIndexFieldName == fieldDef.fieldName) {
            ...
            fieldDef.indexType = IndexConfig::InvertedIndexTypeToStr(indexSchema->GetPrimaryKeyIndexType());
        }
    }
}
...
void IndexPartitionSchemaConverter::convertIndexFieldConfig(
    ...
    if (tableType == IQUAN_TABLE_TYPE_NORMAL || tableType == IQUAN_TABLE_TYPE_SUMMARY) {
        auto indexConfig
            = schema->GetIndexConfig(indexlib::index::INVERTED_INDEX_TYPE_STR, fieldDef.fieldName);
        if (indexConfig) {
        ...
        if (indexConfig) {
            ...
            if (kvConfig->GetKeyFieldName() == fieldDef.fieldName) {
                ...
                fieldDef.indexType = indexlibv2::config::InvertedIndexConfig::InvertedIndexTypeToStr(it_kv);
            }
        }
...
bool IndexPartitionSchemaConverter::convertFields(
    ...
    const auto &attrFields
        = schema->GetIndexFieldConfigs(indexlibv2::GENERALIZED_VALUE_INDEX_TYPE_STR);
