...
static void mergeFrom(InvertedIndexSearchInfo &lhs, const InvertedIndexSearchInfo &rhs) {
    ...
    lhs.set_searchedinmemsegmentcount(lhs.searchedinmemsegmentcount() + rhs.searchedinmemsegmentcount());
...
void SqlSearchInfoCollector::setSqlPlanTime(int64_t useTime) {
    RunSqlTimeInfo *timeInfo = _sqlSearchInfo.mutable_runsqltimeinfo();
...
bool SqlSearchInfoCollector::deserialize(autil::DataBuffer &dataBuffer) {
    autil::ScopedLock lock(_mutex);

    size_t checksum;
    dataBuffer.read(checksum);
    if (checksum != SQL_SEARCHINFO_CHECKSUM_BEGIN) {
        SQL_LOG(ERROR, "invalid checksum, expect [0x%lx] actual [0x%lx]",
                SQL_SEARCHINFO_CHECKSUM_BEGIN, checksum);
        return false;
    }
    size_t len;
    dataBuffer.read(len);
    ...
    if (checksum != SQL_SEARCHINFO_CHECKSUM_END) {
        SQL_LOG(ERROR, "invalid checksum, expect [0x%lx] actual [0x%lx]",
                SQL_SEARCHINFO_CHECKSUM_END, checksum);
        return false;
    }
    return true;
}
