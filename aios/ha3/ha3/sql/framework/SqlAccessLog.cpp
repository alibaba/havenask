...
map<string, bool> SqlAccessLog::getLeaderInfos() const {
    map<string, bool> leaderInfos;
    for (const auto &scanInfo : _info.searchInfo.scaninfos()) {
        leaderInfos[scanInfo.tablename()] = scanInfo.isleader();
    }
    return leaderInfos;
}
