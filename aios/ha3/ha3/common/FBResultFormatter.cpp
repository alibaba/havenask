...
#include "ha3/proto/TsdbColumn_generated.h"
...
namespace isearch {
namespace common {
AUTIL_LOG_SETUP(ha3, FBResultFormatter);
        ...
        if (rowCount > 0) {
            ...
            if (summaryHit) {
                ...
                for (size_t fieldIdx = 0; fieldIdx < fieldCount; fieldIdx++) {
                    const StringView *str = summaryHit->getFieldValue(fieldIdx);
                    if (!str) {
                        continue;
                    }
...
Offset<Column> FBResultFormatter::CreateSummaryTableColumnByString(
    ...
    for (auto &hit : hitVec) {
        ...
        const StringView *str = summaryHit->getFieldValue(summaryFieldIdx);
        if (!str) {
            columnValue.push_back(fbb.CreateString(""));
            continue;
        }
...
#define DECLARE_CREATE_SUMMARY_TABLE_COLUMN_BY_TYPE(fbType, fieldType)  \
    ...
    columnValue.push_back(                                              \
            autil::StringUtil::fromString<fieldType>(str->to_string())); \
    }                                                                   \
