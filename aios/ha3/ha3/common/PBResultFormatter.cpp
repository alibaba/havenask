...
namespace isearch {
namespace common {
AUTIL_LOG_SETUP(ha3, PBResultFormatter);
    ...
    {                                                                   \
        ...
        if (!isMulti) {                                                 \
            for (uint32_t j = 0; j < matchDocs.size(); j++) {           \
                ...
                SingleType value = singleRef->get(matchDocs[j]);        \
            ...
            for (uint32_t j = 0; j < matchDocs.size(); j++) {           \
                ...
                if (!ref->getValueType().isStdType()) {                 \
                    ...
                    value = Type2AttrItemType<MultiExpressType>::convert(multiRef->get(matchDocs[j])); \
                } else {                                                \
                    auto multiRef = static_cast<matchdoc::Reference<MultiType> *>(ref); \
                    value = multiRef->get(matchDocs[j]);                \
                }                                                       \
...
void PBResultFormatter::format(const ResultPtr &resultPtr, stringstream &ss)
{
    ...
    AUTIL_LOG(DEBUG, "pb_format:%s",pbResult->DebugString().c_str());
}
                                   ...
                                   PBResult &pbResult)
{
    ...
    if (hits == NULL) {
        AUTIL_LOG(DEBUG, "There are no hits in the result");
        return;
    }
                                        ...
                                        PBResult &pbResult)
{
    ...
    if (!matchDocs) {
        AUTIL_LOG(DEBUG, "There are no matchDocs to format.");
        return;
    }
    const auto &allocatorPtr = matchDocs->getMatchDocAllocator();
    if (!allocatorPtr) {
        AUTIL_LOG(DEBUG, "Allocator in matchDocs is NULL!");
        return;
    }
        ...
        PBAggregateResults &pbAggregateResults)
{
    ...
    for (matchdoc::MatchDoc aggFunResults : aggResultVector) {
        PBAggregateValue *pbAggregateValue = pbAggregateResults.add_aggregatevalue();
        const string &aggExprValue = ref->toString(aggFunResults);
        pbAggregateValue->set_groupvalue(aggExprValue);

        for (uint32_t i=1; i<=aggFunCount; i++) {
            PBKVPair *funNameResultPair = pbAggregateValue->add_funnameresultpair();
            funNameResultPair->set_key(aggResultPtr->getAggFunName(i-1));
            funNameResultPair->set_value(referenceVec[i]->toString(aggFunResults));
        }
    }
}
