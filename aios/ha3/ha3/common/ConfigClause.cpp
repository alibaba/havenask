...
HA3_LOG_SETUP(common, ConfigClause);
...
void ConfigClause::init()
{
    ...
    _compressType = INVALID_COMPRESS_TYPE;
    _innerResultCompressType = NO_COMPRESS;
...
void ConfigClause::setCompressType(const string& compressTypeStr) {
    _compressType = CompressTypeConvertor::convertCompressType(compressTypeStr);
}

void ConfigClause::setInnerResultCompressType(const string& compressTypeStr) {
    HaCompressType type = 
        CompressTypeConvertor::convertCompressType(compressTypeStr);
    if (type != INVALID_COMPRESS_TYPE) {
        _innerResultCompressType = type;
    }
}
