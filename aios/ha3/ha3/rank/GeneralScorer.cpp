...
HA3_LOG_SETUP(rank, GeneralScorer);
...
bool GeneralScorer::beginRequest(suez::turing::ScoringProvider *provider)
{
    ...
    if (NULL == _provider) {
        HA3_LOG(WARN,"provider pointer is NULL");
        return false;
    }
    ...
    if (!_staticField.empty()) {
        ...
        if (NULL == _staticFieldRef) {
            HA3_LOG(WARN,"Failed to get static score field:[%s]",_staticField.c_str());
            return false;
        }
    }
...
score_t GeneralScorer::computeTermScore(matchdoc::MatchDoc matchDoc, float &totalDocFreq, int32_t &matchNum)
{
    ...
    if (_metaInfo.size() < termsCount) {
        HA3_LOG(WARN,"_metaInfo.size is a wrong value");
        return 0.0;
    }
...
bool GeneralScorer::extractIndexId() 
{
    ...
    if (size < 1) {
        HA3_LOG(WARN, "metaInfo's size less than one.");
        return false;
    }
    indexInfoHelper = _provider->getIndexInfoHelper();
    if ( NULL == indexInfoHelper) {
        HA3_LOG(WARN,"indexInfoHelper is NULL");
        return false;
    } 
    _indexId = new (std::nothrow)indexid_t[size];
    if (NULL == _indexId) {
        HA3_LOG(WARN, "Failed to alloc memory for indexId ");
        return false;
    }
                                 ...
                                 config::ResourceReader *resourceReader)
{
    if ( NULL == resourceReader) {
        HA3_LOG(WARN,"resourceReader pointers is NULL");
        return false;
    }
    ...
    if(it != scorerParameters.end())     
    {
        if (resourceReader->getFileContent(content, it->second)) {
            ...
            HA3_LOG(WARN,"Can not read resource file:%s",it->second.c_str());
            return false;
        }
    } else {
        HA3_LOG(WARN, "Failed to get table File:%s",fileName.c_str());
        return false;
    }
    return false;
}
                                    ...
                                    FloatVecPtr &floatVecPtr)
{
    ...
    if (stringTokenizer.getNumTokens() != 256) {
        HA3_LOG(WARN, "Failed to resolve table value file!!!");
        return false;
    }
    float value = 0.0;
    for(int i = 0; i < 256; ++i)
    {    
        if (!autil::StringUtil::strToFloat(stringTokenizer[i].c_str(),value)) {
            HA3_LOG(WARN, "Failed to convert table value:[%s]",stringTokenizer[i].c_str());
            return false;
        }
...
uint32_t GeneralScorer::MatchedDocmuentProximityExtractor(matchdoc::MatchDoc matchDoc){
    ...
    for(uint32_t i = 0;i<oriSize;i++){
        ...
        index::InDocPositionIteratorPtr posIterPtr = tmd.getInDocPositionIterator();
...
void GeneralScorer::initScoreField(const KeyValueMap &scorerParameters)
{
    ...
    if (it != scorerParameters.end()) {
        if( false == autil::StringUtil::strToFloat((it->second).c_str(), _staticWeight)) {
            HA3_LOG(WARN, "Failed to convert static socre weight: [%s]", (it->second).c_str());
            _staticWeight = 1;
        }
    }
}
...
void GeneralScorer::initPhaseInfo(const KeyValueMap &scorerParameters)
{
    ...
    if (it != scorerParameters.end()) {
        if( false == autil::StringUtil::strToInt32((it->second).c_str(), _phase)) {
            HA3_LOG(WARN, "Failed to convert score phase : [%s]", (it->second).c_str());
            _phase = 1;
        }    
    }
}
