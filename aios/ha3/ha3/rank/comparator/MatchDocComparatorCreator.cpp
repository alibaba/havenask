 ...
 */
#include "ha3/rank/MatchDocComparatorCreator.h"
...
namespace isearch {
namespace rank {
AUTIL_LOG_SETUP(sql, MatchDocComparatorCreator);
...
ComboComparator *MatchDocComparatorCreator::createComparator(
        const vector<string> &refNames,
        const vector<bool> &orders)
{
    assert(refNames.size() == orders.size());
    ComboComparator *comboComp = nullptr;
    if (refNames.size() == 1) {
        comboComp = createOptimizedComparator(refNames[0], !orders[0]);
    } else if (refNames.size() == 2) {
        comboComp = createOptimizedComparator(
                refNames[0], refNames[1], !orders[0], !orders[1]);
    }
...
ComboComparator *MatchDocComparatorCreator::createOptimizedComparator(
...
Comparator *MatchDocComparatorCreator::createComparator(
        const std::string &refName, bool sortFlag)
{
#define COMPARATOR_CREATOR_HELPER(vt_type)                              \
    ...
    {                                                                   \
        ...
        if (comp == nullptr) {                                          \
            SQL_LOG(ERROR, "create sort comparator failed, field [%s] type [%s%s]", \
                    refName.c_str(),                                    \
                    isMulti ? "array " : "",                            \
...
#undef COMPARATOR_CREATOR_HELPER
    default:
        SQL_LOG(ERROR, "create sort comparator failed, unexpected field [%s] type [%s%s]",
                refName.c_str(),
                isMulti ? "array " : "",
                builtinTypeToString(vt.getBuiltinType()).c_str());
        assert(false);
...
template <typename T>
ComboComparator *MatchDocComparatorCreator::createComboComparatorTyped(
        ReferenceBase *ref, bool flag)
{
    ...
    return POOL_NEW_CLASS(_pool, OneRefComparatorTyped<T>, refTyped, flag);
}

template <typename T1, typename T2>
ComboComparator *MatchDocComparatorCreator::createComboComparatorTyped(
        ReferenceBase *ref1, ReferenceBase *ref2,
        bool sortFlag1, bool sortFlag2)
{
    ...
    return new (_pool->allocate(sizeof(TwoRefComparatorTyped<T1, T2>)))
        TwoRefComparatorTyped<T1, T2>(refTyped1, refTyped2, sortFlag1, sortFlag2);
}

template <typename T>
Comparator *MatchDocComparatorCreator::createComparatorTyped(ReferenceBase *ref, bool flag) {
    Reference<T> *refTyped = dynamic_cast<Reference<T> *>(ref);
    if (refTyped == nullptr) {
        SQL_LOG(WARN, "unexpected cast ref [%s] failed", ref->getName().c_str());
        return nullptr;
    }
    return POOL_NEW_CLASS(_pool, ReferenceComparator<T>, refTyped, flag);
}
