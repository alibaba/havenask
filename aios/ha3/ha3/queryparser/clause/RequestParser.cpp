...
AUTIL_LOG_SETUP(ha3, RequestParser);
                                 ...
                                 const ClusterConfigMapPtr &clusterConfigMapPtr)
{
    ...
    if (parseConfigClause(requestPtr) == false) {
        AUTIL_LOG(WARN, "parse config clause failed. request string[%s]",
            configClause->getOriginalString().c_str());
        return false;
    }
    AUTIL_LOG(DEBUG, "parseConfigClause success!");
...
bool RequestParser::parseConfigClause(RequestPtr &requestPtr) {
    ConfigClause *configClause = requestPtr->getConfigClause();
    if (!configClause) {
        AUTIL_LOG(WARN, "failed to parseConfigClause: configClause is null");
    ...
    if (configClauseStr.empty()) {
        AUTIL_LOG(WARN, "Empty Config Clause Original String");
        return true;
    }
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        StringTokenizer st2(*it, CONFIG_KV_SEPERATOR,
                            StringTokenizer::TOKEN_IGNORE_EMPTY | StringTokenizer::TOKEN_TRIM);
        AUTIL_LOG(DEBUG, "Token num[%zd]", st2.getNumTokens());
        if (st2.getNumTokens() != 2) {
            AUTIL_LOG(WARN, "Failed to parse ConfigClause:[%s]", configClauseStr.c_str());
        ...
        if (key == CONFIG_CLAUSE_KEY_CLUSTER) {
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.start:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.hits:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.searcher_return_hits:%s",
                        value.c_str());
                _errorResult.resetError(ERROR_CONFIG_CLAUSE_HIT_VALUE);
                return false;
            }
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.rankSize:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.totalRankSize:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.rerankSize:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.totalRerankSize:%s", value.c_str());
            ...
            if (value == CONFIG_CLAUSE_KEY_JOIN_WEAK_TYPE) {
                ...
                AUTIL_LOG(WARN, "Failed to parse ConfigClause. "
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.actualHitsLimit:%s", value.c_str());
                _errorResult.resetError(ERROR_CONFIG_CLAUSE_HIT_VALUE);
                return false;
            }
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.researchThreshold:%s", value.c_str());
            ...
            if (!ret) {
                AUTIL_LOG(WARN, "Failed to parse ConfigClause.version:%s", value.c_str());
        ...
        const QueryInfo &queryInfo)
{
    ConfigClause *configClause = requestPtr->getConfigClause();
    if (!configClause) {
        AUTIL_LOG(WARN, "failed to parseQueryClause: configClause is null");
        return false;
    }
    QueryClause* queryClause = requestPtr->getQueryClause();
    if (!queryClause) {
        AUTIL_LOG(WARN, "failed to parseQueryClause: queryClause is null");
        return false;
    }
    ...
    AUTIL_LOG(TRACE2, "QueryText: [%s]", queryClauseStr.c_str());
...
bool RequestParser::parseFilterClause(FilterClause *filterClause) {
    if (!filterClause) {
        AUTIL_LOG(WARN, "failed to parseFilterClause: filterClause is null");
        return false;
    }
...
bool RequestParser::parseAggregateClause(RequestPtr &requestPtr) {
    AUTIL_LOG(DEBUG, "parse aggregate clause");
    AggregateClause *aggClause = requestPtr->getAggregateClause();
    if (!aggClause) {
        AUTIL_LOG(WARN, "failed to parseAggregateClause: aggClause is null");
        return false;
    }
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        ...
        if (!ctx.parseAggClause(it->c_str())) {
            AUTIL_LOG(WARN, "parse aggregate clause failed");
        ...
        {
            AUTIL_LOG(WARN, "illegal aggregate desription: [%s]", it->c_str());
            if (aggDes) {
                delete aggDes;
            }
            break;
        }
        AUTIL_LOG(TRACE3, "add Aggregate description: %s", it->c_str());
...
bool RequestParser::parseRankClause(RequestPtr &requestPtr) {
    AUTIL_LOG(TRACE3, "parse rank clause");
    RankClause *rankClause = requestPtr->getRankClause();
    if (!rankClause) {
        AUTIL_LOG(WARN, "failed to parseRankClause: rankClause is null");
        return false;
    }
...
bool RequestParser::parseSortClause(RequestPtr &requestPtr) {
    AUTIL_LOG(TRACE3, "parse sort clause");
    SortClause *sortClause = requestPtr->getSortClause();
    if (!sortClause) {
        AUTIL_LOG(WARN, "failed to parseSortClause: sortClause is null");
        return false;
    }
...
{
    AUTIL_LOG(TRACE3, "parse distinct clauses");
    DistinctClause *distinctClause = requestPtr->getDistinctClause();
    if (!distinctClause) {
        AUTIL_LOG(WARN, "failed to parseDistinctClause: distClause is null");
        return false;
    }
         ...
         it != st.end(); ++ it)
    {
        ...
        if (!ctx.parseDistClause(it->c_str())) {
            AUTIL_LOG(WARN, "parse distinct clause failed");
            _errorResult = ctx.getErrorResult();
            return false;
        }
...
bool RequestParser::validateOriginalRequest(RequestPtr &requestPtr)
{
    ...
    AUTIL_LOG(DEBUG, "validate original request. request string[%s]", oriStr.c_str());
...
bool RequestParser::parseClusterClause(RequestPtr &requestPtr,
                                       const ClusterConfigMapPtr &clusterConfigMapPtr)
{
    AUTIL_LOG(TRACE3, "parse cluster clause");
    ClusterClause *clusterClause = requestPtr->getClusterClause();
    if (!clusterClause) {
        AUTIL_LOG(WARN, "failed to parseClusterClause: clusterClause is null");
        return false;
    }

    const string &clusterClauseStr = clusterClause->getOriginalString();

    AUTIL_LOG(TRACE3, "parseClusterCaluse [%s]", clusterClauseStr.c_str());
...
bool RequestParser::parseHealthCheckClause(common::RequestPtr &requestPtr) {
    ...
    if (!healthCheckClause) {
        AUTIL_LOG(WARN, "healthCheckClause is null");
        return false;
    }
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        ...
        if (key == HEALTHCHECK_CHECK_FLAG) {
            ...
            if (value == HEALTHCHECK_CHECK_FLAG_TRUE) {
                ...
                AUTIL_LOG(TRACE3, "unexpected value[%s]", value.c_str());
...
bool RequestParser::parseAttributeClause(common::RequestPtr &requestPtr)
{
    ...
    if (!attributeClause) {
        AUTIL_LOG(WARN, "attributeClause is null");
        return false;
    }
...
bool RequestParser::parseVirtualAttributeClause(common::RequestPtr &requestPtr)
{
    ...
    if (!virtualAttributeClause) {
        AUTIL_LOG(WARN, "virtualAttributeClause is null");
        return false;
    }
    ...
    if (!ctx.parseVirtualAttributeClause(originalString.c_str())) {
        AUTIL_LOG(WARN, "parse virtualAttribute clause failed");
        _errorResult = ctx.getErrorResult();
        return false;
    }
...
bool RequestParser::parseQueryLayerClause(RequestPtr &requestPtr) {
    AUTIL_LOG(DEBUG, "parse query layer clause");
    QueryLayerClause *layerClause = requestPtr->getQueryLayerClause();
    if (!layerClause) {
        AUTIL_LOG(WARN, "failed to parseQueryLayerClause: layerClause is null");
        return false;
    }
    ...
    if (!ctx.parseLayerClause(layerClauseStr.c_str())) {
        AUTIL_LOG(WARN, "parse query layer clause failed");
        _errorResult = ctx.getErrorResult();
        return false;
    }
...
bool RequestParser::parseSearcherCacheClause(RequestPtr & requestPtr) {
    AUTIL_LOG(DEBUG, "parse searcher cache clause");
    ...
    if (!ctx.parseSearcherCacheClause(cacheClauseStr.c_str())) {
        AUTIL_LOG(WARN, "parse searcher cache clause failed");
        _errorResult = ctx.getErrorResult();
        return false;
    }
...
bool RequestParser::parseOptimizerClause(common::RequestPtr &request) {
    ...
    if (!optimizerClause) {
        AUTIL_LOG(WARN, "failed to parseOptimizerClause: cacheClause is null");
        return false;
    }
    ...
    for (size_t i = 0; i < st.getNumTokens(); ++i) {
        ...
        for (size_t j = 0; j < st1.getNumTokens(); ++j) {
            ...
            if (st2.getNumTokens() != 2) {
                AUTIL_LOG(WARN, "Invalid optimizer[%s]", st1[j].c_str());
            ...
            if (st2[0] == "name") {
                ...
                AUTIL_LOG(WARN, "Invalid optimize key[%s]", st2[0].c_str());
                _errorResult.setErrorCode(ERROR_OPTIMIZER_CLAUSE);
                return false;
            }
        }
                               ...
                               string &clusterName, vector<string> &rawPkVec)
{
    ...
    while (pos < length) {
        if (str[pos] == '\\') {
            if (pos + 1 < length) {
                ...
                AUTIL_LOG(WARN, "raw pk ends with \\, which need be escaped");
                return false;
            }
            pos += 2;
            ...
            if (meetsKvSep) {
                AUTIL_LOG(WARN, "invalid fetch summary clause, "
                        "multi kv seprators");
                return false;
            }
            meetsKvSep = true;
            ...
            autil::StringUtil::trim(temp);
            if (temp.empty()) {
                AUTIL_LOG(WARN, "invalid fetch summary clause, empty raw pk");
                return false;
            }
    ...
    if (!meetsKvSep || clusterName.empty()) {
        AUTIL_LOG(WARN, "parse cluster name from raw pk failed");
        return false;
    }
    autil::StringUtil::trim(temp);
    if (!temp.empty()) {
...
bool RequestParser::parseRankHint(const std::string &rankHintStr, RankHint &rankHint) {
    ...
    if (rankHintStr[0] == '+') {
        rankHint.sortPattern = indexlibv2::config::sp_asc;
    } else if (rankHintStr[0] == '-') {
        rankHint.sortPattern = indexlibv2::config::sp_desc;
    } else {
        return false;
    }
...
bool RequestParser::parseRankSortClause(RequestPtr &requestPtr) {
    AUTIL_LOG(DEBUG, "parse rank sort clause");

    RankSortClause* rankSortClause = requestPtr->getRankSortClause();
    if (!rankSortClause) {
        AUTIL_LOG(WARN, "failed to parseRankSortClause: "
    ...
    if (!ctx.parseRankSortClause(rankSortClauseStr.c_str())) {
        AUTIL_LOG(WARN, "parse rank sort clause failed");
        _errorResult = ctx.getErrorResult();
        return false;
    }
...
bool RequestParser::parseFinalSortClause(RequestPtr &requestPtr) {
    ...
    if (!finalSortClause) {
        AUTIL_LOG(WARN, "failed to parseFinalSortClause: "
    ...
    if (st.getNumTokens() != 1 && st.getNumTokens() != 2) {
        AUTIL_LOG(WARN, "split finalSortClause failed, split num is wrong!");
    ...
    if (sortNameSt.getNumTokens() != 2) {
        AUTIL_LOG(WARN, "parse sortName[%s] failed in finalSortClause",
                sortNameStr.c_str());
    ...
    if (sortNameSt[0] != FINALSORT_SORT_NAME) {
        AUTIL_LOG(WARN, "not specify sorter name in finalSortClause");
    ...
    if (sortParamSt.getNumTokens() != 2) {
        AUTIL_LOG(WARN, "parse sortParam[%s] failed in finalSortClause",
                sortParamStr.c_str());
    ...
    for (size_t i = 0; i < sortParamSt.getNumTokens(); ++i) {
        ...
        if (kvSt.getNumTokens() != 2) {
            AUTIL_LOG(WARN, "parse kv pair[%s] failed in finalSortClause",
                    sortParamSt[i].c_str());
...
bool RequestParser::parseLevelClause(RequestPtr &requestPtr) {
    ...
    if (levelClauseStr.empty()) {
        AUTIL_LOG(DEBUG, "Empty Level Clause Original String");
        return true;
    }
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        StringTokenizer st2(*it, LEVEL_CLAUSE_KV_SEPERATOR,
                            ...
                            | StringTokenizer::TOKEN_TRIM);
        if (st2.getNumTokens() != 2) {
            AUTIL_LOG(WARN, "Failed to parse LevelClause:[%s]",
        ...
        if (key == LEVEL_CLAUSE_KEY_SECOND_LEVEL_CLUSTER) {
            ...
            AUTIL_LOG(WARN, "Unknown key/value in LevelClause:[%s:%s]",
...
bool RequestParser::parseAnalyzerClause(RequestPtr &requestPtr) {
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        StringTokenizer st2(*it, ANALYZER_CLAUSE_SECTION_KV_SEPERATOR,
                            StringTokenizer::TOKEN_IGNORE_EMPTY
                            | StringTokenizer::TOKEN_TRIM);
        AUTIL_LOG(DEBUG, "Analyzer Clause Token num[%zd]", st2.getNumTokens());
        if (st2.getNumTokens() != 2) {
            AUTIL_LOG(WARN, "Failed to parse AnalyzerClause:[%s]",
...
bool RequestParser::parseAuxQueryClause(RequestPtr &requestPtr) {
    ...
    if (!auxQueryClause) {
        AUTIL_LOG(WARN, "failed to parseAuxQueryClause: auxQueryClause is null");
        return false;
    }
    ...
    AUTIL_LOG(TRACE2, "QueryText: [%s]", auxQueryClauseStr.c_str());
...
bool RequestParser::parseAuxFilterClause(AuxFilterClause *auxFilterClause) {
    if (!auxFilterClause) {
        AUTIL_LOG(WARN, "failed to parseAuxFilterClause: auxFilterClause is null");
        return false;
    }
        ...
        ConfigClause *configClause)
{
    ...
    for (StringTokenizer::Iterator it = st.begin(); it != st.end(); it++) {
        StringTokenizer st2(*it, ANALYZER_CLAUSE_INDEX_KV_PAIR_SEPERATOR,
                            StringTokenizer::TOKEN_IGNORE_EMPTY
                            | StringTokenizer::TOKEN_TRIM);
        AUTIL_LOG(DEBUG, "Analyzer Clause specific Index Analyzer Token num[%zd]",
                st2.getNumTokens());
        if (st2.getNumTokens() != 2) {
            AUTIL_LOG(WARN, "Failed to parse AnalyzerClause "
