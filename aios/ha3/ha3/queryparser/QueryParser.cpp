...
HA3_LOG_SETUP(queryparser, QueryParser);

QueryParser::QueryParser(const char *defaultIndex, 
                         QueryOperator defaultOp, bool flag)
...
QueryExpr* QueryParser::createDefaultOpExpr(QueryExpr *exprA, QueryExpr *exprB) {
    HA3_LOG(TRACE2, "createDefaultOpExpr(%p, %p)", exprA, exprB);
    switch (_defaultOP) {
    case OP_AND:
...
AtomicQueryExpr* QueryParser::createOrTermExpr(AtomicQueryExpr *exprA, AtomicQueryExpr *exprB) {
    HA3_LOG(TRACE2, "createOrTermExpr(%p, %p)", exprA, exprB);
...
bool QueryParser::isTerminalTermExpr(AtomicQueryExpr *expr){
    if(expr) {
        ...
        return ((exprTypeName == wordsTermExprTypeName)
                || (exprTypeName == numberTermExprTypeName)
...
AtomicQueryExpr* QueryParser::createMultiTermQueryExpr(
        AtomicQueryExpr *exprA, AtomicQueryExpr *exprB, QueryOperator opExpr)
{
    HA3_LOG(TRACE2, "createMultiTermExpr(%p, %p) %d", exprA, exprB, (int32_t)opExpr);
    ...
    if(isTerminalTermExpr(exprA) && isTerminalTermExpr(exprB)) {
        ...
        return multiExpr;
    } else if(isTerminalTermExpr(exprA) && isMultiTermExpr(exprB)
              && isSameOP(exprB, opExpr)) {
        dynamic_cast<MultiTermQueryExpr*>(exprB)->addTermQueryExpr(exprA);
        return exprB;
    } else if(isMultiTermExpr(exprA) && isTerminalTermExpr(exprB)
              && isSameOP(exprA, opExpr)) {
...
AtomicQueryExpr* QueryParser::createAndTermExpr(AtomicQueryExpr *exprA, AtomicQueryExpr *exprB) {
    HA3_LOG(TRACE2, "createAndTermExpr(%p, %p)", exprA, exprB);
...
AtomicQueryExpr* QueryParser::createWordsTermExpr(std::string *text) {
    ...
    HA3_LOG(TRACE2, "WordsExpr(%p):(%s)", e, text->c_str());
    delete text;
    return e;
}
...
AtomicQueryExpr* QueryParser::createPhraseTermExpr(std::string *text) {
    ...
    HA3_LOG(TRACE2, "PhraseExpr(%p):(%s)", e, text->c_str());
    delete text;
    return e;
}

AtomicQueryExpr* QueryParser::createNumberTermExpr(string *leftStr, bool iLeft,
        string *rightStr, bool iRight, ParserContext *ctx) 
{
    ...
    HA3_LOG(TRACE2, "NumberExpr(%p):(%ld,%d,%ld,%d)",e, left, iLeft, right, iRight);
    return e;
}

AtomicQueryExpr *QueryParser::createWordsWithIndexTermExpr(char* indexName, char* text) {
    AtomicQueryExpr *e = new WordsTermExpr(text);
    HA3_LOG(TRACE2, "WordsExpr(%p):(%s)", e, text);
