...
%parse-param {HA3_NS(queryparser)::ClauseScanner &scanner}
%parse-param {HA3_NS(queryparser)::ClauseParserContext &ctx}
...
%union {
    ...
    HA3_NS(common)::DistinctDescription *distinctDescVal;
    HA3_NS(common)::VirtualAttributeClause *virtualAttributeClauseVal;
    HA3_NS(common)::FilterClause *filterClauseVal;
    std::vector<std::string *> *gradeThresholdsVal;
    HA3_NS(common)::AggregateDescription *aggDescVal;
    HA3_NS(common)::AggFunDescription *aggFunDescVal;
    std::vector<std::string> *rangeExprVal;
    std::vector<HA3_NS(common)::AggFunDescription*> *aggFunDescsVal;
    HA3_NS(common)::QueryLayerClause *layerClause;
    HA3_NS(common)::LayerDescription *layerDesc;
    HA3_NS(common)::LayerKeyRange *layerKeyRange;
    HA3_NS(common)::LayerAttrRange *layerAttrRange;
    std::vector<HA3_NS(common)::LayerKeyRange*> *multiLayerRanges;
    HA3_NS(common)::SearcherCacheClause *searcherCacheClause;
    std::vector<suez::turing::SyntaxExpr*> *extraDistKeyExprVectVal;
    HA3_NS(common)::RankSortClause *rankSortClause;
    HA3_NS(common)::RankSortDescription *rankSortDesc;
    std::vector<HA3_NS(common)::SortDescription*> *sortDescs;
    HA3_NS(common)::SortDescription *sortDesc;
}
...
%token GROUP_KEY RANGE_KEY FUNCTION_KEY MAX_FUN MIN_FUN COUNT SUM_FUN DISTINCT_COUNT_FUN AGGREGATE_FILTER AGG_SAMPLER_THRESHOLD AGG_SAMPLER_STEP MAX_GROUP QUOTA_KEY UNLIMITED_KEY
...
%type <stringVal> sampler_threshold sampler_step max_group
...
%destructor {	      
    for (std::vector<HA3_NS(common)::AggFunDescription*>::iterator it = $$->begin(); it != $$->end(); ++it) {
        delete *it;
    }
    delete $$;
 } aggregate_functions aggregate_functions_sub
%destructor {delete $$;} sampler_threshold sampler_step max_group
...
%destructor {
    for (std::vector<HA3_NS(common)::LayerKeyRange*>::iterator it = $$->begin(); it != $$->end(); ++it) {
        delete *it;
    }
    delete $$;
} multi_layer_ranges
...
%destructor {
    for (std::vector<HA3_NS(common)::SortDescription*>::iterator it = $$->begin(); it != $$->end(); ++it) {
        delete *it;
    }
    delete $$;
} sort_expressions
...
clause_final : clause END {
    ctx.setStatus(HA3_NS(queryparser)::ClauseParserContext::OK);
}
...
multi_layer_ranges : layer_range { $$ = new std::vector<HA3_NS(common)::LayerKeyRange*>; $$->push_back($1);}
                   | multi_layer_ranges '*' layer_range { $1->push_back($3); $$ = $1; }

layer_range : layer_range_key '{' sub_layer_ranges '}' { $3->attrName = *$1; delete $1; $$ = $3; }
            | layer_range_key { $$ = new HA3_NS(common)::LayerKeyRange(); $$->attrName = *$1; delete $1; }

layer_range_key : general_identifier { $$ = $1; }
                | '%' general_identifier {$$ = $2; $$->insert(0, 1, '%');}

sub_layer_ranges : sub_layer_range { $$ = new HA3_NS(common)::LayerKeyRange(); $$->ranges.push_back(*$1); delete $1;}
                 | sub_layer_ranges ',' sub_layer_range { $1->ranges.push_back(*$3); $$ = $1; delete $3; }
                 | layer_range_value { $$ = new HA3_NS(common)::LayerKeyRange(); $$->values.push_back(*$1); delete $1;}
                 | sub_layer_ranges ',' layer_range_value {$1->values.push_back(*$3); $$ = $1; delete $3; }

sub_layer_range : '[' layer_range_value ',' layer_range_value ']' { $$ = new HA3_NS(common)::LayerAttrRange(); $$->from = *$2; $$->to = *$4; delete $2; delete $4;}
                | '[' layer_range_value ',' layer_range_value ')' { $$ = new HA3_NS(common)::LayerAttrRange(); $$->from = *$2; $$->to = *$4; delete $2; delete $4;}
                | '[' ',' layer_range_value ']' {
                    $$ = new HA3_NS(common)::LayerAttrRange();
                    $$->from = HA3_NS(common)::LayerDescription::INFINITE;
                    $$->to = *$3;
                    delete $3;
                  }
                | '[' ',' layer_range_value ')' {
                ...
                | '[' layer_range_value ',' ']' {
                    $$ = new HA3_NS(common)::LayerAttrRange();
                    $$->from = *$2;
                    $$->to = HA3_NS(common)::LayerDescription::INFINITE;
                    delete $2;
                  }
                | '[' layer_range_value ',' ')' {
                    $$ = new HA3_NS(common)::LayerAttrRange();
...
rank_sort_clause : rank_sort_description {
    $$ = new HA3_NS(common)::RankSortClause();
    $$->addRankSortDesc($1);
 }
| rank_sort_clause ';' {
    $$ = $1;
...
rank_sort_description : sort_expressions ',' sort_percent {
    $$ = new HA3_NS(common)::RankSortDescription();
    $$->setSortDescs(*$1);
    $$->setPercent(*$3);
    delete $1;
    delete $3;
...
sort_expressions : SORT_KEY ':' ordered_sort_expr { $$ = new std::vector<HA3_NS(common)::SortDescription*>; $$->push_back($3); }
| sort_expressions '#' ordered_sort_expr { $$ = $1; $$->push_back($3); }

ordered_sort_expr : '+' RANK_EXPRESSION {
    $$ = new HA3_NS(common)::SortDescription();
    $$->setExpressionType(HA3_NS(common)::SortDescription::RS_RANK);
...
sort_expr :
  algebra_expression {
    $$ = new HA3_NS(common)::SortDescription();
    $$->setExpressionType(HA3_NS(common)::SortDescription::RS_NORMAL);
    $$->setRootSyntaxExpr($1);
  }
...
void isearch_bison::ClauseBisonParser::error(
        ...
        const std::string& m)
{
    ctx.setStatus(HA3_NS(queryparser)::ClauseParserContext::SYNTAX_ERROR);
    ctx.setStatusMsg(m);
}
