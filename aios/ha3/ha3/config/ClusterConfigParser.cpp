...
HA3_LOG_SETUP(config, ClusterConfigParser);
...
bool ClusterConfigParser::parse(const string &jsonFile, JsonMap &jsonMap) const {
    string content;
    if (!FileUtil::readFileContent(jsonFile, content)) {
        HA3_LOG(ERROR, "read file %s failed", jsonFile.c_str());
        return false;
    }
    // check loop in an inheritance path
    if (find(_configFileNames.begin(), _configFileNames.end(), jsonFile)
        != _configFileNames.end()) {
        HA3_LOG(ERROR, "inherit loop detected, file: %s", jsonFile.c_str());
        return false;
    }
    ...
    try {
        ...
        HA3_LOG(ERROR, "jsonize %s failed, exception: %s",
                content.c_str(), e.what());
        return false;
    }
    if (!merge(jsonMap)) {
        HA3_LOG(ERROR, "merge inherit of  %s failed",
                content.c_str());
        return false;
    }
...
bool ClusterConfigParser::merge(JsonMap &jsonMap) const {
         ...
         it != jsonMap.end(); it++)
    {
        ...
        if (any.GetType() == typeid(JsonMap)) {
            ...
            if (!merge(childJsonMap)) {
                HA3_LOG(ERROR, "merge child %s fail",
                        key.c_str());
                return false;
            }
        }
    ...
    if (!mergeWithInherit(jsonMap)) {
        HA3_LOG(ERROR, "merge with inherit fail");
        return false;
    }
    return true;
}
...
bool ClusterConfigParser::mergeWithInherit(JsonMap &jsonMap) const {
    ...
    try {
        ...
        HA3_LOG(ERROR, "jsonize inherit_from failed, exception: %s", e.what());
        return false;
    }
    ...
    if (!parse(inheritFromFile, inheritJsonMap)) {
        HA3_LOG(ERROR, "parse %s failed", inheritFromFile.c_str());
        return false;
    }
...
bool ClusterConfigParser::mergeJsonMap(const JsonMap &src, JsonMap &dst) const {
    ...
    for (; it != src.end(); it++) {
        ...
        if (dst.find(key) == dst.end()) {
            ...
            HA3_LOG(ERROR, "merge error, type not match for %s", key.c_str());
            return false;
            ...
            if (!mergeJsonMap(srcChild, dstChild)) {
                HA3_LOG(ERROR, "merge child[%s] failed", key.c_str());
                return false;
            }
    ...
    for (JsonArray::iterator it = dst.begin(); it != dst.end(); it++) {
        if (it->GetType() == typeid(JsonMap)) {
            if (!merge(*AnyCast<JsonMap>(&*it))) {
                HA3_LOG(ERROR, "merge child[%s] failed", ToString(*it).c_str());
                return false;
            }
        ...
        T &config) const
{
    ...
    if (!parse(configFile, jsonMap)) {
        HA3_LOG(ERROR, "failed to parse %s", configFile.c_str());
        return PR_FAIL;
    }
    ...
    try {
        ...
        HA3_LOG(ERROR, "parse section %s failed, exception: %s",
