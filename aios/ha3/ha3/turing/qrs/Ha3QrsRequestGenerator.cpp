...
#include <multi_call/interface/GrpcRequest.h>
...
HA3_LOG_SETUP(turing, Ha3QrsRequestGenerator);
...
void Ha3QrsRequestGenerator::generate(PartIdTy partCnt,
...
void Ha3QrsRequestGenerator::prepareRequest(const std::string &bizName,
        suez::turing::GraphRequest &graphRequest, bool needClone) const
{
...
void Ha3QrsRequestGenerator::generateNormal(PartIdTy partCnt,
        PartRequestMap &requestMap) const
{
    if (partCnt <= 0) {
        return;
    }
    ...
    auto graphRequest0 =
        google::protobuf::Arena::CreateMessage<suez::turing::GraphRequest>(
            arena);
...
void Ha3QrsRequestGenerator::generatePid(PartIdTy partCnt,
                                     PartRequestMap &requestMap) const
{
    if (partCnt <= 0) {
        return;
    ...
    for (iter++; iter != pids.end(); iter++) {
        auto index = *iter;
        auto message =
            (suez::turing::GraphRequest *)graphRequestBegin->New(arena);
        assert(message);
...
set<int32_t> Ha3QrsRequestGenerator::getPartIds(const RangeVec& ranges,
...
multi_call::RequestPtr Ha3QrsRequestGenerator::createQrsRequest(
        const string &methodName,
        suez::turing::GraphRequest *graphRequest) const
{
    multi_call::Request *qrsRequest = NULL;
    if (_useGrpc) {
        auto requestTyped = new GrpcRequest(
            "/suez.turing.GraphService/runGraph", getProtobufArena());
        requestTyped->setMessage(graphRequest);
        requestTyped->setTimeout(_timeout);
        qrsRequest = requestTyped;
    } else {
        qrsRequest = new service::QrsRequest(methodName, graphRequest, _timeout,
                                             getProtobufArena());
    }
