...
#include "ha3/turing/common/Ha3FunctionDef.h"
    ...
    auto reflectParamFn = [&](ParamTypeDef &paramDef) {
        if (paramDef.isMulti) {
            ...
            if (paramDef.type.empty()) {
                throw IquanException("paramDef.type is empty in " +
        ...
        FunctionModels &functionModels)
{
    ...
    for (auto &ha3FunctionDef : ha3FunctionsDef.functionDefList) {
        ...
        functionModel.functionContentVersion =
                ha3FunctionDef.functionContentVersion;
