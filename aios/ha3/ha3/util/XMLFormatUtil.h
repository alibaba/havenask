...
public:
    ...
    static inline void formatCdataValue(const std::string &str, std::stringstream &ss);
    template <typename T>
    static inline void toXMLString(const T &value, std::stringstream &oss);
    template <typename T>
    static inline void toXMLString(const std::vector<T> &value, std::stringstream &oss);

private:
    template <typename T>
    static inline void toXMLString(const T &value, std::stringstream &oss, Unsupport);
    template <typename T>
    static inline void toXMLString(const T &value, std::stringstream &oss, Support);
    template <typename T>
    static inline void toXMLString(const std::vector<T> &value, std::stringstream &oss, Unsupport);
    template <typename T>
    static inline void toXMLString(const std::vector<T> &value, std::stringstream &oss, Support);

private:
    AUTIL_LOG_DECLARE();
};

typedef std::shared_ptr<XMLFormatUtil> XMLFormatUtilPtr;
...
inline void XMLFormatUtil::formatCdataValue(const std::string &str, std::stringstream &ss) {
    ...
    while (pos < str.length()) {
        if (isInvalidXMLChar(str[pos])) {
            ...
            ss << str[pos];
            if (pos + 2 < str.length() && str[pos + 1] == ']' && str[pos + 2] == '>') {
                ss << "]&gt;";
                pos += 2;
            }
        } else {
...
template <typename T>
inline void XMLFormatUtil::toXMLString(const std::vector<T> &value, std::stringstream &oss) {
    toXMLString(value, oss, SupportTraits<DirectOutput<T>::DIRECT_OUTPUT>());
}

template <typename T>
inline void
XMLFormatUtil::toXMLString(const std::vector<T> &value, std::stringstream &oss, Unsupport) {
...
template <typename T>
inline void
XMLFormatUtil::toXMLString(const std::vector<T> &value, std::stringstream &oss, Support) {
...
inline void XMLFormatUtil::toXMLString<int8_t>(const std::vector<int8_t> &value,
                                               ...
                                               Support) {
    for (std::vector<int8_t>::const_iterator it = value.begin(); it != value.end(); ++it) {
        int32_t temp = *it;
        if (it != value.begin())
            oss << " ";
        oss << temp;
...
template <typename T>
inline void XMLFormatUtil::toXMLString(const T &value, std::stringstream &oss, Unsupport) {
    std::string temp = autil::StringUtil::toString(value);
    XMLFormatUtil::formatCdataValue(temp, oss);
}

template <typename T>
inline void XMLFormatUtil::toXMLString(const T &value, std::stringstream &oss, Support) {
    oss << value;
}

template <>
inline void
XMLFormatUtil::toXMLString<int8_t>(const int8_t &value, std::stringstream &oss, Support) {
    toXMLString<int32_t>((int32_t)value, oss, Support());
}

template <>
inline void
XMLFormatUtil::toXMLString<uint8_t>(const uint8_t &value, std::stringstream &oss, Support) {
    toXMLString<uint32_t>((uint32_t)value, oss, Support());
}

template <>
inline void
XMLFormatUtil::toXMLString<std::string>(const std::string &value, std::stringstream &oss, Support) {
