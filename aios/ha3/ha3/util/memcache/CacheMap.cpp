...
u_n32_t CCacheMap::init(void)
{
    ...
    if(m_ppEntry == NULL)
    {
        AUTIL_LOG(ERROR, "CCacheMap: malloc failed!");
        return 0;
    }
    ...
    {
        AUTIL_LOG(ERROR, "CCacheMap: new failed!");
        return 0;
    }
...
bool CCacheMap::insert(HashEntry *item)
{
	u_n32_t nPos = item->key % m_nHashSize;			//计算key的hash值
        // AUTIL_LOG(INFO, "insert %p, key %u, nPos %u", item, item->key, nPos);
    item->hash_next = m_ppEntry[nPos];
	m_ppEntry[nPos] = item;
	atomic64_inc(&m_nHashUsed);
    return true;
}
...
{
	if (atomic64_read(&m_nHashUsed) < m_nRehashLimit || m_nHashSizeStep >= CACHE_MAX_SIZESTEP)
        return 0;
	if(lockBucket() != 0)
		return 0;
    /* 判断容器是否超过限制需重新rehash */
    ...
    if(m_pNodeLock == NULL)
    {
        ...
        unlockBucket();
        AUTIL_LOG(ERROR, "HashMap rehash new mem failed!");
        return 0;
    }
    ...
    if(!ppNewEntries)
    {
		...
		m_nLockSize = nOldLockSize;
        unlockBucket();
        AUTIL_LOG(ERROR, "HashMap rehash calloc mem failed!");
        return 0;
    }
...
HashEntry *CCacheMap::remove(const u_n64_t& _key, const HashEntry *entry)
{
    ...
    if (pCurEntry != NULL && pCurEntry == entry) {	//考虑到可能是多key值的情况
        // AUTIL_LOG(INFO, "remove %p, key %u, nPos %u", pCurEntry, pCurEntry->key, nPos);
...
{
	if (atomic64_read(&m_nHashUsed) > 0) {
...
{
	return atomic64_read(&m_nHashUsed);
}
...
bool CCacheMap::consistencyCheck(std::map<void *, size_t> &hashMap) {
    for (size_t i = 0; i < m_nHashSize; ++i) {
        ...
        while (entry) {
            ...
            if (entry->key % m_nHashSize != i) {
                AUTIL_LOG(ERROR, "wrong position, key %lu, bucket expected %zu, bucket actual %lu",
                        entry->key, i, bucket);
                return false;
            }
            auto iter = hashMap.find(entry);
            if (iter != hashMap.end()) {
                AUTIL_LOG(ERROR, "duplicate entry in hashmap, key %lu, item %p, bucket1: %zu, bucket2: %zu",
    ...
    return (long)hashMap.size() == atomic64_read(&m_nHashUsed);
}
