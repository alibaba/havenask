 ...
 */
#define POLYNOMIAL 0X04c10db7L
#define POLY64NOMIAL 0x95AC9329AC4BC9B5ULL

#include "ha3/util/memcache/crc.h"


const static unsigned int CRC32_TABLE[256]={
...
const static unsigned long long CRC64_TABLE[256]={
...
unsigned long long get_crc64(const char *buf, int buf_len){
	unsigned long long crc=0xFFFFFFFFFFFFFFFFULL;
	int j;
	for (j = 0; j < buf_len; j++) {
		crc = CRC64_TABLE[(crc ^ buf[j]) & 0xff] ^ (crc >> 8);
	}
	return crc;
}


unsigned int get_crc32(const char *buf, int buf_len){
	int i, j;
	int crc_accum = 0xffffffffL;
	for (j = 0; j < buf_len; j++) {
		i = ((crc_accum >> 24) ^ *buf++) & 0xff;
