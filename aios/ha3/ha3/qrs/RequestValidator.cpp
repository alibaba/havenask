...
namespace isearch {
namespace qrs {
AUTIL_LOG_SETUP(ha3, RequestValidator);
...
bool RequestValidator::validateBasciAuxQueryInfo(
        ...
        const std::vector<suez::turing::VirtualAttribute *> &virtualAttributes)
{
    ...
    const string &clusterName = clusterNames[0];
    ClusterConfigMap::const_iterator configIter = _clusterConfigMapPtr->find(clusterName);
    if (configIter == _clusterConfigMapPtr->end()) {
        _errorCode = ERROR_CLUSTER_NAME_NOT_EXIST;
        _errorMsg = "can not find cluster[" + clusterName + "]'s config name";
        return false;
    }

    const ClusterConfigInfo &auxClusterConfigInfo = configIter->second;
    string scanJoinCluster = auxClusterConfigInfo.getJoinConfig().getScanJoinCluster();
    if (!scanJoinCluster.empty()) {
        ...
        TableInfoPtr auxTableInfoPtr = auxIter->second;
...
bool RequestValidator::validate(const RequestPtr& requestPtr) {
    ...
    vector<string> clusterNames = std::move(configClause->getClusterNameVector());
    ...
    if (levelClause && levelClause->maySearchSecondLevel()) {
        ...
        for (size_t i = 0; i < clusterLists.size(); i++) {
            for (size_t j = 0; j < clusterLists[i].size(); j++) {
                ...
                {
                    clusterNames.emplace_back(clusterLists[i][j]);
                }
            }
        }
    }
    return validate(requestPtr, clusterNames);
}
                                ...
                                const vector<string> &clusterNames)
{
    ...
    const OptimizerClause *optimizerClause = requestPtr->getOptimizerClause();
    if (!validateOptimizerClause(optimizerClause)) {
        return false;
    }
    ...
    for (const auto &clusterName : clusterNames) {
        ...
        if (iter == _clusterTableInfoMapPtr->end()) {
            _errorCode = ERROR_CLUSTER_NAME_NOT_EXIST;
            _errorMsg =  "can not find cluster[" + clusterName + "]'s table name";
            return false;
        }
        ...
        //     return false;
        // }
        const AuxQueryClause *auxQueryClause = requestPtr->getAuxQueryClause();
        const AuxFilterClause *auxFilterClause = requestPtr->getAuxFilterClause();
        if (!validateBasciAuxQueryInfo(auxQueryClause, auxFilterClause,
                        clusterNames, virtualAttributes))
        {
            return false;
        }
        ...
        if (!validateAuxQueryClause(auxQueryClause)) {
            return false;
        }

        if (!validateAuxFilterClause(auxFilterClause)) {
            return false;
        }
        ...
        if (!validateVirtualAttributeClause(virtualAttributeClause, virtualAttributes))
        {
            return false;
        }
...
void RequestValidator::addFunctionInfo(const std::vector<std::string> &clusterNames,
    ...
    if (_clusterCavaPluginManagerMapPtr && !_clusterCavaPluginManagerMapPtr->empty()) {
        for (auto name : clusterNames) {
            ...
            if (it != _clusterCavaPluginManagerMapPtr->end()) {
                syntaxExprValidator->addCavaPluginManager(it->second.get());
            }
        }
    }
}
...
bool RequestValidator::validateOptimizerClause(const OptimizerClause* optimizerClause) {
    OptimizerClauseValidator optimizerValidator;
    if (!optimizerValidator.validate(optimizerClause)) {
        _errorCode = optimizerValidator.getErrorCode();
        _errorMsg = haErrorCodeToString(_errorCode);
        return false;
    }
    return true;
...
bool RequestValidator::validateQueryClause(const QueryClause* queryClause) {
    ...
    REQUEST_TRACE(DEBUG, "validate query string[%s]",
                  queryClause->getOriginalString().c_str());
    ...
    for (uint32_t i = 0; i < queryCount; ++i) {
        ...
        REQUEST_TRACE(DEBUG, "validate query name[%s]", query->getQueryName().c_str());
        ...
        const vector<VirtualAttribute*> &virtualAttributes, const string &name)
{
    ...
    {
        AUTIL_LOG(WARN, "virtual attribute name [%s] conflict with attribute.",
                name.c_str());
        return false;
    }
         ...
         it != virtualAttributes.end(); ++it)
    {
        if (name == (*it)->getVirtualAttributeName()) {
            AUTIL_LOG(WARN, "virtual attribute name [%s] conflict with other "
...
bool RequestValidator::validateDistinctClause(const DistinctClause* distinctClause)
{
        ...
        distDescCount > DistinctClause::MAX_DISTINCT_DESC_COUNT)
    {
        ...
        _errorMsg = "wrong distinct description count, "
                    "it must be >0 and <=2.";
        AUTIL_LOG(WARN, "%s original string: %s", _errorMsg.c_str(),
        ...
        const AggregateClause* aggregateClause)
{
         ...
         it != aggDesVec.end(); ++it)
    {
        ...
        if(!groupKeyExpr || !validateSyntaxExpr(groupKeyExpr, location)) {
            AUTIL_LOG(WARN, "AggregateClause: groupKeyExpr validate error");
            return false;
        }
        ...
        if (aggFunDesVec.size() == 0){
            AUTIL_LOG(WARN, "aggFunDesVec.size() == 0");
            return false;
        }
        ...
        for (size_t i = 0; i < aggFunDesVec.size(); i++) {
            if (!validateAggFunDescription(aggFunDesVec[i], allowSubExpr)) {
                AUTIL_LOG(WARN, "validate the %lu aggregate Function error", i);
                return false;
            }
        }
        ...
        if (!validateAggregateRange(exprResultType, ranges)) {
            AUTIL_LOG(WARN, "validate aggregate range error");
            return false;
        }
    }
    return true;
...
{
    REQUEST_TRACE(DEBUG, "validate agg ranges.size: %ld", ranges.size());
    if (ranges.size() < 2) {
        return true;
    }
        ...
        const AggFunDescription* aggFunDes, bool allowSubExpr)
{
    ...
    if (funName != AGGREGATE_FUNCTION_COUNT) {
        ...
        if (!validateSyntaxExpr(syntaxExpr, AGGFUN_SYNTAX_LOCATION)) {
            AUTIL_LOG(WARN, "aggregate function expression validate error");
            return false;
        }
        ...
        REQUEST_TRACE(DEBUG, "validate agg funName:%s, ExprResultType: %d",
...
bool RequestValidator::validateAggFunName(const string &aggFunName) {
    ...
    if (!isValidate) {
        AUTIL_LOG(WARN, "not supported aggregate function name: [%s]",
            aggFunName.c_str());
...
{
    if (syntaxExpr == NULL) {
        AUTIL_LOG(WARN, "the parameter of syntaxExpr is NULL.");
        return false;
    }
...
bool RequestValidator::validateAuxQueryClause(
    ...
    REQUEST_TRACE(DEBUG, "validate aux query string[%s]",
    ...
    for (uint32_t i = 0; i < queryCount; ++i) {
        ...
        REQUEST_TRACE(DEBUG, "validate aux query name[%s]",
                      auxQuery->getQueryName().c_str());
