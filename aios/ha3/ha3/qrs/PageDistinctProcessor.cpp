...
namespace isearch {
namespace qrs {
AUTIL_LOG_SETUP(ha3, PageDistinctProcessor);
                                    ...
                                    ResultPtr &resultPtr) 
{
    ...
    if (!getPageDistParams(requestPtr, pageDistParam, errorResult)) {
        resultPtr.reset(new common::Result);
        resultPtr->addErrorResult(errorResult);
        AUTIL_LOG(WARN,"%s", errorResult.getErrorMsg().c_str());
        return;
    }
...
#define CREATE_PAGEDISTSELECTOR_CASE(vt)                                \
    ...
    default:
        AUTIL_LOG(WARN, "invalid variable type: [%s]", vt2TypeName(vt).c_str());
        return PageDistinctSelectorPtr();
    }
                                        ...
                                        ResultPtr &resultPtr)
{
    ...
    if (pageDistSelectorPtr == NULL) {
        AUTIL_LOG(WARN, "create PageDistinctSelector failed");
