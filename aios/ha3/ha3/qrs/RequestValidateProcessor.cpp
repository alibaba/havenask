...
HA3_LOG_SETUP(qrs, RequestValidateProcessor);
        ...
        ResultPtr &resultPtr)
{
    ...
    if (NULL == configClause) {
        ...
        HA3_LOG(WARN, "%s", haErrorCodeToString(ERROR_NO_CONFIG_CLAUSE).c_str());
        return false;
    }
    ...
    if (!validateMultiCluster(configClause, requestPtr->getLevelClause(),
                              errorResultPtr))
    {
        return false;
    }
    ...
    if (queryClause == NULL || queryClause->getQueryCount() == 0) {
        ...
        HA3_LOG(WARN, "%s", haErrorCodeToString(ERROR_NO_QUERY_CLAUSE).c_str());
        return false;
    }
    ...
    for (uint32_t i = 0; i < queryCount; ++i) {
        ...
        if (cleanQuery) {
            REQUEST_TRACE(DEBUG, "no stop word query:%s",
    ...
    if (auxQueryClause != NULL) {
        ...
        const string &clusterName = configClause->getClusterName();
        if (!getAuxTableInfo(clusterName, 
                             errorResultPtr, 
                             auxTableInfoPtr)) {
...
bool RequestValidateProcessor::validateMultiCluster(
        const ConfigClause *configClause, const LevelClause *levelClause,
        MultiErrorResultPtr &errorResultPtr)
{
    ...
    if (levelClause && levelClause->maySearchSecondLevel()) {
        string secondLevelCluster = levelClause->getSecondLevelCluster();
        ...
        for (size_t i = 0; i < clusterLists.size(); i++) {
            for (size_t j = 0; j < clusterLists[i].size(); j++) {
                ...
                {
                    clusterNameVec.push_back(clusterLists[i][j]);
                }
            }
        }
    }
    ...
    if (!getTableInfo(configClause->getClusterName(), errorResultPtr, tableInfoPtr1)) {
        return false;
    }
    ...
    for (size_t i = 1; i < clusterNameVec.size(); ++i) {
        ...
        if (!schemaCompatible(tableInfoPtr1, tableInfoPtr2)) {
            ...
            HA3_LOG(WARN, "schema not compatible between cluster[%s] and cluster[%s]",
                    configClause->getClusterName().c_str(), clusterNameVec[i].c_str());
            return false;
        }
    }
    return true;
        ...
        ConfigClause *configClause, MultiErrorResultPtr &errorResultPtr)
{
        ...
        || !getClusterConfigInfo(fetchSummaryCluster, clusterConfig))
    {
        ...
        HA3_LOG(WARN, "%s", errorString.c_str());
        return false;
    }

    if (!sameCluster && fetchSummaryType != BY_RAWPK) {
        errorResultPtr->addError(ERROR_ONLY_SUPPORT_FETCH_SUMMARY_BY_RAWPK);
        HA3_LOG(WARN, "%s", haErrorCodeToString(ERROR_ONLY_SUPPORT_FETCH_SUMMARY_BY_RAWPK).c_str());
        return false;
    }
        ...
        ConfigClause *configClause, MultiErrorResultPtr &errorResultPtr)
{
    ...
    if (!configClause->isNoSummary()) {
        ...
        HA3_LOG(WARN, "cannot format pb matchdocs in phase2.");
        return false;
    }
        ...
        const IndexInfos* indexInfo2)
{
         ...
         it != indexInfo1->end(); ++it)
    {
        ...
        if (info1->getIndexType() != info2->getIndexType()) {
            HA3_LOG(WARN, "index info not compatible, different index types."
                    "index name:%s", indexName.c_str());
            return false;
        }
        if (info1->getAnalyzerName() != info2->getAnalyzerName()) {
            HA3_LOG(WARN, "index info not compatible, different analyzers."
                    "index name:%s", indexName.c_str());
            return false;
        }
    }
        ...
        const AttributeInfos* attrInfo1, const AttributeInfos* attrInfo2)
{
         ...
         it != attrs.end(); ++it)
    {
        ...
        if (info1->getMultiValueFlag() != info2->getMultiValueFlag()) {
            HA3_LOG(WARN, "attribute info not compatible, different multivalue flags."
                    "attribute name:%s", attrName.c_str());
            return false;
        }
        if (info1->getFieldType() != info2->getFieldType()) {
            HA3_LOG(WARN, "index info not compatible, different field types."
                    "attribute name:%s", attrName.c_str());
            return false;
        }
    }
        ...
        Query **ppCleanQuery)
{
                    ...
                    indexInfos, defaultOP))
    {
        ...
        HA3_LOG(WARN, "%s", haErrorCodeToString(errorResult.getErrorCode()).c_str());
        return false;
    }
    Query *tokenizedQuery = _queryTokenizer.stealQuery();
    StopWordsCleaner stopWordsCleaner;
    if (tokenizedQuery) {
        REQUEST_TRACE(DEBUG, "tokenized query:%s",
                      tokenizedQuery->toString().c_str());
        bool ret = stopWordsCleaner.clean(tokenizedQuery);
        delete tokenizedQuery;
        tokenizedQuery = NULL;
        if (!ret) {
            errorResultPtr->addError(stopWordsCleaner.getErrorCode(),
                    stopWordsCleaner.getErrorMsg());
            HA3_LOG(WARN, "%s", haErrorCodeToString(
                            stopWordsCleaner.getErrorCode()).c_str());
            return false;
        }
    }
    *ppCleanQuery = stopWordsCleaner.stealQuery();
    return true;
}
