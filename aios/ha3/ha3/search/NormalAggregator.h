...
#include "matchdoc/SubDocAccessor.h" // IWYU pragma: keep
...
struct UnorderedMapTraits {
    typedef std::unordered_map<KeyType, matchdoc::MatchDoc> GroupMapType;
};
...
template <typename TT, typename T>
struct MapTransformTraits<
    TT, std::unordered_map<T, matchdoc::MatchDoc> > {
...
AUTIL_LOG_SETUP_TEMPLATE_3(ha3, NormalAggregator, GroupMapType, KeyType, ExprType);
...
suez::turing::CavaAggModuleInfoPtr NormalAggregator<KeyType, ExprType, GroupMapType>::codegen() {
    ...
    return std::dynamic_pointer_cast<suez::turing::CavaAggModuleInfo>(ptr);
}
