...
class MultiTermAndQueryExecutorTest : public TESTBASE {
public:
    MultiTermAndQueryExecutorTest();
    ~MultiTermAndQueryExecutorTest();
public:
    void setUp();
    void tearDown();
...
HA3_LOG_SETUP(search, MultiTermAndQueryExecutorTest);


MultiTermAndQueryExecutorTest::MultiTermAndQueryExecutorTest() { 
    _queryExecutor = NULL;
...
void MultiTermAndQueryExecutorTest::setUp() { 
    HA3_LOG(DEBUG, "setUp!");
}

void MultiTermAndQueryExecutorTest::tearDown() { 
...
TEST_F(MultiTermAndQueryExecutorTest, testAndQueryTwoTermQuery1) { 
    HA3_LOG(DEBUG, "Begin Test!");
    string one = "one:1[1];3[1];5[1];\n";
    string two = "two:2[1];4[1];\n";
    createIndexPartitionReaderWrapper(one + two);
    _queryExecutor = QueryExecutorConstructor::prepareMultiTermAndQueryExecutor(
...
TEST_F(MultiTermAndQueryExecutorTest, testAndQueryTwoTermQuery2) { 
    HA3_LOG(DEBUG, "Begin Test!");
...
TEST_F(MultiTermAndQueryExecutorTest, testAndQueryTwoTermQuery3) { 
    HA3_LOG(DEBUG, "Begin Test!");
...
TEST_F(MultiTermAndQueryExecutorTest, testAndQueryThreeTermQuery) { 
    HA3_LOG(DEBUG, "Begin Test!");
...
TEST_F(MultiTermAndQueryExecutorTest, testSubDocSeekTwoExecutor) {
    FakeIndex fakeIndex;
...
TEST_F(MultiTermAndQueryExecutorTest, testSubDocSeekAllSubExecutor) {
    FakeIndex fakeIndex;
    fakeIndex.tablePrefix.push_back("sub_");
...
TEST_F(MultiTermAndQueryExecutorTest, testSubDocSeekMultiExecutor) {
    FakeIndex fakeIndex;
    fakeIndex.tablePrefix.push_back("sub_");
...
TEST_F(MultiTermAndQueryExecutorTest, testSubDocSeekTwoExecutorWhenNotSetMainJoin) {
    FakeIndex fakeIndex;
    fakeIndex.tablePrefix.push_back("sub_");
...
void MultiTermAndQueryExecutorTest::createIndexPartitionReaderWrapper(
