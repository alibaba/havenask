...
namespace isearch {
namespace search {
AUTIL_LOG_SETUP(ha3, AggregatorCreator);
...
Aggregator* AggregatorCreator::createAggregator(const AggregateClause* aggClause) {
    if (!aggClause) {
        AUTIL_LOG(TRACE3, "AggregateClause is NULL");
        return NULL;
    }
         ...
         it != aggDescriptions.end(); it++)
    {
        ...
        if (aggregator) {
            ...
            AUTIL_LOG(WARN, "Create aggregator error from aggDes[%s]",
    ...
    if (aggVec.empty()) {
        AUTIL_LOG(WARN, "Create aggregator error from aggClause[%s]",
...
AggregateFunction* AggregatorCreator::doCreateAggregateFunction(const AggFunDescription* aggFunDes) {
    ...
    if (!attrExpr) {
        AUTIL_LOG(WARN, "create aggregate function expression[%s] failed.",
    ...
    AttributeExpressionTyped<T> *expr =
     ...
     if (!expr) {
         AUTIL_LOG(WARN, "unexpected type for aggregator function[%s].",
                 syntaxExpr->getExprString().c_str());
         return NULL;
    }
...
AggregateFunction* AggregatorCreator::doCreateDistinctCountAggFunction(const AggFunDescription* aggFunDes) {
    ...
    AttributeExpression *attrExpr =
        _attrExprCreator->createAttributeExpression(syntaxExpr);
    if (!attrExpr) {
        AUTIL_LOG(WARN, "create distinct_count aggregate function expression[%s] failed.",
                syntaxExpr->getExprString().c_str());
        return NULL;
    }
...
#define CREATE_AGG_FUN_HELPER(vt_type, ResultType)                      \
    ...
    if (aggFun == NULL) {
        AUTIL_LOG(WARN, "does not support this Aggregator Function Result type: %d",
            variableType);
...
Filter* AggregatorCreator::createAggregateFilter(const AggregateDescription *aggDes) {
    ...
    if (filterClause == nullptr) {
        return nullptr;
    }
    return Filter::createFilter(filterClause->getRootSyntaxExpr(), _attrExprCreator, _pool);
}
...
Aggregator * AggregatorCreator::doCreateSingleAggregatorDelegator(const AggregateDescription *aggDes) {
    ...
    if (enableDenseMode) {
        ...
        typedef std::unordered_map<KeyType, matchdoc::MatchDoc> GroupSparseMapType;
        if (groupKeyExpr->isMultiValue()) {
...
Aggregator * AggregatorCreator::doCreateSingleAggregator(const AggregateDescription *aggDes) {
    ...
    AttributeExpressionTyped<ExprType> *expr
        = dynamic_cast<AttributeExpressionTyped<ExprType> *>(attrExpr);
    if (!expr) {
        AUTIL_LOG(WARN, "unexpected group key type for aggregator[%s].",
                groupKeyExpr->getExprString().c_str());
        return NULL;
    }
         ...
         it != aggFuns.end(); it++)
    {
        ...
        if(!aggFun) {
            AUTIL_LOG(WARN, "Create AggregateFunction error: %s", aggDes->getOriginalString().c_str());
            continue;
        }
...
#define CREATE_SINGLE_AGG_DELEGATOR_HELPER(vt_type)                     \
    ...
    default:
        ...
        AUTIL_LOG(WARN, "does not support this Aggregator type: %s",
                typeStr.c_str());
        aggregator = NULL;
        stringstream ss;
