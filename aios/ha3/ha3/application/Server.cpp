 ...
 */
#include "ha3/application/Server.h"
...
void Server::prepareArpcDesc(multi_call::ArpcServerDescription& desc) {
    desc.port = autil::EnvUtil::getEnv(RPC_PORT, "0");
    if (desc.port == "0") {
        desc.port = randomPort();
    }
    desc.name = "Ha3Arpc";
...
string Server::randomPort() const {
    static int32_t MAX_PORT = 60000;
    static int32_t MIN_PORT = 50000;
    srand((unsigned int)TimeUtility::currentTimeInSeconds());
    int32_t port = random() % (MAX_PORT - MIN_PORT) + MIN_PORT;
    return StringUtil::toString(port);
}
