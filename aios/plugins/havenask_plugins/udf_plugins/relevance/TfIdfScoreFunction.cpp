...
AUTIL_LOG_SETUP(tfidfscore, TfIdfScoreFunction);

TfIdfScoreFunction::TfIdfScoreFunction(const FunctionSubExprVec & exprs) : 
    _exprs(exprs) {
}

TfIdfScoreFunction::~TfIdfScoreFunction() {
    if(_fieldMetaHelper != nullptr) {
        delete _fieldMetaHelper;
    }
}

bool TfIdfScoreFunction::beginRequest(FunctionProvider* functionProvider) {
    ...
    if(_matchInfoReader == nullptr) {
        SQL_LOG(ERROR, "get matchInfo failed");
        return false;
    }
    ...
    _metaInfo = _matchInfoReader->getMetaInfoPtr();
    if(_metaInfo == nullptr) {
        SQL_LOG(ERROR, "get metaInfo failed");
        return false;
    }

    _partTotalDocCount = _metaInfo->getPartTotalDocCount();
...
bool TfIdfScoreFunction::initMetas() {
    ...
    for(size_t i=0;i<_metaInfo->size();i++) {
        ...
        // 过滤符合udf index参数的用户query term, 预存避免重复计算
...
void TfIdfScoreFunction::endRequest() {

}

float TfIdfScoreFunction::evaluate(matchdoc::MatchDoc matchDoc) {
    TfidfRelEvaluator evaluator(_partTotalDocCount);
    ...
    for(size_t k=0;k<_filteredTermMetaIdx.size();k++) {
        ...
        if (termMatchData.isMatched()) {
            ...
            std::string termIndexName = termMetaInfo.getIndexName();
            int64_t termFreq = termMatchData.getTermFreq();
            int64_t docFreq = termMetaInfo.getDocFreq();
            SQL_LOG(DEBUG, "docid: [%d] tfidf term: [%s]", matchDoc.getDocId(), termMetaInfo.getTermText().c_str());
            evaluator.addTermParam(termFreq, docFreq);
        }
    }
...
AUTIL_LOG_SETUP(tfidfscore, TfIdfScoreFunctionCreatorImpl);

FunctionInterface* TfIdfScoreFunctionCreatorImpl::createFunction(
    ...
    return new TfIdfScoreFunction(funcSubExprVec);
}
