...
class PQKnnParamsSelector : public NoneLinearKnnParamsSelector {
 public:
    ...
    virtual bool InitStreamerParams(const indexlib::util::KeyValueMap &parameters,
                                    aitheta::IndexParams &indexParams, bool printParam = true) override {
        INDEXLIB_FATAL_ERROR(UnImplement, "PQKnnParamsSelector does not support the function");
        return false;
    }
    ...
    const std::string &GetStreamerName() override { return INVALID_PQ_STREAMER; }
