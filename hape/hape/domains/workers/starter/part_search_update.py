...
examples:
    ...
    def _loadQrsTarget(self, timeout = 300):
        ...
        target = {
            "service_info" : {
                "cm2_config" : {
                    "local" : self.readyZones #localTopoStr
                },
                "part_count" : 0,
                "part_id" : 0,
                "zone_name": "qrs"
        ...
        with open("qrs_port") as f:
            data = json.load(f)
            portList = [data["http_arpc_port"], data["arpc_port"]]
    ...
    def _loadSearcherTarget(self, targetInfos, timeout = 300):
        ...
        while timeout > 0:
            ...
            timeout -= 5
            count = 0
            for targetInfo in targetInfos[:1]:
                # portList = self._getSearcherPortList(count)
                with open("searcher_port") as f:
                    data = json.load(f)[0][1]
                    portList = [data["http_arpc_port"], data["arpc_port"]]
                count += 1
                print('read', targetInfo)
                zoneName = targetInfo[0][0]
                partId = self.partitionIndex
                ...
                if len(targetInfos) - 1 == len(self.readyZones):
