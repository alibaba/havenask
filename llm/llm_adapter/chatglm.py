...
class ChatGLM(LLM):
    ...
    _CHATGLM_INSTANCE = RemoteChatGLM.get_instance() if CHATGLM_ENDPOINT else LocalChatGLM.get_instance()
    _EMBEDDING_INSTANCE = RemoteEmbedding.get_instance() if EMBEDDING_ENDPOINT else LocalEmbedding.get_instance()
    ...
    def get_tokenizer(cls):
        return cls._EMBEDDING_INSTANCE.get_tokenizer()

    @classmethod
    async def embed_text(cls, text: str) -> List[float]:
        return await cls._EMBEDDING_INSTANCE.embed_text(text)

    @classmethod
    async def embed_query(cls, text: str) -> List[float]:
        return await cls._EMBEDDING_INSTANCE.embed_query(text)
    ...
    async def chat(cls, contexts: List[HAResult], query: str, history: List[List[str]], max_length=2048, top_p=0.7, temperature=0.01):
        ...
        return await cls._CHATGLM_INSTANCE.chat(prompt, history, max_length=max_length, top_p=top_p, temperature=temperature)
    ...
    async def stream_chat(cls, contexts: List[HAResult], query: str, history: List[List[str]], max_length=2048, top_p=0.7, temperature=0.01):
        prompt = cls._create_prompt(contexts, query)
        return cls._CHATGLM_INSTANCE.stream_chat(prompt, history, max_length=max_length, top_p=top_p, temperature=temperature)
